...
11. [x] M1 共享收敛已通过并以 `b38ac20e` 形成独立提交，M2 已据此进入实施。
...
6. [x] M1 已形成独立、可审计提交 `b38ac20e` 后才进入 M2。
...
1. [x] 页面、adapter、CSS 和测试 fixture 未复制业务常量。
2. [x] `visibleBars?: number` 已从 public shared props 删除。
3. [x] 没有新增 env、Vite flag、策略中心 key 或 localStorage 配置。

### 3.2 初始范围

1. [x] 使用 K 线 host 宽度减 56px 价格轴计算真实绘图区宽度。
2. [x] 除以 9.5 后四舍五入到最接近的 15 根，再 clamp 到 75～150。
3. [x] 1600×1200 页面精确得到 120 根。
4. [x] 宽度为 0/NaN/不可测时回退 120 根。
5. [x] 初始右边界为最新一根；数据不足时显示全部。

### 3.3 点击与锚点

1. [x] 放大镜加号每次减少 15 根，最低 45；45 根时该按钮 disabled。
2. [x] 放大镜减号每次增加 15 根，最高 `min(180, pointCount)`；到达上界时该按钮 disabled。
3. [x] 当前贴近最新时固定最新右边界。
4. [x] 历史区间缩放保持区间中心；边界 clamp 不静默缩短目标范围。
5. [x] 四个 pane 接收完全相同的 logical range。
6. [x] 点击按钮不触发 pointer drag、`fitContent()`、API 请求或 chart 重建。
7. [x] `buildChartOptions()` 显式设置 `rightPriceScale.autoScale=true`；没有 CSS scale 或行情值倍率转换。

### 3.4 生命周期

1. [x] 四个 adapter 均传稳定且必填的 `dataKey`。
2. [x] dataKey 改变时重置自适应默认。
3. [x] MA/BOLL/趋势通道图层切换保留当前范围。
4. [x] 用户交互后 resize 保持选择；未交互时 resize 允许更新自适应默认。
5. [x] 同一 dataKey 新增最新 bar 时，最新视图跟随；历史视图不跳回最新。
6. [x] 卸载时 observer、subscription 和 pointer listener 全部清除。

## 4. 控件与视觉门禁

1. [x] 两个真实 button 位于 K 线主图右下角。
2. [x] 排列为 Phosphor `magnifying-glass-minus`（缩小）后 `magnifying-glass-plus`（放大），单按钮 28×28px、间距 4px、组合宽 60px。
3. [x] 控件右侧距价格轴左边 8px，底部距绘图区 8px。
4. [x] 图标从正式 Figma 组件 `583:534` 导出完整同源 SVG 并固化：16×16、内部约 13.014×13.014、保留约 1.493px 左右的 x/y 居中位移、`currentColor`、`aria-hidden`；没有纯文本、左上角偏移或近似图标。
5. [x] 缩小按钮 aria-label 为“缩小K线，增加可见根数”。
6. [x] 放大按钮 aria-label 为“放大K线，减少可见根数”。
7. [x] disabled 同时具备原生属性、45% 视觉弱化和不可点击行为。
8. [x] focus-visible 清晰，颜色和圆角使用现有 Design Token。
9. [x] 控件不遮挡价格轴、Y 轴浮标、Tooltip、十字线、时间轴或重要 K 线。
10. [x] 页面、左右栏、pane、底部指标栏尺寸不变。
11. [x] 不新增 Phosphor/Supericons 运行时依赖。
...
1. [x] 切换标的/周期时旧缩放状态不串入新图表。
2. [x] 状态切换不展示旧图表的缩放按钮。
3. [x] 本功能没有新增 API 错误或异常码。

## 6. 测试门禁

### 6.1 纯函数测试

1. [x] 75/120/150 自适应样例。
2. [x] 45/180 和 pointCount 边界。
3. [x] pointCount 为 0、30、60、100、300、500。
4. [x] latest 锚点、历史中心、左右 clamp。
5. [x] logical span 始终等于 `visibleCount-1`。

### 6.2 Shared 组件测试

1. [x] 默认 120 根和四 pane 同步。
2. [x] 连续放大至 45、连续缩小至 180。
3. [x] 数据少于 45、介于 45 与默认、少于 180 三类边界。
4. [x] 点击按钮时 `createChart` 次数不变、fetch 次数为 0。
5. [x] overlay 切换、resize、dataKey 和 append bar 行为。
6. [x] drag/crosshair/tooltip/时间轴/axis label 回归。
7. [x] 通过 aria-label 找到两个按钮，并断言 45/最大值/ShortData 的 disabled 方向与组件集 `585:550` 一致。
8. [x] SVG source/样式断言证明没有纯文本 `−`/`＋` 或近似 icon 回退。
...
1. [x] 单次点击只执行四个 chart 的 `setVisibleLogicalRange()`。
2. [x] 单次点击不销毁/重建 chart 或 series。
3. [x] 单次点击不触发 React 页面请求和网络请求。
4. [x] ResizeObserver 不形成同步回调循环或持续重绘。
5. [x] 300/500 点输入下连续点击与拖动无可见卡顿。
