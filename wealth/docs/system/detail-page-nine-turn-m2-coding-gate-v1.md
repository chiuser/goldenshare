...
2. 生产日线 API 的真实窗口、状态、权限和代表性性能样本已验收；仍缺至少 10 次同口径登录态 HTTP 样本形成正式 P95。
3. 1600×1200 日线 Loaded 和代表分钟、切周期、缩放行为已验收；仍缺生产 45/180 根边界截图及未覆盖状态的最终视觉收口。
4. 三个股票九转 sensor 当前虽为实例 `RUNNING`，但最近 tick 均被门禁跳过；必须完成自然日常更新和 freshness 验收，不能把 `RUNNING` 当作已投产。后续启停仍须单独审批，当前状态见总方案第 3.3 节。
