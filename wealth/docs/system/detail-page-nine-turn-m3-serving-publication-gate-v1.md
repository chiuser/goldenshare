...
> 状态：M3-A/M3-B 已完成。M3-C 的生产日线接口、权限、真实数据对齐和 1600×1200 Loaded 视觉已完成代表性验收；正式 P95 仍缺足量登录态 HTTP 样本，生产页缩放到 45/180 根边界的浏览器截图仍待补齐，因此 M3-C 尚未最终收口。两个 sensor 仍为 `STOPPED`，分钟九转验收尚未执行；该表述不否定分钟 K 线 Gold 已就绪。
...
3. 用户恢复分钟专项后，再完成股票 30/60/90/120 分钟与 1/5/15 禁用周期验收；当前按明确要求暂停。
...
6. 分钟验收继续暂停；最后做一次自然日常链路验收，仍需单独审批才可启用 sensor。
