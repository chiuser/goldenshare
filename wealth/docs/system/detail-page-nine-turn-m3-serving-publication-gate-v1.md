...
> 状态：M3-A/M3-B 已完成。M3-C 的生产日线接口、权限、真实数据对齐和 1600×1200 Loaded 视觉已完成代表性验收；正式 P95 仍缺足量登录态 HTTP 样本，生产页缩放到 45/180 根边界的浏览器截图仍待补齐，因此完整 M3-C 尚未最终收口。M3-C-Minute 的 30/60/90/120 分钟接口、性能、内存、浏览器和视觉行为验收已完成，1/5/15 分钟禁用口径已验证为零九转请求；2026-08-15 去价格专项又完成四个分钟资产的八列正式迁移、事件恢复、Reader/readiness 和分钟 sensor 最近窗口自然评估。三个股票九转 sensor 的正式实例状态均为 `RUNNING`；分钟 sensor 最近评估为最近 5 日均 ready，日线 Gold sensor 仍被既有聚合 check 阻断，serving sensor 返回空结果。定义默认 `STOPPED` 不能替代当前实例状态，详细快照见总方案第 3.3 节。
...
3. 完成一次股票日线自然更新与 serving 发布链路验收；当前三个股票 sensor 均为实例 `RUNNING`，但日线 Gold sensor 仍被既有聚合 check 阻断，serving sensor 返回空结果。分钟 sensor 的最近窗口自然评估已经通过，不再是本条日线待办。本文不授权任何启停动作，后续处置须单独审批。
...
M3-C-Minute 的页面/API/性能行为和无价格八列物理合同均已完成：四频各 3,068 个正式文件，共 12,272 个文件、197,753,897 行；12,272 条 materialization、最近 20 日共 80 条 check、Reader/readiness 和分钟 sensor 最近窗口自然评估均已通过。它不等于完整 M3-C 完成；生产日线 P95、45/180 根边界截图和日线自然更新/serving 链路仍按 8.5 保持待验收。指数 M4-B 后端、正式历史和 serving 已在后续阶段完成，指数页面接入仍属于 M5。
...
6. M3-C-Minute 行为验收和无价格八列物理合同迁移均已完成；完整 M3-C 只剩生产日线 P95、生产 45/180 根边界截图和日线自然更新/serving 链路。三个股票 sensor 当前均为实例 `RUNNING`，分钟最近窗口 ready，日线仍被既有门禁跳过；任何后续停止、恢复或运行口径调整仍需单独审批。
