...
> 状态：M3-A/M3-B/M3-C 均已收口，后续 M4-B/M5 也已完成；M6-0 发布准备审计与测试稳定性修复已于 2026-08-15 完成并通过，M6-A 尚未开始。M3-C 的生产日线接口、权限、真实数据对齐和 1600×1200 Loaded 视觉已完成代表性验收；2026-08-15 用户明确取消“补足登录态正式 P95 样本”和“补拍生产页 45/180 根缩放边界截图”两项补充验收，它们不再是退出条件。M3-C-Minute 的 30/60/90/120 分钟接口、性能、内存、浏览器和视觉行为验收已完成，1/5/15 分钟禁用口径已验证为零九转请求；去价格专项又完成四个分钟资产的八列正式迁移、事件恢复、Reader/readiness 和分钟 sensor 最近窗口自然评估。股票日线价格字段和自然更新阻塞已经移交独立数据治理专项，不属于 M3-C 页面收口范围。三个股票九转 sensor 的实例状态只引用总方案第 3.3 节的带时间戳快照，本次未刷新为实时状态。
...
| 定义默认停止的 sensor | `prod_core_stock_daily_qfq_nineturn_sync_job_sensor`；最近实例快照见总方案第 3.3 节 |
...
4. 登录态页面真实请求：`600683.SH` 返回 HTTP 200、143.07ms；另一个生产样本 `688300.SH` 返回 HTTP 200、211.78ms。两次均低于 1.5s 目标；它们只作为代表性可用性样本，不宣称正式 P95。用户已取消补足正式 P95 样本的要求。
...
4. Chrome 验收连接在连续点击缩放按钮时超时，未稳定取得 45 根放大上限和 180 根缩小上限的生产截图；共享组件自动化测试已证明 15 根步长、45/180 根禁用边界、不重建图表且不发请求。用户已取消补拍这两个生产截图的要求。
...
生产日线的功能、权限、数据映射和 Loaded 视觉主体门禁已通过。用户取消登录态正式 P95 和生产 45/180 根截图两项补充验收后，M3-C 页面切片据此收口。股票日线自然更新与 serving 阻塞、冗余价格字段继续作为独立数据治理/M6 运维事项保留，不回写为 M3-C 页面退出条件；本文不授权任何启停动作。
...
M3-C-Minute 的页面/API/性能行为和无价格八列物理合同均已完成：四频各 3,068 个正式文件，共 12,272 个文件、197,753,897 行；12,272 条 materialization、最近 20 日共 80 条 check、Reader/readiness 和分钟 sensor 最近窗口自然评估均已通过。结合 8.5 的日线页面收口，完整 M3-C 已完成。指数 M4-B 后端、正式历史和 serving 以及 M5 页面接入也已在后续阶段完成；当前只剩 M6 发布、自然更新与最终生产验收，以及独立的股票日线数据治理任务。
...
5. M3-C 生产日线接口、权限、数据对齐和 Loaded 视觉主体门禁已通过；用户取消正式 P95 与缩放边界截图两项补充要求后，日线页面切片已收口。
6. M3-C-Minute 行为验收和无价格八列物理合同迁移均已完成，因此 M3-C 整体完成。2026-08-15 12:58 的最近快照中三个股票 sensor 均为实例 `RUNNING`，分钟最近窗口 ready；该快照本次未刷新，不能当作实时状态。日线自然链路问题已移交独立专项，任何后续停止、恢复或运行口径调整仍需单独审批。
7. M4-B、M5 与 M6-0 已完成；板块测试竞态已修复并通过 Wealth 全量回归。本地六个发布候选提交仍未推送，后续分别审批推送部署、Dagster 实例只读刷新、指数 sensor 启用和下一交易日自然更新观察；本门禁不自动授权这些动作。
...
| v1.7 | 2026-08-15 | 同步分钟去价格 S0～S5 完成事实：12,272 个八列正式文件、12,272 materialization、80 check、Reader/readiness 和分钟 sensor 最近窗口评估通过；保留生产日线 P95、缩放边界截图和日线自然链路待办 | Codex |
| v1.8 | 2026-08-15 | 按用户决定取消登录态正式 P95 与生产 45/180 根截图两项补充验收；M3-C 页面切片收口，日线价格与自然更新问题移交独立数据治理/M6 运维范围 | Codex |
| v1.9 | 2026-08-15 | 同步 M4-B/M5 完成事实；明确本地提交不等于推送部署，并把后续边界收敛为 M6-0 发布评审、受控发布、sensor 两段审批和自然更新验收 | Codex |
| v1.10 | 2026-08-15 | 同步 M6-0 通过：六提交、生产版本/路由/配置、platform-only 部署和 M5 定向回滚已审计；板块测试竞态修复并通过 Wealth 全量回归，M6-A 尚未获批 | Codex |
