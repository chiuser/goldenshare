...
> 状态：M3-A/M3-B/M3-C 均已收口。M3-C 的生产日线接口、权限、真实数据对齐和 1600×1200 Loaded 视觉已完成代表性验收；2026-08-15 用户明确取消“补足登录态正式 P95 样本”和“补拍生产页 45/180 根缩放边界截图”两项补充验收，它们不再是退出条件。M3-C-Minute 的 30/60/90/120 分钟接口、性能、内存、浏览器和视觉行为验收已完成，1/5/15 分钟禁用口径已验证为零九转请求；去价格专项又完成四个分钟资产的八列正式迁移、事件恢复、Reader/readiness 和分钟 sensor 最近窗口自然评估。股票日线价格字段和自然更新阻塞已经移交独立数据治理专项，不属于 M3-C 页面收口范围。三个股票九转 sensor 的正式实例状态均为 `RUNNING`；详细快照见总方案第 3.3 节。
...
M3-C-Minute 的页面/API/性能行为和无价格八列物理合同均已完成：四频各 3,068 个正式文件，共 12,272 个文件、197,753,897 行；12,272 条 materialization、最近 20 日共 80 条 check、Reader/readiness 和分钟 sensor 最近窗口自然评估均已通过。结合 8.5 的日线页面收口，完整 M3-C 已完成。指数 M4-B 后端、正式历史和 serving 已在后续阶段完成，指数页面接入属于 M5。
...
6. M3-C-Minute 行为验收和无价格八列物理合同迁移均已完成，因此 M3-C 整体完成。三个股票 sensor 当前均为实例 `RUNNING`，分钟最近窗口 ready；日线自然链路问题已移交独立专项，任何后续停止、恢复或运行口径调整仍需单独审批。
