...
> 状态：M3-A/M3-B 门禁代码已完成；Gold 日线修复与全历史验收已完成。生产 serving 已发布并验收到 1,113/3,066 个交易日、2,770,508 行，因内存与超大控制台输出风险主动暂停；发布器修正及正式只读内存验收已通过，恢复发布仍须获得用户确认。分钟未执行，Web 生产验收未开始。
...
| serving 当前检查点 | 1,113 / 3,066 个交易日；2,770,508 行 |
| serving 当前状态 | 主动暂停；检查点与生产表日期数一致 |
...
2026-08-13 本轮内存与输出修正后的定向代码验收为 21 项通过；九转相关完整回归为 96 项通过、另有 14 个子测试通过，Ruff 与 `git diff --check` 通过。测试明确证明 plan 不再调用行装载器、publish 不再重建深度计划、CLI 摘要不含完整分区数组、进度仅按 batch 发出。正式只读 plan 与 checkpoint 回验也已通过上述内存门禁。`dg list defs` 已发现 serving asset、blocking check、独立 job 和 sensor。真实 Gold 修复已完成；serving 仍是部分发布，不能标记为生产就绪。
...
4. serving 已发布至 1,113/3,066 个交易日并主动暂停。代码修正后先执行不写 Prod 的全历史计划和 checkpoint 流式回验内存验收；通过且用户再次确认后，才能以单进程最多 200 日的短进程续跑。两个 sensor 继续保持 STOPPED。
5. 全量 Prod 对账通过后进行生产日线 API、页面和性能验收。
