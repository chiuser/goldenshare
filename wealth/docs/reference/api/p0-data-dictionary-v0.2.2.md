...
# 附录 A：Tushare / 落库字段名词表

> 目的：本附录作为前端展示名称、表格列名、Tooltip、字段解释的统一名词来源。  
> 原则：字段名保持 Tushare / 当前 PostgreSQL 落库口径；中文展示名由财势乾坤统一定义。  
> 使用建议：前端展示指标名称时，优先使用本表的“统一中文名”；需要解释口径时，使用“字段说明”。

## A.1 通用行情字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `ts_code` | 证券代码 | Tushare 统一证券代码，通常带交易所后缀 | string | `stock_basic`、`daily`、`index_daily`、`limit_list_d` |
| `symbol` | 股票代码 | 不带交易所后缀的股票代码 | string | `stock_basic` |
| `name` | 证券名称 | 股票、指数或板块名称 | string | `stock_basic`、`limit_list_d`、`sw_daily` |
| `trade_date` | 交易日期 | 数据所属交易日 | `YYYYMMDD` | `daily`、`daily_basic`、`index_daily`、`limit_list_d` |
| `trade_time` | 交易时间 | 分钟行情或分时行情时间 | datetime/string | `stk_mins` |
| `open` | 开盘价 | 当日或当前周期开盘价 | 元/点 | `daily`、`index_daily`、`sw_daily`、`stk_mins` |
| `high` | 最高价 | 当日或当前周期最高价 | 元/点 | `daily`、`index_daily`、`sw_daily`、`stk_mins` |
| `low` | 最低价 | 当日或当前周期最低价 | 元/点 | `daily`、`index_daily`、`sw_daily`、`stk_mins` |
| `close` | 收盘价 | 当日或当前周期收盘价 | 元/点 | `daily`、`index_daily`、`sw_daily`、`stk_mins` |
| `pre_close` | 昨收价 | 上一交易日收盘价，或上一周期收盘价 | 元/点 | `daily`、`index_daily`、`stk_limit` |
| `change` | 涨跌额 | 当前收盘价相对昨收价的涨跌额 | 元/点 | `daily`、`index_daily` |
| `pct_chg` | 涨跌幅 | 当前收盘价相对昨收价的涨跌幅 | % | `daily`、`index_daily` |
| `pct_change` | 涨跌幅 | 板块或行业指数涨跌幅 | % | `sw_daily`、`moneyflow_ind_dc` |
| `vol` | 成交量 | 成交数量；不同数据集单位不同，需看具体来源 | 手/万股等 | `daily`、`index_daily`、`sw_daily` |
| `volume` | 成交量 | 成交数量；部分接口直接使用 volume 命名 | 股/手，按来源 | `stk_mins`、部分扩展数据 |
| `amount` | 成交额 | 成交金额；不同数据集单位不同，需看具体来源 | 千元/万元/元 | `daily`、`index_daily`、`sw_daily`、`stk_mins` |

## A.2 交易日历字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `exchange` | 交易所 | 交易所代码，如 SSE、SZSE | string | `trade_cal`、`stock_basic` |
| `cal_date` | 日历日期 | 自然日期 | `YYYYMMDD` | `trade_cal` |
| `is_open` | 是否开市 | 是否为交易日；当前项目落库已改为 boolean | boolean | `trade_cal` |
...
| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `area` | 所属地域 | 上市公司所属地域 | string | `stock_basic` |
| `industry` | 所属行业 | 股票基础行业字段 | string | `stock_basic`、`limit_list_d` |
| `fullname` | 公司全称 | 上市公司全称 | string | `stock_basic` |
| `enname` | 英文名称 | 上市公司英文名称 | string | `stock_basic` |
| `cnspell` | 拼音缩写 | 股票名称拼音缩写 | string | `stock_basic` |
| `market` | 市场类型 | 主板、创业板、科创板等市场类型 | string | `stock_basic` |
| `curr_type` | 交易货币 | 交易币种 | string | `stock_basic` |
| `list_status` | 上市状态 | L 上市、D 退市、P 暂停上市等 | string | `stock_basic` |
| `list_date` | 上市日期 | 股票上市日期 | `YYYYMMDD` | `stock_basic` |
| `delist_date` | 退市日期 | 股票退市日期 | `YYYYMMDD` / null | `stock_basic` |
| `is_hs` | 是否沪深港通 | 是否为沪股通/深股通标的 | string | `stock_basic` |

## A.4 每日指标字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `turnover_rate` | 换手率 | 成交量相对流通股本的比例 | % | `daily_basic` |
...
| `ps` | 市销率 | 市值相对营业收入的比值 | 倍 | `daily_basic` |
| `ps_ttm` | 市销率 TTM | 滚动市销率 | 倍 | `daily_basic` |
| `dv_ratio` | 股息率 | 股息率 | % | `daily_basic` |
| `dv_ttm` | 股息率 TTM | 滚动股息率 | % | `daily_basic` |
| `total_share` | 总股本 | 公司总股本 | 万股 | `daily_basic` |
| `float_share` | 流通股本 | 可流通股本 | 万股 | `daily_basic` |
| `free_share` | 自由流通股本 | 自由流通股本 | 万股 | `daily_basic` |
| `total_mv` | 总市值 | 股票总市值 | 万元 | `daily_basic` |
| `circ_mv` | 流通市值 | 股票流通市值 | 万元 | `daily_basic` |
...
| `pre_close` | 昨收价 | 计算涨跌停价格使用的昨收价 | 元 | `stk_limit` |

## A.6 涨跌停 / 炸板字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `limit` | 涨跌停状态 | U 涨停、D 跌停、Z 炸板等，具体以源文档为准 | enum/string | `limit_list_d` |
| `fd_amount` | 封单金额 | 封板时封单金额 | 按源口径 | `limit_list_d` |
| `first_time` | 首次封板时间 | 当日首次封板时间 | HHMMSS/string | `limit_list_d` |
| `last_time` | 最后封板时间 | 当日最后一次封板时间 | HHMMSS/string | `limit_list_d` |
| `open_times` | 开板次数 | 当日打开涨停/跌停板次数 | 次 | `limit_list_d` |
| `up_stat` | 连板统计 | 源接口提供的涨停统计字符串 | string | `limit_list_d` |
| `limit_times` | 连板数 | 当前连续涨停板数量 | 板 | `limit_list_d` |
| `strth` | 涨停强度 | 源接口提供的封板强度字段 | number | `limit_list_d` |
| `fc_ratio` | 封成比 | 封单金额与成交额等相关比例，按源口径 | ratio/%，按源 | `limit_list_d` |
| `fl_ratio` | 封流比 | 封单量与流通盘相关比例，按源口径 | ratio/%，按源 | `limit_list_d` |

## A.7 大盘资金流字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `net_amount` | 主力净流入 | 主力资金净流入金额 | 元 | `moneyflow_mkt_dc` |
| `net_amount_rate` | 主力净流入占比 | 主力净流入金额占比 | % | `moneyflow_mkt_dc` |
| `buy_elg_amount` | 超大单流入金额 | 超大单主动买入金额 | 元 | `moneyflow_mkt_dc` |
| `buy_elg_amount_rate` | 超大单流入占比 | 超大单主动买入金额占比 | % | `moneyflow_mkt_dc` |
| `buy_lg_amount` | 大单流入金额 | 大单主动买入金额 | 元 | `moneyflow_mkt_dc` |
| `buy_lg_amount_rate` | 大单流入占比 | 大单主动买入金额占比 | % | `moneyflow_mkt_dc` |
| `buy_md_amount` | 中单流入金额 | 中单主动买入金额 | 元 | `moneyflow_mkt_dc` |
| `buy_md_amount_rate` | 中单流入占比 | 中单主动买入金额占比 | % | `moneyflow_mkt_dc` |
| `buy_sm_amount` | 小单流入金额 | 小单主动买入金额 | 元 | `moneyflow_mkt_dc` |
| `buy_sm_amount_rate` | 小单流入占比 | 小单主动买入金额占比 | % | `moneyflow_mkt_dc` |
| `close_sh` | 上证收盘点位 | 上证指数收盘点位 | 点 | `moneyflow_mkt_dc` |
| `close_sz` | 深证收盘点位 | 深证成指收盘点位 | 点 | `moneyflow_mkt_dc` |

## A.8 个股资金流字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `net_amount` | 主力净流入 | 个股主力资金净流入金额 | 万元 | `moneyflow_dc` |
| `net_amount_rate` | 主力净流入占比 | 个股主力净流入占比 | % | `moneyflow_dc` |
| `buy_elg_amount` | 超大单流入金额 | 个股超大单主动买入金额 | 万元 | `moneyflow_dc` |
| `buy_elg_amount_rate` | 超大单流入占比 | 个股超大单主动买入占比 | % | `moneyflow_dc` |
| `buy_lg_amount` | 大单流入金额 | 个股大单主动买入金额 | 万元 | `moneyflow_dc` |
| `buy_lg_amount_rate` | 大单流入占比 | 个股大单主动买入占比 | % | `moneyflow_dc` |
| `buy_md_amount` | 中单流入金额 | 个股中单主动买入金额 | 万元 | `moneyflow_dc` |
| `buy_md_amount_rate` | 中单流入占比 | 个股中单主动买入占比 | % | `moneyflow_dc` |
| `buy_sm_amount` | 小单流入金额 | 个股小单主动买入金额 | 万元 | `moneyflow_dc` |
| `buy_sm_amount_rate` | 小单流入占比 | 个股小单主动买入占比 | % | `moneyflow_dc` |

## A.9 板块 / 行业资金流字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `content_type` | 板块类型 | 行业、概念、地域等分类类型 | string | `moneyflow_ind_dc` |
| `rank` | 排名 | 当前排序口径下的排名 | integer | `moneyflow_ind_dc` |
| `pct_change` | 涨跌幅 | 板块涨跌幅 | % | `moneyflow_ind_dc`、`sw_daily` |
| `net_amount` | 主力净流入 | 板块主力资金净流入金额 | 元 | `moneyflow_ind_dc` |
| `net_amount_rate` | 主力净流入占比 | 板块主力净流入占比 | % | `moneyflow_ind_dc` |
| `buy_elg_amount` | 超大单流入金额 | 板块超大单主动买入金额 | 元 | `moneyflow_ind_dc` |
| `buy_lg_amount` | 大单流入金额 | 板块大单主动买入金额 | 元 | `moneyflow_ind_dc` |
| `buy_md_amount` | 中单流入金额 | 板块中单主动买入金额 | 元 | `moneyflow_ind_dc` |
| `buy_sm_amount` | 小单流入金额 | 板块小单主动买入金额 | 元 | `moneyflow_ind_dc` |

## A.10 申万行业行情字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `ts_code` | 行业指数代码 | 申万行业指数代码 | string | `sw_daily` |
| `name` | 行业名称 | 申万行业名称 | string | `sw_daily` |
| `change_pct` | 涨跌幅 | 申万行业指数涨跌幅；若落库字段为 `pct_change`，以实际表结构为准 | % | `sw_daily` |
| `pct_change` | 涨跌幅 | 申万行业指数涨跌幅 | % | `sw_daily` |
| `vol` | 成交量 | 行业指数成交量 | 万股 | `sw_daily` |
| `amount` | 成交额 | 行业指数成交额 | 万元 | `sw_daily` |
| `float_mv` | 流通市值 | 行业成分流通市值 | 万元 | `sw_daily` |
| `total_mv` | 总市值 | 行业成分总市值 | 万元 | `sw_daily` |
| `pe` | 市盈率 | 行业市盈率 | 倍 | `sw_daily` |
| `pb` | 市净率 | 行业市净率 | 倍 | `sw_daily` |

## A.11 分钟行情字段

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见来源 |
|---|---|---|---|---|
| `trade_time` | 交易时间 | 分钟 K 线时间 | datetime/string | `stk_mins` |
| `open` | 分钟开盘价 | 当前分钟 K 线开盘价 | 元 | `stk_mins` |
| `close` | 分钟收盘价 | 当前分钟 K 线收盘价 | 元 | `stk_mins` |
| `high` | 分钟最高价 | 当前分钟 K 线最高价 | 元 | `stk_mins` |
| `low` | 分钟最低价 | 当前分钟 K 线最低价 | 元 | `stk_mins` |
| `vol` | 分钟成交量 | 当前分钟成交量；以落库字段单位为准 | 按源口径 | `stk_mins` |
| `amount` | 分钟成交额 | 当前分钟成交额；以落库字段单位为准 | 按源口径 | `stk_mins` |

## A.12 财势乾坤派生字段名词

| 字段名 | 统一中文名 | 字段说明 | 单位/类型 | 常见使用对象 |
|---|---|---|---|---|
| `direction` | 涨跌方向 | 根据涨跌幅派生：`UP` 上涨、`DOWN` 下跌、`FLAT` 平盘 | enum | `IndexSnapshot`、`QuoteSnapshot`、`StockRankItem` |
| `redRate` | 红盘率 | 上涨家数 / 有效样本数 | ratio | `MarketBreadth`、`SectorRankItem` |
| `upCount` | 上涨家数 | 有效样本中上涨股票数量 | 只 | `MarketBreadth` |
| `downCount` | 下跌家数 | 有效样本中下跌股票数量 | 只 | `MarketBreadth` |
| `flatCount` | 平盘家数 | 有效样本中平盘股票数量 | 只 | `MarketBreadth` |
| `medianChangePct` | 中位涨跌幅 | 有效样本涨跌幅中位数 | % | `MarketBreadth` |
| `upGt3Pct` | 涨超 3% 占比 | 涨幅大于等于 3% 的样本占比 | ratio | `MarketBreadth` |
| `downGt3Pct` | 跌超 3% 占比 | 跌幅小于等于 -3% 的样本占比 | ratio | `MarketBreadth` |
| `sealRate` | 封板率 | 涨停数 / 触及涨停数 | ratio | `LimitUpSummary` |
| `failedLimitUpCount` | 炸板数 | 触及涨停但未封住的股票数量 | 只 | `LimitUpSummary` |
| `touchedLimitUpCount` | 触板数 | 涨停数 + 炸板数 | 只 | `LimitUpSummary` |
| `highestStreak` | 最高连板高度 | 当日最高连续涨停板数量 | 板 | `LimitUpSummary`、`LimitUpStreakLadder` |
| `amount20dMedian` | 近 20 日成交额中位数 | 近 20 个交易日成交额中位数，单位跟随源聚合口径 | 按源口径 | `TurnoverSummary` |
| `amountRatio20dMedian` | 成交额相对 20 日中位倍数 | 当日成交额 / 近 20 日成交额中位数 | 倍 | `TurnoverSummary` |
| `smallVsLargeSpreadPct` | 小盘相对大盘强弱差 | 小盘代表指数涨跌幅 - 大盘代表指数涨跌幅 | pct point | `MarketStyle` |
| `styleLeader` | 领先风格 | 当前市场相对占优风格 | enum | `MarketStyle` |
| `dataStatus` | 数据状态 | `READY` / `PARTIAL` / `DELAYED` / `UNAVAILABLE` | enum | 全部行情/统计对象 |
| `asOf` | 数据时间 | 当前对象的数据更新时间或同步时间 | datetime | 全部行情/统计对象 |
| `sourceRefs` | 数据来源引用 | 标识数据来自哪些数据集和文档 | array | API 响应对象 |
