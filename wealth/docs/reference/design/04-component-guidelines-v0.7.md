...
> 合并说明：本版以 Google Drive 公共区 `04-component-guidelines.md` v0.6 merged-full 为完整基线，完整保留此前已确认内容，并合并 Review v4 的局部修订。本轮 Review v4 只处理“涨跌停统计与分布中的领涨股涨停表现”和“大盘资金流向单型资金净流向饼图说明样式”两个点名区域，不主动重构其它组件。请勿用局部 delta 文档覆盖本文件。
