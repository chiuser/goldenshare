...
> 当前收口：第 1～9 节保留 2026-08-11 的阶段快照语义；2026-08-22 的最终生产复验见第 10 节。当前生产版本为 `57ece8a3`，2024 覆盖、动态 MA、数据库计划和生产 Web 主机同拓扑服务链均已通过；真实 2000 行按 `N/A — accepted physical limitation` 关闭，不再阻塞本项目交付。
...
## 10. 2026-08-22 最终生产复验与关闭结论

本节是对第 1～9 节历史快照的后续收口，不回写或改名旧样本。审计时间为 `2026-08-22 15:57:52 CST`，生产 SHA 为 `57ece8a3`。数据库检查全部使用显式只读事务；Web 主机样本使用当前部署代码、正式环境配置和短生命周期 Session，覆盖 Query Service、数据库往返、DTO 构造及 JSON 序列化，但不包含公网、ASGI Server 网络或登录态正式 HTTP，因此只能称为“生产 Web 主机同拓扑服务链”。

### 10.1 十指数物理覆盖与 2024 对齐

| tsCode | factor 行数 | 日期范围 | 与 daily 最新日对齐 |
|---|---:|---|---|
| `000001.SH` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `399001.SZ` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `399006.SZ` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `000688.SH` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `000300.SH` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `000905.SH` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `000852.SH` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `899050.BJ` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |
| `000510.SH` | 464 | 2024-09-23 ~ 2026-08-21 | 是 |
| `000016.SH` | 881 | 2023-01-03 ~ 2026-08-21 | 是 |

十指数 `(ts_code, trade_date)` 重复键组数为 0。2024 年内，九个既有指数各有 factor/daily `242/242` 行，范围为 2024-01-02～2024-12-31；`000510.SH` 从自身正式起点 2024-09-23 起有 `67/67` 行。两组的 daily 无 factor、factor 无 daily 均为 0，2024 当前可用历史覆盖复审通过。

### 10.2 动态 MA 复审

当前历史中的合理 warm-up 空值和达到周期后的异常空值如下：

| tsCode | MA5/10/20/30/60/90/250 合理 warm-up 空值 | 达到周期后的异常空值 |
|---|---|---|
| `000510.SH` | 4/9/19/29/59/89/249 | 全部 0 |
| `899050.BJ` | 0/0/9/19/49/79/239 | 全部 0 |
| 其余八个指数 | 全部 0 | 全部 0 |

`000510.SH` 最近 300 根仍有 85 行 MA250 为 null，但这些 bar 截至当日的实际有效历史均少于 250 根，接口正确返回 READY。结果证明分类由实际历史根数动态决定，没有按指数代码、固定日期或旧快照预设；未来回填仍沿用同一规则。

### 10.3 数据库执行计划

样本为 10 code × 5 轮，截止 `2026-08-21`，P95 使用 nearest-rank：

| 查询 | 实返行数 | P50 | P95 | max |
|---|---:|---:|---:|---:|
| factor-only `LIMIT 300` | 300 | 0.261ms | 0.799ms | 0.903ms |
| factor-only `LIMIT 2000` | 464/881 | 1.414ms | 2.326ms | 2.426ms |
| MA 历史基数条件查询 | 164～581 | 0.454ms | 0.718ms | 1.260ms |

300 根走 `(ts_code, trade_date)` 主键反向索引；2000 上限和历史基数查询走同一主键 Bitmap Index/Heap Scan，当前最大 881 行仅做内存排序。无顺序全表扫描或无界排序。

### 10.4 生产 Web 主机同拓扑服务链

| 链路 | 样本 | P50 | P95 | max | JSON 大小 |
|---|---:|---:|---:|---:|---:|
| page-init | 10 code × 5 | 12.060ms | 26.649ms | 80.751ms | 2,205～2,266 B |
| Kline 300 | 10 code × 5 | 14.578ms | 17.373ms | 57.706ms | 164,096～170,956 B |
| Kline `limit=2000` 当前最大历史 | 10 code × 5 | 38.993ms | 78.741ms | 86.947ms | 253,460～496,748 B |

page-init、Kline 300 和当前物理最大窗口均低于既定目标/硬门禁。300 根的十指数全部 READY；最大历史窗口中，`000510.SH`、`899050.BJ` 因返回最早期合理 warm-up 空值而为 PARTIAL，其余八个指数为 READY。生产活服务要求登录，匿名请求为 401；本表不冒充登录态 HTTP 性能。

### 10.5 最终关闭判定

| 门禁 | 最终判定 | 说明 |
|---|---|---|
| 2024 覆盖与动态 MA | PASS | 当前可用历史双向对齐，达到周期后的异常 MA null 为 0 |
| factor-only、历史基数与同拓扑性能 | PASS | 300 根和当前最大 464/881 根均通过 |
| 真实 2000 行 | `N/A — accepted physical limitation` | 当前九个指数最多 881 行、A500 最多 464 行；2000 继续作为 API 参数上限并保留边界测试，未来自然达到后进入常规性能回归，不再作为当前项目未完成项 |
| 权重贡献排序发布 | PASS | `874f647b` 已包含在生产 `57ece8a3`，贡献点数值降序、null 置底与稳定决胜已在线 |
| 九转 M6-D | PASS | 10 code × 5 同拓扑样本 P95 12.795ms；固定日线/15/30/60/90/120 六行已在线；生产指数分钟及分钟九转路由均保持 404 |
| 视觉收口 | PASS（用户验收） | 用户已自行完成当前生产页面视觉检查；本轮未创建新的截图文件，不把历史 `/private/tmp` 临时文件当作当前证据 |
