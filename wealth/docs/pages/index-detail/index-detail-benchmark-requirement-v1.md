...
> 状态：M1–M4 与 M5-A 已完成并通过验证；M5-A 使用真实 Silver K 线与可见开发态 Mock 指标，真实 Gold 对接保留到 M5-B。
...
6. 本地环境在正式 Silver 与 Lake capability 通过门禁后支持 `1/5/15/30/60/90/120` 分钟；M5-A 技术指标使用明确标注的开发态 Mock，不阻塞真实 K 线，正式 Gold 对接保留到 M5-B。
...
15. 五个状态画板共享 TopMarketBar、面包屑和周期工具栏；错误、空数据、权限和部分缺失不得用旧行情或 mock 冒充 Loaded。唯一例外是 M5-A 明确标注“模拟指标”的本地分钟技术图层；页面身份、日线、Silver 分钟 K 线、权重和趋势仍禁止 mock/fallback。
...
12. M5-A 本地七频率均可切换，分钟请求按 code/frequency/date 独立缓存且旧响应不能串标；北证50只显示左侧分钟 EMPTY，切回日线恢复；分钟 Tooltip 只显示冻结的七项字段，并有可见“模拟指标”标识。
