...
> 状态：M1–M4 与 M5-A 已完成；M5-B 正式 Gold 物理覆盖、全历史对齐和性能已通过只读验收，真实指标 provider 切换按冻结合同推进。
...
6. 本地环境在正式 Silver 与 Lake capability 通过门禁后支持 `1/5/15/30/60/90/120` 分钟；M5-B 使用正式 Gold 技术指标，指标失败只使技术图层 PARTIAL，不阻塞真实 K 线，也不得回退 M5-A Mock。
...
15. 五个状态画板共享 TopMarketBar、面包屑和周期工具栏；错误、空数据、权限和部分缺失不得用旧行情或 mock 冒充 Loaded。M5-B 删除 M5-A 的“模拟指标”例外；页面身份、日线、Silver 分钟 K 线、Gold 技术指标、权重和趋势均禁止 mock/fallback。
...
2026-08-13 正式重跑：Silver/Gold technical 七频率各 4,276 个分区，29,932 个频率-日期分区对全历史对齐零失败；Technical/state 共 59,864 个正式文件、10,147,176 行。页面可用九个指数 × 七频率 × 10 次的 630 个默认 500 根样本全部 READY，频率级 P95 为 295.855–324.620ms。代表性指标 DTO 在 7,862 根时为 4,999,968 bytes，7,863 根时触发 5MB 拒绝；该数据只用于证明 5MB 优先门禁，不作为新的公共 limit。
