...
> 状态：指数详情开发与生产验收已完成。M1～M5-B、P10、共享图表、九转 S7/M5/M6-A～M6-D 和日线最终数据/性能门禁均已闭环；bars/indicators 只读正式 Gold，Mock 已清零。本地按 capability 开放七个分钟周期，生产只开放日线且分钟路由保持 404。
...
> 九转专项说明：M1–M5-B 的“九转为空、`supportsNineTurn=false`、不发请求”是九转立项前的历史验收事实，不是当前实现。S7/M5 与 M6-A～M6-D 已完成指数日线及 5/15/30/60/90/120 分钟九转接入、生产发布、自然更新和最终验收；当前 `supportsNineTurn=true`，生产 `nineTurnPeriods=["day"]`，local/dev 在分钟能力就绪时增加 5/15/30/60/90/120，指数 1 分钟始终不支持。
...
1. 展示当前有效源权重批次中的全部 A 股成分，按四位小数的贡献点数值降序排列；贡献点为 `null` 的行置底，同贡献点按 `weight DESC, conCode ASC`，不按贡献绝对值排序。不截断、不提供前端任意 limit；B 股不返回，也不占 coverage。
...
5. 权重列表按贡献点数值降序展示完整 A 股子集，`null` 置底，同贡献点按权重降序、代码升序；表头固定、首屏 10 行、内部滚动可达末行。生产验收的权重批次为 `2026-07-31`，上证响应应为 2184 行且不包含 B 股。
...
### 9.1 2026-08-22 最终生产验收

1. 生产 `dev-interface@57ece8a3` 工作区干净且与远端一致，已包含权重贡献排序 `874f647b`、固定六周期摘要 `673161cf` 和指数详情九转代码。
2. 十指数 factor 最新日均为 2026-08-21、重复键为 0；2024 可用历史与 daily 双向零缺口，MA 空值只按实际有效历史根数动态分类，达到周期后的异常空值为 0。
3. 生产 Web 主机同拓扑 Query/Service/DTO 样本中，page-init P95 26.649ms、Kline 300 P95 17.373ms、`limit=2000` 当前最大 464/881 根 P95 78.741ms，均通过预算。由于正式历史尚无 2000 根，真实 2000 行按 `N/A — accepted physical limitation` 关闭，2000 仍是 API 参数上限。
4. 十指数日线九转 50 次真实服务请求全部 HTTP 200/READY、300 根完全对齐，P95 12.795ms；`000680.SH` 为 404，生产分钟 K 线、指标和九转路由均为 404。
5. 固定日线/15/30/60/90/120 六周期摘要已经上线；用户于 2026-08-22 完成当前生产页面视觉检查并批准收口。本轮不把历史 `/private/tmp` 临时截图冒充为当前持久证据。

## 10. 已接受的历史设计残留

1. Weights/Technical Loaded 根画板已物理归位到 `08` 页面，原节点 ID 和页面尺寸未改变；这是已完成历史事实，不是待评审项。
2. `425:190` 保留的是旧基本行情概述文案，开发和测试永久不得引用该节点。用户已检查并接受这一历史残留，不再把删除或改写它列为本项目待办；未来若重做对应 Figma 说明页，可随该设计任务一并清理。

## 11. 版本记录

| 版本 | 日期 | 变更摘要 | 负责人 |
|---|---|---|---|
| v1.24 | 2026-08-22 | 收口指数详情最终生产验收：登记当前 SHA、2024/dynamic MA、Web-host 同拓扑性能、真实 2000 行 N/A 物理边界、权重排序在线、九转 M6-D、固定六周期和用户视觉验收；Figma 旧文案作为已接受历史残留关闭，不再列为项目待办 | Codex |
| v1.23 | 2026-08-17 | 权重股贡献列表改为按贡献点数值降序、null 置底；同贡献点按权重降序和代码升序，完整批次、十行视窗与滚动合同不变 | Codex |
