...
> 状态：指数详情开发与生产验收已完成。M1～M5-B、P10、共享图表、九转 S7/M5/M6-A～M6-D 和日线最终数据/性能门禁均已闭环；bars 与 indicators 只读正式 Gold，无 Silver fallback 或 Mock。本地按 capability 开放七个分钟周期，生产只开放日线且分钟路由保持 404。
...
3. `technical` 的趋势通道仍仅上证指数展示四个客观轨道值；九转摘要按 capability 独立读取固定 day/15/30/60/90/120 六周期的 `latestMarker`，未开放、源空或无 marker 显示 `--`。技术结论继续显示 `--`，九转不得被解释为交易动作。
...
| M6 最终生产验收（已完成） | prod 日线能力、分钟路由不存在、日线数据/性能、九转自然运行与全链路收口 | 构建/测试/生产 smoke、2024/MA/性能、M6-D 和用户视觉验收通过 |
...
2026-08-15 的生产窄发布与 smoke 确认生产只提供指数日线，指数分钟路由为 404；2026-08-22 当前生产 `57ece8a3` 又完成日线数据/性能、权重排序、九转 M6-D 和固定六周期验收。至此，指数详情从数据合同、Reader/API、前端状态机、共享图表、正式 Gold、性能、视觉到环境隔离均已完成。

技术结论 API 仍不属于本方案，后续需独立立项。九转已由 S7/M5 独立合同接入并完成 M6-A～M6-D，不向指数详情核心日线/分钟 DTO 回填 marker payload。

### 11.1 2026-08-22 最终生产复验

1. 十指数 factor 最新日均为 2026-08-21、重复键为 0；九个指数 881 根，A500 464 根。2024 可用历史与 daily 双向零缺口，达到对应周期后的异常 MA null 为 0。
2. 数据库 factor-only 300/当前最大窗口 P95 为 0.799/2.326ms，MA 历史基数 P95 0.718ms；生产 Web 主机同拓扑 page-init、Kline 300、当前最大窗口 P95 为 26.649/17.373/78.741ms。
3. 正式历史尚无 2000 根，真实 2000 行按 `N/A — accepted physical limitation` 关闭；2000 继续作为 API 参数上限，未来自然达到后进入常规回归。
4. 生产 `57ece8a3` 已包含 `874f647b` 权重贡献排序和 `673161cf` 固定六周期摘要。十指数日线九转 50 次服务请求全部 200/READY、300 根完全对齐，P95 12.795ms；`000680.SH` 为 404，生产分钟 K 线、指标、九转路由均为 404。
5. 用户于 2026-08-22 完成生产页面视觉检查并批准收口。本轮未生成新的持久截图，不把已经不存在的 `/private/tmp` 历史临时文件作为当前证据。
...
| 历史回填会改变 MA 可计算边界 | 把某次 A500 空值前缀固化为 code/date 规则 | 按实际有效历史根数动态判断；2024 同步后的复审已证明达到周期后的异常 null 为 0，不改代码特例 |
...
| 版本 | 日期 | 变更摘要 | 负责人 |
|---|---|---|---|
| v1.25 | 2026-08-22 | 最终收口指数详情：更新生产 SHA、2024/动态 MA、数据库与 Web-host 同拓扑性能；真实 2000 行按物理限制 N/A 关闭；同步权重排序、九转 M6-D、固定六周期与用户视觉验收 | Codex |
