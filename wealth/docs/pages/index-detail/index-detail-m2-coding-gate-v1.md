...
7. [x] 九转首期为空的历史边界已由独立专项升级；当前按 `nineTurnPeriods` 调用独立 API，不进入指数详情核心 DTO。
...
12. [x] 指数分钟后端、前端、架构/产品门禁均已完成；生产只日线、本地按 capability 开放分钟的边界不再待评审。
...
5. [x] 技术结论显示 `--`；九转按 capability/API 显示客观 marker，未开放、源空或无 marker 时为 `--`，不引用 Figma 示例文案。
...
1. [x] 技术结论和交易流程未扩散；九转只通过已评审的独立合同接入，不扩展指数详情核心 DTO。
