...
> 状态：首版已提交 `d143dec4`；五项展示修正开发与自测完成；本次未提交、未部署
>
> 日期：2026-09-03
>
...
本次用户明确修订按钮居中、价格着色、估值拆列及资金显示单位；这些口径覆盖首版 Figma 的对应细节。本轮只改 watchlist 前端展示、测试及三份页面文档，不改 API、持久化、行情来源、其它页面或 Figma。执行与验证见 LLD 第 18 节。
...
| 表头承担展示单位，单元格只显示数值 | 成交量按万手、资金净流入按千万、涨跌幅与换手率按百分比格式化；不得在值后重复单位 |
...
源单位：`price=元`、`changePct=%`、`vol=手`、`netAmount=万元`、`turnoverRate=%`。后端只返回数值和结构化方向；前端将 `vol / 10000` 显示为万手、`netAmount / 1000` 显示为千万，并把单位放在表头而不是单元格。此次仅调整展示换算，API 原始单位不变。
...
7. 可见列顺序固定为：股票代码、股票名称、最新价（元）、涨跌幅（%）、成交量（万手）、市盈率（PE TTM）、市净率（PB）、量比、换手率（%）、资金净流入（千万）、所属板块、操作，共 12 列。
8. PE/PB 为两个单项数值列，不添加 `PE`/`PB` 前缀；所属板块标签在行内垂直居中；最新价与涨跌幅共享行情色。
