...
按钮居中、价格着色和估值拆列已在 `2b6da8e2` 提交。资金显示单位从千万改为万；所属板块采用用户选定的 A 金色细边。改动限定于 watchlist 金额格式化、表头、板块标签、对应测试和三份页面文档；不改 API、持久化、行情来源、其它页面、全局 token 或 Figma。执行与验证见 LLD 第 19、20 节。
...
| 数据列表头与对应单元格内容左对齐；操作/搜索状态列居中 | 数据列复用同一列宽及左侧内边距；按钮格及按钮内容双轴居中，不以单侧 padding 偏移文案 |
...
| 表头承担展示单位，单元格只显示数值 | 成交量按万手、资金净流入按万、涨跌幅与换手率按百分比格式化；不得在值后重复单位 |
| PE、PB、量比、换手率各自成列 | 禁止估值组合列和“活跃度”组合列；PE/PB 每格只有单项数值，不带字段前缀 |
| 最新价与涨跌幅同色 | 复用 `quote.direction` 对应的行情 token；价格缺失保持中性占位，不沿用资金方向 |
...
以下是编码前的事实，已由本轮 D1—D4 逐项实现，不再表示当前代码状态。交付证据见 [LLD 第 17 节](./watchlist-low-level-design-v1.md#17-开发交付记录2026-09-03)。
...
分页日期变化处理（用户已确认）：后续分页或添加后的尾部补读若返回不同的 `observedTradeDate`，前端丢弃该批次，不与现有行合并；重新请求首批并回到列表顶部，继续按新日期分页。不扩展 API 日期参数或写接口响应。首次无行情日期、后续出现日期也按日期变化处理。
...
源单位：`price=元`、`changePct=%`、`vol=手`、`netAmount=万元`、`turnoverRate=%`。后端只返回数值和结构化方向；前端将 `vol / 10000` 显示为万手，`netAmount` 不缩放，按“万”保留两位小数和正负号；单位只放在表头。此次取消上一轮资金 `/1000` 展示换算，API 原始单位不变。
...
7. 可见列顺序固定为：股票代码、股票名称、最新价（元）、涨跌幅（%）、成交量（万手）、市盈率（PE TTM）、市净率（PB）、量比、换手率（%）、资金净流入（万）、所属板块、操作，共 12 列。
8. PE/PB 为两个单项数值列，不添加 `PE`/`PB` 前缀；所属板块采用 A 金色细边，复用 `brand-weak` / `border-strong` / `brand`，高 24px、字 12px、横向内距 8px、圆角 4px。文字左边界保持与表头一致，行内垂直居中；长名称单行省略并保留完整 title，缺失值无标签装饰。不增加标签动作或改变行导航。最新价与涨跌幅共享行情色。
9. 数据列的表头和内容保持相同左边界；操作列与搜索状态列按最新指令居中，按钮内容同样居中。移除按钮保留系统暗底、描边、危险操作文字及 hover/disabled/focus 行为。
...
6. 添加成功后原位更新状态，对话框不关闭。全部页已加载时，以已读最大 id 做有界尾部补读；仍有未加载页时只更新服务端数量，新行随后续分页出现在全局末尾，不提前插入当前批末尾。
...
8. 数据列表头和表体内容左边界一致且均左对齐；操作/搜索状态列及按钮内容居中。
...
5. D4：股票详情“+自选”、契约清理、全量回归和浏览器开发自测；部署及正式验收由用户执行。

每个阶段只推进自选功能，不顺手修改其它首页卡片或详情动作。

2026-09-03：D0—D4 已完成。新增迁移 `20260903_000169` 接真实 head `20260831_000168`；本轮只在新建隔离 PostgreSQL 中执行该迁移和合成样本写入，未操作既有数据库。后端定向回归 60 项、Wealth 前端全量 610 项通过；性能、真实 API 展示、逐条代码/测试映射和用户后续部署边界统一记录于 LLD 第 17 节。无业务范围或已冻结 API 的扩展。
...
| v1 展示修正 | 2026-09-03 | 修正按钮居中，最新价使用涨跌色，PE/PB 拆列，资金显示单位为千万；API 原始单位不变 | Codex |
| v1 单位与标签评审 | 2026-09-03 | 资金显示单位改为万、取消缩放；标签三方案仅供选择，正式 CSS 不变 | Codex |
| v1 标签 A 定稿 | 2026-09-03 | 用户选择 A 金色细边；限定 feature 样式，覆盖长名和缺值，不改全局 token 或 API | 用户 / Codex |
