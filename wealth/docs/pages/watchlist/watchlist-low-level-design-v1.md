...
> 状态：第 18 节修正已提交 `2b6da8e2`；第 19 节资金单位万与第 20 节用户选定的 A 金色细边标签已完成开发自测；本轮未提交、未部署
>
> 日期：2026-09-03
>
...
按钮/状态格居中、最新价跟随涨跌色、PE/PB 拆列继续沿用第 18 节已提交实现；资金单位按第 19 节改为万。所属板块按第 20 节用户选定的 A 金色细边实现。本轮不修改 Figma；其余布局、搜索、增删、滚动和详情交互不变。
...
12. 表头承担单位；成交量以万手、资金净流入以万、涨跌幅与换手率以百分比展示，单元格不重复单位。
...
10. 表头文案严格为：股票代码、股票名称、最新价（元）、涨跌幅（%）、成交量（万手）、市盈率（PE TTM）、市净率（PB）、量比、换手率（%）、资金净流入（万）、所属板块、操作，共 12 列。
11. `vol` 源单位手，显示值为 `vol / 10000`；`netAmount` 源单位万元，不再缩放，保留两位小数和正负号；表体不追加“万手”“万”或 `%`。
12. `peTtm` 与 `pb` 各占一个独立单元格，只显示单行格式化数值；删除旧 `valuation-column` 与两行布局，不保留兼容组合列。
13. `.sector-column` 内标签按第 20 节 A 金色细边实现，使用 flex 垂直居中，标签文字保持与表头左边界一致；不得使用水平居中覆盖整列左对齐合同。长名省略但保留完整 title，缺失显示无装饰 `--`。
...
| v1 单位与标签评审 | 2026-09-03 | 资金单位改为万并完成专项验证；标签三方案等待用户选择，全量回归的非自选失败如实记录 | Codex |
| v1 标签 A 定稿 | 2026-09-03 | 用户选择 A 金色细边；实现范围、样式合同、长名/缺值与验证落点见第 20 节 | 用户 / Codex |
...
本节为已提交 `2b6da8e2` 的历史开发记录；资金“千万”已被第 19 节“万”取代，其余展示修正保留。本节“本次未提交”等说明仅描述当时开发交付状态。
...
## 19. 资金单位改为万与标签备选（2026-09-03）

本节保留用户选型前的开发与评审记录；“三方案待选择”和“正式标签 CSS 不变”仅描述当时状态，后续选型与实现以第 20 节为准。

### 19.1 冻结范围与代码落点

1. `WatchlistTable` 表头改为“资金净流入（万）”；`buildWatchlistRow` 的 `netAmount` 使用原数值格式化，不除以 1000 或 10000。正负号、两位小数、缺失 `--` 和独立资金方向不变。成交量仍除以 10000 展示万手。
2. API DTO、查询和原始万元单位不变；不改分页、增删、详情、列宽、冻结边界和正式标签 CSS。
3. adapter 测试覆盖正、负、零、缺失、小数及大金额；页面与真实 API 浏览器 smoke 同步检查“万”表头和 `2189.4 → +2189.40`，防止仅改表头或继续错误缩放。

### 19.2 标签设计评审边界

目标是“所属板块”的分类标签，不是按钮或数据状态；三套方案仅在对话预览中比较，不新增点击、选中、发光或行业随机配色，不提前落到正式页面。全部使用当前 `design-tokens.css` 的背景、文字、品牌、边框、间距及圆角 token；颜色不复用行情红绿。

| 待选方案 | 视觉 | token 使用 |
|---|---|---|
| A 金色细边 | 轻金底、细金框、金色文字；识别最强 | `brand-weak` / `border-strong` / `brand` |
| B 冷灰胶囊 | 暗蓝灰底、灰边、亮灰文字；最克制 | `bg-chip` / `border-default` / `text-primary` |
| C 金色侧签 | 浅金暗底、单侧金线、中性文字；建议优先看此方案 | `brand-weak` / `brand` / `text-primary` |

备选按 24px 高、12px 中文、8px 横向间距与现有 60px 行高对比，正文左对齐、行内垂直居中。长名称保持单行；缺失值显示无装饰 `--`，不伪造板块。这里只记录设计提议，不把建议当作已确认规范；用户选择后再同步正式标签与相关测试。

### 19.3 本轮验证

1. 资金单位修改完成：自选相关 **6 文件 / 44 项测试通过**，涵盖 `0.01 → +0.01`、`2189.4 → +2189.40`、`-2189.4 → -2189.40`、大金额、零值和缺值；原成交量、PE/PB、方向及交互断言保留。执行命令：`cd wealth && npm run test -- src/features/watchlist src/pages/watchlist src/features/wealth-exploration/sector-analysis/price-volume/ui/PriceVolumeWorkspace.test.tsx`，其中另有 13 项量价模块复核，合计 57 项通过。
2. `npm run typecheck`、`npm run build` 通过；构建保留已有大于 500KB 的 chunk 提醒（约 955.18KB，gzip 284.63KB）。文档完整性与差异检查通过。
3. 隔离真实 API 浏览器 smoke 通过：表头“资金净流入（万）”、正负原值、12 列、100→200 行分页、置尾添加和移除、两处按钮居中、默认日 K 均保持正确；无主流程 console/pageerror。通过第 17.2 节相同 fixture 与脚本复现，本次证据位于 `/private/tmp/wealth-watchlist-wan-review.OHMBGA/`。只操作新建隔离测试库，测试服务及其 PostgreSQL 已停止，未操作生产或既有数据库。
4. 标签预览按现有 token 绘制，736 / 360 / 320px 宽度下无横向溢出；标签 24px 高、行内中心误差 0.5px。长名称与行悬停预览切换通过，浏览器无脚本错误；已人工查看并排效果。正式 `watchlist.css` 和全局 token 未修改，三方案仍待用户选择。
5. 两次默认全量回归均为 **630 passed / 1 failed**，不可标为全量通过：首次是 `PriceVolumeWorkspace.test.tsx:34` 在列表出现后立即断言 Details 请求次数；第二次是 `market-overview-sector-overview-real-api.test.tsx:503` 在骨架出现后立即断言 FORBIDDEN 文案。已读相关代码并分别复跑：前者 **13 passed**，后者 **36 passed**。现象与异步状态等待窗口有关，但未在本轮做完整根因定论或修改这两个无关模块；保留为回归稳定性风险。
6. CodeGraph 使用 `codegraph_explore` 核验表格与 adapter 的局部影响面，开发后 sync/status 正常；未改共享契约或依赖矩阵。三份 watchlist 文档同步最新单位，保留第 18 节历史记录；所有无关工作区改动保留。

本轮资金单位开发与专项自测完成，标签仅交付备选。未提交、未推送、未部署；下一步是用户选择标签方案，再按该方案修改正式标签。

## 20. 用户选定 A 金色细边标签（2026-09-03）

### 20.1 范围与实现合同

用户明确选择 A，B/C 不采用。依据第 19 节评审预览，只替换自选列表“所属板块”标签，不扩散到其它标签，不改变 API、行导航、列宽、冻结边界、资金单位或全局 token。

| 硬口径 | 代码落点 | 验证落点 |
|---|---|---|
| A 轻金底、细金框、金色文字 | `watchlist.css`：`brand-weak` / `border-strong` / `brand`，1px 内描边；24px 高、12px 字号、8px 横向内距、4px 圆角 | 浏览器 computed style 检查及截图 |
| 与表头文字左对齐、60px 行内垂直居中 | 保留标签内距对应的负 margin；不改 table / sticky 布局 | 浏览器测量文字左边界、行内中心、横滚冻结边界 |
| 长名不溢出；完整名称可读取 | 标签单行省略，`WatchlistTable` 保留完整文本及原生 `title` | 页面测试；隔离真实 API fixture 的长名样本及浏览器尺寸断言 |
| 缺失无金色标签、不伪造行业 | `WatchlistTable` 对 `--` 使用独立中性文本样式 | 页面测试与隔离真实 API fixture 的空行业样本 |
| 不新增标签按钮或交互状态 | 标签仍为 span，沿用所属行导航；无独立 hover/选中效果 | 页面测试点击长名仍跳股票详情；浏览器检查悬停样式 |

真实浏览器边界样本只在 `tests/wealth_watchlist_browser_fixture.py` 新建隔离数据库中设置 `000008.SZ` 的长行业名和 `000009.SZ` 的空行业，不修改共享 API 测试样本或任何既有数据库。

### 20.2 开发验证

1. `cd wealth && npm run test -- src/features/watchlist src/pages/watchlist`：**6 文件 / 46 项通过**，新增真实标签、长名称完整文本与 title、缺失值无装饰及不新增标签动作的断言。
2. `cd wealth && npm run typecheck && npm run test && npm run build` 对应三项命令均通过；本轮默认全量为 **94 文件 / 633 项通过**。第 19.3 节两次历史异步失败仍保留，不宣称已修复其稳定性问题。构建仍有已有大于 500KB 的 chunk 提醒（955.28KB，gzip 284.66KB），未扩展到打包优化。
3. 隔离真实 API 浏览器 smoke 通过并已人工查看 `watchlist-tag-a.png`：金色文字、10% 轻金底、56% 金框、24px 高 / 12px 字号 / 4px 圆角均与 A 一致；表头与标签文字左边界误差 0px，垂直中心误差 0.5px。长名宽 118px、内容宽 136px，正确省略且不侵入操作列；空行业为中性无装饰 `--`。
4. 原有核心链路继续通过：资金单位万及正负原值、12 列和冻结边界、100→200 行加载、置尾添加/移除、按钮居中、首页真实数量和详情默认日 K。首 100 行可用 177ms，横纵滚动无长任务，主流程 console/pageerror 为空。浏览器命令：`node wealth/scripts/watchlist-browser-smoke.mjs <fixture-url> <evidence-dir> <playwright-module>`，fixture 启动方式见第 17.2 节；本轮证据目录 `/private/tmp/wealth-watchlist-tag-a.ewfIaH/`。
5. 只操作本轮新建隔离测试库；测试服务及其 PostgreSQL 已停止，不涉及生产数据或既有数据库。三份 watchlist 文档同步用户选型；文档完整性检查和 `git diff --check` 通过。
6. CodeGraph `codegraph_explore` 核验 `WatchlistTable`、页面、adapter 与测试消费者；开发后 `codegraph sync` / `codegraph status` 正常。不改变 API、全局 token、共享组件、子系统边界或依赖矩阵，所有无关改动保留。

本轮开发和自测完成，未提交、未推送、未部署；后续提交等待用户指令，部署及正式验收由用户执行。
