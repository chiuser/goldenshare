...
> 状态：第一阶段后端已实现，开发自测与边界见 LLD 第 22 节，待用户阶段验收；前端未开发，未提交、未部署、未迁移正式数据库
>
...
两轮修订对账见 LLD 第 21.1～21.2 节，名称依赖准入见第 21.3 节。随后用户授权按计划完成第一阶段后端：现工作区为 v2 后端，前端仍为 v1，不能单独发布。逐条实现、测试、隔离 PostgreSQL 迁移及未执行项集中记录在 LLD 第 22 节；本文不代表完整产品已验收或正式库已升级。
...
搜索 groupId 合同已统一到第 5.4 节，无新增产品拍板项。第一阶段已获开发授权并实现；正式 Policy、API 与隔离数据库验证见 LLD 第 22 节，前端及完整页面验收仍留给后续阶段。其它依赖安装/升级仍须另行报批，不能扩大环境变更范围。
