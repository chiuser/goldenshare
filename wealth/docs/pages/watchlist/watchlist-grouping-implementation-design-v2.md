...
> 状态：十项评审修订已获用户确认并回填；尚未进入开发、数据库迁移、部署或验收
>
...
>
> 当前实现基线：[我的自选技术实施方案 v1](./watchlist-implementation-design-v1.md)
>
...
本文只形成实现方案，不修改业务代码、API、数据库和运行环境。用户本轮确认十项修订并要求修改、提交文档；本次授权不包含业务编码、依赖安装或生产操作。十项修订对账见 LLD 第 21.1 节。
...
| `name_key` | text | not null | 用于用户内重名约束的规范化键，不直接展示 |
...
1. `(user_id, name_key)` 唯一，名称提交统一 NFC 后 trim，以规范值生成重名键。
2. 每个用户通过部分唯一索引保证最多一个 `is_default=true` 的分组。
3. 默认组固定名称“我的自选”、`color is null`。
4. 自定义组 `name_key` 不能等于“我的自选”的规范化键，且 `color` 必须属于固定八色。
...
4. move/add-to-groups 依赖唯一约束幂等，不重置目标组已有成员的顺序或置顶。
...
2. Items Controller：当前组行情、排序、游标加载、日期一致性和写后重载。
3. Edit Controller：展示/编辑模式、已加载成员选择、动作弹层和提交锁。
...
14. 八个数值列均覆盖首次降序、升序、置顶分区、空值后置、同值 tie-break 和跨页无重复无遗漏。
...
4. 多页排序无重复、无遗漏；请求失败没有假成功或 mock 数据。
...
1. M0：PRD、交互稿、代码审计、技术方案和 LLD；当前十项评审修订已获确认并落档，本次只提交文档。
...
用户已确认全部十项修订，无新增产品拍板项。其中基础排序从时间戳改为成员 ID 是明确批准的技术调整，并已同步默认排序、同值排序、游标、迁移对账和 PRD 加入顺序措辞。

后续开工仍需开发授权和实际环境门禁。名称字素簇库不能用简化字符计数替代；如需要安装依赖须先按仓库规则报批。未完成依赖版本与目标浏览器一致性验证前，不声称该项已经实现或通过验收。
