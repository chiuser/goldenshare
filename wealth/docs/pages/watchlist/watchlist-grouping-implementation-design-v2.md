...
> 状态：后端 `a7840e0a`、前端 `532a7b13` 已提交；最新真实接口联调与技术验证见 LLD 第 24 节。部署、全部交互测试及验收由用户负责；未部署、未迁移正式数据库
>
...
两轮修订对账见 LLD 第 21.1～21.2 节，名称依赖准入见第 21.3 节。后端实现与隔离 PostgreSQL 验证记录在第 22 节，前端实现与开发自测记录在第 23 节。运行时代码已统一为 v2，提交后真实接口联调及用户确认的新责任边界见第 24 节；交互验收和部署由用户负责，本文不代表完整产品已验收或正式库已升级。
...
搜索 groupId 合同已统一到第 5.4 节，无新增产品拍板项。后端和前端已分别获开发授权并提交；开发自测见 LLD 第 22～23 节，真实接口联调见第 24 节。完整页面与部署验收由用户负责。其它依赖安装/升级仍须另行报批，不能扩大环境变更范围。
