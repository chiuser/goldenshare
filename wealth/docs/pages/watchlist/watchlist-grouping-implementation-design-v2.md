...
> 状态：前后端及联调已提交；用户已部署并反馈初步验证符合预期。部署后只读核验正常，正式库为 `20260907_000170`，证据见 LLD 第 25 节；最终交互验收仍由用户负责
>
...
两轮修订对账见 LLD 第 21.1～21.2 节，名称依赖准入见第 21.3 节。后端实现与隔离 PostgreSQL 验证记录在第 22 节，前端实现与开发自测记录在第 23 节，真实接口联调见第 24 节。用户随后完成部署，正式库版本、运行依赖及数据只读核验见第 25 节；当前为已部署、用户初步验证符合预期，不能据此宣称完整产品已验收。
...
责任边界（前端提交 `532a7b13` 后用户确认）：Codex 完成隔离环境中的真实前后端接口联调和技术验证，到需要部署时停止；部署、正式数据库迁移、全部浏览器交互测试及交互验收由用户负责。LLD 第 18 节为调整后的执行安排，不再要求 Codex 升级或运行浏览器 smoke。
...
4. 开发阶段三：Codex 完成正式前端客户端与真实后端 HTTP 联调、隔离 PostgreSQL 迁移/性能验证、相关回归和文档对账，不执行浏览器交互测试。
5. 发布与交互验收：由用户负责；Codex 到需要部署时停止。任何开发阶段都不构成后端单独发布许可。
...
搜索 groupId 合同已统一到第 5.4 节，无新增产品拍板项。后端和前端已分别获开发授权并提交；开发自测见 LLD 第 22～23 节，真实接口联调见第 24 节。用户已部署并初步验证，部署后只读核验见第 25 节；最终交互验收仍由用户负责。其它依赖安装/升级仍须另行报批，不能扩大环境变更范围。

## 15. 版本记录

| 版本 | 日期 | 变更摘要 | 负责人 |
|---|---|---|---|
| v2.8 | 2026-09-09 | 同步已部署、用户初步验证及正式库只读核验状态，证据集中引用 LLD 第 25 节；不改变技术方案或验收责任 | 用户 / Codex |
| v2.7 | 2026-09-08 | 记录前端已提交和真实接口联调；按用户新分工将部署、全部交互测试与验收交用户，证据见 LLD 第 24 节 | 用户 / Codex |
