...
4. 结果编排：
   ...
   - 当前 UI 支持 `涨跌分布` 与 `涨跌家数` 两种图表模式；
   - `涨跌分布` 位于左侧并默认选中，使用当日分桶事实渲染柱状图，不展示参考图底部汇总文字；
   - `涨跌分布` 柱状图横轴标签只展示区间与 `平盘`，不展示 `涨` / `跌` 字样；
   - `涨跌分布` 柱状图数值贴近对应柱顶展示，距离柱顶约 `10px`，不固定排列在图表顶部；
   - `涨跌家数` 位于右侧，复用 1 个月上涨/下跌双线。
...
1. 单元测试：
   ...
   - `1m/3m` 点数固定；
   - `涨跌分布` 模式默认选中，消费当日 `distributionBuckets + flatCount` 渲染分桶柱状图；
   - `涨跌家数` 模式只消费 up/down；
...
3. 冒烟验证：
   - 返回结构稳定；
   - 前端切换按钮文案与顺序为 `涨跌分布/涨跌家数`，不得继续显示 `1个月/3个月`；
   - `涨跌分布` 默认选中，分桶柱状图可渲染，柱状图顺序固定为：左侧下跌分桶 `>10%`、`7~10%`、`5~7%`、`3~5%`、`0~3%`，中间 `平盘`，右侧上涨分桶 `0~3%`、`3~5%`、`5~7%`、`7~10%`、`>10%`；
   - `涨跌分布` 横轴标签不得展示 `涨` / `跌` 字样；
   - `涨跌分布` 分桶数值必须贴近对应柱顶约 `10px`；
   - `涨跌家数` 双趋势线（上涨/下跌）可渲染；
   - `涨跌分布` 不展示参考图底部汇总文字；
...
1. 本模块无配置能力，不接策略配置中心。
2. 仅支持 `涨跌分布/涨跌家数` 两种图表模式，默认展示 `涨跌分布`；`1m/3m` 只保留为 API 历史数据键，不作为按钮文案。
...
| v1.4 | 2026-06-06 | 对齐生产 fact 表新 schema：`>7%` 拆为 `7~10%` 与 `>10%`；新增 `涨跌分布/涨跌家数` 图表模式切换方案，默认展示分桶柱状图 | Codex |
