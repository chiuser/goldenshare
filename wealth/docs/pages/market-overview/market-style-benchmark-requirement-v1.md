...
5. 真实源优先：接入真实 API 后，数据未返回前显示 `loading`，不得展示 mock 数据冒充 ready。
6. 超时显式失败：请求超过 5 秒必须进入 `error`，不得静默回退 mock。
7. 模块级渐进替换：本轮只允许 `marketStyle` 模块 source 从 `mock` 切到 `real`，其他模块 source 不变。
...
5. 行为验收：真实源请求 pending 显示 loading；超过 5 秒显示 error；两者都不得回填 mock。
6. 范围验收：本轮仅 `marketStyle` 模块 source 变化，其余模块 source 保持不变。
7. 展示验收：不新增常驻图下注释（除非后续需求文档明确新增）。
...
| v1.1 | 2026-05-09 | 对齐通用 checklist：补充真实源加载态、5 秒超时、单模块切换纪律与说明文案约束 | Codex |
