...
> 状态：双来源新闻、阅读器及 2026-08-27 确认的新闻速览标题提取增量均已开发完成，待用户部署与页面验收。
> 日期：2026-08-24；标题提取增量确认日期：2026-08-27。
...
7. `新闻速览` 的源标题若以完整的 `【...】` 开头，列表和阅读器标题只展示第一组括号内的非空文本，不展示右括号后的摘要正文。
...
8. 标题提取只作用于 `contentSource=news` 的新闻速览，不改变新闻通讯、股票详情新闻或数据库中的原始 `title`。
...
3. 正文：trim 后非空。
4. 不再读取或判断 `channels`。
5. 展示标题：按 7.1.1 的统一规则处理；未命中开头 `【...】` 时继续优先使用 trim 后非空 `title`，标题为空时取 trim 后 `content` 前 80 字。
...
仅对 `core_serving_light.news.title` 使用以下确定性规则：

1. 先对原始 `title` 去除首尾空白。
2. 只有标题以全角左方头括号 `【` 开头、并且后续存在第一个全角右方头括号 `】` 时，才进入提取分支。
3. 取第一组 `【` 与 `】` 之间的文本并再次去除首尾空白；结果非空时，它就是最终展示标题，右书名号后的摘要、来源提示或其它文本全部丢弃。
4. 缺少右书名号、括号内容为空、标题不是以 `【` 开头时，不做猜测，继续使用原有标题展示逻辑。
5. 原始标题为空时，继续使用现有正文前 80 字 fallback；不得从正文中的 `【...】` 反向提取标题。
6. 标题提取必须发生在同来源标题去重之前。因此 `【同一标题】摘要 A` 和 `【同一标题】摘要 B` 视为同一个最终展示标题，只保留发布时间最新的新闻。
...
11. 缺少右书名号、空括号、非开头括号和无标题正文 fallback 均保持既定回退语义。
12. 两条原始标题尾部摘要不同、但第一组 `【...】` 相同的新闻按提取后的展示标题去重，只保留最新一条。
13. 新闻通讯标题不应用该提取规则。
