...
2. 图表模式：右上角只提供 `涨跌分布` 与 `涨跌家数` 两个切换按钮；左侧为 `涨跌分布`，右侧为 `涨跌家数`。
3. `涨跌分布`：默认选中，使用当日 `distributionBuckets + flatCount` 渲染分桶柱状图，仅展示柱状图本体，不展示底部“涨跌 / 涨停 / 跌停”等汇总文字。
4. 分桶图标签：横轴标签只展示区间与 `平盘`，不得展示 `涨` / `跌` 字样；涨跌方向由左右位置和红绿柱表达。
5. 分桶图数值：每个分桶数值贴近对应柱子顶部展示，距离柱顶约 `10px`，不得统一固定在图表顶部。
6. `涨跌家数`：复用 API `historyByRange["1m"]` 上涨家数 / 下跌家数双折线图，不改变折线图视觉。
7. API 契约：后端返回当日与历史点的完整市场宽度事实字段，包括涨跌幅分桶字段。
8. 前端接收：前端 DTO 必须接收分桶字段；当前 UI 默认在 `涨跌分布` 模式使用当日分桶字段渲染柱状图，切到 `涨跌家数` 模式后使用 up/down 折线。
9. 模块级 debug 状态（仅调试模式）。

### 2.2 本期不覆盖

1. 不做可配置时间范围；API 内部仍保留 `1m/3m` 历史数据键，但它们不再作为 breadth 面板按钮文案。
2. 不展示参考图底部“涨跌 / 涨停 / 跌停”等汇总文字。
3. 不做跨模块样式/交互改造。
...
2. 固定图表模式：仅支持 `涨跌分布` 与 `涨跌家数`，默认展示 `涨跌分布`。
...
5. 历史趋势展示：`涨跌家数` 模式仍只画 1 个月上涨/下跌两条线；历史点返回的平盘、总数、红盘率和分桶字段继续保留为 API 完整事实，不作为本期图表输入。
6. 分桶柱状图展示顺序固定：左侧下跌分桶 `>10%`、`7~10%`、`5~7%`、`3~5%`、`0~3%`，中间 `平盘`，右侧上涨分桶 `0~3%`、`3~5%`、`5~7%`、`7~10%`、`>10%`；横轴展示标签不得带 `涨` / `跌` 字样。其中 `平盘` 使用 `flatCount`，其余使用 `distributionBuckets`。
7. 分桶柱状图每个数值贴近对应柱顶展示，距离柱顶约 `10px`，不得固定排列在图表顶部。
8. 分桶比例不作为 API 独立字段；如前端展示比例，只能基于 `bucketCount / totalCount` 派生，不允许后端和前端各自定义不同口径。
...
1. 功能验收：返回当日 3 指标 + `1m/3m` 历史数据；UI 按 `涨跌分布/涨跌家数` 两种图表模式切换，默认展示 `涨跌分布`。
...
11. 分桶横轴标签不得展示 `涨` / `跌` 字样，分桶数值必须贴近对应柱顶约 `10px`。
...
| v1.3 | 2026-06-05 | 数据源口径切到 ClickHouse 市场宽度事实表；分桶字段先纳入契约 | Codex |
| v1.4 | 2026-06-06 | 对齐生产 fact 表新 schema：`>7%` 拆为 `7~10%` 与 `>10%`；按钮改为 `涨跌分布/涨跌家数`，默认展示分桶柱状图 | Codex |
