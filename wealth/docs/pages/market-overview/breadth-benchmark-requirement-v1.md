...
3. `涨跌家数`：复用 API `historyByRange["1m"]` 上涨家数 / 下跌家数双折线图，不改变折线图视觉。
4. `涨跌分布`：使用当日 `distributionBuckets + flatCount` 渲染分桶柱状图，仅展示柱状图本体，不展示底部“涨跌 / 涨停 / 跌停”等汇总文字。
5. API 契约：后端返回当日与历史点的完整市场宽度事实字段，包括涨跌幅分桶字段。
6. 前端接收：前端 DTO 必须接收分桶字段；当前 UI 在 `涨跌家数` 模式使用 up/down 折线，在 `涨跌分布` 模式使用当日分桶字段渲染柱状图。
7. 模块级 debug 状态（仅调试模式）。
