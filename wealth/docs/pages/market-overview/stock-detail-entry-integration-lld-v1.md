...
| M4：连板天梯接入 | 已落地 | `wealth/src/features/market-overview/limit-up/StreakLadderPanel.tsx`、`wealth/src/pages/market-overview/MarketOverviewPage.tsx`、`wealth/src/pages/market-overview/MarketOverviewPage.test.tsx` | 股票卡片调用 `onStockSelect(stock.stockCode)`；停牌/缺行情展示态不拦截跳转；展开按钮 `stopPropagation` 并测试不改变路径 |
| M5：涨跌停统计与分布接入 | 已落地 | `wealth/src/features/market-overview/limit-up/LimitBoardPanel.tsx`、`wealth/src/pages/market-overview/MarketOverviewPage.tsx`、`wealth/src/pages/market-overview/MarketOverviewPage.test.tsx` | `LeaderPerformanceRow` 整行调用 `onStockSelect(stock.stockCode)`；板块分布行只保留 hover 切换；页面测试覆盖领涨股行跳转 |
| M6：边界与负向门禁 | 已落地 | `wealth/src/pages/market-overview/MarketOverviewPage.test.tsx` | 页面测试覆盖主要指数、板块排行、板块热力图、个股新闻点击后不进入 `/wealth/market/stock/*`；股票详情入口继续限定为榜单、连板天梯、涨跌停领涨股三类 |
| M7：最终对账与回归 | 已落地 | `wealth/src/pages/market-overview/MarketOverviewPage.test.tsx`、计划内模块文件 | `cd wealth && npm run typecheck`、`cd wealth && npm run test`、`cd wealth && npm run build` 均通过；旧 `onAction` 字符串式股票详情跳转残留检查无结果 |
