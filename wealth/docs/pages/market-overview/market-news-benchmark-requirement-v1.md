...
> 2026-08-23 增量说明：本文保留新闻列表首版业务基线；其中“item 不可点击”“不打开内容”的条款已由 [新闻弹窗阅读器技术实施方案 v1](./market-news-reader-implementation-design-v1.md) 替代。当前代码在阅读器开发完成前仍是不可点击状态，后续实现与验收以增量方案为准。
...
3. 滚动行为：
   ...
   - 滚动速度必须按候选新闻条数动态放慢：最少 `40s` 一轮，默认按每条约 `2s` 推进，避免 `queryLimit=300` 时高速扫屏。
4. 自动刷新行为：
   - 新闻速览和个股新闻每 10 分钟各自重新请求一次后端接口；
   - 自动刷新只影响新闻模块，不触发整页刷新，也不影响其它市场总览模块；
   - 刷新过程中保留当前列表，不切换到 loading，不清空内容，不产生闪烁；
   - 刷新成功后替换当前列表，并从最新新闻开始重新滚动；
   - 刷新失败时保留旧列表，不把当前板块切到 error。
...
6. 如果 10 分钟自动刷新时出现 loading、清空列表或整页刷新，说明局部静默刷新失效，验收必须失败。
