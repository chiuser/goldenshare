...
行业、概念和地域三个关键画板基线均为 `1564 × 680`。Slice 9 已将概念 Heat 历史冻结为 20 个已发布交易日槽位，移除三个工作台的板块详情路由入口，并补齐正式交互与数据契约；修改前后截图和节点属性保存在 `figma-pixel-artifacts/20260813-sector-overview-v2-slice9/`。Web 落地时允许板块速览模块高度增加，但不得改变首页其它模块、左右栏和页面主宽度。

### 1.3 已确认产品结论

1. 默认 Tab 为“行业”，可切换到“概念”或“地域”；三个视图各自保留排序与选择状态。
2. 行业展示一级、二级、三级三列联动排名；每列固定 5 个可见排名项。
3. 行业排名维度固定为：`涨幅榜`、`跌幅榜`、`主力净流入`、`上涨家数`；涨跌幅不得再合并成单一页签。
...
10. Heat Model EOD V1 是既有首发口径；有效行情补入 `close` 后，修正版固定为 Heat Model EOD V2 / `concept-heat-eod-v2`，必须完成新版本生产回放，不能沿用 v1 行作为完成证据。
...
5. 概念 Heat Model EOD V2、热度等级、日度变化和趋势标签。
...
1. 不建设板块详情页；行业、概念、地域板块名称本期只承担选择与联动，正式 Figma 不保留“进入××行情”入口。后续若建设路由必须重新产品评审和设计，不得从本版实现推断。
...
| `scoreVersion` | 版本 | 否 | 修正版固定 `concept-heat-eod-v2`；v1 仅为既有历史 |
...
| 排名维度 | 主排序 | 空值规则 |
|---|---|---|
| `CHANGE_PCT_UP`（涨幅榜） | `changePct desc` | 空值不入 Top5 |
| `CHANGE_PCT_DOWN`（跌幅榜） | `changePct asc` | 空值不入 Top5 |
| `MAIN_NET_INFLOW` | `mainNetInflow desc` | 空值不入 Top5 |
| `UP_COUNT` | `upCount desc` | 空值不入 Top5 |

涨幅榜与跌幅榜是两个独立排序页签，默认进入涨幅榜；二者使用同一批非空候选，分别取涨跌幅最高与最低的 5 个同层级兄弟节点，不按正负号删行。所有维度使用 `sectorCode asc` 作为最终次排序，每列最多 5 行。
...
4. `moneyflow_ind_dc` 提供盘后正式板块资金流字段，其已入库的 prod `board_moneyflow_dc` 是 Heat Model EOD V1/V2 的运行数据源。
...
| `PARTIAL` | 当前可见榜单行或当前详情缺少必需热度/成员行情，但其余事实仍可用 | 保留骨架，缺字段显示 `--` 或 `UNKNOWN`，debug 记录原因 |
...
3. 行业“涨幅榜”和“跌幅榜”分别返回每个层级的降序 Top5 与升序 Bottom5，切换后不复用旧榜单顺序。
4. 点击一级/二级后下级列表和详情按规则联动。
5. 每个排名项突出板块名、主指标和领涨股。
6. 概念默认按有效 `heatScore` 排名，等级和日度趋势标签可独立变化；全量概念池中未进入当前榜单的 `INVALID` 不触发页面 `PARTIAL`。
7. 行业、概念与地域详情均展示最多 5 个真实成分股及盘后涨跌幅。
8. 地域榜单只包含 31 个地域板块，固定 7 行可视区，不出现行业层级或概念热度标签。
...
6. Heat Model EOD V2 对固定样本可复算，权重、分位、阈值、日度变化和版本一致；v1 仅作为既有历史对照。
...
| 版本 | 日期 | 变更摘要 | 负责人 |
|---|---|---|---|
| v2.11 | 2026-08-15 | 纠正行业涨跌幅排序为“涨幅榜/跌幅榜”两个独立页签；概念 `PARTIAL` 收敛到当前可见榜单/详情；正式概念排行采用表头与内容共享固定列网格 | Codex |
...
| v2.4 | 2026-08-13 | 完成 Slice 9 Figma 与文档纠偏：正式概念 Heat 改为 20 日、移除三个工作台板块详情入口、补齐视图专属字段与状态说明；同步真实阶段状态为 Slice 1-5 完成、6-8 已实现未验收、A01-A19 OPEN | Codex |
