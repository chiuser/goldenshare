...
本表与[首页股票搜索低层设计 v1](./homepage-stock-search-low-level-design-v1.md)共同记录实现门禁与完成证据；LLD 已补齐代码符号、SQL、通用清单映射和测试门禁。
...
产品范围没有新增待拍板项。D1-D4 已完成：Wealth 自有 API、当前上市 A 股候选池、500ms 联想、标准组件、首页唯一挂载和详情默认日 K 均已落地。生产数据库只读性能门禁和本地真实页面 smoke 已通过；用户已于 2026-09-02 确认完成部署并验收通过，本功能结案。
...
| v1.3 | 2026-09-02 | 用户确认已完成部署并验收通过；功能结案 | Codex |
