...
2026-08-24 用户部署复验确认标题、发布时间和来源内容正确，但要求阅读器头部改为居中排版。本轮视觉合同冻结如下：

1. 标题相对于整个阅读器宽度水平居中，不得因为右上角关闭按钮占位而向左偏移。
2. 发布时间与来源组成同一行，位于标题正下方，并相对于整个阅读器宽度水平居中。
3. 标题继续允许自然折行；多行标题的每一行均居中，不恢复截断或省略号。
4. 关闭按钮继续固定在右上角，保持现有尺寸、图标、焦点和关闭行为，不参与标题与元信息的布局宽度计算。
5. 正文、URL/HTML/TEXT 渲染、详情数据合同、弹窗尺寸和状态机均保持不变。
6. Figma `13 News Reader - Components and States` 的 8 个桌面状态已同步为上述结构；移动端仍不属于本期实现范围。

本轮只允许修改共享阅读器头部样式、对应回归测试和本方案/LLD，不修改新闻 API、adapter、controller、列表组件或正文内容排版。

修正已完成：Figma 8 个桌面状态与共享阅读器 CSS 已统一为全宽居中头部；阅读器目标测试 `10 passed`，Wealth 全量回归 `52` 个测试文件、`334 passed`，typecheck 和 build 均通过。当前状态为“开发完成，待用户部署与视觉复验”。
