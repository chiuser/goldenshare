...
- 状态：v1.22；既有动量排名 M0～M4 与双动量 M5～M8 保持完成结论；相对轮动 M10 后端与 M11 前端已完成；M12 首轮真实部署审计已完成，但 Results 稳态 P95 未通过；下一步固定为 M12R 性能纠偏。
- 编写日期：2026-08-29。
...
状态：`AUDIT/LLD PASS；CODE NOT STARTED`。
...
| G39 M12R 纠偏合同 | 真实瓶颈证据、稀疏等价方案、旧路径安全删除、共享回归和失败停止门禁完整 | PASS (audit/LLD)；代码与部署验收 OPEN |
...
30. M12R CodeGraph、当前代码、共享消费者、性能单测和运行剖析已经完成对账；确认全历史全行业排名／坐标过度物化及事实行读取／物化成本，排除 JSON 瓶颈和无证据索引判断。第12.2～12.3节已冻结稀疏等价修正、安全删除和失败停止顺序，尚未修改业务代码。
...
M12R 的问题和修正边界已经达到可编码程度：当前代码确实过度构造21,905个历史排名事实和20,220个历史坐标事实；31,614行事实读取／物化仍是最大单阶段；JSON 瓶颈、缺索引和删 `pct_change` 均没有证据。下一步只允许先实施经等价原型证明的稀疏内存计算，并在部署后重新执行两轮20次最大 Results。若仍失败，必须停止并另审查询投影；不得自动增加缓存、索引、结果表、分页、截断或迁移。M12R 通过后才返回 M12 完成四档最终视觉、交互和用户验收。
