...
- 状态：v1.21；既有动量排名 M0～M4 与双动量 M5～M8 保持完成结论；相对轮动 M10 后端与 M11 前端已完成；下一步固定为 M12 真实 API 联调与交付验收。
- 编写日期：2026-08-28。
...
本文定义财势探查页面结构、已完成的“横截面动量排名”、M3A 三级行业成分股明细和“双动量”，并在第 6.21～8.22 节冻结第三个独立方法“相对轮动”的代码级方案。相对轮动只描述行业当前同组强度位置及相对 5 个交易日前的变化，不做预测、传统 RRG、综合评分或发布。M10 后端与 M11 前端均已完成，相对轮动现已使用第三条精确路由和独立工作区；成员广度和量价分布始终只保留按钮与提示，不得生成路由、controller、API、Mock、隐藏工作区或计算逻辑。M3A 成分股明细只是已选三级行业的事实下钻，不属于“成员广度”。
...
相对轮动 M10 后端及 M11 前端已经完成，G35、G36 与已覆盖阶段的 G38 均已关闭。相对轮动现为第三个正式方法，使用独立路由、controller 和响应式工作区；成员广度、量价分布和其他研究能力继续排除在本需求之外。下一步只允许进入 M12 真实 API 联调与交付验收；部署同拓扑真实 HTTP P95、3/5 SQL、payload、四档最终视觉和用户验收仍属于 M12，不得用本地 Mock、SQLite／TestClient 或本轮浏览器超时页面冒充上线结论。
