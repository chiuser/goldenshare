...
| 五方法只能等价切读 | typed fact reader + 逐字段 oracle | 数值公式与公开 URL/query保持不变；日期按已批准PUBLISHED规则校准，量价`dateCoverageBasis`变更见M24.6.1；未通过全scope/周期/缺失逐字段等价前继续现算，禁止运行时双读和永久fallback |
| 量价分布独立公式与状态 | `sector_price_volume_contract.py` + calculator | `1/5/10/20/30`、两段等长成交额窗口、四种客观状态；不生成综合分或预测 |
| 量价分布发布日期口径 | 共享发布覆盖＋量价Meta | 默认取最近PUBLISHED；PARTIAL／零可计算不退旧日；日期计数只代表已发布1日动量可计算行业数，不代表量价坐标完整。显式未发布日Empty且零原行情读取 |
...
contractVersion = 1
templateVersion = sector-daily-insight-template@2
```
...
4. 文本、颜色、字号、圆角、边框和间距复用现有 Wealth token；行业名称／路径单行省略并提供 Tooltip。1日、5日、20日、名次四列的表头及数值均在列内水平居中；`.num` 仅复用数字字体与等宽数字特性，不承担对齐规则，也不修改全局 `.num` 或其它方法的数值对齐。
...
#### 3.6.1 当前列表布局基线（2026-09-04 用户确认）

旧正式三个Ready画板的表头左内距为0、行左内距为12px，七列整体错位12px；旧行宽746px、行高76px、118×28事实标签不再作为编码基线。以下统一包含六项视觉修正及后续居中、删除提示文字、去掉末行横线的确认，作为当前编码要求，不再并行执行旧布局。目标区域以最新Figma建立像素基线，非目标区域仍执行原2px门槛；具体组件与样式落点见第8.34节。

1. 面板保留776×348、左右14px；表头和行统一748px宽、左右12px内距、8px列间隔。七列依次为行业116、1日74、5日74、20日74、名次78、事实标签80、说明180px，合计676px；`676 + 6×8 + 2×12 = 748`。表头／行共享一份Grid定义，禁止各自补坐标。1600基准之外，四个数字列和80px标签槽保持宽度，行业／说明以116:180比例吸收剩余宽度并允许省略，禁止固定最小748px导致溢出。
2. 行高60px、上下8px，所有列在44px内容区垂直居中；行业名称15px／22px行高，路径11px／16px行高，间隔4px，均单行省略并保留完整Tooltip。按用户补充确认，1日、5日、20日、名次四列的表头和13px数值均在各自列内水平居中，两者中心线一致；不再右对齐，列间留白不能被相邻数字占用。
3. 事实标签复用当前`wealth/src/features/watchlist/ui/watchlist.css`的A金色细边视觉：24px高、12px常规字、左右8px、4px圆角、1px内描边，`brand-weak / border-strong / brand`。移除原圆点；标签按内容Hug宽度，放在80px槽内居中，不再让整个槽成为大按钮。六种状态只通过既有文字表达；不改eventType映射，不增加点击、hover、选中语义。不要复制自选表专用的负margin布局。
4. 说明文字与表头左边界严格一致，11px／16px行高，最多两行、超出末尾省略，完整`renderedText`原文仍由Tooltip可读；不截断API数据、不重写句子、不修改业务事实。Figma采用固定宽、`textAutoResize=HEIGHT`、`textTruncation=ENDING`、`maxLines=2`，真实高度16或32px；不能使用会使maxLines失效的固定高单行截断模式。代码使用两行line-clamp及完整原文Tooltip。
5. 行密度改变不缩小面板／工作区／正式画板；四个列表继续独立纵向滚动、表头固定，设计稿行数不是TopN限制。删除面板标题右侧“列表可滚动”提示文字，不删除滚动能力。最后一条数据行不画底部分隔线，行与行之间仍保留1px分隔线，代码对实际列表末行使用`:last-child`而非写死第三行。Figma的262px滚动区仅放3条60px示例，所以留下82px空白；这不是默认3／4行或空行占位，运行时应展示完整数据并在超出区域时滚动。本次不修改固定面板高度，也不修改工具栏、概览、公共骨架和非Ready状态的外形。
6. 编码验收补充：行业／说明的表头和数据左边界误差≤1px，四个数字列的表头／数值水平中心线误差≤1px；同一行数字、标签垂直中心误差≤1px；标签宽度随文案且不侵入两侧；长说明最多两行并保留全量Tooltip；不显示“列表可滚动”，滚动仍有效；四档宽度无模块溢出。Figma先修共享行，再校准12个面板表头及正式实例，不脱离组件、不一次重建整页。
...
| 修改 | `tests/test_wealth_sector_price_volume_query_service.py` | PUBLISHED日期、五scope、3/5/5 SQL、119/120、状态、409、payload和性能；PARTIAL／零可计算不回退 |
| 修改 | `tests/web/test_wealth_sector_analysis_api.py` | 三只endpoint的strict／401／400／409／500正反例及既有十一只响应零变化 |
| 修改 | `tests/architecture/test_wealth_sector_analysis_guardrails.py` | 量价计算输入维持日历／层级／行业日行情，日期共享发布批次＋1日动量覆盖；无成员／股票／因子／资金／Heat／QTF／DG/Lake／迁移／缓存／配置 |
...
| 新增／修改 | `src/biz/services/wealth/market/sector_analysis/daily_facts/source_query.py` | 单日一致性读取六张白名单来源并生成实际source hash；M25R另提供联合窗口聚合审计query，禁止逐日60日全量读取；无来源副本 |
...
| 新增 | `src/biz/services/wealth/market/sector_analysis/daily_facts/template_renderer.py` | `sector-daily-insight-template@2`，固定证据优先级、最多2项、字节确定性 |
...
| 修改 | `src/biz/services/wealth/market/sector_analysis/daily_facts/materialization_service.py` | M25R让每日期一次`_build()`同时产生hash、写入和read-back期望；read-back不重算公式。保留单日自动物化所需入口 |
| 重构 | `src/biz/services/wealth/market/sector_analysis/daily_facts/replay_planner.py` | 保留`MIN_PUBLISH_DATE=2025-08-22`、日期／预热／层级解析；以轻量auditor替代单日完整preview，删除历史`preview_unit()`与expected-content合同，不保留兼容分支 |
| 新增 | `src/biz/services/wealth/market/sector_analysis/daily_facts/history_input_auditor.py` | 联合窗口日期、层级、业务键、有效值和来源覆盖准入；只返回聚合证据与schema v2 audit hash，零Calculator／Builder／业务写入 |
...
| 修改 | `src/ops/action_catalog.py` | 历史动作增加静态`execution_style=audit_then_apply`并移除其PLAN/APPLY参数；Heat等动作仍使用通用PLAN/APPLY，目标九表不变 |
...
| 修改 | `src/ops/runtime/maintenance_executor.py` | 保留既有通用PLAN/APPLY协议给Heat；M25R所需上下文只表达审计进度、取消和逐unit执行，不认识板块公式 |
| 修改 | `src/ops/services/task_run_maintenance_plan_context.py` | 复用独立短session保存审计阶段、audit计数、当前对象和最后更新时间；业务`unitDone`只随已发布／幂等核验日期推进 |
| 修改 | `src/ops/runtime/task_run_dispatcher.py` | 仅对`audit_then_apply`动作在同一TaskRun内先审计后自动执行units；审计失败零执行。默认、Heat及其他registered行为不变，`IngestionCanceledError`继续收口为canceled |
...
| 修改 | `src/app/runtime/sector_analysis_daily_task_executor.py` | App编排轻量auditor与逐日单次materialize；同TaskRun自动衔接，业务和观测使用不同session，旧10958/schema v1拒绝 |
| 保留 | `src/app/runtime/sector_analysis_daily_readiness_evaluator.py` | 仅服务单日自动动作的既有来源齐备判定；M25R历史输入审计不得调用完整preview |
...
| 修改 | 五方法现有 QueryService | M24按动量→双动量→相对轮动→成员广度→量价分布顺序逐个切行业级事实；数值公式及URL不变，日期按PUBLISHED校准；量价日期合同明确变更见M24.6.1 |
...
### 6.36 量价日期与发布语义（M24.6.1修正）

Meta是自动日期决策唯一责任方。按2026-09-04用户批准，取消原始`dc_daily.close/pct_change/amount`全字段齐全才可作为默认日的限制。复用`SectorAnalysisFactReader.load_momentum_coverage()`：SSE日历、当前PUBLISHED批次与全局一级／二级／三级的动量1日事实；不读取或重新证明原始行情完整性。

`dateCoverageBasis`由旧`INDUSTRY_PRICE_AMOUNT_DAILY`改为`INDUSTRY_DAILY`，不保留旧别名。`expectedSectorCount`为当前全部行业数；`validSectorCount`为已发布1日涨跌幅可计算数量，三类全局范围每个行业只计一次。它不代表收盘价、成交额或所选周期量价坐标完整性。覆盖查询1条SQL，返回全部SSE日期槽及：
...
自动模式：公共`MarketPageContextQuery`给出`expectedTradeDate`；从`coverage.published_dates`选不晚于它的最后一天。同日为READY，更早为DELAYED，没有任何已发布日才为EMPTY。已发布PARTIAL甚至零可计算的MISSING均不能自动退旧日；发布与可计算数量是独立事实。前后端只校验默认日期存在于覆盖、READY等于期望日、DELAYED早于期望日，不再强制默认日COMPLETE。批次身份由服务器验证，不让前端用availability猜发布状态。
...
4. 显式已发布日保留可用数据和独立缺失原因；显式未发布日返回Empty，不借用旧日、不读取原始行情。仅`validSectorCount=0`不得抹掉量价的其他可用事实。
...
| `template_version` | VARCHAR(64) | 固定 `sector-daily-insight-template@2` |
...
API原因代码依次为 `HISTORY/DATE/PRICE/MEMBER/AMOUNT/ADJ_FACTOR/GROUP_SIZE/COVERAGE/PREVIOUS_BATCH/OTHER`，分别一一对应上述十个 `missing_*_count` 列；只输出非零项。不同方法的缺失可重叠，不得把原因之和强制校验成 `missing_count`。固定映射由专属schema的 `MISSING_REASON_FIELDS` 定义，QueryService与响应校验共用；前端只翻译原因名称，不重新统计。

`breadth_up_share_above_50_count`只计既有成员广度资格ELIGIBLE且有限占比严格>50%的行业，不能仅凭非空或100%就计入；不合格的原始占比仍保留在方法事实中。本规则及文案纠偏见M25.0。
...
`preview_trade_date()`是M22单日自动readiness阶段已有的只读入口；只有全量消费者审计证明单日链路不再需要时才允许另行删除。M25R历史刷新明确不调用它。历史`materialize_trade_date()`只接收日期、层级、公式、模板和审计合同等静态身份，在该日期内部只执行一次`_build()`；实际source／plan／content hash、写入事实和read-back期望全部由同一构建结果产生，不再接收旧PLAN冻结的expected content hashes后重复构建比较。
...
### 6.50 历史实现记录：回补 PLAN/APPLY（M25R后不再执行）

本节保留M23/M23R当时已经执行过的合同，目的仅是解释历史TaskRun和既有生产数据如何产生。它不是M25R之后的新任务合同；其中“逐日完整preview”“独立PLAN TaskRun”“APPLY引用旧PLAN”和“APPLY前重算preview”的要求均已废止。现行编码与执行要求以第6.50R节为唯一依据。
...
### 6.50R M25R现行合同：轻量输入审计与单次计算APPLY

#### 6.50R.1 当前代码问题与影响面

CodeGraph与源码审计确认当前历史刷新存在三次完整计算：

```text
SectorAnalysisDailyTaskExecutor.plan_for_task_run()
  -> SectorAnalysisReplayPlanner.preview_unit()
  -> SectorAnalysisDailyFactsMaterializationService.preview_trade_date()
  -> _build()                                      # 第1遍完整计算，结果只写PLAN证据

SectorAnalysisDailyTaskExecutor.execute_unit()
  -> preview_trade_date()
  -> _build()                                      # 第2遍完整计算，只为和旧PLAN对账
  -> materialize_trade_date()
  -> _build()                                      # 第3遍完整计算，唯一真正写库的一遍
```

每次 `_build()` 都会通过 `SectorAnalysisDailyFactsSourceQuery.load_bundle()` 读取目标日最多60个SSE交易日的日历、层级、行业行情、成员、股票日线和复权因子，然后执行五方法Calculator、洞察Builder和hash。生产TaskRun `10958`从2026-09-05 11:25:36运行至13:41:40，最终success／FROZEN、247/247、`rowsSaved=0`，且没有找到引用它的新APPLY TaskRun；这证明第一遍完整计算没有写入九张业务表。继续旧APPLY还会再做两遍。因此：

1. `10958`只能作为不可变历史证据，不得引用执行。
2. 不允许用增加缓存、来源快照、新表或新Worker来掩盖重复计算。
3. M25R只改板块历史刷新主链；Heat、每日单日物化和其他maintenance动作不改变。
...
- 对应板块历史刷新、TaskRun、Heat回归和架构边界测试

不受影响的公开消费者：两只每日洞察API、五方法API、Wealth前端、九表schema、Alembic、scheduler、QTF、DG/Lake。依赖方向仍为Ops定义通用意图、App组合、Biz审计／计算／发布；Ops不得import Biz。

#### 6.50R.2 单TaskRun状态机

运营只创建一次 `maintenance.replay_wealth_sector_analysis_history`：

```text
request(start_date, end_date)
  -> AUDITING_INPUT
  -> audit failed: snapshot BLOCKED + TaskRun failed, zero business writes
  -> audit passed: APPLYING
  -> trade_date 1: build once -> write -> read-back -> publish -> commit
  -> trade_date N: build once -> write -> read-back -> publish -> commit
  -> READBACK_COMPLETE -> success
```

该动作不再公开 `execution_mode`、`plan_task_run_id`、`plan_hash`。dispatcher通过该action的静态`execution_config.execution_style='audit_then_apply'`进入专属通用流程；没有该值的动作维持既有逻辑，Heat的PLAN/APPLY合同不得变化。该字段是代码目录中的静态动作元数据，不是环境变量、Settings、数据库配置或页面开关。
...
新增板块历史专属`SectorAnalysisHistoryInputAuditor`，只做以下工作：

1. 用SSE日历解析`max(requestedStart, 2025-08-22)`至结束日的升序目标日期，以及首个目标日向前60个SSE交易日的预热起点；目标日期为空即阻断。
2. 一次读取当前唯一发布层级版本，验证一级、二级、三级父子闭包、根路径、代码唯一性和五类比较池；层级版本或闭包不唯一即阻断。
3. 对`warmupStartDate..effectiveEndDate`的六张来源执行有界聚合／存在性审计。查询返回日期、业务键重复数、空值／非法值数、按日总行数和必要覆盖计数，不返回来源明细或股票日级全量行。
4. `dc_daily`验证目标行业分类、业务键唯一、日期合法、目标窗口不为空及价格／涨跌／成交字段的硬有效性；行业级可表达缺失继续由Calculator输出typed missing，不扩大成整日阻断。
5. `dc_member`验证业务键唯一、行业代码属于冻结层级、成员代码非空和联合窗口总体覆盖；停牌、上市时间等导致的单行业／单日成员差异不是全局阻断。
6. `equity_daily_bar`与`equity_adj_factor`验证业务键唯一、日期合法、数值有限、价格／因子满足现有硬约束及联合窗口总体覆盖；不要求停牌股票每天都有行情，不用补零或前向填充。
7. 所有查询必须在短`REPEATABLE READ, READ ONLY`事务内，`SET TRANSACTION`位于第一条SQL。审计只对联合窗口做一次集合读取，不允许为247个目标日分别重扫60日窗口。

审计问题按现有安全异常输出来源、日期范围、计数和有限摘要，不输出SQL、连接或原始行。硬问题使TaskRun停止且九表零写入；审计通过后同一TaskRun自动进入APPLY，不需要第二次用户操作。

#### 6.50R.4 APPLY每个交易日只计算一次

`materialize_trade_date()`调整为单次构建合同：

```text
validate frozen static identity
  -> built = _build(trade_date, previous_published_batch)   # 唯一一次
  -> derive source_hash / daily_plan_hash / content_hash from built
  -> if identical PUBLISHED exists: idempotent verified skip
  -> write BUILDING batch and child facts from built
  -> read persisted rows/counts/keys/hash against built
  -> atomically publish new batch and supersede old batch
```

1. `BuiltDailyFacts`作为当前日期进程内不可变结果，只存活于该日期执行周期；进入下一日期前释放，不形成跨日内存缓存。
...
1. 审计阶段显示`phase=AUDITING_INPUT`、当前审计项、`auditDone/auditTotal`和`lastCheckpointAt`；业务`unitDone`固定为0，因为尚无任何日期发布。
2. 审计通过后`unitTotal=目标日期数`、`phase=APPLYING`；只有日期已发布或同内容幂等读回通过，`unitDone`才加1。
3. 每次审计查询前后、每日期开始前、`_build()`完成后写入前、发布后进入下一日前检查取消。收到取消后不得领取新日期；当前未提交业务事务回滚，已提交日期保留。
4. 进度、当前对象、阶段和最后更新时间至少每完成一个有界步骤写回；运行中不得连续30秒只有心跳而没有阶段或业务进度解释。无法估算ETA时明确为不可估算，不伪造倒计时。
5. 成功、失败、取消时TaskRun及全部pending/running节点必须一致收口；Ops状态写入失败不得回滚已发布业务批次。

#### 6.50R.6 安全修改与删除顺序

1. 先新增轻量auditor、schema v2和纯合同测试；证明审计只调用日历／层级／聚合覆盖query，五Calculator、InsightBuilder、repository写入调用均为0。
2. 改造`materialize_trade_date()`接收冻结的静态身份，并让一次`_build()`产生hash、事实和read-back期望；先保持单日自动动作及其测试通过。
3. 在action catalog只给`maintenance.replay_wealth_sector_analysis_history`登记`audit_then_apply`，dispatcher加入该静态风格的单TaskRun分支；Heat、news、普通maintenance、QTF和分钟lane测试必须先通过。
4. 切换`SectorAnalysisDailyTaskExecutor`历史路径：审计通过后直接逐日materialize；删除`execute_unit()`中历史动作对`preview_trade_date()`的调用和`_validate_replay_preview()`。
5. 从历史unit/snapshot删除仅为旧完整preview服务的`expected_source_hash`、source日期／计数hash、逐表计数范围等字段；新schema必须拒绝10958等旧snapshot。不得修改或物理删除历史TaskRun记录。
6. 删除`SectorAnalysisReplayPlanner.preview_unit()`及其旧全量计算路径；`resolve_scope()`中日期／层级能力可下沉给auditor。`finalize()`、`_fact_count_ranges`、`_warmup_start`只有在全量引用审计证明零消费者后才删除。
7. 从板块历史动作参数中移除`execution_mode/plan_task_run_id/plan_hash`；这些通用参数定义不能删除，因为Heat等动作仍使用。
8. `preview_trade_date()`若仍被单日readiness或其他消费者使用则保留；只有CodeGraph和文本引用均为0且相关测试通过时才允许删除，禁止为“清理干净”误伤单日自动任务。
9. 最后执行全量消费者、架构、TaskRun和Heat回归；M25R未部署前禁止新建生产刷新任务，部署后也禁止继续10958。

#### 6.50R.7 必须通过的正反例

1. 247日审计只扫描一次联合窗口，不出现247次60日来源读取；spy证明公式、洞察生成、完整preview和业务写入均为0。
2. 审计缺日期、重复键、非法值或层级闭包错误时同一TaskRun停止，APPLY调用为0；合法的行业级局部缺失不误伤全局任务。
3. 审计通过后无需第二个请求，自动进入APPLY；每个交易日`_build()`、五方法Calculator和洞察Builder各只执行一次。
4. read-back删行、增行、改键或改值时当前新batch失败、旧PUBLISHED不变，并证明read-back没有再次执行公式。
5. 日期／层级／公式／模板／审计合同漂移拒绝；审计后来源内容正常变化由该日唯一构建记录实际source hash，不触发第二遍计算。
6. 中途取消、进程退出、续跑、同范围幂等重试、Ops观察写失败、TaskRun／节点终态一致全部有正反例。
7. `10958`及所有schema v1旧PLAN即使`FROZEN/applyReady=true`也被新历史路径拒绝。
8. Heat仍要求其原PLAN/APPLY参数和两个TaskRun；默认registered maintenance、GENERAL lane、QTF和分钟任务行为零变化。

测试落点固定为：新增`tests/test_wealth_sector_analysis_history_input_auditor.py`；修改`tests/test_wealth_sector_analysis_daily_replay.py`、`tests/test_wealth_sector_analysis_daily_materialization.py`、`tests/test_sector_analysis_daily_task_executor.py`、`tests/test_ops_action_catalog.py`、`tests/web/test_wealth_sector_analysis_replay_runtime.py`与`tests/architecture/test_wealth_sector_analysis_guardrails.py`。若dispatcher通用分支发生变化，还必须运行既有`tests/test_worker_lane.py`、Heat、news和普通maintenance相关回归；不得靠删除旧反例让新路径通过。
...
| 模板 | `sector-daily-insight-template@2` | template renderer |
...
| 历史动作执行风格 | `audit_then_apply` | `src/ops/action_catalog.py`内该动作的静态`execution_config`；仅registered maintenance dispatcher消费，未配置动作保持原行为 |
| 输入审计合同 | `sector-analysis-history-input-audit@1` | Biz auditor、TaskRun schema v2／integrity hash；用于拒绝旧式PLAN snapshot |
| 物理存储 | `gs_raw_cold_hdd` | Alembic migration 常量 |

`audit_then_apply`不新增运行时配置：默认值为缺失，来源和持久化位置均为action catalog代码，作用域仅`maintenance.replay_wealth_sector_analysis_history`，部署后由现有GENERAL Worker读取；Heat是必须验证的不受影响消费者。禁止把同一值复制到页面常量、脚本或env。前端从 API 获取公式／模板身份，不参与判定。若以后需要可运营配置，必须另做配置审计和版本迁移，不在本期预埋。
...
列表枚举顺序必须与contract一致；`tradeDates`升序唯一、count非负且`valid<=expected`。COMPLETE要求`expected>0 && valid==expected`，PARTIAL要求`0<valid<expected`，MISSING要求`valid==0`。`defaultTradeDate`非空时必须出现在列表中，不要求COMPLETE；服务端按PUBLISHED选择，READY要求等于`expectedTradeDate`，DELAYED要求早于它，EMPTY要求无默认日。不得让前端根据可计算数量重新选择默认日；旧`dateCoverageBasis`枚举拒绝。
...
本节只细化第3.6节已确认的视觉要求。代码落在既定 `daily-insight/ui/DailyInsightPanel.tsx`、`DailyInsightRow.tsx` 与 `sector-daily-insight.css`；不抽取跨方法公共表格，不修改自选列表、全局 `.num`、五方法样式或 Design Token。

1. **容器尺寸**：1600基线下四面板两列等宽、列距12px；每面板776×348、左右14px、上下10px。内部依次为38px标题、28px表头和262px滚动区，三者间距0，`10 + 38 + 28 + 262 + 10 = 348`。使用正常流布局；滚动区内行不能被Flex压缩或拉伸。面板及行的内描边不得额外占用布局宽高，例如使用现有边框token的内阴影表达内描边，避免CSS边框挤掉Figma内容区的2px。只去掉实际末行底线，面板外框与其它行间线不变。
2. **唯一列定义**：表头和数据行共用同一组Grid列、12px左右内距和8px间隔。声明为 `minmax(0, 116fr) 74px 74px 74px 78px 80px minmax(0, 180fr)`；1600基线内容行宽748px，列宽依次为116／74／74／74／78／80／180px。父容器和可变列使用 `min-width: 0`，不得设置748px最小宽、额外补偿坐标或CSS缩放。有无纵向滚动条时，表头与行都必须按相同可用宽度计算，不能只让数据行缩窄造成再次错列。
3. **单元格对齐**：行固定60px、上下8px、内容区44px，各列垂直居中。四个数字列同时为表头和内容设置局部居中样式，数值元素保留 `.num`、13px和既有涨跌颜色；不得以 `justify-self: end`、右内距或旧方法右对齐类覆盖。行业与说明左对齐；事实标签列及其表头居中。行业名称／路径的字号、行高与截断按第3.6.1节执行。
...
在既定 `sectorDailyInsightAdapter.ts` 中维护穷尽的 `eventType` 文案映射，由 `DailyInsightRow` 消费；它是枚举翻译，不是业务分类。不得增设入选原因字段、解析 `renderedText`，或根据 `returnPct1d/currentPercentile20d/previousPercentile20d/percentileChangePp` 自行改判。
...
| `STRENGTHENING` | 显著转强 |
| `WEAKENING` | 显著转弱 |
| `COUNTER_TREND_STRENGTHENING` | 逆势抗跌 |
| `RISING_BUT_WEAKENING` | 上涨滞后 |

1. 不显示“进入前20%／进入后20%”子标签；`+12pp/-13pp` 等数值不再作为事实状态标签。说明区仍直接展示后端 `renderedText`，保留后端提供的两日名次／分母、百分位变化及补充证据；文本省略时完整 Tooltip 展示原文。
2. 当前 `_event_type()` 对普通转强／转弱可返回平盘或涨跌幅缺失样本，因此标签不能固定写成“上涨转强／下跌转弱”。具体涨跌数值继续独立展示，不补零、不改状态。
3. strict adapter拒绝未知枚举，不根据所属列表猜测状态；API四个数组已经表达后端 `category` 分组，`eventType` 负责行状态，两者不得混用，不为前端增加category字段。
4. 后台 `DailyInsightBuilder` 的候选条件、排序和已存 `event_type` 不改；不增加迁移、模板版本或本次专属生产刷新。既有M25.0模板@2更新步骤仍保留。
...
| `SA_DAILY_FACT_PLAN_DRIFT` | error | true | 历史刷新日期／层级／公式／模板／审计合同身份漂移；拒绝继续APPLY |
...
8. M24.6.1替换原完整日测试：Meta只按PUBLISHED选日，COMPLETE/PARTIAL/零可计算均保留已发布日；目标未发布才回退，无发布EMPTY。原行情完整与否不得改变Meta；来源读取、计数和默认日校验必须有反例。
9. Snapshot／Details精确tradeDate不回退；显式已发布日依量价坐标事实展示，1日覆盖为MISSING也不清空独立成交值；显式未发布Empty且零源读取／计算。响应observedTradeDate严格等于请求。
...
15. 计算只读日历／层级／行业日行情，日期只复用既有发布批次和1日动量覆盖；原完整日谓词及方法不得恢复。无成员、股票、因子、资金、Heat、宽基、申万、QTF、DG/Lake、写事务、迁移、缓存、配置或新依赖。
...
5. M25R单TaskRun审计日期升序、`2025-08-22`起点、首日60个SSE交易日预热和BLOCKED零APPLY；审计只做联合窗口完整性校验，零公式、零preview、零业务写入。
6. 审计通过后自动APPLY，每个交易日完整构建恰好一次；部分失败从持久化日期恢复，同范围幂等重试零新增，日期洞、跨批次previous和自然日空行均失败。旧TaskRun 10958和schema v1 PLAN不可执行，Heat原PLAN/APPLY零变化。
...
6. 8张正式Figma、`1600/1512/1460/1366`、固定表头、四独立滚动区、2px基线、无模块横向溢出；每日洞察列表专项按下表验收，不以历史118×28标签／76px行截图替代最新基线。
...
| 场景 | 正例与反例验收 |
|---|---|
| 表头／数据列 | 正数、负数、0、缺失占位及不同位数名次：四个数字列的表头／内容中心线误差≤1px，行业／说明左边界误差≤1px；有无滚动条都不得错列或侵入相邻列 |
| 密度与高度 | 1600基线面板776×348、标题38、表头28、列表262、数据行60px；各列垂直中心误差≤1px。其它三档仅按既定宽度规则伸缩，不增加面板高度、补偿空白或模块横向溢出 |
| 行数与末行线 | 0／1／3／4／5及80条真实样本：无TopN截断、假行或固定第四行；3条时剩余82px；5及80条可滚动到末项且表头固定；实际末行无底线、其余相邻行仍有1px线，四个列表互不联动 |
| 六类事实标签 | 原事件映射与文字不变；24px高、紧凑金色、无圆点、无按钮交互且不铺满80px槽；不出现“列表可滚动”提示，不能因删提示而移除滚动 |
| 长文本 | 行业／路径单行省略；说明分别用1行、2行和超过2行原文验证，视觉最多2行、不撑行、不越列；Tooltip含完整原文，API字段及adapter结果不被裁切或改写 |

M22的远程迁移、catalog、受控单日与M23历史回补均已有真实生产证据并已关闭。M25R新合同TaskRun `10997`已经完成248个交易日的联合窗口审计、自动APPLY和生产read-back，当前248个PUBLISHED批次全部为模板@2，G66A据此关闭；M26仍须完成自动任务、公开API性能和最终页面交付验收。
...
4. 幂等重试：相同日期范围重新执行时，已存在相同`daily_plan_hash + content_hash`的日期只读回核验并跳过，零新增、零PUBLISHED变化；不要求先生成独立全量PLAN。
...
状态：`PASS / CLOSED`（2026-09-04）。动量、双动量、相对轮动此前已关闭；成员广度及量价分布以用户本次自行验收确认结案，五方法切读全部通过。以下实施及验收记录按发生顺序保留，最终结论见M24.5.5、M24.6.3。
...
状态：`PASS / CLOSED`（2026-09-03）。用户已明确自行完成相对轮动验收，要求不再重复验收并推进成员广度。下列实施与只读预验收证据继续保留；不将用户验收写成Codex新增认证HTTP计时，也不推定本次获得新的性能样本。
...
这是开发代码对Prod的直调证据，不是生产部署版本的认证HTTP／页面验收；前两方法的HTTPS豁免不扩展。仅有既有Starlette测试客户端弃用提示和前端bundle体积警告，没有为消除提示修改依赖或构建策略。实施时停止于待提交／部署；后续已提交`2b031bcf`，用户明确自行验收完成并免除Codex重复验收，因此M24.4关闭。没有新增HTTP计时证据，不整体关闭G63，不进入M25。

### M24.5：成员广度切读开工核对

状态：`PASS / CLOSED`（2026-09-04）。完整切读提交`6f124782`，用户已验证无问题并批准直接通过。行业级Rankings、Details组成与历史趋势已改读发布事实，完整股票明细保留按需读取；公式、排名、缺失语义、页面和数据库不变。以下开工及精度小修保留为历史过程，开发证据见M24.5.4，最终用户验收结论见M24.5.5。

#### M24.5.1 已核实的冲突与影响面

| 当前证据 | 核实结果 | 影响 |
|---|---|---|
| `wealth_sector_member_breadth_daily.py`、`wealth_sector_member_ma_breadth_daily.py` | 占比与覆盖率为`Numeric(8,4)`；成员方向计数、MA方向计数为整数，方向成交额为`Numeric(24,6)` | 比例字段与原在线高精度响应不是逐字段完全相同 |
| `daily_facts/repository.py::_normalize_record` | 按实际列scale使用`ROUND_HALF_UP`，不是只在显示时格式化 | 直接读取落表百分比不能恢复被舍入的小数 |
| `sector_member_breadth.py::SectorMemberBreadthCompositionDto.validate_composition` | 三项比例之和与100的误差不得超过`1e-6` | `33.3333×3=99.9999`会拒绝；2026-09-03直接构造真实响应模型已复现，无数据库或网络操作 |
| `sectorMemberBreadthAdapter.ts::parseComposition/validateCoverage` | 前端同样使用`1e-6`校验组成和计数对应的覆盖率 | 后端仅调松校验仍不能解决；5/6覆盖率落表为83.3333时前端也会拒绝 |
| 后端Meta响应验证及前端`validateDefaultDate` | 当前强制最近COMPLETE日 | 必须随切读同步到已批准PUBLISHED日期策略；仅改后端reader会导致前端拒绝PARTIAL默认日 |
| CodeGraph `status/explore/search/impact` | 覆盖广度Service、三只API、共享reader、离线builder、测试及前端adapter／工作区；Service影响45个符号，同名模糊命中已按文件源码复核 | 不可删除仍供物化builder使用的纯计算器；跨子系统依赖无需改变 |

这里不是来源数据质量问题，不能交给Prod补数据，也不能通过旧原行情兜底隐藏。上述为修改前审计证据；用户随后明确只调整舍入误差校验，批准口径如下，不再保留待拍板阻断。

#### M24.5.2 已批准的舍入容差修正

用户拍板：只修改校验容纳正常舍入误差，不采用恢复高精度比例的建议。切读时直接返回已存占比，不按计数／金额重新生成占比，不补尾差、不迁移表或重跑历史。

| 校验 | 固定容差（百分点） | 编码位置 |
|---|---:|---|
| 三项组成之和相对100 | `3 × 0.00005 = 0.00015` | 后端`SectorMemberBreadthCompositionDto.validate_composition`；前端`parseComposition` |
| 覆盖率相对已有计数计算的期望值 | `0.00005` | 前端`validateCoverage`，同时覆盖榜单行与详情组成；只验证，不改写返回值 |

4位小数单项舍入误差最多半个末位单位，三项按误差上界相加。两端实现另加`1e-12`处理浮点表示边界，不额外扩大业务容差。这是固定字段精度合同，不新增运营配置或可调开关。

安全边界：只改上述三处比较及局部具名常量，保留0～100范围、有限值、空值同有同空、方向计数平衡、资格、排名、身份与日期等现有检查。服务读取、公式、数据库、UI/CSS、网络超时与依赖均不改。日期消费者校准及行业级切读留在M24.5后续实现，不把本次校验完成等同于切读完成。
...
#### M24.5.4 开发收口与证据（2026-09-04，历史记录）

当时开发阶段结论：`PASS（代码、自动化与有界Prod只读预检）`。该阶段未部署，未把新切读代码标记为远程已上线，未重复用户已确认的既有版本验收，也未整体关闭M24／G63。后续用户验收结案见M24.5.5。
...
该阶段停止点：成员广度开发完成，随后按用户指令提交`6f124782`；本轮未核验该提交的远程部署。量价日期合同后续修正见M24.6.1，不进入M25或M26。

#### M24.5.5 用户验收与结案（2026-09-04）

用户明确确认成员广度和量价分布两个子板块已验证无问题，批准直接通过。成员广度完整切读实现提交为`6f124782`；M24.5状态更新为`PASS / CLOSED`，无剩余验收项。

本次依据用户自行验收确认，不重复Codex远程验收；M24.5.4的测试、Prod只读预检与冷请求记录完整保留，不改写为本次已部署认证HTTP测量。未新增远程HEAD核验或性能样本，未调整2秒行业级完整事实目标、数据口径或逐只股票明细按需读取范围。

### M24.6：量价分布切读

状态：`PASS / CLOSED`（2026-09-04）。日期修正提交`625ae045`、完整数值切读提交`cd81c702`；用户已验证无问题并批准直接通过，最终结案见M24.6.3。以下合同修正、开发及预检记录按发生顺序保留。
...
1. QueryService注入既有`SectorAnalysisFactReader`，Meta消费`load_momentum_coverage`及`published_dates`；Snapshot/Details从同一`calendar_dates`精确选择日期，保留batch是否存在的信息。层级版本／scope／父级／对象校验仍在第3条SQL前；公共日期仍只来自`MarketPageContextQuery`。
2. 未发布Snapshot保留完整行业行、null值及`DATE_MISSING`；未发布Details保留请求日空槽并返回Empty。该分支零源行情查询、零公式执行；已发布但1日覆盖为MISSING不能走此分支。正常已发布日仍调用现有量价计算器，数值完整切读另行实施。
3. 删除`_valid_price_volume_predicate`、`load_trade_date_coverage`、`load_exact_trade_date_status`及无消费者的`SectorPriceVolumeCoverageFacts/SectorPriceVolumeDateAvailabilityFact`；保留`SectorPriceVolumeDailyFact`和有界`load_open_dates/load_facts`。全量引用核对后再删除，不新增替代完整性检查。
4. 后端contract/schema、前端types/adapter/fixtures的`dateCoverageBasis`统一为`INDUSTRY_DAILY`；拒绝旧枚举，不兼容双口径。schema和adapter取消“默认日期必须COMPLETE”，其余日期、数量、有限值、空值、身份检查保留。不改controller请求参数、路由、文案、视觉、公式或排序。
5. 自动化覆盖：已发布PARTIAL留当天；已发布零可计算留当天；未发布回退；无发布EMPTY；显式历史不回退；未发布零源查询／零公式；源数据再齐全也不决定Meta；1日覆盖缺失不清空独立成交事实；旧枚举拒绝；401、409、SQL3/5/5及已有量价回归。新增前端真实响应驱动日期联动case，禁止只测宽松mock。
6. 允许改动为量价QueryService/Query/contract/schema及对应后端测试、真实路由测试、架构护栏；量价前端types/adapter/fixtures及对应测试；本技术方案和LLD。共享reader只增加内部`allow_empty=False`参数，量价三个入口显式传True；截至公共目标日完全没有发布时，返回目标开市日一个MISSING空槽，Meta无默认日。原有消费者不传参数，仍按既有约定拒绝无发布数据；层级／公式／计数校验不放宽。该扩展不增加公开配置，不改物化writer、表、迁移、账号或其他方法。

实施证据（2026-09-04）：

| 约束 | 当前落点及验证 |
|---|---|
| 按发布选日，缺失不触发自动退旧日 | `sector_price_volume_query_service.py`消费共享coverage；unit及真实API测试覆盖PARTIAL、零可计算、未发布、显式历史和无发布。`validSectorCount`与`coordinateCount`不混同 |
| 安全删除旧入口 | `sector_price_volume_query.py`与contract删除两个原始覆盖方法、完整性谓词和两只无消费者事实类；引用搜索及架构反例防止恢复。正常公式输入仍只读有界原行情，无新兜底 |
| 空发布不扩散为其他方法行为变更 | `sector_analysis_fact_reader.py`内部可选参数仅量价显式启用；真实SQL测试验证量价EMPTY且既有默认调用仍拒绝无发布；身份、计数及公式不放宽 |
| 前后端同一日期合同 | schema及量价types／adapter／fixtures统一`INDUSTRY_DAILY`、拒绝旧枚举；adapter和真实controller组件测试覆盖PARTIAL／零可计算默认日期。页面布局／公式／路由不变 |
| 回归与SQL | 12个后端测试文件共300项通过，包含真实路由、401、409、SQL3/5/5和其他已切方法；整个板块分析前端21个文件214项通过，typecheck和build通过。未发布Snapshot／Details仅3 SQL、零原行情读取／计算 |
| 有界生产只读 | 使用既有`.env → Settings.database_url → SessionLocal`，首语句`REPEATABLE READ, READ ONLY`、单条15秒超时，调用一次新Meta；仅四张`core_serving`日历／层级／发布批次／动量事实表，无原行情。公共日2026-09-03、最近发布2026-08-31、DELAYED、起点2025-08-22；251日期槽、496行业、所选日496/496，3 SQL，848.78ms。这是单次服务只读连通／口径证据，不是P95或已部署认证HTTP验收 |
| 影响面及停点 | CodeGraph status/explore/impact覆盖量价Service→三只API→schema／adapter、共享reader及已切方法消费者，源码逐项复核；未改变子系统依赖、表、迁移、配置、公式、UI或生产数据 |
...
状态：`PASS / CLOSED`，完整数值切读已提交`cd81c702`，前置日期修正为`625ae045`；2026-09-04用户验收确认见M24.6.3。只替换量价Snapshot／Details数值来源，不改前端、接口字段、公式、表、配置、发布任务或上游质量治理，不进入M25。以下保留该阶段的开发和预验收证据。
...
只读计时脚本为`/private/tmp/verify_m24_6_serving.py`，仅输出聚合计时／数量，不保存来源行。保留既有Starlette弃用提示，不扩展修改依赖。该开发阶段停止于待提交及用户部署后的量价切读验收，当时未整体关闭M24／G63；后续结案如下。

#### M24.6.3 用户验收与M24整体结案（2026-09-04）

1. 用户确认成员广度与量价分布均已验证无问题，并批准直接通过。量价分布的日期修正`625ae045`与完整数值切读`cd81c702`对应M24.6，状态为`PASS / CLOSED`，无剩余验收项。
2. 连同此前已关闭的动量、双动量、相对轮动，以及本次关闭的成员广度M24.5，五方法切读全部通过；M24及G63整体更新为`PASS / CLOSED`。
3. 验收依据明确为用户自行验证与批准。Codex本轮不重复远程页面／认证HTTP测试，不声称重新核验部署HEAD或生成新的P95；原自动化、等价对账、只读预检及冷请求证据保留其原阶段属性。
4. 该验收决定只覆盖M24.5与M24.6，不修改公式、完整数据范围、缺失语义、性能门槛或其他阶段验收要求，不自动扩展到后续方法或需求。
5. 下一步为M25每日洞察后端与前端；M25、M26及G64～G66仍未开始／OPEN。本轮仅更新技术方案和LLD状态，不开发、不部署、不执行生产任务，也不自动提交。
...
CodeGraph影响面：`SectorDailyInsightBuilder`只由物化服务调用，renderer只由builder调用；追到repository、版本常量、PLAN hash以及五方法typed reader。已有实测反例：只保留两只有效股票时，广度为100%但资格INELIGIBLE，旧概览仍计1；旧句子仅列“佐证：量价状态、成分股广度”。这不是新需求，也不是五公式错误。

| 顺序 | 文件／实现点 | 固定修正与验收 |
| --- | --- | --- |
| 1 | 本LLD及原技术方案M25 | 冻结本节后才编码；不修改产品阈值、五公式、数据库结构、上游数据或worker |
| 2 | `daily_facts/insight_builder.py` | `breadth_up_share_above_50_count`同时要求ELIGIBLE、Decimal有限、严格>50；50／缺失／不足资格反例均不计。只选有限可用且资格有效的证据，最多两项，保持量价→数量广度→成交额广度→双动量→轮动→MA20顺序 |
| 3 | `daily_facts/template_renderer.py` | 接收证据类型code而非中文标题，输出实际状态或数值；HEAD写1日方向幅度、当前20日同层级rank/count、5日收益；显著变化写前后20日rank/count及百分位变化；COUNTER_TREND_STRENGTHENING写下跌但相对抗跌，RISING_BUT_WEAKENING写上涨但相对滞后。缺少可选事实省略分句，不输出伪0、`--%`或空泛佐证。名次用“由第x/a变为第y/b”，与百分位升降分别陈述，避免分母变化时误称名次上升／下降 |
| 4 | `daily_facts/contract.py`、`repository.py` | TEMPLATE_VERSION升为@2，表示本次洞察生成合同；FORMULA_BUNDLE_VERSION不变。repository批次版本引用常量。内部PreviousSectorEvidence补充三项广度资格，repository直接投影既有资格列，不新增SQL／表；保留前日数值原貌。当前合格而前日不合格时，只描述当前值，不描述跨日变化 |
| 5 | Figma正式每日洞察8画板 | 将四个既有方法按钮的“待建设／不得改变URL”说明改为按既有正式路由切换、只传tradeDate；逐画板记录节点ID和属性核对，视觉及布局不变 |
| 6 | 新增`tests/test_wealth_sector_daily_insight_generation.py`及既有物化／reader测试 | 覆盖资格门槛、有限数值、四类列表及逆势文案、独立分母、两证据优先级、未知／缺值／不合格不生成证据、前日不足资格不生成变化、相同输入字节一致、公式事实不变、batch/item/PLAN版本一致及旧版本五方法读合同 |

格式固定：百分比及百分点保留2位、Decimal ROUND_HALF_UP、中文标点；下跌／下降写绝对幅度，0不得显示负零；状态名称沿用既有工作区中文名称。广度证据均写具体“上涨成分股占比／上涨成分股成交额占比／站上MA20成分股占比”，前日可用且变化在两位小数展示精度下可见时写“由x%升至／降至y%”，否则写当前值；不得因为当前未舍入值与前日落库四位小数的差异写出“由33.33%升至33.33%”。双动量只把QUALIFIED/NOT_QUALIFIED作为可解释状态，未知不描述进入／退出；轮动同理只描述既有有效状态。基础量价当前状态也占两项证据中的一项，不重复添加第三项。`primary_evidence_type`保存第一项，`secondary_evidence_type_1`保存第二项，`secondary_evidence_type_2`本版恒为空。

版本边界：@1已生成的数据不自动变为@2；本次汇总修正与话术修正一同由@2批次身份及content hash追踪。五方法typed reader不以templateVersion过滤，因此原已验收五方法继续可用；前日洞察比较读取前日方法事实而非旧句子，templateVersion不同不构成禁止比较的理由。生产上线每日洞察前须核对被展示批次确实经@2重新生成；不能在API响应上把旧@1伪标为@2。

**旧结果更新合同（已由M25R替代）：**

部署代码不会自动修改已PUBLISHED数据，也禁止直接UPDATE summary/item。历史范围必须按日期升序重发完整九表批次，五方法公式不变，旧批次保留为SUPERSEDED；但不得继续使用本段早先设计的“逐日完整PLAN后另建APPLY”方式。现行流程统一引用第6.50R节：一个TaskRun先做联合窗口输入审计，通过后自动逐日APPLY，每个日期只完整计算一次。该流程已由TaskRun `10997`完成生产验证；旧TaskRun `10958`永久不得执行，M26只继续最终自动化与交付验收。

**2026-09-04实现与验收记录：**

- 生成侧四个文件及专项测试已实现本节约束；基础算法与五方法事实结构无变化，未增加迁移、API、配置、worker或前端代码。CodeGraph `impact`核对builder→materialization、renderer→builder，源码审计覆盖repository、常量→单日与历史PLAN hash、previous资格投影及typed reader；无新增跨子系统依赖，不调整依赖矩阵。
- 专项测试32项通过；含专项、既有物化、历史planner、typed reader、Task executor、五方法API及子系统依赖矩阵的最终回归237项通过（66.06秒）。仅1条现存TestClient依赖弃用警告，不在本轮修改依赖。文档完整性及`git diff --check`均通过。SQLite内的新旧发布演练证明：同来源六张方法表内容hash完全一致；不足资格100%对应概览由1修为0；旧PLAN拒绝；新批次唯一PUBLISHED、旧批次保留SUPERSEDED。此为本地测试，不是生产更新。
- Figma仅更新下表32个INSTANCE的annotations，没有新增／删除节点，没有修改文字、颜色、字号、组件结构或prototype reactions。每组对全部子节点的ID、父子关系、名称、类型、x/y、宽高、Auto Layout及文本做前后比较，8画板均一致；尺寸保持1600×1292.390625。四组代表截图已工具内检查，最终8画板旧“点击只显示待建设／不得改变URL”注释残留为0。原prototype reactions为空，本轮仍为空，跳转要求由更新后的说明表达，不声称新增了可点击原型连线。

| 正式画板 | 双动量／相对轮动／成员广度／量价分布修改节点ID |
| --- | --- |
| 一级 `1227:22298` | `1227:22308`、`1227:22309`、`1227:22310`、`1227:22311` |
| 二级 `1227:23074` | `1227:23084`、`1227:23085`、`1227:23086`、`1227:23087` |
| 三级 `1227:23850` | `1227:23860`、`1227:23861`、`1227:23862`、`1227:23863` |
| Delayed `1227:24626` | `1227:24636`、`1227:24637`、`1227:24638`、`1227:24639` |
| Loading `1227:25085` | `1227:25095`、`1227:25096`、`1227:25097`、`1227:25098` |
| Empty `1227:25544` | `1227:25554`、`1227:25555`、`1227:25556`、`1227:25557` |
| Error `1227:26003` | `1227:26013`、`1227:26014`、`1227:26015`、`1227:26016` |
| Contract `1227:26462` | `1227:26472`、`1227:26473`、`1227:26474`、`1227:26475` |

本小步通过不关闭G64／G65／G66；下一开发项仍为以下M25 API／前端，生产洞察刷新另行批准。本轮未启动本地常驻Web／前端服务，未写生产库，未部署、提交或推送。

本次回归命令（仅本地测试）：

```text
uv run pytest -q tests/test_wealth_sector_daily_insight_generation.py tests/test_wealth_sector_analysis_daily_materialization.py tests/test_wealth_sector_analysis_daily_replay.py tests/test_wealth_sector_analysis_fact_reader.py tests/test_sector_analysis_daily_task_executor.py tests/web/test_wealth_sector_analysis_api.py tests/architecture/test_subsystem_dependency_matrix.py
...
3. 实现概览、四类完整滚动列表、确定性文案、五态和方法证据跳转；列表严格使用第3.6节当前Figma基线与第8.34节组件／布局编码要求，不从历史修改记录恢复旧行或旧标签。
4. 完成8张Figma、四档、一次409、竞态、401、超时、按需挂载和五方法零回归；执行第11.7节列表专项，覆盖数值列居中、长说明、真实末行无底线及完整独立滚动。

#### M25第1项：后端接口实现记录（2026-09-04）

状态：`本地实现与只读预检通过`；不是M25整体结案。第2～4项前端尚未实施，G64仍等待部署态验收，G65／G66 OPEN。

| 硬口径 | 实现点 | 验证 |
| --- | --- | --- |
| strict、四项身份必填、未知／重复／非法值拒绝 | `schemas/wealth/market/sector_daily_insight.py`及既有`api/wealth/market/sector_analysis.py`两只新GET | 缺字段、重复、非法日期／层级／UUID／debug、非有限数值、字符串伪数值和bool伪数字反例 |
| Meta唯一自动回退、公共20:00、发布覆盖 | `sector_daily_insight_query_service.py::build_meta`复用MarketPageContextQuery；`sector_daily_insight_query.py::load_coverage`只连接日历和PUBLISHED批次 | Ready／Delayed／Empty、20点前后、未来批次不可见；已发布局部缺失不回退；真实SQL计数2 |
| Snapshot不回退、完整四列表、整页单批次 | `build_snapshot`→`load_batch/load_summary/load_items`，三个查询均显式PUBLISHED | 旧batch／错误hierarchy／无该日批次在子表读取前409；summary或items读取中发布被替换同样409；真实SQL计数3 |
| 原始typed事实与模板原文、缺失不编造 | 专属summary／item DTO、明确投影列表、固定原因映射 | 四类排序、count守恒、rank/count、百分位差、阈值／逆势、跨panel同一行业事实一致、最多两证据及模板版本；80项列表无TopN |
| 只读四表、不重算五方法、身份不泄密 | Query仅日历／publish_batch／summary／item；无来源或方法事实表查询，API不返回ORM | SQL监听白名单；真实生成器→repository→查询→DTO测试；FastAPI应用真实路由400／401／409／500及敏感错误反例 |

DTO状态以是否存在可展示事实判断：1日可计算计数、四类事件或其余概览事实仍有内容时保持Ready；全盘平盘的0是有效事实。某个事件列表为空或某项缺失不得升级整页Empty。Meta和Snapshot如实输出读到的templateVersion；旧@1不得伪标@2。此规则不为旧@1汇总／文案正确性背书，正式入口交付仍受M25.0的数据更新条件约束。

新增测试为 `tests/test_wealth_sector_daily_insight_query_service.py` 与 `tests/web/test_wealth_sector_daily_insight_api.py`，共57项；没有修改历史测试来迁就新实现。与上轮32项生成测试、物化／回补／typed reader／Task executor／原五方法API／依赖矩阵共同运行294项通过（65.38秒）。仅有既有Starlette测试客户端弃用提示，不改依赖。新增文件静态检查通过。

**有界生产只读证据：** 使用现有Settings→SessionLocal连接；每次首先设置 `REPEATABLE READ, READ ONLY` 及15秒单SQL超时，结束rollback。白名单仅 `core_serving.trade_calendar`、`wealth_sector_analysis_publish_batch`、`wealth_sector_daily_insight_summary`、`wealth_sector_daily_insight_item`；投影为第7.23节字段，不读取hash、来源JSON或其他方法表。日期覆盖限定2025-08-22至公共目标日，Snapshot只取最新已发布日的三个层级，不导出明细，无生产写入。

| 直接服务读取 | SQL数 | 完整规模／列表 | JSON bytes | 两次单次耗时ms |
| --- | ---: | --- | ---: | --- |
| Meta | 2 | 248个已发布日；第一次251个日历槽，20:00公共日期切换后252槽 | 54,162／54,273 | 84.94／89.05 |
| Snapshot一级 | 3 | 31行业；上涨20、下跌11、转强6、转弱6 | 48,850 | 61.89／128.42 |
| Snapshot二级 | 3 | 128行业；上涨81、下跌47、转强17、转弱21 | 188,240 | 64.92／67.54 |
| Snapshot三级 | 3 | 337行业；上涨196、下跌139、转强47、转弱53 | 497,964 | 78.13／69.61 |

两次最新发布日均为2026-08-31、templateVersion均为@1；公共目标日由2026-09-03切换为2026-09-04，Meta均为Delayed，Snapshot严格保持同日同批内容。各层级1日缺失行业数均为0；此为存量结构、SQL数量、版本和完整列表的只读核验，不是@2生产刷新验收或部署HTTP P95。三级payload低于1MiB，但未缩减字段、列表或行业。临时执行脚本 `/private/tmp/verify_m25_insight_api_readonly.py` 只输出以上统计，没有落地业务行。

CodeGraph使用status／explore／callers及开发后sync，覆盖原sector_analysis路由、公共认证及session、现有五方法QueryService／reader、新增专属schema和查询；当前前端仍只消费原五方法，新API尚无前端消费者。新增依赖只在Biz内及Biz→Foundation，依赖矩阵不变。正式Ready一级Figma `1227:22298`已用设计上下文读取，本轮没有改Figma或前端，也没有做页面像素验收。

本项交付范围为schema、Query、QueryService、原router、新增两份测试与原技术方案／LLD。无迁移、账号、配置、worker、生产任务、部署、提交或推送；没有启动本地常驻Web／Vite服务。下一项继续本M25每日洞察前端；生产@2更新与M26仍单独保留。

#### M25 前端开工纠偏：事实标签（2026-09-04）

用户已确认不再区分前20%／后20%入选原因，直接展示后端已有状态。代码核对为 `sector_daily_insight.py.Event` 六种枚举与 `insight_builder.py._event_type()`；没有独立“进入前／后20%”枚举。本轮只调整三份原文档及正式Figma，不改筛选公式、生成器、API、生产数据或前端代码。

Figma修改记录（file key `RADlZzREU4lPVviYfkLy6x`，page `965:2`）：

| 工作区组件／正式画板 | 修改文本节点 ID | 修改内容 |
|---|---|---|
| L1 `1221:21622` → `1227:22298` | `I1221:21843;1219:21644`、`I1221:21860;1219:21644`、`I1221:21877;1219:21644`、`I1221:21915;1219:21644`、`I1221:21932;1219:21644`、`I1221:21949;1219:21644` | 前三项由+12pp、+11pp、进入前20%改为显著转强；后三项由-13pp、-11pp、进入后20%改为显著转弱 |
| L1说明 | `I1221:21877;1219:21646`、`I1221:21949;1219:21646` | 删除前／后20%示例表述，分别改为“20日同组强度显著转强／转弱” |
| L2 `1225:21837` → `1227:23074` | `I1225:21939;1219:21644`、`I1225:21940;1219:21644`、`I1225:21941;1219:21644`、`I1225:21963;1219:21644`、`I1225:21964;1219:21644`、`I1225:21965;1219:21644`；说明 `I1225:21941;1219:21646`、`I1225:21965;1219:21646` | 同L1的六标签、两说明修正 |
| L3 `1225:22183` → `1227:23850` | `I1225:22285;1219:21644`、`I1225:22286;1219:21644`、`I1225:22287;1219:21644`、`I1225:22309;1219:21644`、`I1225:22310;1219:21644`、`I1225:22311;1219:21644`；说明 `I1225:22287;1219:21646`、`I1225:22311;1219:21646` | 同L1的六标签、两说明修正 |
| 合同 `1226:22295` → `1227:26462` | `1226:22349`、`1226:22350`，以及组件 `1226:22295` 的description | 标题改为“状态与文案”；说明“状态直接映射后端；说明使用后端模板，不使用AI。”；description登记六状态映射及不重新分类、后台阈值不变 |

正式实例均随组件同步：L1实例前缀 `I1227:22730;`、L2 `I1227:23506;`、L3 `I1227:24282;`、合同 `I1227:26894;`，后接上表原组件内节点路径（去掉开头的 `I`）即为实例节点ID。没有新增、删除或脱离组件的节点。

修改前后已分别取得三个Ready和合同画板的1倍截图。全部8张正式画板仍为 `1600×1292.390625`，工作区仍为 `1564×1006`；三Ready组件各344节点、正式画板各436节点；合同组件68节点、正式画板160节点；四状态正式画板各119节点，数量及父子顺序无变化。对全部节点比较布局／样式属性摘要：画板与行容器位置尺寸、Auto Layout、Fixed/Hug/Fill、间距、字体、字号、颜色和变量绑定不变。状态文字仍为11px、内容宽度自适应、标签容器118×28；仅字数变化带来的文字宽度与内部居中位置自然变化，不改容器布局。截图未见新增换行、裁剪或重叠。

原说明中的12／11／13／11个百分点示例保留；原先只写前／后20%的两行没有前日名次和变化值，本轮不补造示例数值，实际开发必须展示后端完整 `renderedText`。合同中的后台阈值说明 `1226:22342/1226:22345` 保持原文，与“页面不细分入选原因”是不同职责。

该段是2026-09-04设计阶段历史记录；其前端工作随后已经完成。模板@2历史刷新不再按M25.0旧式独立PLAN/APPLY执行，现统一受M25R第6.50R节约束。

#### M25 前端开工纠偏：六项列表视觉修正（2026-09-04）

状态：`Figma已修正并核验；前端待实施`。上一节118×28标签、76px行高和原节点数量只记录上一轮文案修改的历史证据，本轮起由第3.6节紧凑布局替代。本轮没有修改业务代码、生产数据、其它模块或“我的自选”组件本身。

共享行`1219:21630`保留横向Auto Layout，改为748×60及8px列距；修改原七列、文字与标签节点`1219:21631`～`1219:21642`、`1219:21644`～`1219:21646`的尺寸／样式，新增80×44的Auto Layout标签槽`1344:24138`，原标签`1219:21642`移入槽内并改为Hug×24；删除旧圆点`1219:21643`。共享标签示例由旧“进入头部区”校准为既定“头部上涨”，36行正式示例的业务文本与数字不变。组件和实例均未脱离或重建。

以下12个面板均保留776×348；每个表头及其七个直属列容器同步尺寸和对齐方式。原三个行实例移入新建的748×262纵向Auto Layout滚动区，表头留在滚动区外；示例行不是TopN限制。

| 层级／列表 | 面板ID | 表头ID | 原行实例ID | 新增滚动区ID |
|---|---|---|---|---|
| 一级／上涨 | `1221:21678` | `1221:21684` | `1221:21699`、`1221:21716`、`1221:21733` | `1347:30303` |
| 一级／下跌 | `1221:21750` | `1221:21756` | `1221:21771`、`1221:21788`、`1221:21805` | `1349:30304` |
| 一级／转强 | `1221:21822` | `1221:21828` | `1221:21843`、`1221:21860`、`1221:21877` | `1349:30357` |
| 一级／转弱 | `1221:21894` | `1221:21900` | `1221:21915`、`1221:21932`、`1221:21949` | `1349:30410` |
| 二级／上涨 | `1225:21869` | `1225:21875` | `1225:21890`、`1225:21891`、`1225:21892` | `1349:30463` |
| 二级／下跌 | `1225:21893` | `1225:21899` | `1225:21914`、`1225:21915`、`1225:21916` | `1349:30516` |
| 二级／转强 | `1225:21918` | `1225:21924` | `1225:21939`、`1225:21940`、`1225:21941` | `1349:30569` |
| 二级／转弱 | `1225:21942` | `1225:21948` | `1225:21963`、`1225:21964`、`1225:21965` | `1349:30622` |
| 三级／上涨 | `1225:22215` | `1225:22221` | `1225:22236`、`1225:22237`、`1225:22238` | `1349:30675` |
| 三级／下跌 | `1225:22239` | `1225:22245` | `1225:22260`、`1225:22261`、`1225:22262` | `1349:30728` |
| 三级／转强 | `1225:22264` | `1225:22270` | `1225:22285`、`1225:22286`、`1225:22287` | `1349:30781` |
| 三级／转弱 | `1225:22288` | `1225:22294` | `1225:22309`、`1225:22310`、`1225:22311` | `1349:30834` |

表头七个直属列ID为同表头前缀下编号分别加1／3／5／7／9／11／13。每个行实例另将`I<行实例ID>;1219:21642`的描边和`I<行实例ID>;1219:21644`的文字颜色覆盖同步为现有`Border/Strong`与`Brand/Primary`，保留原涨跌数字颜色。共享行、三个Ready工作区`1221:21622 / 1225:21837 / 1225:22183`和合同组件`1226:22295`的description补充本轮布局约束。

验收通过：三个工作区组件及正式实例`1227:22730 / 1227:23506 / 1227:24282`合计24个面板、72个行出现位置逐项读取；七列x坐标与表头完全一致（误差0），行均60px，标签均64×24、4px圆角、金字／金边／轻金底，无圆点；说明左对齐、最多2行且高度不超过32px。12个滚动区均为262px、`overflowDirection=VERTICAL`，三个工作区仍1564×1006；8张正式画板仍1600×1292.390625。一级上涨及三级转强面板2倍截图、三级正式整页1倍截图已在本轮工具结果中核对，长说明确实显示两行，未见相邻列挤压。截图仅为本轮Figma工具内证据，不声称存在仓库PNG文件。前端四档响应式、Tooltip及完整列表运行验收仍属于M25编码，不关闭G65／M25。

2026-09-04 用户补充修正：四个数字列从右对齐改为列内水平居中，标题及数值共享中心线；共享行修改`1219:21634`～`1219:21641`，上表12个表头各修改四个数字列及文字（表头编号加3～10）。删除“列表可滚动”文本节点：一级`1221:21683 / 1221:21755 / 1221:21827 / 1221:21899`，二级`1225:21874 / 1225:21898 / 1225:21923 / 1225:21947`，三级`1225:22220 / 1225:22244 / 1225:22269 / 1225:22293`；标题容器自然回流，尺寸不变。共享行、三个工作区及合同组件说明同步新规则；实际滚动、列宽、60px行高、行业／说明左对齐、标签样式均保留。本项仅修改Figma和三份原文档，不改变M25编码／部署状态。

2026-09-04 末行分隔线修正：仅将12个列表最后一行的`strokeBottomWeight`由1改为0：`1221:21733 / 1221:21805 / 1221:21877 / 1221:21949 / 1225:21892 / 1225:21916 / 1225:21941 / 1225:21965 / 1225:22238 / 1225:22262 / 1225:22287 / 1225:22311`，正式实例随工作区同步。行间分隔线与面板外框保留；未删除数据节点、未缩小面板、未填充假示例。共享行和三个工作区的description同步“实际末行无底线、示例3行不是数量限制”，仅设计与文档变更。
...
最后一个浏览器参数使用本机已有Playwright入口，不为复验增加项目依赖。**停止点：** G65只记录本地通过；G64部署验收及G66保持OPEN。默认入口切换已写入当前代码，但历史@2刷新仍受M25R约束。不得执行旧TaskRun／PLAN，也不把本地合成样本当作生产成功。

### M25R：历史洞察刷新执行链去重纠偏

状态：`PASS（2026-09-06，代码、本地自动化、部署、生产刷新与read-back）`。
...
3. 只给板块历史动作切换`audit_then_apply`，一个TaskRun审计通过后自动升序APPLY；Heat及其他动作的PLAN/APPLY保持不变，不新增Worker／Lane／systemd／队列。
...
6. 部署后已按用户指令创建新单TaskRun `10997`，覆盖包含247个模板`@1`日期及1个既有`@2`日期的完整248日窗口；执行后已核对唯一PUBLISHED、模板@2、九表计数、previous链和日期完整性。既有`@2`日期通过同一主链重放后形成一个SUPERSEDED旧批次和一个当前PUBLISHED批次，不构成重复发布。
...
8. 生产验收证据：部署提交`58d316c07eb892144a07f19225de4fcb74088374`后创建新TaskRun `10997`，于`2026-09-05 20:32:53+08`开始、`23:30:29+08`结束。schema v2输入审计为`AUDIT_PASSED`，审计日期248、问题0、预热起点`2025-05-30`；1个审计节点和248个日期节点全部success，`unitDone=unitTotal=248`、`unitFailed=0`、`rowsSaved=6,112,135`。生产read-back确认248个SSE开市日均有且仅有一个当前PUBLISHED批次，模板均为`sector-daily-insight-template@2`、公式包均为`sector-analysis-daily-facts@1`、层级均为`eastmoney_dc_industry_hierarchy.cn_a.v1`，expected／actual计数差异0、previous链差异0；九表批次计数合计与TaskRun一致。旧@1和被重放替换的旧@2批次只保留为SUPERSEDED，不参与公开读取。

M25R本地最低验证命令：

```text
uv run pytest -q \
  tests/test_wealth_sector_analysis_history_input_auditor.py \
  tests/test_wealth_sector_analysis_daily_replay.py \
  tests/test_wealth_sector_analysis_daily_materialization.py \
  tests/test_sector_analysis_daily_task_executor.py \
...
| G52 量价后端 | 三只strict API、PUBLISHED日期／量价切片、四状态、独立原因、完整列表与60日槽；公式仅离线执行 | PASS（M24.6.2代码／测试／等价／性能预检；M24.6.3用户验收结案，G63已关闭） |
...
| M24.5／M24.6 本次成员广度与量价分布验收 | 用户已自行验证两个子板块无问题，批准直接通过，不再要求Codex重复远程验收 | 原公式、完整数据、缺失语义及性能门槛不变；保留已有测试／等价／只读预检，不能将其改写为新增认证HTTP测量 | 2026-09-04明确批准并随本次结案结束；仅覆盖这两个子板块，不扩展到M25／M26。详见M24.5.5、M24.6.3 |
...
4. LLD已决定首版九张非分区表，并冻结comparisonKey、复合FK、partial unique、typed业务列、HDD全对象、source/hash/read-back与原子发布。M23的旧PLAN/APPLY只作为历史事实；后续历史刷新以第6.50R节单TaskRun轻量审计和逐日单次构建为现行合同。
...
10. 2026-09-05旧式PLAN `10958`完成247/247但未写业务表；代码审计确认旧PLAN一遍完整计算、旧APPLY两遍完整计算。该任务永久不得APPLY。M25R随后按第6.50R节完成纠偏，并由2026-09-05至06日执行的新单TaskRun `10997`完成248日生产刷新和read-back，当前发布模板已全部统一为@2。
...
| 历史刷新执行意图漂移 | 输入审计后日期清单、层级版本、公式包、模板或审计合同发生变化仍继续APPLY | 审计snapshot冻结静态身份并在APPLY前复核；来源内容由当日唯一构建记录实际hash，不通过第二次完整计算对账 |
...
28. 单日唯一构建、写入或read-back的batch、日期、层级版本、公式包、来源hash、计划hash、逐表计数或内容hash任一不一致；不得发布部分结果。read-back不得通过重跑公式来“修正”不一致。
29. Ops readiness／dispatcher 通用化改变 Heat、新闻、分钟、QTF 或其他 registered maintenance action 的计划、lane、重试、截止时间、状态或事务边界。
30. `2025-08-22`起历史刷新的审计日期、层级／公式／模板／审计合同与APPLY不一致，或某日失败后无法保持已完成日期可验证、失败日期不可见和升序幂等续跑。
...
33. M25R输入审计调用任何Calculator、InsightBuilder、`preview_trade_date()`、`_build()`或业务repository写入，或者按目标日重复扫描60日完整来源窗口。
34. M25R的同一APPLY日期调用`_build()`超过一次，read-back重跑公式，审计未通过仍进入APPLY，取消被记成success／failed，或取消后继续领取新日期。
35. stale cancel 后 TaskRun 已是 canceled 但仍存在 pending/running 节点，或节点收口改写已终结节点、其他 TaskRun、真实完成数、业务事实；不得重跑 M23。
36. 板块历史动作仍要求用户创建独立PLAN／APPLY TaskRun，旧10958或其他schema v1 PLAN可被新主链执行，或者为本次纠偏新增Worker、Lane、systemd、队列、业务表、来源快照或缓存。
...
| v1.72 | 2026-09-04 | 完成M25第1项后端：strict Meta／Snapshot、2／3SQL、完整列表、批次竞态409、安全错误和57项专项，合并294项回归通过。记录三层级生产有限只读数量／版本／payload；旧@1未更新、前端未开发，G64部署验收及G65/G66仍OPEN |
| v1.71 | 2026-09-04 | 完成获批M25.0纠偏的编码及正反例：广度资格、真实数值／状态模板、@2批次版本与前日资格传递；记录32处Figma说明修正及几何无变化，冻结旧洞察正式更新步骤。M25两只API及前端尚未开发，不关闭后续Gate |
| v1.70 | 2026-09-04 | 根据用户自行验收确认，关闭M24.5成员广度、M24.6量价分布及整体M24/G63；登记本次免重复远程验收的边界，保留历史测试、只读预检和冷请求证据。仅文档状态更新，下一步M25，M25～M26未开始 |
