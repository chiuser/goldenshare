...
- 状态：基础版本、5 日/20 日均值补充功能及十指数成交额扩展 M7 均已完成开发、验证、部署和用户 UI 验收，正式闭环
- 编写日期：2026-08-21
- 本次修订：2026-09-02
...
2026-09-02 十指数扩展开发与本地验收完成：固定十指数批量 Gold Reader、独立 Biz/API 查询链、共享 panel 原语和 2×5 网格均已按 M7 LLD 落地。后端 99 项目标与非目标回归、前端 88 个文件 578 项测试、类型检查、生产构建、正式 Gold 独立 SQL 对账、真实 API 性能/体积门禁和浏览器/Figma 交互验收均通过。Figma 实图复核中发现卡头缺少“成交额”、副标题和卡级日期，已在最终实现中校准；该阶段仅等待用户 UI 验收，未提前标记业务闭环。

2026-09-02 十指数扩展验收闭环：用户已完成部署与 UI 验收并明确确认通过，M7 正式闭环。本次验收只关闭本文定义的功能与交互范围，不改变远程 prod/staging 尚无指数分钟 serving、生产化须另立方案的既有边界。
...
- 5 日均值卡金额使用品牌金色，20 日均值卡金额使用紫色；卡片标题继续使用弱化文字色，不用颜色承担字段含义。
...
- 图例横线与文字垂直居中。
- 图例固定包含四项：当日累计红色实线、昨日累计白色实线、5 日均值品牌金色虚线、20 日均值紫色虚线；均值图例只解释参考线含义，不显示具体数值。
- 5 日均值使用品牌金色水平虚线，20 日均值使用紫色水平虚线；两条线均使用 `1px`、`4/4` 的短密虚线节奏。
- 两个均值都存在时，金额较高者的完整标签放在线段上方 `2px`，金额较低者放在线段下方 `2px`，不按 5 日/20 日身份预设上下位置。金额完全相等时固定为 5 日标签在上、20 日标签在下；只有一个均值时，其标签位于线上方。
- 标签固定锚定绘图区右端，直接显示后端提供的完整文案，例如 `5日均值 23,771亿`、`20日均值 28,064亿`。
...
- `TurnoverLegendItem` 基础组件继续提供 Current、Previous；Loaded 主组件在同一右对齐图例组中增加 `Legend Item / Average 5`（`867:50`）和 `Legend Item / Average 20`（`867:53`），其 swatch 使用与参考线一致的短密虚线。
...
- Loaded 组件（`805:130`）新增：
  ...
  - 两条参考线虚线节奏为 `4/4`；当前样本中 20 日值较高，标签在线上方，5 日值较低，标签在线下方。
...
- 5 日/20 日均值虚线只出现在上图，虚线节奏为 `4/4`；较高值标签位于线上方 `2px`、较低值标签位于线下方 `2px`，下图和 tooltip 不增加均值内容。
- 图例固定显示当日、昨日、5 日均值、20 日均值四项；均值 swatch 的颜色和虚线节奏与图内参考线一致。
- 5 日/20 日均值卡金额分别使用品牌金色/紫色，标题色与其它卡片一致。
- 当日红线、昨日白线、正红负绿差值柱。
- 15 分钟标签和午间休市行为正确。
...
| 两个均值接近时标签都放在线上方 | 文案重叠，无法辨认 | 按金额高低分配上方/下方；相等时固定 5 日在上、20 日在下 |
| 参考线虚线过长、图例缺少均值说明 | 参考线视觉过重且用户无法从图例识别 | 统一使用 `4/4` 短密虚线，并在右侧图例增加两项均值说明 |
| 均值卡金额与参考线颜色脱节 | 卡片和图内事实映射不直观 | 5 日金额使用品牌金、20 日金额使用紫色 |
...
- [x] 用户已完成部署与 UI 验收，本文和 LLD 已同步闭环。
...
技术方案和原 LLD 已于 2026-09-02 评审通过，`ITI_*` 异常码已完成登记。M7-B、M7-C、M7-D 的实现、自动化、正式 Gold、真实 API、Figma、浏览器以及用户部署/UI 验收均已严格按 LLD 完成，本需求正式闭环。远程 prod/staging 没有指数分钟 serving，生产化仍须另立方案。
...
| v1.1 | 2026-09-02 | 选择 `major_index_mins`，完成十指数 2×5 扩展设计、开发、验证、部署和用户 UI 验收；8 个 `ITI_*` 已登记，M7 正式闭环 |
