...
> - 当前状态：v1.83；M22、M23、M24／G63已关闭。M25进行中：生成侧纠偏和两只每日洞察后端API已实现；下一项为前端，默认入口尚未改动，M26未开始。事实标签按用户拍板直接展示后端状态，不细分前20%／后20%入选原因。旧生产洞察尚未更新，不把本地测试或生产只读预检冒充部署验收。
...
说明继续直接展示服务端版本化 `renderedText`，保留两日名次、分母与百分位变化等已有事实；不在前端重拼话术。第12E节后台入选条件（变化至少10个百分点或跨越前／后20%边界）、排序、API字段和生产数据均不变。Figma只同步状态标签和对应说明示例，保留原布局；本次调整本身不要求重算生产事实，原M25.0的模板@2生产更新仍独立按既定步骤执行。
