...
> - 当前状态：v1.43；横截面动量排名 M0～M3A、双动量 M5～M8 与相对轮动 M9～M12R 均已完成并关闭；成员广度 M14 后端与 M15 前端已完成，M16R 等价计算内核纠偏和 M16I 趋势查看均已完成本地代码、自动化与浏览器验收。成员广度仍须部署同一版本并重新执行完整 M16，尚未交付。
...
5. 成员广度作为第四个独立方法，使用同日来源成分关系、股票日行情和复权因子，分别计算成分股数量、成交额参与和均线位置三项客观广度；不合成为综合分、不解释为预测。其正式交互、数据合同和分期方案由第 12C 节冻结；M14 后端、M15 前端、M16R 本地等价计算纠偏及 M16I 趋势查看本地实现均已完成。部署 M16R+M16I 同一版本并重跑完整 M16 后才能宣告交付完成。
...
| 成员广度 | `/wealth/exploration/sector-analysis/member-breadth` | M14 后端与 M15 前端已完成；M16 事实与载荷通过、性能不通过，交付未关闭 |
...
M16R 与 M16I 当前均已完成本地代码和验收。下一步固定为部署包含 M16R 与 M16I 的同一版本，并按原一秒／两秒门禁从头重跑 M16；在真实认证 HTTP 与页面验收通过前，成员广度仍不得关闭。
...
1. 已用真实认证 API 核对生产日期、496行业、五类范围、门槛分布、最大625成员详情、六均线、三趋势范围和 payload；事实与载荷通过。
2. Meta 两轮20次通过；三级 MA20 Rankings 已完成样本约 `19.926～22.471s`，三级 MA60 Rankings 单次服务端耗时 `64.580s`，明确不满足一秒／两秒门禁，因此停止其余性能采样。
3. 已定位为计算器重复构建整批行情索引、重复扫描关系导致的生产规模放大；当前没有证据支持调整数据库、查询数量、产品合同或性能阈值。
4. 性能失败后未继续13状态、真实页面和四档宽度验收，不能用 M15 fixture 代替，成员广度不得关闭。
5. 该失败已经触发 M16R，M16R 与 M16I 本地代码和自动化均已通过；完整 M16 仍须部署二者的同一版本后从头重跑。
...
3. 本地纠偏不能代替部署验收；与 M16I 一起部署后重新执行完整 M16。
...
4. M16I 在本地验收后停止，不自动提交、部署或宣告成员广度关闭；下一步是部署 M16R+M16I 并完整重跑 M16。
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md) 已覆盖已完成的动量排名、双动量、相对轮动及成员广度 M14/M15。M16 已证明生产事实与 payload 正确，但 MA20／MA60 Rankings 真实性能不通过；M16R 已完成本地等价内核纠偏和现实规模自动化，M16I 已完成本地代码、自动化和浏览器验收。下一步固定为部署 M16R+M16I 并完整重跑 M16；在此之前成员广度不能关闭，量价分布不属于本需求。
