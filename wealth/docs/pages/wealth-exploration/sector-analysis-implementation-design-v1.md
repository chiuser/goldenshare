...
> - 当前状态：v1.30；横截面动量排名 M0～M3A 与双动量 M5～M8 已完成并关闭；相对轮动 M10 后端与 M11 前端已完成；M12R 稀疏计算代码与本地全矩阵门禁已通过，下一步只允许部署同一提交并复测 Results，两轮稳态 P95 通过前不得关闭 M12。
...
1. 前端路由已分别识别动量排名、双动量和相对轮动三个精确方法；`SectorAnalysisPage` 按显式方法只挂载一个 controller，方法栏仅将成员广度和量价分布保留为“待建设”。三个已完成方法分别使用独立 API、adapter、URL、controller 和工作区，没有在既有 controller 中增加模式分支。
...
| 相对轮动 | `/wealth/exploration/sector-analysis/relative-rotation` | M10 后端与 M11 前端已完成；M12 真实联调与交付验收待执行 |
...
状态：`CORRECTION CODE PASS / DEPLOYMENT RETEST PENDING (2026-08-29)`。

M12 已在远程部署版本 `6b07ae96c9dab353f801c80c9d77006e12ecc404` 上完成最大窗口真实只读审计。结果不是“可能较慢”，而是已经证明相对轮动 Results 没有达到本文冻结的部署同拓扑稳态 `P95 <= 500ms`：
...
3. **R2 部署复测**：部署同一提交后，对 Meta 和最大 Results 各预热一次，再分别执行两轮、每轮20次 localhost 认证 HTTP；两轮 Results P95 都必须 `<=500ms`，同时保持5 SQL、337行、60轨迹槽和256KB门禁。
4. **失败即停止**：稀疏计算若仍未通过500ms，不得继续猜测或顺手改 SQL。下一步只能先提交一个独立的、只读的事实投影／行物化缩减可行性方案，说明 SQL 语义、缺失传播、共享消费者影响和真实等价证据，等待用户另行批准后才可编码。

M12R 没有改变产品交互、Figma、前端状态、异常码、数据库、索引、缓存或部署配置。代码已按上述边界完成稀疏修正：最大窗口每次只执行2次完整排名、其余63日只投影选中行业，并只构造337个当前点和60个轨迹点；旧生产 `calculate_grid()` 已在等价门禁通过后删除。部署态两轮 HTTP 复测仍未执行，不能把代码通过写成 M12 性能通过。
...
2. Results 两轮20次稳态 HTTP P95 分别为 `1250.883ms/1275.006ms`，未达到500ms，M12 不得标记完成。
3. 四档宽度、普通图／放大图、完整交互和最终用户验收须在 M12R 通过并重新部署后继续，不得以当前失败版本关闭。
4. 完成后仍须由用户明确关闭；不得自动进入成员广度或量价分布。
...
3. 两轮稳态 P95 都通过500ms后，才返回 M12 完成剩余视觉、交互和用户验收。
4. 若仍失败，停在查询投影方案评审前；不得自动增加缓存、索引、结果表、分页、截断或迁移。
...
| 未经证明扩大性能方案 | 稀疏计算仍未达到500ms后直接加索引、缓存或结果表 | 立即停止；先做独立只读查询投影可行性方案并等待批准 |
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md) v1.23 已完成 M12R 稀疏计算代码与本地门禁：API、DTO、公式、SQL、事实源和前端零变化，旧全历史全行业坐标网格已安全删除。下一步固定为部署同一提交并执行第12B.14节两轮最大 Results 复测；通过后再返回 M12 完成四档宽度、交互和用户验收。成员广度和量价分布仍显示“待建设”。

M9～M12R 期间若发现当前数据字段、索引、消费者、真实性能或 Figma 与本文/LLD 冲突，必须停止并回到方案层修正，禁止边编码边改口径。任何新增索引、迁移、缓存、结果表、第三方依赖或范围扩张都不在本方案授权内。
