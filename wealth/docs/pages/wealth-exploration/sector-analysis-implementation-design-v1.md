...
> - 当前状态：v1.78；M22、M23、动量、双动量和相对轮动切读已关闭。成员广度M24.5已提交6f124782，该提交的部署状态未在本轮核验。M24.6.1量价日期合同修正通过：按PUBLISHED选日、取消原始完整日限制，300项后端／214项前端、typecheck/build及有界Prod只读检查通过。本轮量价修改未提交部署，量价数值完整切读仍待下一步；M24／G63进行中，M25～M26未开始。
...
1. 量价计算只使用当前发布行业层级、SSE交易日和`dc_daily`行业日行情；M24.6.1日期元数据改用既有发布批次和1日动量事实，全部只读Prod，不新增上游完整性检查。
...
职责固定为：contract定义版本、枚举、不可变事实和严格解析；calculator只做Decimal纯计算；量价query只做日历和`dc_daily`有界读取与业务键唯一性校验；共享HierarchyQuery读取行业层级，共享FactReader读取发布覆盖；query_service组合公共日期、层级、scope、版本、状态和DTO；API只校验query shape、注入session、调用service和映射已登记异常。
...
M24.6.1使用`SectorAnalysisFactReader.load_momentum_coverage(allow_empty=True)`；若公共目标日及此前尚无发布，Meta返回EMPTY、不编造默认日期。该内部参数仅量价使用，其余方法默认严格行为不变，身份与计数校验仍保留。当前本轮只修正日期合同，Snapshot/Details数值仍沿用既有有界计算；完整量价数值物化切读属于M24.6后续步骤，不得把本轮记为切读完成。
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md)已覆盖第12E节和M21～M26。M22、M23、动量、双动量及相对轮动已关闭；成员广度M24.5已提交6f124782。量价M24.6.1日期合同修正通过，代码与前后端消费者、原方案同步，本轮未提交部署。下一步是量价数值完整切读，仍需单独等价对账。M24／G63仍进行中，M25～M26未开始；M23历史TaskRun只保留证据，不重用。
