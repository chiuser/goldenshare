...
> - 当前状态：v1.3 编码基线；产品口径和技术方案已确认，代码级 LLD 与 Figma 开发交付审计已完成；等待用户确认 LLD 后从 M0 进入开发。
...
2026-08-27 开发交付收口已完成：四张 Ready 统一使用“序号 / 同组强度排名”，上下两图只保留一套共用 `20/30/60` 显示范围，榜单 viewport 已具备纵向滚动语义，并新增 Loading、Delayed、Empty、Error 四张正式状态画板。图表、涨跌数据条和滚动条叠层继续保留绝对坐标；页面壳、工具栏、榜单行、摘要和状态面板继续使用 Auto Layout。
...
| 可观测与异常标准化原则 | 只输出用户状态和已登记异常码；debug 不泄露 SQL | 第 9 节 | READY/DELAYED/EMPTY/ERROR 和权限反例 |
...
| query | 允许值 | 默认值 | 说明 |
|---|---|---|---|
| `tradeDate` | `YYYY-MM-DD` 有效已发布交易日 | 公共默认日期 | 只有用户历史选日时写入 URL |
...
`percentile` 始终表达“强弱位置”，不因涨跌榜切换改变：
...
1. 显示范围只允许 `20/30/60` 个 SSE 已开市且行业日行情已发布的交易日；`observedTradeDate` 是两条历史序列的共同结束日期。
...
| Endpoint | 职责 | 触发时机 |
|---|---|---|
| `GET /meta` | 返回公式身份、允许枚举、当前层级和可选已发布交易日 | 进入动量排名时一次 |
...
`availableTradeDates` 是 `trade_calendar` 开市日与 `dc_daily(category='行业板块')` 已发布日期的交集，不返回自然日，不设置拍脑袋的固定 250 日上限。
...
2. `market` 首期只允许 `CN_A`。
3. `tradeDate` 必须为 `YYYY-MM-DD` 且是 SSE 开市日；开市日尚无行业行情时返回 EMPTY，不把来源缺失误报为参数错误。页面日期选择器只提供 Meta 返回的已发布日期交集。
...
4. 默认模式下当天行业日行情未发布，板块 API 可返回最近已发布交易日和 `DELAYED`；页面显示“当前展示 YYYY-MM-DD 盘后数据”。
...
| `READY` | 目标日有至少一个可计算行业 | 展示完整比较池；缺值行显示 `--` |
| `DELAYED` | 默认目标日未发布，返回较早完整盘后日 | 展示旧数据并明确日期提示 |
| `EMPTY` | 显式日无数据，或比较池无任何可计算事实 | 稳定空态，不展示旧数据 |
| `ERROR` | 查询、层级或合同失败 | 稳定错误态和重试，不用 mock 兜底 |

本期不新增 `PARTIAL` 视觉状态。存在个别缺值时，完整行业行仍返回，`calculableCount/totalCount` 使覆盖事实可观测；只有完全不可计算时进入 EMPTY。
...
1. 三个 endpoint 复用现有 `require_quote_access`，不新增角色或权限模型。
...
3. 涨幅／跌幅排序、`listPosition`、方向无关的 `strengthRank`、并列排名、稳定次序、null 末尾和完整列表。
4. 二级／三级全局排名和父级内部排名同时正确。
5. 历史 `20/30/60` 日、预热、缺点断线、无补值、方向无关强度排名和逐日可计算分母。
6. 默认 20:00 日期、默认延迟回退、显式历史严格命中。
7. 非交易日、非法日期、非法 scope、错层父级、跨父级行业和未知参数拒绝。
8. 层级为空、多版本、错误 parent/root 闭包进入 ERROR。
9. Meta、rankings、history 真实路由返回前端核心字段，不 mock service/query。
10. 现有 `/sector-overview` 在公共层级查询提取后响应和测试零回退。
...
1. 1600px 宽桌面基线逐状态截图对照上述 Figma 节点。
2. `TopMarketBar`、面包屑和市场总览快捷入口不发生视觉漂移。
3. 一级总榜涨／跌、一级内二级、二级内三级分别验收，不用一个默认截图代替。
...
| 版本 | 日期 | 变更摘要 | 负责人 |
|---|---|---|---|
| v1.3 | 2026-08-27 | 完成代码级 LLD 和 Figma 开发交付收口；补齐四个正式异常态、单一显示范围、滚动语义、精确 90 日历史边界和编码入口 | Codex |
