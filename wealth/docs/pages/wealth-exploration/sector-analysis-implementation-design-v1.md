...
> - 当前状态：v1.86；M22、M23、M24／G63已关闭。M25生成侧、两只后端API及前端已完成本地开发验证；默认入口、紧凑完整列表和屏幕居中说明弹窗已落地，尚未部署。G65本地通过，G64部署验收及G66保持OPEN；旧生产洞察@2更新尚未执行，M25未整体结案，M26未开始。
...
状态：`IN_PROGRESS / 本地开发通过、部署待验收`。生成侧、两只API及下列前端项已实现；2026-09-05前端验证见本节末尾。不改变M24已验收结论，旧生产洞察更新和M26未执行。以下开工与后端记录保留其对应日期的历史状态。
...
旧数据更新边界：部署代码不会修改已PUBLISHED的summary和rendered_text。后续须另获生产执行批准，用既有逐日PLAN／APPLY／read-back／原子发布主链更新已批准日期，不能直接UPDATE两张洞察表而留下旧content hash。现有主链以完整九表批次发布，仍会重新生成五方法事实，但公式不变；本轮不新建“洞察专用”任务或worker。先以一个批准日期验证新旧六张方法事实等价、summary差异可解释、句子／版本正确及幂等，再确定历史范围和费用；如连续更新多日，按日期正序逐日重建PLAN并发布，后日使用最新的紧邻前日batch，不能复用绑定旧previous/content hash的历史PLAN。旧批次保留为SUPERSEDED。历史更新尚未执行，不得宣称历史洞察已纠正，也不能提前开放每日洞察默认入口作为已完成交付。
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md)已覆盖第12E节和M21～M26。M22、M23、M24／G63已关闭；成员广度及量价分布以2026-09-04用户自行验收确认结案，不追加Codex远程测量。M25生成侧、后端与前端均已本地实现，下一步是用户审阅／提交部署及独立获批的旧生产洞察更新；M26未开始，历史TaskRun与旧PLAN不重用。不得把本地默认入口实现当作生产入口及@2数据已交付。
