...
> - 当前状态：v1.87；M22、M23、M24／G63已关闭。每日洞察模板 `@2` 已产生一个生产发布日，历史范围仍有247个交易日使用 `@1`。旧式历史 PLAN TaskRun `10958` 虽已完成247/247个日期，但其“PLAN完整计算并丢弃、APPLY再次预览并再次计算”的执行合同已被否决，不得进入 APPLY。M25R纠偏方案已确认、待编码；M26未开始。
...
状态：`IN_PROGRESS / 功能已实现，历史刷新等待M25R`。生成侧、两只API及前端项已有实现证据；生产当前仅一个日期为模板@2，仍有247个交易日为@1。M25整体结案与M26必须等待M25R代码、部署和生产刷新。以下开工与后端记录保留其对应日期的历史状态。
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md)已覆盖第12E节和M21～M26。M22、M23、M24／G63已关闭；成员广度及量价分布以2026-09-04用户自行验收确认结案。当前下一步固定为M25R：先编码并验证“轻量输入审计→同TaskRun自动APPLY→每日单次构建”，再创建新合同的历史刷新任务。旧TaskRun `10958`不得进入APPLY；M25R通过前不得进入M26。
