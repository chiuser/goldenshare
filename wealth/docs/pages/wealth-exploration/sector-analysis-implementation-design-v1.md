...
> - 当前状态：v1.47；横截面动量排名 M0～M3A、双动量 M5～M8 与相对轮动 M9～M12R 均已完成并关闭；成员广度 M14、M15、M16R 和 M16I 已部署，用户页面验收无明显问题。M16R2 已完成本地等价实现和 Prod 只读最大样本复测；用户基于当前机器和完整数据规模，将 Details 最终部署稳态 P95 临时调整为8秒。当前为“代码完成、新门禁已批准、待提交部署验收”，尚未关闭 M16R2/G47。
...
5. 成员广度作为第四个独立方法，使用同日来源成分关系、股票日行情和复权因子，分别计算成分股数量、成交额参与和均线位置三项客观广度；不合成为综合分、不解释为预测。其正式交互、数据合同和分期方案由第 12C 节冻结；M14、M15、M16R 和 M16I 已部署，页面验收无明显问题。M16R2 已保持全量成员、完整趋势和公开合同完成等价投影；旧一秒／两秒门禁已被现实数据规模证伪，用户已批准 Details 临时8秒门禁，待同一提交的部署 HTTP 复测后决定是否关闭。
...
| 成员广度 | `/wealth/exploration/sector-analysis/member-breadth` | M14 后端与 M15 前端已完成；M16R2 事实、载荷、等价和新8秒直接 service 预门禁通过，待提交部署和两轮 HTTP 验收后关闭 |
...
5. 成员广度已由 M15 接入精确路由并按第12C节请求数据；M16I 已补齐趋势图本地查看增量，不能退回“待建设” toast，也不能接入第二套趋势交互。量价分布始终只触发 toast，不改变 URL、页面状态或已加载事实。
...
4. 首屏 Meta 完成后，有合法 `sectorCode` 时 Rankings 与 Details 并发；首次无合法 `sectorCode` 时按“Rankings 返回规范默认选择后再发 Details”的必要顺序执行。Rankings 仍承担一秒／两秒的首要可用目标，Details 是局部重计算区，不阻塞已返回的榜单和工具栏。基于完整625成员、60趋势槽的真实测量，Details 全部均线周期的最终部署稳态 P95 临时统一为8秒；该放宽不得扩散到 Meta、Rankings 或其他方法。
5. Meta 可按 `hierarchyVersion` 在页面生命周期复用；Rankings 和 Details 仅做前端完整 query-key 短期复用，不把缓存当事实源。
6. 前端 Meta 和 Rankings 仍使用既5秒超时；仅 Details 使用10秒超时，为8秒服务门禁保留网络、调度和响应解析余量。快速切换、Abort、request key、409重载和局部重试语义不变。
...
6. 用户已对部署页面做大体验收并确认无明显问题，M16I 的页面交互可记为通过；性能是独立硬门禁，因此 G47 和成员广度整体仍不能关闭。
...
5. Details 分段剖析和 M16R2 等价纠偏已经完成；当时按旧门禁停止是正确执行结果。用户现已根据 MA20/MA60 最大样本证据批准 Details 临时8秒 P95 门禁；M16R2 恢复为待提交、部署和两轮 HTTP 验收，不再继续修改内部算法。
...
1. Details 后端内部查询投影和计算复用已完成；Rankings、Meta、三只公开 API、DTO、Figma和产品数据口径不变。前端仅把 Details 局部请求超时从5秒调为10秒，Meta/Rankings仍为5秒。
...
5. 本地等价、4 SQL、685行和载荷门禁已经通过；Prod只读 MA20/MA60 最大样本完整 service P95 分别为`1,645.836ms/5,502.148ms`，都低于新批准的8秒 Details 门禁。下一步为提交当前等价候选、用户部署同一提交，再以预热后两轮、每轮20次 localhost 认证 HTTP 验收 Details `P95<=8,000ms`。未通过前不关闭 G47/G47A，也不自动进入量价分布。
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md) 已覆盖已完成的动量排名、双动量、相对轮动及成员广度 M14/M15。M16R/M16I 已部署，M16I 页面验收和 M16R Rankings 性能门禁通过；M16R2 等价投影已完成本地代码、自动化和 Prod 只读最大样本复测。用户已将 Details 最终 P95 临时调整为8秒，下一步固定为提交、部署和同一提交的两轮真实 HTTP 验收；量价分布不属于本需求。
