...
> - 当前状态：v1.89；M21～M26、G61及G63～G66A均已完成并关闭。生产历史`2025-08-22～2026-09-07`共253个交易日全部使用每日洞察模板`@2`；2026-09-07工作日20:05自动任务已完成首次真实触发、发布和读回验收。本轮板块分析与每日洞察需求结案。
...
5. 成员广度作为第四个独立方法，使用同日来源成分关系、股票日行情和复权因子，分别计算成分股数量、成交额参与和均线位置三项客观广度；不合成为综合分、不解释为预测。其正式交互、数据合同和分期方案由第12C节冻结；M14～M16系列已收口，当时较重的60日在线计算超过15秒等待，作为历史限制保留。后续M24.5已将行业级榜单、组成及趋势改读物化事实，2026-09-04由用户验收通过并关闭；逐只股票明细仍按需读取，不把原超时TODO继续描述为尚未实施。
6. 量价分布作为第五个独立方法，使用同一行业体系的区间涨跌幅和两段等长窗口成交额均值变化，形成当前横截面散点、完整列表和所选行业双历史趋势；不生成预测、信号、综合分或交易建议。第12D节冻结三只只读API、独立交互、状态和性能；M18～M20已关闭，M24.6.2把在线最大119日原行情计算替换为当前切片／最多60日已发布事实，公式保持不变。M24.6已于2026-09-04由用户验收通过并关闭。
...
| 契约先行与冻结原则 | 既有五方法合同默认保持不变；已批准的量价日期语义及`dateCoverageBasis`修正见M24.6.1，前后端同步，不兼容旧枚举；每日洞察只在技术方案、LLD、迁移和等价对账依次通过后新增两只 endpoint 并切换默认路由 | 第 7、12A.5、12B.6、12C.6、12D.5、12E.8、13 节 | schema 正反例、未知字段拒绝；除明确批准的日期变更外，既有消费者逐字段零变化 |
...
7. M18在线量价曾需要119日原行情窗口；M24.6.2已删除该Query，线上改为最多60日选中行业物化事实。不扩大全局日历上限，不恢复在线预热读取；119日只保留为历史实现和离线公式对账说明。
...
price-volume.wealth_sector_price_volume_daily = PUBLISHED batch × 精确比较键 × period；Snapshot当前完整池，Details单个行业最多60日
```

### 5.2 不读取的已有表

`dc_index`、`board_moneyflow_dc`、`wealth_sector_heat_daily`、股票分钟行情和其他股票衍生表不参与板块分析五个已批准方法。动量排名中的 `dc_member/equity_daily_bar` 仍严格限于三级行业成分股列表；成员广度使用同两张表及 `core.equity_adj_factor` 建立自己的独立事实链。量价分布线上只读日历、层级、PUBLISHED覆盖与量价物化结果，原行情仅供离线发布；不得把成员广度字段反向写入其它方法 DTO，也不得把 `amount` 扩入既有共享 `SectorDailyFact`。
...
生效范围：本节公式和交互合同继续有效；M14～M16的在线行业计算、原始行情日期覆盖与119日Details读取属于历史实现，已由M24.5整体替换。当前行业榜单／组成／趋势读取PUBLISHED物化事实，股票明细只读取目标日成员和最多MA周期数据；日期消费者按12E.9选择已发布日，而非按COMPLETE强制回退。不得重新实现旧在线行业聚合。
...
当前生效范围：公式／交互合同保持不变，公式由离线物化任务执行；M24.6.2线上只消费PUBLISHED量价结果。下文119日和前缀计算说明属于公式及历史实现依据，当前请求边界以12D.6、12D.9及LLD M24.6.2为准。
...
1. 离线量价公式使用当前发布行业层级、SSE交易日和`dc_daily`行业日行情；M24.6.1日期元数据改用既有发布批次和1日动量事实，M24.6.2线上数值改读量价物化结果，全部只读Prod，不新增上游完整性检查。
...
sortDirection = DESC
tradeDate = Meta.defaultTradeDate（以pageContext.tradeDate为上界选最近PUBLISHED；URL未显式选日）
...
返回 `formulaKey/formulaVersion`、周期与范围枚举、defaults、`dateCoverageBasis=INDUSTRY_DAILY`、dateContext、当前层级和日期覆盖。M24.6.1按用户批准修正日期合同：复用已发布批次和动量1日事实，`validSectorCount`只表示当前全部行业中已发布1日涨跌幅可计算的数量，不代表收盘价、成交额齐全，也不代表当前所选周期的量价坐标数。后者仍由Snapshot的`coordinateCount`表达。日期是否可作默认值由PUBLISHED决定，不由COMPLETE决定；已发布但部分或全部不可计算仍保留该日，只有尚未发布才自动选此前最近已发布日。默认日期可对应PARTIAL或MISSING；MISSING本身不能用来推断该日未发布。
...
src/biz/
  ...
  queries/wealth/market/sector_analysis/
    sector_analysis_fact_reader.py             # PUBLISHED量价投影，替代原行情Query
...
职责固定为：contract定义版本、枚举、不可变事实和严格解析；calculator供离线任务做Decimal纯计算；共享HierarchyQuery读取行业层级，FactReader读取发布覆盖、量价当前切片和选中行业历史；query_service组合公共日期、scope、版本、状态和DTO，不实例化计算器；API只校验query shape、注入session、调用service和映射已登记异常。

查询链路固定为：

```text
Meta = 公共页面日期1条 + 当前层级1条 + 已发布1日动量覆盖1条

Snapshot = 公共页面日期1条
         + 当前层级1条
         + 已发布1日动量覆盖1条（精确取请求日，不回退；未发布则返回Empty）
         + 当前比较池量价PUBLISHED物化事实1条

Details = 公共页面日期1条
        + 当前层级1条
        + 已发布1日动量覆盖1条（精确取请求日，不回退；未发布则返回Empty）
        + 最多60个展示用SSE开市日1条
        + 当前选中行业窗口内PUBLISHED量价事实1条
```

Meta 是自动日期解析的唯一责任方：目标日未发布才返回此前最近已发布日及Delayed；已发布但部分缺失不回退。Snapshot和Details严格使用请求`tradeDate`，并从同一发布覆盖读取日期状态；未发布日期返回Empty且不读取原始行情。已发布日不根据1日可计算数量清空独立的价格或成交事实。前端自动模式使用Meta的`defaultTradeDate/defaultStatus`，显式模式使用URL日期；不得自行寻找COMPLETE日。hierarchyVersion、scope、父级、SSE开市日及覆盖区间的安全检查保留。

M24.6.1使用`SectorAnalysisFactReader.load_momentum_coverage(allow_empty=True)`；若公共目标日及此前尚无发布，Meta返回EMPTY、不编造默认日期。该内部参数仅量价使用，其余方法默认严格行为不变，身份与计数校验仍保留。M24.6.2已将Snapshot/Details数值切至上述物化链路，不保留原行情兜底或运行时双读。
...
M24.6.2改读既有结果表；本轮不新增缓存、表、视图、索引、后台任务或迁移，性能门槛不变：

| 接口 | SQL上限 | 最大业务规模 | 部署稳态P95 | payload上限 |
|---|---:|---:|---:|---:|
| Meta | 3 | 当前496节点层级＋全历史日期覆盖聚合 | `<=1,000ms` | `<=256KB` |
| Snapshot | 4 | 当前PUBLISHED批次完整337行 | `<=2,000ms` | `<=256KB` |
| Details | 5 | 1个行业×最多60个交易日的PUBLISHED事实／空槽 | `<=1,000ms` | `<=64KB` |

线上不再构建原行情索引或计算前缀；Snapshot映射当前完整切片，Details只映射单行业各日事实。SQL数量不得随行业或日期增长；公式和前缀算法保留给离线发布与等价测试。
...
6. Details `20/30/60` 点升序、最多60日物化读取、未发布日期槽断线、已发布缺行错误和所选行业不在scope的400。
7. strict query／DTO未知字段拒绝、401、一次409恢复所需合同和安全异常。
8. Meta/Snapshot/Details `3/4/5` SQL、最大337行／60点、payload与响应预算；无逐行业／逐日SQL、零在线公式执行。
...
8. 当前发布层级版本变化时禁止把新层级当日事实与旧层级上一日事实比较。每日动作必须先阻断并要求以新层级版本创建一次“输入审计→升序APPLY”的历史刷新任务；至少重新发布上一 SSE 日和目标日后才能恢复当日洞察，`2025-08-22` 以来历史随后按同一执行合同重放。
...
历史刷新改为一次用户动作内的两阶段执行，不再要求用户先创建一个完整计算的 PLAN TaskRun，再创建另一个 APPLY TaskRun：

```text
同一个 TaskRun
  -> AUDITING_INPUT：只审计日期、层级和来源覆盖，不运行五个公式
  -> 审计不通过：审计结果BLOCKED，TaskRun失败，零业务写入
  -> 审计通过：自动进入 APPLYING
  -> 按交易日升序，每日只构建一次完整结果
  -> 写入、read-back并原子发布该日
  -> READBACK_COMPLETE
...
1. 一个运营动作只接收 `start_date/end_date`；不再让该动作暴露 `execution_mode`、`plan_task_run_id` 或 `plan_hash`。通用 PLAN/APPLY 参数仍保留给 Heat 等既有动作，不能全局删除。
2. 输入审计只做完整性与准入检查：生成从 `max(requestedStart, 2025-08-22)` 到结束日的 SSE 开市日清单，解析首日60交易日预热起点，验证唯一发布层级版本、父子闭包、对象池和六张来源在整个联合窗口内的日期、关键键及有效值覆盖。审计以有界聚合和存在性查询一次覆盖整个范围，禁止逐日加载60日明细。
3. 输入审计禁止调用五个 Calculator、洞察 Builder、`preview_trade_date()` 或 `_build()`，不生成每日期望业务内容、`content_hash` 或完整事实行数。审计通过后冻结的是执行意图：日期清单、预热起点、层级／公式包／模板／审计合同版本、来源覆盖计数、目标表和 `audit_hash`。
4. 输入审计发现整体来源未齐、重复业务键、非法日期、层级闭包错误或其他硬阻断时，snapshot记录`BLOCKED`，同一TaskRun以安全失败终止，不创建BUILDING batch。个别行业可按现有产品合同表达的typed missing不得被扩大为全局阻断。
5. 审计通过后，同一 TaskRun 自动进入 APPLY，不再要求第二次人工提交。每个交易日只调用一次完整构建：同一份构建结果同时产生实际 `source_hash/plan_hash/content_hash`、待写事实和read-back期望；read-back只核对已落盘内容，禁止再次执行公式。
6. APPLY仍一交易日一提交并按日期升序。相同日相同 `plan_hash + content_hash` 幂等跳过；不同结果形成新批次，旧PUBLISHED只在新批次read-back通过后SUPERSEDED。取消或失败只回滚当前未发布日期，已发布日期保留，重试同范围从持久化结果继续。
7. 审计与APPLY之间只把日期清单、层级版本、公式包、模板和审计合同漂移视为执行意图漂移。来源内容可能在审计后正常补齐或变化，APPLY记录实际 `source_hash` 并在该日单次构建中执行硬输入校验；不再用“重新完整计算并比对旧预览hash”防漂移。
8. 进度必须显示阶段、当前日期、已完成／总日期、百分比和最后更新时间。`unitDone`只统计已发布或已幂等核验的交易日；审计期间为0，仅更新阶段和审计项进度。取消点位于每次审计查询前后、每个日期开始前、构建完成写入前和发布后，取消后不得领取新日期。
9. 历史刷新与每日任务继续复用同一计算／发布服务、GENERAL Worker和现有TaskRun／节点表；不新增Lane、Worker、systemd、队列、业务表、API、账号、连接或来源副本。

#### 12E.6.1 旧 M23/M23R 仅作为历史证据

M23/M23R当时的分段PLAN、草稿检查点、取消一致性和一日一提交已经证明长任务可恢复；其生产结果和TaskRun必须保留，不删除、不改写。它们不再定义新历史刷新的执行合同。

旧式 PLAN TaskRun `10958` 于2026-09-05完成247/247个日期并冻结，但执行过程中已经逐日读取完整60日来源并运行五方法计算；结果未写业务表。当前实现随后还会在APPLY的unit执行前完整预览一次，并在materialize内部再次完整构建一次，因此按旧合同继续APPLY会造成额外两遍计算。该TaskRun只保留为不可变审计证据，明确禁止引用执行；部署新合同后必须创建一个新的单TaskRun执行“轻量审计→自动APPLY”。

#### 12E.6.2 M25R：刷新主链去重纠偏

M25R只纠正历史洞察刷新执行链，不改变五方法公式、洞察模板、九表schema、页面或公开API。编码顺序固定为：先加入轻量输入审计与合同版本；再让单日materialize以一次`_build()`同时服务hash、写入和read-back；随后仅为板块历史动作切换成`audit_then_apply`；最后删除该动作的旧逐日完整preview和独立PLAN/APPLY参数。完整代码级删除顺序、影响面和测试见LLD第6.50R节。
...
status / defaultTradeDate / defaultBatchKey
hierarchyVersion
```
...
SA_DAILY_FACT_PLAN_DRIFT           历史刷新日期／层级／公式／模板／审计合同身份漂移
```
...
- Meta 最多3条SQL，Results 从原5条降为最多4条；P95 500ms、256KB及全部对象要求不放宽。实施与只读预验收、用户部署后的正式验收分阶段执行；双动量已完成独立验收并结案，见M24.3。当时其余三方法尚未关闭；M24整体最终结案见下文“M24验收结案”。
...
#### M24.6.1 量价日期合同修正（2026-09-04批准）

本小节保留已提交625ae045的日期纠偏历史；其中“本轮”指M24.6.1。当前数值链路以紧随其后的M24.6.2为准。

取消量价旧的原始行情完整日门槛。日期元数据沿用既有发布覆盖：`dateCoverageBasis=INDUSTRY_DAILY`，可用数量仅为已发布1日涨跌幅可计算行业数，不能声称收盘价或成交额完整；前后端一起移除旧枚举和COMPLETE默认日限制。PUBLISHED决定是否可选默认日，实际坐标能否计算由量价结果决定，两者不混同。

自动模式优先当天PUBLISHED，只有未发布才选此前最近PUBLISHED并提示实际日期；已发布PARTIAL／零可计算不退旧日。显式日期精确命中，未发布Empty且不查原始行情；不能以1日覆盖MISSING清空一个已发布日中独立可用的价格／成交指标。

本轮只修正日期读取、合同、测试和原方案；Snapshot/Details正常路径暂仍为既有有界量价计算，完整数值切读留在M24.6下一步。无新表、补数、配置、兜底或视觉改动；Meta/Snapshot/Details仍3/5/5 SQL。详细删除清单、文件范围及正反例见LLD M24.6.1，不能提前关闭M24／G63。

实施结论：日期合同修正`PASS`。300项后端、214项前端回归、typecheck/build通过；一次Prod只读新Meta返回公共目标2026-09-03、最近发布2026-08-31、DELAYED，251日期槽和496行业，3 SQL，未查原行情。单次848.78ms仅为连接和合同证据，不声称P95验收。修改覆盖15个相关文件，CodeGraph及真实源码消费者复核完成，子系统边界不变。未提交／部署；新旧枚举不兼容，部署时前后端须同批更新。完整证据及数值切读停止点见LLD M24.6.1。

#### M24.6.2 量价数值切读实施合同（2026-09-04）

前置日期合同已提交`625ae045`。数值切读前对两天全部162比较池×五周期、五类范围选中行业60日历史做有限Prod只读对账：11,060条物化记录、11,110次比较，差异0，包含275次单项独立可用。详细范围及证据见LLD M24.6.2。

Snapshot／Details数值仅消费`wealth_sector_price_volume_daily`的PUBLISHED精确切片，价格、成交、名次、四态和缺失原因不在线重算。缺指标行存储分母为NULL，接口的统一组内分母从同次完整物化榜单非空指标数映射，并与可计算行已存分母核对；不新增数据、不修改公式。已发布缺行报错，未发布历史保留日期空槽，不回查原行情填充。

 Meta／Snapshot／Details最多3／4／5条SQL；详情只读选中行业最多60日事实，不加载全组历史。移除已替代在线Query与Calculator接线，保留离线发布使用的计算器。内部debug天数改为实际展示／读取日期数（Snapshot1，Details H），不再报告原始计算预热窗口。业务接口／前端／性能门槛不变。代码、回归和只读预验收完成后停止；远程部署及M24／G63整体结案不能提前声明。

开发阶段证据（历史）：开发与预验收通过，当时未提交部署。13个后端／架构文件371项、21个前端文件214项及typecheck通过。三级30日完整337行、158,554 bytes；所选行业60槽、9,314 bytes；Meta／Snapshot／Details为3／4／5 SQL，固定预热后各20次直接服务P95为259.71／296.98／292.89ms，均在原1秒／2秒／1秒门槛内。只读使用既有连接且无原行情请求；无本地服务启动、数据写入、前端改动或依赖变更。CodeGraph同步及消费者审计完成。完整范围、冷请求和风险见LLD M24.6.2；后续用户验收结案如下。

#### M24验收结案（2026-09-04）

用户明确确认成员广度和量价分布两个子板块已验证无问题，批准直接通过。M24.5成员广度（实现提交`6f124782`）及M24.6量价分布（日期修正`625ae045`、完整数值切读`cd81c702`）均为`PASS / CLOSED`。动量、双动量、相对轮动此前已关闭，因此五方法切读M24及G63整体关闭。

本次以用户验收确认结案，不重复执行Codex远程验收；此前测试和Prod只读计时仍是原阶段证据，不改写为本次认证HTTP测量，也不声称重新核验远程HEAD。公式、完整数据、缺失处理与性能门槛不变；该验收决定仅覆盖这两个子板块，不扩展到M25／M26。详细记录见LLD M24.5.5、M24.6.3。
...
wealth/src/features/wealth-exploration/sector-analysis/daily-insight/
  ...
  ui/
    ...
    DailyInsightRow.tsx
    DailyInsightExplanationDialog.tsx
...
`batchKey`、公式包、模板版本属于内部一致性合同，不写入用户 URL。请求时序为：精确路由激活 → Meta → 确定唯一 observedTradeDate／batchKey → Snapshot；切换层级只请求 Snapshot，切换历史日期按LLD第8.34节先刷新Meta并校准日期，再请求对应 Snapshot。快速切换使用完整 request key 和 AbortController 丢弃旧响应；未选每日洞察时零请求、零 DOM。

交互跳转固定为：

1. 点击行业主体进入现有动量排名工作区，带入同一业务日期、对应全局层级 scope、20日周期和 `sectorCode`；不新增独立“行业详情页”。
2. 点击说明单元格打开居中说明弹窗；弹窗内的方法证据入口进入对应既有方法，带入同一事实日期、层级 scope、行业和该方法已批准的默认参数。事实标签本身不承担跳转。
3. 方法按钮栏普通切换只保留公共 `tradeDate`，目标方法使用自己的默认范围和参数；行业主体／证据点击才按LLD第8.34节显式携带范围与行业。两类跳转均不得把洞察的内部 batch 身份带入用户 URL。

四类列表完整渲染在各自固定高度、固定表头、独立纵向滚动 viewport 中。名称和路径单行省略并提供 Tooltip；1日、5日、20日、名次的表头与数值均在各自列内水平居中，数值使用 `.num` 的字体样式，不修改全局 `.num` 或其它方法的对齐。页面只使用现有背景、边框、文字、行情方向、warning、间距和圆角 token；不复制 Figma 的绝对 `x/y`，不新增硬编码颜色或第二套状态组件。布局编码与边界验收分别见LLD第8.34、11.7节。

2026-09-04 用户确认：事实标签直接映射服务端 `eventType`，不再展示“进入前20%／进入后20%”，也不用 `+12pp` 等变化值充当状态。固定文案为 `HEAD_GAINER → 头部上涨`、`HEAD_LOSER → 头部下跌`、`STRENGTHENING → 显著转强`、`WEAKENING → 显著转弱`、`COUNTER_TREND_STRENGTHENING → 逆势抗跌`、`RISING_BUT_WEAKENING → 上涨滞后`。普通转强／转弱使用中性方向文案，避免把平盘或缺失涨跌幅错误解释为上涨／下跌。页面不根据涨跌幅、百分位或说明文字再判定事件。

说明继续直接展示服务端版本化 `renderedText`，保留两日名次、分母与百分位变化等已有事实；不在前端重拼话术。第12E节后台入选条件（变化至少10个百分点或跨越前／后20%边界）、排序、API字段和生产数据均不变。标签文案纠偏本身不要求重算生产事实，原M25.0的模板@2生产更新仍独立按既定步骤执行。

**2026-09-04 用户六项视觉修正及补充确认：** 三层四类列表统一表头与行左右内距12px，消除原12px错位。1600基准面板仍776×348，行／表头宽748px；行业列压至116px，1／5／20日分别74px、名次78px，列间距8px，事实标签槽80px、说明180px。行高由76px改60px，上下8px、各列垂直居中；1日、5日、20日、名次的表头与数值均在各列水平居中，中心线一致，不再右对齐。删除“列表可滚动”提示文字，保留固定表头和独立纵向滚动。事实标签采用“我的自选”当前A金色细边样式（24px高、12px字、4px圆角、轻金底／细金框／金字），去掉圆点、宽度随文字，不铺满标签列。说明与表头左对齐、11px／16px行高、最多两行，溢出省略但完整原文Tooltip保留。响应式只收缩行业／说明可变列，不缩减数据或侵入相邻列；不改变面板、页面尺寸和其它模块。本轮目标区域按修正后的Figma重新立基线，旧118×28大标签、76px行高和表头错位不再适用；具体Grid、token和验收见LLD第3.6节。

列表最后一条数据行不画底部分隔线，行间分隔线保留。当前Figma列表区固定262px、3条示例各60px，剩余82px是尚未填满的容器空间，不代表默认展示3／4行，也不允许前端生成空行补齐。运行时按完整返回列表渲染并滚动；本轮只去掉末行横线，不改变面板高度。

**2026-09-04 已批准的说明弹窗：** 以补充画板 `1368:24576`、组件 `1367:24138`、实例 `1368:25015` 为依据；380px宽、16px内距、12px间距和圆角，内容高度自适应，示例194px高。使用浏览器原生模态 dialog 的顶层与焦点约束，固定相对可视 viewport 居中；透明外部点击层，不新增遮暗样式。完整原文不截断、不重写；`primaryEvidenceType + secondaryEvidenceTypes` 按后端顺序最多两项，无证据隐藏整个入口区。关闭按钮／外部点击／Escape／背景滚动／日期层级改变关闭，关闭后恢复有效触发元素的焦点。长原文在有限可视高度内滚动，弹窗内部滚动不关闭。保留七列、60px行高与两行摘要，不扩大事实标签或在行内追加按钮。新增组件仅归属daily feature，不引入全局弹窗库或修改其它方法。
...
5. 历史刷新中的每个交易日都只执行一次完整构建；同一构建结果同时提供source／plan／content hash、待写事实和read-back期望。测试必须以spy证明历史审计零公式执行、每个APPLY日期恰好一次 `_build()`，read-back不重跑公式。单日自动动作不属于M25R改动范围。
...
10. `2025-08-22` 以来历史刷新只创建一个TaskRun：输入审计使用联合窗口聚合查询且零公式、零业务写入，审计通过后自动进入APPLY；审计不通过零APPLY。`unitDone`只统计已发布或幂等核验的日期，阶段、当前日期和最后更新时间持续可见。
...
12. `2025-08-22` 以来单任务审计／APPLY／read-back和重试幂等，无自然日空行、无日期洞、无跨批次引用；取消或失败后保留已提交日期并能从持久化结果续跑，回补期间SSD WAL／临时文件峰值受控。
...
4. 行业主体与说明弹窗内的方法证据按钮进入正确既有方法，并带入日期、全局层级、行业和方法默认参数；事实标签不点击。
...
| 模板 | `sector-daily-insight-template@2` | Biz 模板生成器、item、API | 随部署提交生效 |
...
| 历史动作执行风格 | `audit_then_apply` | `src/ops/action_catalog.py`中该action的静态`execution_config`；仅由registered maintenance dispatcher消费 | 随部署提交生效；无env／Settings／数据库配置／前端开关；未配置的Heat等动作保持原PLAN/APPLY行为 |
| 输入审计合同 | `sector-analysis-history-input-audit@1` | Biz审计结果、TaskRun snapshot与integrity hash | 随代码版本生效；旧式PLAN snapshot（包括10958）不得跨版本执行 |
| 物理存储 | `gs_raw_cold_hdd` | 九张表 heap／TOAST、主键和全部索引 | migration 显式DDL并由catalog read-back；缺失时fail-closed |

当前没有新增产品拍板项：完整滚动列表、三层级、默认公共日期、显著变化阈值、确定性模板、`2025-08-22` 起回补和约94%物化范围均已获批准；行业主体进入既有动量排名，说明弹窗的方法证据按钮进入对应方法，事实标签不点击，列表无Top N截断；不增加新页面或新业务能力。
...
状态：`PASS / CLOSED`（2026-09-04）。五方法切读全部通过；成员广度与量价分布依据用户本次自行验收确认结案。以下分阶段实施及验收记录按发生顺序保留，最终结论见第12E.9节“M24验收结案”和LLD M24.6.3。
...
9. 动量首切片的本地冻结回归为439项通过；当时旧在线Snapshot／Calculator仍服务双动量、相对轮动等方法及测试oracle，不能在该切片删除。随后M24.3已移除双动量线上依赖；其他未切方法及离线消费者仍在使用的共享代码继续保留。
...
5. M24R结案时，用户已确认第5节等效验收口径：M24R与动量serving切读子阶段均为 `PASS / CLOSED`，无未决验收项；M24／G63整体仍为 `IN PROGRESS`，当时其余四方法为 `OPEN`。此处保留该轮历史结论，双动量后续结案见M24.3；M24R的验收例外不自动扩展。

#### M24.3：双动量切读实施与验收结案

2026-09-03：`PASS / CLOSED`。部署、接口、页面与数据对账通过，用户单独批准本次HTTPS等效验收后关闭双动量子阶段。双动量Meta／Results已改为PUBLISHED事实读取；只保留既有字段／批次安全检查，不新增完整性计数或上游保障，不回查原行情兜底。目标日已发布PARTIAL保留该日；默认未发布才使用最近已发布日，显式未发布Empty。公开字段、参数、默认值和页面样式不变。以下第1～4项保留实施与只读预验收记录，第5～7项为后续部署验收与结案。
...
7. 验收时公共目标日为 `2026-09-03`，最近PUBLISHED为 `2026-08-31`；默认DELAYED显示真实盘后日期，不代表9月1日以后已物化。M24.3无剩余验收项。当时M24／G63整体仍为 `IN PROGRESS`，相对轮动、成员广度、量价分布切读仍OPEN，下一步为相对轮动；此处保留该轮仅文档结案与提交的历史记录，后续整体结案见第12E.9节“M24验收结案”。
...
#### M24.5：成员广度切读与已批准精度处理

当前状态：`PASS / CLOSED`（2026-09-04）。完整切读提交`6f124782`，用户已验证无问题并批准直接通过，不重复Codex远程验收。以下开工／精度及预检记录按发生顺序保留；开发证据见LLD M24.5.4，最终用户验收结论见M24.5.5。

历史步骤（2026-09-03精度小修阶段）：当时先修正前后端比例校验，行业级排名、组成及趋势切读留待下一步；该完整切读现已完成，见本节末尾和LLD M24.5.4。逐只股票明细仍按需读取，公式、页面布局、表结构、物化任务及生产数据均未改动。

当前代码证实：两张广度物化表的占比与覆盖率均保存4位小数，写入仓储明确按列精度四舍五入；原在线接口返回未按4位截断的比例。现有后端组成校验和前端组成／覆盖率校验使用`1e-6`个百分点误差，直接搬用物化占比会被拒绝。例如6只股票中涨／平／跌各2只，落表后为`33.3333 + 33.3333 + 33.3333 = 99.9999`；已用真实响应模型复现拒绝。5/6覆盖率落表为`83.3333`，同样超出现有前端误差。此问题在切读前暴露，不代表当前已上线页面正在消费这些舍入值。

用户明确拍板：直接使用已存占比，修改校验容纳正常舍入误差，不采用此前“用计数／金额恢复高精度比例”的建议。4位小数单项最大舍入误差为0.00005个百分点；三项组成之和允许0.00015个百分点误差，覆盖率与计数比例的比较允许0.00005个百分点误差。实现仅额外留1e-12用于二进制浮点比较，不能作为数据容差扩张。前后端统一该口径，原样返回数据，不补尾差、不重新计算占比、不改变计数／资格／排名／原因、数据库或历史批次。

本轮修正只涉及组成求和与覆盖率的容差，数值范围、有限值、空值一致性、计数平衡等检查不变，超出正常舍入误差仍拒绝。Meta日期校验留在完整切读步骤按12E.9批准口径处理，不混入本轮小修；页面与公开字段不变。编码和测试要求见LLD M24.5.2。

校验修正已完成：后端组成校验与前端组成／覆盖率校验同步；后端174项、前端板块分析203项通过，含舍入正例、超容差与非法字段反例。typecheck、production build、Ruff、文档与diff检查通过。没有启动服务、请求Prod、修改页面或提交；本结论只关闭舍入校验问题，不代表完整物化切读完成。

M24.5完整切读按用户本轮指令优先完成，量价分布暂停：行业榜单、三项组成和20/30/60交易日趋势只读已发布的两张广度事实表；逐只股票明细继续按需读取目标日成员和最多MA周期日行情／因子。Meta沿用`INDUSTRY_DAILY`身份和已发布1日行业事实覆盖，不新增完整性检查；默认选最近PUBLISHED，部分缺失不退旧日。Meta／Rankings／Details上限为3／4／4条SQL，行业级完整事实目标仍为2秒。物化名次不可由舍入后的占比重新推导：四位小数相同允许已有不同名次，仍严格保留竞争排名及代码稳定次序。具体字段、安全删除和正反例见LLD M24.5.3；不改变公式、页面、数据库或上游数据。

M24.5已完成行业榜单、组成、历史趋势物化切读，以及目标日完整股票明细投影；旧在线行业聚合入口已删除，离线公式保留。后端368项、前端209项、typecheck/build通过。有界Prod样本为2026-08-31、三级337行、BK1205.DC的625成员及60日期槽；Meta／Rankings／Details为3／4／4 SQL，榜单单次159.67～255.31ms，最大Details一次预热后20次直接服务P95为1,081.11ms，满足2秒预检；首次2,061.19ms冷请求保留，不冒充HTTP验收。字段、只读白名单、完整矩阵与脚本路径见LLD M24.5.4。没有生产写入、部署或启动常驻本机服务。

### M25：每日洞察后端与前端

状态：`PASS / CLOSED（2026-09-07）`。生成侧、两只API、前端、历史模板`@2`刷新及生产读回均已完成；以下开工与后端记录保留其对应日期的历史状态。

#### M25开工纠偏：先保证汇总和句子正确

当前代码核验发现两处偏差：`SectorDailyInsightBuilder`把广度不足资格但占比超过50%的行业也计入概览；`SectorDailyInsightTemplateRenderer`只输出当日涨跌和“量价状态、成分股广度”等名称，没有产品基线要求的排名分母、数值、状态及逆势表述。按以下顺序修正：

1. 概览“上涨成分股占比超过50%的行业数”同时要求既有 `member_qualification=ELIGIBLE`、有限占比严格大于50%；等于50%、缺值、不合格均不计。只修汇总，不修改广度公式、门槛或五方法原始事实。
2. 头部涨跌文案写当日涨跌、当前20日同层级名次／分母、近5日涨跌；显著变化写前后20日名次／各自分母、百分位变化。名次变化和百分位变化分开描述，不能因为百分位提高就假称名次上升。逆势抗跌和上涨滞后分别明确当日方向与相对变化；缺失的可选事实整段省略，不编造0或排名。
3. 补充证据按量价、成员数量广度、成交额广度、双动量、相对轮动、MA20广度固定顺序，最多两项。写实际值／中文状态，有可用前日事实才写变化，否则只写可证明的当前状态；未知状态不是证据。广度当前及前日各自检查既有资格，不拿前日不合格占比描述改善。保留表内原始事实，不用清洗或补值掩盖缺失。
4. 文案生成合同升级为 `sector-daily-insight-template@2`，同时标识本次概览资格修正；五公式包仍为 `sector-analysis-daily-facts@1`。batch、item、PLAN均引用同一版本常量；禁止repository继续硬编码旧版本。上一日比较仍按日期、层级和公式包匹配，不因话术升级丢弃真实前日五方法事实。
5. Figma每日洞察正式画板中，清理双动量、相对轮动、成员广度、量价分布的旧“待建设”交互说明，改为进入既有正式方法、只带入日期；不改视觉、尺寸、节点结构或已验收工作区。
6. 自动化证明资格正反例、四类句子／逆势、独立分母、证据最多两项、缺失省略、字节确定性、batch/item版本一致、旧版本五方法仍可读。然后继续下列M25开发项。

旧数据更新边界：部署代码不会修改已PUBLISHED的summary和rendered_text，也不能直接UPDATE两张洞察表而留下旧content hash。生产刷新必须走M25R的新单TaskRun主链，以完整九表批次按日期升序重发；五方法公式不变，旧批次保留为SUPERSEDED。输入审计只做一次联合窗口准入检查，审计通过后自动逐日APPLY；每个日期只完整计算一次。旧PLAN `10958`不得引用，M25R部署和生产read-back完成前不得宣称历史洞察已纠正。

前一轮实现：上述生成侧纠偏和32处Figma导航说明已完成，本地专项32项通过。原有物化、历史planner、typed reader、Task executor与五方法API的回归记录见LLD M25.0。2026-09-04继续完成下列第1项后端，前端第2～5项仍未实施，不将M25整体标为完成。
...
4. 行业主体进入现有动量排名，说明弹窗内的方法证据按钮进入对应方法；带入日期、层级、行业和方法默认参数，不新增行业详情页。事实标签不点击。
5. 完成8张 Figma、四档宽度、未选零请求、401／409、超时、竞态和既有五方法零回归测试。

**第1项后端实现与证据（2026-09-04）：**

- 新增专属schema、Query、QueryService，并在原sector_analysis router增加Meta／Snapshot；只读日历、发布批次、洞察summary与item四表。公开字段逐项投影，保留批次的真实模板版本；不调用五公式、五个工作区QueryService，不读来源行情，不改表、任务或运行配置。
- Meta遵守公共20:00日期及完整PUBLISHED批次回退，Snapshot精确绑定日期／batch／hierarchy，读取summary与items时再次在同一SQL检查PUBLISHED。读取中批次被替换返回409，不增加补查SQL，不拼接两批事实。四列表全量、稳定排序，原文直接返回，不在API重写模板。
- 固定缺失原因按LLD顺序输出；多方法缺失计数可重叠，不强制它们之和等于1日行情缺失行业数。全盘平盘仍为Ready；仍有其他可展示事实时，不因为1日字段缺失而清空整页。
- 新增后端专项57项通过；连同上一轮生成侧、物化／回补／reader／Task executor、五方法API及依赖矩阵共294项通过（65.38秒）。其中真实FastAPI应用＋SQLite路由测试验证序列化、400／401／409／500；生产只读预检不是部署HTTP验收。
- 生产既有连接两次有限直调：最新发布日2026-08-31，模板仍为@1；一级／二级／三级完整数量31／128／337，三级四列表196／139／47／53，约486KiB。Meta固定2SQL、Snapshot固定3SQL，单次约62～129ms；这是有限冷读观察，不是稳态P95，也不证明旧@1文案已修正。只查询四表、只读事务最终回滚，无明细导出或生产写入。
- 已读取正式Ready一级Figma设计上下文，仅核对下一步前端输入；没有修改Figma或页面。下一项固定为本M25前端，不进入M26，不提交、部署或刷新生产洞察。G64保留部署验收OPEN，G65/G66保持OPEN。

#### M25前端本地实现与验证（2026-09-05）

- 按第12E.10节及批准的说明展开稿实现daily feature、精确路由、根地址跳转和六按钮。共享Shell／Panel／MetricCard／DataStatusBadge及Design Token复用；没有修改后端、公式、数据库或其它五方法业务逻辑。
- 两只真实API串行消费：Meta决定日期／batch，Snapshot只读对应事实；层级只重取Snapshot，日期先刷新Meta。保留显式未发布Empty、自动Delayed、一次409重载、5秒超时、旧请求丢弃、401公共认证及完整数组。
- 四面板使用同一七列Grid、60px行、24px紧凑标签、两行原文和独立262px滚动区；实际末行无底线。说明弹窗固定在可视屏幕中心，完整原文、0／1／2证据与关闭规则按已批准设计实现。
- 前端专项91项、全量104文件／803项、typecheck与生产构建通过；重跑既有后端专项57项通过。四档浏览器验证及0／1／3／4／5／80行、各层级、Loading／Delayed／Empty／Error、居中弹窗均有本地证据，详见LLD本次记录。
- 浏览器使用真实应用路由／查询／序列化配合临时内存测试数据；局部布局和状态用明确标注的合成响应验证。不是生产数据、生产性能或最终用户验收。临时服务已退出，不占用默认开发端口。
- G65记录本地通过，G64部署验收及G66仍OPEN。当前代码包含默认入口切换，但正式交付仍须先按M25.0完成获批生产@2更新与核验，再进入M26；本轮未提交、部署、执行生产任务或自动推进下一阶段。

### M25R：历史洞察刷新执行链去重纠偏

状态：`PASS / CLOSED（2026-09-06）`。

1. 代码审计确认旧链路存在三次完整计算：`plan_for_task_run()`逐日调用`preview_unit()`完成第一遍；APPLY的`execute_unit()`先调用`preview_trade_date()`完成第二遍；`materialize_trade_date()`内部再调用`_build()`完成第三遍。三次都会读取同一日最多60个交易日的来源并运行五方法，只有第三遍写库。
2. 把历史动作改为单TaskRun的`AUDITING_INPUT -> APPLYING -> READBACK_COMPLETE`主链。输入审计只做联合窗口完整性、层级和关键键检查，严禁调用公式或生成结果hash；通过后自动APPLY，失败则零业务写入。
3. 每个APPLY交易日只调用一次完整构建；hash、事实写入和read-back期望必须来自同一个不可变构建结果。read-back只核对数据库中实际结果，不得重算公式。
4. 仅板块历史动作使用action catalog中的`audit_then_apply`静态执行风格；Heat及其他通用PLAN/APPLY动作保持原样。继续使用GENERAL，不增加Worker、Lane或部署单元。
5. 旧TaskRun `10958`只保留为历史证据，不得APPLY。完成代码、自动化和部署后，重新创建一个新合同的单TaskRun刷新仍为模板`@1`的247个交易日。
6. M25R完成并通过生产read-back前，M25历史刷新与M26保持OPEN；不得继续旧APPLY，也不得直接改写洞察表。
7. 本地实现已新增联合窗口输入审计、schema v2审计snapshot、单TaskRun自动APPLY、每日期一次构建、PUBLISHED幂等read-back和旧参数／旧合同拒绝；Heat继续使用原PLAN/APPLY。196项板块、Ops、Heat、news、worker和架构回归通过，Alembic仍为单一head `20260903_000169`。本轮没有新增迁移、表、Worker、Lane、API或前端，也未连接Prod、创建TaskRun或刷新生产数据。
8. 生产TaskRun `10997`已完成248个历史交易日的模板`@2`刷新；截至2026-09-07，当前253个PUBLISHED交易日全部为模板`@2`，旧式TaskRun `10958`继续保持不可执行。
...
1. 此前M26只读终验已确认生产基础设施、每日洞察Meta／Snapshot、五个分析模块物化切读及页面交付满足冻结合同。
2. 自动计划`41`（`maintenance.materialize_wealth_sector_analysis_daily`）在2026-09-07 20:05:22首次真实触发；TaskRun `11210`于20:07:50以`success`结束，`unitDone=unitTotal=1`、`unitFailed=0`、进度100%、保存24,680行、拒绝0行。
3. 唯一维护节点`17823`为`success`且无issue；2026-09-07批次`a36be146-aecd-435b-a629-22c6734ea862`为PUBLISHED，expected／actual／物理九表计数三者完全一致，总计24,680行，hash格式与previous批次链均通过。
4. 上一交易日2026-09-04批次继续为PUBLISHED；生产覆盖已扩展为`2025-08-22～2026-09-07`共253个PUBLISHED交易日，253日全部使用模板`sector-daily-insight-template@2`。
5. 远程`dev-interface`部署、Web／GENERAL Worker／Scheduler的active与enabled状态、Web入口及两个健康接口均通过；计划下一次运行时间为2026-09-08 20:05。M26据此关闭，不再安排后续开发里程碑。
...
| 量价物化历史错误恢复原行情窗口 | 切读后仍读取119日原行情或全池历史 | M24.6.2只读取单行业最多60日物化事实；不恢复已删除Query，不放宽共享上限 |
...
| 历史输入审计被实现成完整计算 | 审计逐日加载60日来源并运行五方法，结果随即丢弃 | M25R只对联合窗口做聚合完整性／关键键／层级准入检查；自动化spy证明Calculator、Builder、preview和业务写入调用均为0 |
| APPLY重复计算 | unit执行前先完整preview，materialize内部再次 `_build()` | 每日只构建一次不可变结果；同一对象产生hash、写入内容和read-back期望，read-back只查询已落盘事实 |
| 长周期刷新无进度且取消后全部丢失 | 执行只在完整返回后写100%，或未按日持久化 | 同一TaskRun持续显示审计／APPLY阶段；完成数只认已发布或幂等核验日期，一日一提交，取消只回滚当前未发布日期 |
...
| 旧PLAN被误用于新刷新 | `10958`等旧snapshot仍显示FROZEN/applyReady | 以`sector-analysis-history-input-audit@1`拒绝旧schema；旧TaskRun只留审计证据，不得执行 |
...
[板块分析低层设计 v1](./sector-analysis-low-level-design-v1.md)已覆盖第12E节和M21～M26。M21～M26及G61、G63～G66A均已关闭；成员广度及量价分布以2026-09-04用户自行验收确认结案，M25R历史刷新和M26首次自动任务也已通过生产读回。旧TaskRun `10958`永久不得进入APPLY。本轮需求已关闭，没有后续开发里程碑。
...
| 版本 | 日期 | 变更摘要 | 负责人 |
|---|---|---|---|
| v1.89 | 2026-09-07 | 完成M26生产结案：此前基础设施、两只每日洞察API、五方法物化切读及页面交付终验已通过；自动计划41首次真实运行生成TaskRun 11210并成功发布2026-09-07批次，1/1单元、24,680行、0失败、0拒绝，九表expected／actual／physical计数一致，previous链及服务健康通过。当前2025-08-22～2026-09-07共253个PUBLISHED交易日全部为模板@2；关闭M25、M26及G61、G64～G66，本轮需求结案 | Codex |
| v1.88 | 2026-09-05 | 完成M25R本地实现：板块历史动作改为联合窗口轻量输入审计、schema v2审计snapshot和同TaskRun自动APPLY；每日期只构建一次，PUBLISHED幂等路径从数据库read-back，旧PLAN参数及旧合同拒绝。196项板块、Ops、Heat、news、worker和架构回归及单一Alembic head通过；未部署新代码、未连接Prod或刷新247日历史 | Codex |
| v1.87 | 2026-09-05 | 按用户确认纠正历史刷新执行合同：PLAN不再逐日完整计算，而只做联合窗口输入完整性审计；审计通过后同一TaskRun自动APPLY，每个交易日只构建一次，read-back不重算公式。登记M25R、`audit_then_apply`静态作用域、旧TaskRun 10958禁用、安全删除顺序与测试门禁；本轮只修文档，不改代码或执行生产任务 | Codex |
| v1.86 | 2026-09-05 | 按批准的屏幕居中说明稿完成M25前端：独立route、三参数URL、两API、六状态标签、紧凑四列表、原文弹窗及方法跳转；专项91／全量803、typecheck/build、后端57及四档本地浏览器通过。G65本地通过，生产@2更新／部署／M26仍未执行 | Codex |
| v1.85 | 2026-09-04 | 配合最新Figma的LLD收口，清除每日洞察旧“数值右对齐”要求，统一为四个数字列的表头／数值居中，并关联LLD布局编码与验收章节；其它方法、API和数据口径不变，M25前端待实施 | Codex |
| v1.84 | 2026-09-04 | 按六项视觉意见统一每日洞察列表列宽与内距，采用自选列表同款紧凑金色标签、两行说明及60px行；正式Figma三层12面板与实例已同步，LLD登记节点和核验。未改代码／API／数据，M25前端仍待实施 | Codex |
| v1.83 | 2026-09-04 | 按用户拍板将每日洞察事实标签统一为后端六种eventType的固定文案，不细分前／后20%入选原因；同步正式Figma与LLD展示及反例要求。后台条件、API、排序和生产数据不变，前端及原@2生产更新仍待实施 | Codex |
| v1.82 | 2026-09-04 | 完成M25第1项Meta／Snapshot后端及57项专项测试，294项回归通过；记录生产四表有限只读的版本、完整数量、2／3SQL和payload。与LLD统一Meta状态字段、日期刷新和普通按钮仅传日期；前端未开发，旧@1结果未更新，不关闭M25／M26 | Codex |
| v1.81 | 2026-09-04 | 执行获批M25开工纠偏：广度概览遵守既有资格、实际数值与状态话术及两证据上限、模板@2统一版本、32处Figma导航说明；明确旧生产数据更新的正式发布步骤，保持五公式及M24验收不变。M25未整体完成 | Codex |
| v1.80 | 2026-09-04 | 根据用户自行验收确认，关闭M24.5成员广度、M24.6量价分布及整体M24/G63；保留历史测试与只读预检，不声称新增远程计时或部署核验。仅更新文档状态，M25每日洞察为下一步，M25～M26未开始 | Codex |
| v1.79 | 2026-09-04 | 完成M24.6.2量价数值切读，删除旧原行情Query与在线计算器接线，保留离线公式；统一分母／独立缺失／单行业60日槽和3/4/5 SQL。11,110次Prod对账零差异，371后端／214前端／typecheck通过，直接服务P95 259.71/296.98/292.89ms通过预检；未提交部署，不整体关闭M24/G63 | Codex |
| v1.78 | 2026-09-04 | 完成M24.6.1量价旧日期合同修正：按PUBLISHED选择默认日，INDUSTRY_DAILY只表示已发布1日涨跌幅可计算数量；删除原始完整日入口，前后端共同接受PARTIAL／零可计算默认日，显式未发布Empty且不读源行情。300后端／214前端、typecheck/build及3 SQL只读检查通过；完整量价数值切读未执行，本轮未提交部署 | Codex |
| v1.77 | 2026-09-04 | 完成M24.5成员广度物化切读：榜单／组成／历史只读发布事实，股票明细保留；删除在线行业聚合，校准日期、舍入和真实名次。368后端／209前端通过；337行、625成员、60槽及3/4/4 SQL，最大详情只读服务P95 1,081.11ms。新代码未提交部署，量价分布不推进 | Codex |
| v1.76 | 2026-09-03 | 用户拍板舍入精度处理：只调整校验，三项合计容差0.00015个百分点、覆盖率容差0.00005个百分点；不恢复比例、不修改数据。替代上一版待确认建议，先修校验，完整成员广度切读仍待实施 | Codex |
| v1.75 | 2026-09-03 | 按用户自行验收结论关闭M24.4，不重复验收；推进M24.5开工核对，复现物化4位占比与原前后端1e-6校验冲突，记录利用已存计数／金额恢复响应百分比的待确认建议与日期消费者校准边界。未修改代码或生产数据 | Codex |
