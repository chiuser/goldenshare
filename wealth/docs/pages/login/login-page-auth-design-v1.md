...
5. 浅色背景下，登录表单采用深海蓝文字与暖金边框，保证与天空背景的对比度。
6. 输入框内容与 placeholder 使用市场总览页同口径的基础无衬线字体，不使用“用户名/密码”标签的书法字体。
7. 表单字段包含：
   - 用户名
   - 密码
8. 按钮包含：
...
10. 右下角有数据接入状态提示，当前实现为 `数据接入状态：登录保护已启用`。
...
5. 浅色背景下输入框内容必须使用 `--cs-font-family-base`，不得复用标签的书法字体。
6. 不出现运营后台风格组件。
7. 不出现浅色后台登录页。
