...
- [财势乾坤交易助手标杆需求 v1（v1.35；输入恢复仅保证服务端已留存内容，其他填写数据丢失排除；既有算法不变）](./pages/trading-assistant/trading-assistant-benchmark-requirement-v1.md)
- [财势乾坤交易助手技术实施方案 v1（草稿修订 44；输入恢复边界已确认；低层设计直接复用为 LLD，整体未完成，未编码）](./pages/trading-assistant/trading-assistant-implementation-design-v1.md)
