...
export function buildSectorMemberBreadthRankingsViewModel(payload: unknown, request: SectorMemberBreadthRankingsRequest): SectorMemberBreadthRankingsAdapterResult {
  ...
  if (status === "EMPTY" && (exceptionCode !== "SA_SOURCE_EMPTY" || availability.calculableSectorCount !== 0)) fail("EMPTY Rankings 必须使用空来源事实壳。 ");
...
function validateDefaultDate(dates: SectorTradeDateAvailability[], selected: string | null, status: "READY" | "DELAYED" | "EMPTY") {
  // The server selects PUBLISHED dates; availability only describes calculable facts.
  if (selected !== null && !dates.some((day) => day.tradeDate === selected)) fail("默认日期不在公共日期范围内。");
  const expected = selected === null ? "EMPTY" : selected === dates.at(-1)!.tradeDate ? "READY" : "DELAYED";
  if (status !== expected) fail("默认日期与状态不一致。");
}
...
function parseRankingRow(value: unknown): SectorMemberBreadthRankingRow {
  ...
  if ((calculableCount >= 5 && coveragePct >= 80) !== (qualificationStatus === "ELIGIBLE") && !(qualificationStatus === "INELIGIBLE" && reasonCodes(row.reasonCodes).includes("AMOUNT_NON_POSITIVE"))) fail("排名资格与5+80%门禁不一致。 ");
...
function validateRankingOrder(rows: SectorMemberBreadthRankingRow[]) {
  ...
  for (let index = 0; index < eligible.length; index += 1) {
    ...
    if (row.rankTotal !== eligible.length) fail("rankTotal 必须等于有资格行业数。 ");
    if (previous && previous.metricValuePct! < row.metricValuePct!) fail("有资格行业未按指标降序。 ");
    const tied = previous !== undefined && previous.rank === row.rank;
    if (tied && (previous.metricValuePct !== row.metricValuePct || previous.sectorCode > row.sectorCode)) fail("并列行业未按代码稳定排序。 ");
    // Do not reconstruct pre-rounding competition ranks from four-decimal display values.
    if (!tied && row.rank !== index + 1) fail("行业排名不是标准竞争排名。 ");
  }
