...
export function PositionDetailDialog({ selected, row, token, onClose, onRefresh, onSell }: {
  selected: string; row: PositionRow; token: string; onClose: () => void; onRefresh: () => void;
  ...
  useEffect(() => {
    ...
    void getPositionDetail(selected, row.stockRef.tsCode, token, abort.signal).then(value => {
      if (!abort.signal.aborted && epoch === getAuthEpoch()) { setDetail(value); if (value.accountRounds.length === 1) setAccount(value.accountRounds[0].accountRef.accountId); }
    }).catch(reason => {
    ...
    return () => abort.abort();
  }, [selected, row.stockRef.tsCode, token, retry]); // callbacks do not define a read identity
  ...
  return <TradingAssistantDialog variant="drawer" title={`${row.stockRef.name} ${row.stockRef.tsCode}`} subtitle="当前持仓 · 分账户核对" onClose={onClose}
    footer={<><TradingAssistantAction onClick={onClose}>关闭</TradingAssistantAction><TradingAssistantAction primary disabled={!chosen || chosen.availableQuantity === null || BigInt(chosen.availableQuantity) === 0n}
