...
import { positionNumber as money } from "../model/positionPresentation";
...
export function RecordDetailDialog({ value, onClose, onMaintain }: { value: SavedRecordDetail; onClose: () => void;
  ...
  return <TradingAssistantDialog variant="fees" title={value.kind === "TRADE" ? "原始成交详情" : "资金流水 · 原始收支"} onClose={onClose}
    footer={<TradingAssistantAction onClick={onClose}>关闭</TradingAssistantAction>}><RecordDetailContent value={value} onMaintain={onMaintain} /></TradingAssistantDialog>;
}
export function RecordDetailContent({ value, onMaintain }: { value: SavedRecordDetail;
  onMaintain: (source: MaintenanceRecord, action: "CORRECT" | "VOID") => void }) {
  ...
  return <>
    <div className="ta-recovery-summary">{value.kind === "TRADE" ? `${value.detail.record.stockRef.name} ${value.detail.record.stockRef.tsCode} · ${record.direction === "BUY" ? "买入" : "卖出"}`
    ...
    {value.kind === "CASH_FLOW" && <div className="ta-record-profit"><strong>{money(value.detail.record.amount)}</strong></div>}
    <dl className="ta-record-facts">
      {value.kind === "TRADE" ? <><dt>成交数量</dt><dd>{value.detail.record.quantity} 股</dd><dt>成交价格</dt><dd>{value.detail.record.price}</dd>
        ...
        <dt>印花税</dt><dd>{value.detail.record.stampTaxAmount}</dd><dt>现金变动</dt><dd>{record.netCashChange}</dd></> : <><dt>所属账户</dt><dd>{record.accountRef.brokerName} · {record.accountRef.name}</dd><dt>发生日期</dt><dd>{value.detail.record.occurredOn}</dd><dt>资金方向</dt><dd>{record.direction === "IN" ? "转入" : "转出"}</dd></>}
      <dt>备注</dt><dd>{record.note || "—"}</dd>
    </dl>
