...
export function PositionsWorkspace({ selected, revision, onSell, actions }: { selected: string; revision: number; actions: ReactNode; onSell: (account: string, stock: StockRef) => void }) {
  ...
  return <section className="ta-positions" aria-label="持仓股">
    ...
    {data && <>
      ...
      <div className="ta-position-toolbar"><div className="ta-segments" role="group" aria-label="持仓视图">{([['list', '列表'], ['bar', '条形图'], ['pie', '饼图'], ['map', '盈亏地图']] as const).map(([key, label]) => <button type="button" key={key} aria-pressed={view === key} onClick={() => setView(key)}>{label}</button>)}</div>{actions}<TradingAssistantAction onClick={read.refresh}>重新读取</TradingAssistantAction></div>
