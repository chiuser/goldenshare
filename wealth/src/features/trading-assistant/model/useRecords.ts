...
export function useRecords(selected: string, category: RecordCategory, filter: RecordFilter, round: RoundRecordsScope | null, revision: number) {
  ...
  const key = JSON.stringify([selected, category, filter, round, revision, reload, pageIndex]);
  const identity = JSON.stringify([selected, category, filter, round, revision]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!totals || totals.requestedStartDate !== month.start || totals.requestedEndDate !== month.end
        ...
        totals = await getRecordsSummary(recordRange(selected, month), abort.signal);
      }
      if (!current()) return;
      ...
      if (current()) { summary.current = totals; setReading({ key, scopeKey: `${selected}:${revision}`, summary: totals, page }); }
    })().catch(reason => {
      if (!current()) return;
      if (reason instanceof TradingAssistantApiError && reason.details.code === "TA_READ_CONTEXT_CHANGED") refresh();
  ...
  return { summary: reading?.scopeKey === `${selected}:${revision}` ? reading.summary : null, page: visible?.page ?? null, error, busy, pageIndex, refresh,
