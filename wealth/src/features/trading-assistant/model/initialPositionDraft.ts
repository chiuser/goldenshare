...
export interface PositionDraft { clientRowId: string; stock: StockRef | null; quantity: string; availableQuantity: string; costPrice: string }
export function positionDrafts(rows: InitializationPositionInput[]): PositionDraft[] {
  return rows.map(row => ({ clientRowId: row.clientRowId, stock: { tsCode: row.tsCode, name: "" },
    quantity: String(row.quantity), availableQuantity: String(row.availableQuantity), costPrice: row.costPrice }));
}
export function validateInitialPositions(rows: PositionDraft[]) {
