...
export function buildWatchlistRow(item: WatchlistItemDto): WatchlistRowViewModel {
  return {
    membershipId: item.membershipId,
    isPinned: item.isPinned,
    groupMarks: item.groupMarks,
    ...
    peTtm: item.valuation.peTtm !== null && item.valuation.peTtm > 0 ? number(item.valuation.peTtm) : "--",
    pb: item.valuation.pb !== null && item.valuation.pb > 0 ? number(item.valuation.pb) : "--",
