...
describe("add watchlist dialog", () => {
  it("opens empty with a fixed results area, then renders three columns after exactly 500 ms", async () => {
    ...
    const onAdd = vi.fn().mockResolvedValue(result);
    const props = {
      open: true,
      onClose: vi.fn(),
      onAdd,
    ...
    const {
      rerender
    } = render(<AddWatchlistDialog {...props} />);
    ...
    expect(screen.getAllByRole("columnheader").map(node => node.textContent)).toEqual(["代码", "名称", "状态"]);
    ...
    expect(addButton.querySelector(".watchlist-plus-icon")).toHaveAttribute("aria-hidden", "true");
    expect(screen.getByRole("columnheader", {
      name: "状态"
    })).toHaveClass("watchlist-search-status");
    ...
    rerender(<AddWatchlistDialog {...props} mutation={{
      kind: "succeeded",
      action: {
        groupId: 1,
        tsCode: "000001.SZ"
  ...
  it("cancels old searches, suppresses out-of-order responses, resets on close, and respects IME", async () => {
    ...
    search.mockReturnValueOnce(old.promise).mockResolvedValueOnce({
      groupId: 1,
      keyword: "NEW",
      items: []
    });
  ...
  it("preserves input on failure, retries search, and locks all interactions while adding", async () => {
    search.mockRejectedValueOnce(new Error("搜索失败"));
    render(<AddWatchlistDialog open onClose={vi.fn()} onAdd={vi.fn()} groupId={1} groupName="我的自选" mutation={{
      kind: "pending",
      action: {
        groupId: 1,
        tsCode: "000001.SZ"
