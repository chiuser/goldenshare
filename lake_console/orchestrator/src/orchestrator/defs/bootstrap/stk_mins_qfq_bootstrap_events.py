...
def audit_stk_mins_qfq_bootstrap_batch(
    *,
    ...
    with connect_configured_duckdb() as connection:
        ...
        identity_counts = _batch_identity_coverage_counts(
            connection,
            partition_keys=batch.partition_keys,
            gold_paths=existing_paths,
            silver_paths=silver_paths,
    ...
    for partition_key in batch.partition_keys:
        ...
        counts = stk_mins_checks.GoldStkMinsQfqCheckCounts(
            ...
            invalid_trade_adj_factor_row_count=int(
                factor.get("invalid_trade_adj_factor_row_count", 0)
            ),
            invalid_as_of_adj_factor_row_count=int(
                factor.get("invalid_as_of_adj_factor_row_count", 0)
            ),
    ...
    rows = connection.execute(
        ...
        SELECT
          ...
          count(joined_rows.ts_code) FILTER (
            WHERE isfinite(joined_rows.trade_adj_factor)
              AND joined_rows.trade_adj_factor > 0
              AND isfinite(joined_rows.as_of_adj_factor)
              AND joined_rows.as_of_adj_factor > 0
          ...
          count(joined_rows.ts_code) FILTER (
            WHERE joined_rows.trade_adj_factor IS NOT NULL
              AND (
                NOT isfinite(joined_rows.trade_adj_factor)
                OR joined_rows.trade_adj_factor <= 0
              )
          ) AS invalid_trade_adj_factor_row_count,
          count(joined_rows.ts_code) FILTER (
            WHERE joined_rows.as_of_adj_factor IS NOT NULL
              AND (
                NOT isfinite(joined_rows.as_of_adj_factor)
                OR joined_rows.as_of_adj_factor <= 0
              )
...
def _batch_identity_coverage_counts(
    connection,
    *,
    partition_keys: Sequence[str],
    gold_paths: Sequence[Path],
