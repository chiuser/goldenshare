...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="stk mins qfq derived history helper")
    ...
    add_history_selection_arguments(
        plan_gold_qfq_derived, default_start_date=STK_MINS_QFQ_HISTORY_START_DATE
    )
    ...
    add_history_selection_arguments(
        generate_gold_qfq_derived, default_start_date=STK_MINS_QFQ_HISTORY_START_DATE
    )
    plan_gold_qfq_derived_events = subparsers.add_parser("plan-gold-qfq-derived-events")
    plan_gold_qfq_derived_events.add_argument("--lake-root", default=DEFAULT_LAKE_ROOT)
    add_history_selection_arguments(
        plan_gold_qfq_derived_events, default_start_date=STK_MINS_QFQ_HISTORY_START_DATE
    )
    ...
    add_history_selection_arguments(
        report_gold_qfq_derived_events,
    ...
    audit_gold_qfq_derived_final = subparsers.add_parser("audit-gold-qfq-derived-final")
    audit_gold_qfq_derived_final.add_argument("--lake-root", default=DEFAULT_LAKE_ROOT)
    add_history_selection_arguments(
        audit_gold_qfq_derived_final, default_start_date=STK_MINS_QFQ_HISTORY_START_DATE
    )
    audit_gold_qfq_derived_final.add_argument(
        "--mode", choices=("full", "quick"), default="full"
    )
    args = parser.parse_args(argv)
    if args.command == "plan-gold-qfq-derived-history":
        ...
        print(
            {
                ...
                "planned_source_stock_day_count": report.planned_source_stock_day_count,
