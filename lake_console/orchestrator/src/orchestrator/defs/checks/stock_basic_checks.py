...
def _missing_file_result(path: Path) -> dg.AssetCheckResult:
    return dg.AssetCheckResult(
        ...
        metadata=build_check_metadata(
            ...
            extra_metadata={
                "summary": "股票基础信息文件不存在，当前 check 无法继续验证。",
                "next_action": "先运行对应 stock_basic update job 生成 raw 或 silver 文件。",
        ...
        metadata=build_check_metadata(
            ...
            extra_metadata={
                "summary": (
                    ...
                    else "股票基础信息聚合检查失败，请先看 failed_rule_names。"
                ),
                "next_action": (
                    "无需处理，等待下游消费。"
                    if not failed_rule_names
                    else "按 failed_rule_names 修复 raw/silver 股票基础信息后再重跑。"
                ),
...
def raw_stock_basic_required_columns(
    ...
    missing_columns = [
        column for column in STOCK_BASIC_RAW_REQUIRED_COLUMNS if column not in columns
    ]
    return dg.AssetCheckResult(
...
def silver_stock_basic_required_columns_non_null(
    ...
    with connect_configured_duckdb() as connection:
        ...
        missing_columns = [
            column
            for column in STOCK_BASIC_SILVER_REQUIRED_COLUMNS
