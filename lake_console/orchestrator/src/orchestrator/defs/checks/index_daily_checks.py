...
IndexDailyCheckEvaluatorFn = Callable[
    [tuple[str, ...], Path, DuckDBResource],
    dg.AssetCheckResult,
]


@dataclass(frozen=True)
class IndexDailyCheckEvaluator:
    asset_key: dg.AssetKey
    check_name: str
    blocking: bool
    evaluate: IndexDailyCheckEvaluatorFn
...
def evaluate_raw_index_daily_file_exists(
    ...
    del duckdb
    missing_paths = [
        str(raw_index_daily_path(lake_root_path, partition_key))
        for partition_key in partition_keys
        if not raw_index_daily_path(lake_root_path, partition_key).exists()
    ]
    return dg.AssetCheckResult(
...
def evaluate_raw_index_daily_row_count_positive(
    partition_keys: tuple[str, ...],
    lake_root_path: Path,
    duckdb: DuckDBResource,
) -> dg.AssetCheckResult:
...
def evaluate_raw_index_daily_required_columns_and_types(
    partition_keys: tuple[str, ...],
    lake_root_path: Path,
    duckdb: DuckDBResource,
) -> dg.AssetCheckResult:
...
def evaluate_raw_index_daily_partition_date_matches(
    partition_keys: tuple[str, ...],
    lake_root_path: Path,
    duckdb: DuckDBResource,
) -> dg.AssetCheckResult:
    ...
    with duckdb.connect() as connection:
        for partition_key in partition_keys:
            ...
            mismatch_count = int(
                connection.execute(
                    ...
                    WHERE try_strptime(trade_date, '%Y%m%d') IS NULL
                       OR CAST(strptime(trade_date, '%Y%m%d') AS DATE) != DATE '{partition_key}'
                    """
                ).fetchone()[0]
            )
            rows = connection.execute(
            ...
            mismatch_counts[partition_key] = mismatch_count
...
def evaluate_raw_index_daily_unique_ts_code_trade_date(
    partition_keys: tuple[str, ...],
    lake_root_path: Path,
    duckdb: DuckDBResource,
) -> dg.AssetCheckResult:
...
INDEX_DAILY_HISTORY_BACKFILL_CHECK_EVALUATORS = (
    IndexDailyCheckEvaluator(
        ...
        blocking=True,
        evaluate=evaluate_raw_index_daily_file_exists,
    ),
    IndexDailyCheckEvaluator(
        asset_key=RAW_INDEX_DAILY_ASSET_KEY,
        check_name="raw_index_daily_row_count_positive",
        blocking=True,
        evaluate=evaluate_raw_index_daily_row_count_positive,
    ),
    IndexDailyCheckEvaluator(
        asset_key=RAW_INDEX_DAILY_ASSET_KEY,
        check_name="raw_index_daily_required_columns_and_types",
        blocking=True,
        evaluate=evaluate_raw_index_daily_required_columns_and_types,
    ),
    IndexDailyCheckEvaluator(
        asset_key=RAW_INDEX_DAILY_ASSET_KEY,
        check_name="raw_index_daily_partition_date_matches",
        blocking=True,
        evaluate=evaluate_raw_index_daily_partition_date_matches,
    ),
    IndexDailyCheckEvaluator(
        asset_key=RAW_INDEX_DAILY_ASSET_KEY,
        check_name="raw_index_daily_unique_ts_code_trade_date",
        blocking=True,
        evaluate=evaluate_raw_index_daily_unique_ts_code_trade_date,
    ),
    IndexDailyCheckEvaluator(
        asset_key=SILVER_INDEX_DAILY_ASSET_KEY,
        check_name="silver_index_daily_row_count_positive",
        blocking=True,
        evaluate=evaluate_silver_index_daily_row_count_positive,
    ),
    IndexDailyCheckEvaluator(
        asset_key=SILVER_INDEX_DAILY_ASSET_KEY,
        check_name="silver_index_daily_required_columns_and_types",
        blocking=True,
        evaluate=evaluate_silver_index_daily_required_columns_and_types,
    ),
    IndexDailyCheckEvaluator(
        asset_key=SILVER_INDEX_DAILY_ASSET_KEY,
        check_name="silver_index_daily_unique_ts_code_trade_date",
        blocking=True,
        evaluate=evaluate_silver_index_daily_unique_ts_code_trade_date,
    ),
    IndexDailyCheckEvaluator(
        asset_key=SILVER_INDEX_DAILY_ASSET_KEY,
        check_name="silver_index_daily_conflicting_duplicate_absent",
        blocking=True,
        evaluate=evaluate_silver_index_daily_conflicting_duplicate_absent,
    ),
    IndexDailyCheckEvaluator(
        asset_key=SILVER_INDEX_DAILY_ASSET_KEY,
...
@dg.asset_check(asset=raw_tushare_index_daily, blocking=True)
def raw_index_daily_file_exists(
    context: dg.AssetCheckExecutionContext,
    lake_root: LakeRootResource,
    duckdb: DuckDBResource,
) -> dg.AssetCheckResult:
    return evaluate_raw_index_daily_file_exists(
...
def raw_index_daily_row_count_positive(
    ...
    return evaluate_raw_index_daily_row_count_positive(
        _selected_partition_keys(context),
        lake_root.root(),
        duckdb,
    )
...
def raw_index_daily_required_columns_and_types(
    ...
    return evaluate_raw_index_daily_required_columns_and_types(
        _selected_partition_keys(context),
        lake_root.root(),
        duckdb,
    )
...
def raw_index_daily_partition_date_matches(
    ...
    return evaluate_raw_index_daily_partition_date_matches(
        _selected_partition_keys(context),
        lake_root.root(),
        duckdb,
    )
