"""Canonical Gold business-bar assets for major-index minutes."""
...
from orchestrator.defs.paths import (
    ...
    gold_major_index_mins_staging_path,
    lake_path_template,
...
_SILVER_BY_FREQ = dict(
    zip(CN_A_GOLD_MINUTE_FREQS, SILVER_MAJOR_INDEX_MINS_ASSETS, strict=True)
)
...
def _build_asset(*, asset_name: str, target_freq: int) -> dg.AssetsDefinition:
    ...
    @dg.asset(
        ...
        description=f"主要指数 {target_freq} 分钟 Gold 业务 K 线。",
    )
    def asset(
        ...
        result = write_canonical_gold_minute_partition(
            ...
            staging_path=gold_major_index_mins_staging_path(
            ...
            expected_codes=effective_silver_codes_for_date(context.partition_key),
        )
        return dg.MaterializeResult(
            metadata=build_materialization_metadata(
                ...
                observed_columns=tuple(
                    column.name for column in GOLD_MAJOR_INDEX_MINS_SCHEMA
                ),
...
GOLD_MAJOR_INDEX_MINS_ASSETS = tuple(
    _build_asset(asset_name=asset_name, target_freq=target_freq)
    for asset_name, target_freq in zip(
        MAJOR_INDEX_MINS_GOLD_ASSET_KEYS, CN_A_GOLD_MINUTE_FREQS, strict=True
    )
)
...
__all__ = ["GOLD_MAJOR_INDEX_MINS_ASSETS"]
