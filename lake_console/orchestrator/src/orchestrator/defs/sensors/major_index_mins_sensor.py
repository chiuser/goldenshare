        ...
        if not source.ready:
            return dg.SensorResult(
                ...
                cursor=_cursor(
                    ...
                    summary="bounded closing source probe is incomplete",
                    next_action="wait for all online indices to publish closing bars",
                    source=source,
                ),
            )
        return dg.SensorResult(
            run_requests=[
                build_run_request(
                    run_key=build_asset_update_run_key(
                        subject=MAJOR_INDEX_MINS_RAW_JOB_NAME.removesuffix("_job"),
                        unit_id=target_date,
                    ),
            ...
            cursor=_cursor(
                ...
                blocked_component="none",
                summary="bounded source probe is ready; request the first Raw gap",
