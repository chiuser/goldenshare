...
4. 新增 sensor 时必须同步更新[运行合同治理](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-run-contract-governance.html)、[资产与任务拓扑](/Users/congming/github/goldenshare/lake_console/docs/architecture/dagster-asset-job-topology.html)中的 sensor 分类口径和静态门禁测试，禁止只新增 Python sensor definition 导致 Automation 页面分类退化。
...
以下保留 Dagster 1.13.6 时形成的历史验证记录与设计注意事项，不代表当前版本已重新验收。
版本应核对本项目 `pyproject.toml`、`uv.lock` 与实际运行环境；后续涉及这些 API 的新增或行为变更时，按具体场景做定向核验，不能直接把旧版本结论当成当前验收证据。
