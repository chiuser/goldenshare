...
class DcBoardPerformanceTests(unittest.TestCase):
    def test_1022_member_codes_stay_within_budget_and_emit_temp_report(self):
        ...
        self.assertEqual(result.source_closure_diagnostics["member_repair_code_count"], 0)
        self.assertEqual(result.source_closure_diagnostics["member_repair_request_count"], 0)
        Path("/private/tmp/dc_board_m3_performance_20260714.json").write_text(
            json.dumps(report, ensure_ascii=False, indent=2) + "\n"
        )

    def test_1022_member_codes_retry_only_the_missing_board_within_shared_budget(self):
        codes = tuple(f"BK{index:04d}.DC" for index in range(1, 1023))
        ...
        class _RepairingTushare:
            def call(self, api_name, params, fields):
                ...
                rows = []
                if code != "BK1022.DC" or call_counts[code] > 1:
                    rows = [
                        {
                            "trade_date": "20260714",
                            "ts_code": code,
        ...
        self.assertEqual(result.source_closure_diagnostics["member_repair_code_count"], 1)
        self.assertEqual(result.source_closure_diagnostics["member_repair_request_count"], 1)
        self.assertLess(result.request_count, policy.max_requests)
