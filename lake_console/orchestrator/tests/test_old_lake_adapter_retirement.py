...
RETIRED_MODULES = (
    "source_method", "dataset_spec", "old_lake_executor",
    "specs", "adj_factor_raw_bootstrap_events", "adj_factor_silver_bootstrap_events",
)
RETIRED_TEMPLATES = (
    "TRADE_CALENDAR", "STOCK_BASIC", "STOCK_DAILY", "STK_MINS",
    "STOCK_IDENTITY_MAP", "ADJ_FACTOR", "SUSPEND_D",
)
...
def test_retired_module_has_no_runtime_source(name):
    ...
    if name == "specs":
        ...
        if spec is not None:
            for dataset in ("adj_factor", "stk_mins", "stock_basic", "stock_daily",
                            "stock_identity_map", "suspend_d", "trade_calendar"):
                assert importlib.util.find_spec(f"orchestrator.defs.bootstrap.specs.{dataset}") is None
    else:
        assert spec is None
...
def test_formal_code_has_no_retired_imports_or_source_values():
    ...
    for path in root.rglob("*.py"):
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Import):
                issues.extend((path, node.lineno) for alias in node.names if alias.name.startswith(forbidden))
    ...
    assert all(not hasattr(duckdb_sql, f"{name}_BOOTSTRAP_SELECT_TEMPLATE") for name in RETIRED_TEMPLATES)
...
def test_exact_seventeen_catalog_entries_keep_current_sources():
    ...
    for freq in (1, 5, 15, 30, 60):
        ...
        assert raw.ingestion_sources == (IngestionSource.TUSHARE_API, IngestionSource.PROD_DB_READONLY)
...
@pytest.mark.parametrize("name", (
    "adj_factor_silver_history", "stk_mins_raw_replace_from_prod",
    "stk_mins_raw_replace_from_prod_cli", "stk_mins_silver_history_cli",
    "stk_mins_qfq_history_cli", "stk_mins_qfq_derived_history_cli",
    "stk_mins_qfq_macd_kdj_history_cli", "cn_a_minute_gold_history",
))
...
def test_recovery_has_no_backup_or_active_event_entry():
    ...
    assert all(token not in source for token in (
        "_quarantine", "_backup_path", "_restore_backups", "shutil.rmtree",
        "report_runless_asset_event", "@dg.asset", "@dg.sensor",
    ))
