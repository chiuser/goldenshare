...
def test_repair_clean_next_20241030_multifreq_dry_run_and_apply(tmp_path: Path) -> None:
    pytest.importorskip("pandas")
    pytest.importorskip("pyarrow")

    affected_code = "920001.BJ"
    unaffected_code = "000001.SZ"
    _write_clean_next_source_partition(tmp_path, ts_code=affected_code)
    ...
    service = StkMinsCleanNext20241030MultifreqRepairService(lake_root=tmp_path, progress=lambda _: None)
    dry_summary = service.repair(dry_run=True, apply=False)

    assert dry_summary["operation"] == "repair_stk_mins_clean_next_20241030_multifreq"
    ...
    assert dry_summary["schema_mode"] == "formal_clean_next_11_columns"
    ...
    for item in dry_summary["dry_run_freq_stats"]:
        ...
        assert item["vwap_stats"]["missing_vwap_rows"] == 0
    ...
    for freq in TARGET_FREQS:
        ...
        assert tuple(rows[0].keys()) == TARGET_SCHEMA
        ...
        backup_partition = (
            tmp_path
            / "_tmp"
            / apply_summary["run_id"]
            / "_backup"
            / CLEAN_NEXT_ROOT
            / f"freq={freq}"
            / f"trade_date={TARGET_TRADE_DATE.isoformat()}"
        )
        assert backup_partition.exists()
        backup_rows = read_parquet_files(sorted(backup_partition.glob("*.parquet")))
        assert len([row for row in backup_rows if str(row["ts_code"]) == affected_code]) == 271


def _write_clean_next_source_partition(root: Path, *, ts_code: str) -> None:
...
def _partition_file(root: Path, *, freq: int, trade_date: date) -> Path:
    return root / CLEAN_NEXT_ROOT / f"freq={freq}" / f"trade_date={trade_date.isoformat()}" / "part-00000.parquet"


def _read_partition_rows(root: Path, *, freq: int, trade_date: date) -> list[dict]:
    files = sorted((root / CLEAN_NEXT_ROOT / f"freq={freq}" / f"trade_date={trade_date.isoformat()}").glob("*.parquet"))
