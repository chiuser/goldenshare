...
def test_repair_clean_next_2022_bj_freq30_dry_run_and_apply(tmp_path: Path) -> None:
    ...
    service = StkMinsCleanNext2022BjFreq30RepairService(lake_root=tmp_path, progress=lambda _: None)

    dry_summary = service.repair(dry_run=True, apply=False)
    assert dry_summary["operation"] == "repair_stk_mins_clean_next_2022_bj_freq30"
    ...
    assert dry_summary["vwap_stats"]["missing_vwap_rows"] == 0
    ...
    assert len([row for row in backup_rows if str(row["ts_code"]) == affected_code]) == 6
