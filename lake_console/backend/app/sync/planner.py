...
class LakeSyncPlanner:
    ...
    def plan(
        ...
        *,
        ...
        if dataset_key == "stk_mins":
            return self._stk_mins_note_plan(definition, trade_date=trade_date, start_date=start_date, end_date=end_date, ts_code=ts_code)
    ...
    def _trade_date_plan(
        ...
        *,
        ...
        if trade_date:
            ...
            if end_date < start_date:
                raise ValueError("--end-date 不能早于 --start-date。")
            # M4 仅做请求计划骨架，先按自然日估算上限；真实同步会读取本地交易日历筛选交易日。
        ...
        notes = ["区间计划按日期估算请求数量；真实同步必须读取本地交易日历，只请求开市交易日。"]
        if ts_code:
        ...
        return LakeSyncPlan(
            ...
            required_manifests=("manifest/trading_calendar/tushare_trade_cal.parquet",),
            parameters={
    ...
    def _stk_mins_note_plan(
        ...
        *,
        ...
        notes = [
            "stk_mins 已有专用 sync-stk-mins / sync-stk-mins-range 命令；本计划仅说明依赖与写入范围。",
            "全市场模式依赖本地股票池 manifest，不访问远程数据库。",
        ]
        parameters = {
            "trade_date": trade_date.isoformat() if trade_date else None,
            "start_date": start_date.isoformat() if start_date else None,
