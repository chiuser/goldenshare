...
class StkMinsCleanService:
    """Build and audit the cleaned stk_mins by_date fact layer.

    The service intentionally separates two phases:
    - bootstrap: copy raw partitions into research/stk_mins_by_date_clean.
    - rebuild dry-run/audit: calculate what a true cleaned partition would keep/filter.
    """
    ...
    def bootstrap_clean_from_raw(
        ...
        *,
        ...
        if dry_run == apply:
            raise ValueError("bootstrap 必须且只能指定 dry_run 或 apply。")
        _validate_freqs(freqs)
        partitions = self._discover_partitions(layer="raw_tushare", freqs=freqs, start_date=start_date, end_date=end_date)
        existing_targets = [
            partition for partition in partitions if self._clean_partition(freq=partition.freq, trade_date=partition.trade_date).exists()
        ]
        total_rows = sum(partition.row_count for partition in partitions)
        total_bytes = sum(partition.byte_count for partition in partitions)
        summary: dict[str, Any] = {
            "operation": "bootstrap_stk_mins_by_date_clean_from_raw",
            ...
            "raw_files": sum(len(partition.files) for partition in partitions),
            "raw_rows": total_rows,
            "raw_bytes": total_bytes,
            "existing_clean_partitions": len(existing_targets),
            "replace_existing": replace_existing,
            "samples": [_partition_summary(partition) for partition in partitions[:sample_limit]],
            "write_intent": apply,
        }
        if dry_run:
            return summary
        if existing_targets and not replace_existing:
            ...
            raise RuntimeError(
                ...
                "如确认要重新初始化，请显式传 --replace-existing。"
            )

        LakeRootService(self.lake_root).require_ready_for_write()
        run_id = _run_id("bootstrap-stk-mins-clean")
        started = time.monotonic()
        written_partitions = 0
        written_files = 0
        written_rows = 0
        self.progress(
            f"[stk_mins_clean_bootstrap] start run_id={run_id} partitions={len(partitions)} "
            f"files={summary['raw_files']} rows={total_rows}"
        )
        for index, partition in enumerate(partitions, start=1):
            self.progress(
                f"[stk_mins_clean_bootstrap] partition={index}/{len(partitions)} "
                f"freq={partition.freq} trade_date={partition.trade_date.isoformat()} "
                f"files={len(partition.files)} rows={partition.row_count}"
            )
            result = self._copy_raw_partition_to_clean(run_id=run_id, partition=partition)
            written_partitions += 1
            written_files += int(result["written_files"])
        ...
        self.progress(
            f"[stk_mins_clean_bootstrap] done partitions={written_partitions} files={written_files} "
            f"rows={written_rows} elapsed={round(elapsed, 1)}s"
        )
        return summary
    ...
    def audit_clean_layer(
        ...
        *,
        ...
        return self._summarize_clean_rules(
            operation="audit_stk_mins_by_date_clean",
    ...
    def audit_clean_completeness(
        ...
        *,
        ...
        """Audit clean partitions and optionally record issues in the G6 ledger.

        The ledger is intentionally separate from clean rows: clean/derived/research
        data must not gain explanatory columns just because audit found an issue.
        """
        _validate_freqs(freqs)
        identity_by_source, diagnostics = self._load_or_build_identity_map()
        ...
        self.progress(
            f"[audit_stk_mins_clean_completeness] start partitions={len(partitions)} write_ledger={write_ledger}"
        )
        for index, partition in enumerate(partitions, start=1):
            records = self._audit_clean_partition_completeness(
                partition=partition,
            ...
            if index == 1 or index == len(partitions) or index % 250 == 0 or records:
                self.progress(
                    f"[audit_stk_mins_clean_completeness] partition={index}/{len(partitions)} "
                    f"freq={partition.freq} trade_date={partition.trade_date.isoformat()} issues={len(records)}"
                )

        global_status = _global_audit_status(status_counts)
        ledger_summary = None
        if write_ledger:
            ledger_summary = self._write_clean_issue_ledger(issue_records=issue_records)

        return {
            "operation": "audit_stk_mins_clean_completeness",
    ...
    def plan_rebuild_clean(
        ...
        *,
        ...
        return self._summarize_clean_rules(
            operation="rebuild_stk_mins_by_date_clean",
            partitions=partitions,
            identity_by_source=identity_by_source,
            diagnostics=diagnostics,
            sample_limit=sample_limit,
    ...
    def rebuild_clean_from_raw(
        ...
        *,
        ...
        if dry_run == apply:
            raise ValueError("clean rebuild 必须且只能指定 dry_run 或 apply。")
        if dry_run:
            return self.plan_rebuild_clean(freqs=freqs, start_date=start_date, end_date=end_date, sample_limit=sample_limit)
        ...
        run_id = _run_id("rebuild-stk-mins-clean")
        started = time.monotonic()
        total_raw_rows = 0
        total_kept_rows = 0
        ...
        self.progress(f"[rebuild_stk_mins_by_date_clean] start run_id={run_id} partitions={len(partitions)} mode=apply")
        for index, partition in enumerate(partitions, start=1):
            build = self._build_clean_partition(partition=partition, identity_by_source=identity_by_source, include_frame=True)
            stats = build.stats
            conflict_count = int(stats["duplicate_reasons"].get("duplicate_conflict_payload", 0))
            if conflict_count:
                raise RuntimeError(
                    "clean rebuild 遇到同键不同内容冲突，已停止。"
                    f" freq={partition.freq} trade_date={partition.trade_date.isoformat()} conflicts={conflict_count}"
                )
            self._write_clean_partition(run_id=run_id, partition=partition, frame=build.frame, expected_rows=int(stats["kept_rows"]))
            ...
            if index == 1 or index == len(partitions) or index % 250 == 0 or stats["filtered_rows"] or duplicate_rows:
                self.progress(
                    f"[rebuild_stk_mins_by_date_clean] partition={index}/{len(partitions)} "
                    f"freq={partition.freq} trade_date={partition.trade_date.isoformat()} "
    ...
    def _copy_raw_partition_to_clean(self, *, run_id: str, partition: StkMinsPartition) -> dict[str, Any]:
        tmp_partition = (
            self.lake_root
            / "_tmp"
            / run_id
        ...
        final_partition = self._clean_partition(freq=partition.freq, trade_date=partition.trade_date)
        backup_partition = self.lake_root / "_tmp" / run_id / "_backup" / f"freq={partition.freq}" / f"trade_date={partition.trade_date.isoformat()}"
        if tmp_partition.exists():
            shutil.rmtree(tmp_partition)
        tmp_partition.mkdir(parents=True, exist_ok=True)
        for source_file in partition.files:
            shutil.copy2(source_file, tmp_partition / source_file.name)
        copied_files = _partition_files(tmp_partition)
        copied_rows = _row_count(copied_files)
        if copied_rows != partition.row_count:
            raise RuntimeError(
                f"clean bootstrap 校验失败：freq={partition.freq} trade_date={partition.trade_date.isoformat()} "
                f"raw_rows={partition.row_count} copied_rows={copied_rows}"
            )
        replace_directory_atomically(tmp_dir=tmp_partition, final_dir=final_partition, backup_root=backup_partition.parent)
        return {
            "freq": partition.freq,
            "trade_date": partition.trade_date.isoformat(),
            "written_files": len(copied_files),
            "written_rows": copied_rows,
            "final_partition": str(final_partition),
        }

    def _summarize_clean_rules(
        self,
        *,
        operation: str,
        ...
        duplicate_totals: Counter[str] = Counter()
        samples: list[dict[str, Any]] = []
        self.progress(f"[{operation}] start partitions={len(partitions)} source_is_clean={source_is_clean}")
        for index, partition in enumerate(partitions, start=1):
            stats = self._build_clean_partition(partition=partition, identity_by_source=identity_by_source, include_frame=False).stats
            total_raw_rows += int(stats["raw_rows"])
            total_kept_rows += int(stats["kept_rows"])
            reason_totals.update(stats["filter_reasons"])
            duplicate_totals.update(stats["duplicate_reasons"])
        ...
        status = "success" if not reason_totals and not duplicate_totals else "needs_rebuild" if source_is_clean else "planned"
        return {
            "operation": operation,
            "mode": "read_only" if source_is_clean else "dry_run",
            "lake_root": str(self.lake_root),
            "partitions": len(partitions),
            "raw_rows": total_raw_rows,
            "kept_rows": total_kept_rows,
    ...
    def _build_clean_partition(
        ...
        *,
        ...
        if frame.empty:
            ...
            return StkMinsCleanBuildResult(stats=stats, frame=_empty_clean_frame(pd) if include_frame else None)
        ...
        reasons.loc[reasons.eq("") & invalid_price] = "invalid_price"
        ...
        if kept.empty:
            ...
            kept_rows = 0
            clean_frame = _empty_clean_frame(pd) if include_frame else None
        else:
            ...
            if duplicate_counts["duplicate_conflict_payload"]:
                ...
                clean_frame = self._to_clean_frame(
                    frame=clean_source,
                    pd=pd,
                    partition=partition,
    ...
    def _audit_clean_partition_completeness(
        self,
        *,
        partition: StkMinsPartition,
        ...
        pd = _require_pandas()
        frames = [pd.read_parquet(path, engine="pyarrow") for path in partition.files]
        frame = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=list(CLEAN_STK_MINS_FIELDS))
        ...
        if list(frame.columns) != list(CLEAN_STK_MINS_FIELDS):
            records.append(
                _issue_record(
                    gate="G5",
                    ...
                    expected_value=",".join(CLEAN_STK_MINS_FIELDS),
                    actual_value=",".join(str(column) for column in frame.columns),
                    evidence_dataset="research.stk_mins_by_date_clean",
                    evidence_ref=evidence_ref,
                    action="block",
                    reason="clean 分区字段不等于标准行情字段，不能通过审计。",
                )
            )
        ...
        if invalid_trade_time_count:
            records.append(
                _issue_record(
                    ...
                    evidence_ref=evidence_ref,
                    action="block",
                    reason="clean 分区存在无法解析的 trade_time。",
                )
            )
        ...
        if invalid_price_count:
            records.append(
                _issue_record(
                    ...
                    freq=partition.freq,
                    trade_date=partition.trade_date,
                    expected_value="open/close/high/low > 0 and high >= low",
                    actual_value=f"invalid_rows={invalid_price_count}",
                    evidence_dataset="research.stk_mins_by_date_clean",
                    evidence_ref=evidence_ref,
                    action="exclude_from_clean",
                    reason="clean 分区存在非法价格行，问题行必须入账后才能剔除。",
                )
            )
        ...
        if invalid_volume_amount_count:
            records.append(
                _issue_record(
                    ...
                    evidence_ref=evidence_ref,
                    action="exclude_from_clean",
                    reason="clean 分区存在非法成交量或成交额，问题行必须入账后才能剔除。",
                )
            )
        ...
        if identity_missing_count:
            records.append(
                _issue_record(
                    ...
                    evidence_ref=evidence_ref,
                    action="block",
                    reason="clean 中存在身份账本无法解释的 ts_code。",
                )
            )
        ...
        for ts_code in delisted_codes:
            ...
            records.append(
                _issue_record(
                    ...
                    freq=partition.freq,
                    trade_date=partition.trade_date,
                    expected_value="delisted security excluded from clean",
                    actual_value=f"rows={row_count}",
                    evidence_dataset="manifest.security_identity.security_identity_map",
                    evidence_ref=evidence_ref,
                    action="exclude_from_clean",
                    reason="已退市股票整只不允许进入 clean。",
                )
            )
        ...
        if {"ts_code", "freq", "trade_time"}.issubset(frame.columns):
            ...
            if not duplicate_rows.empty:
                payload_cols = [field for field in ("open", "close", "high", "low", "vol", "amount") if field in duplicate_rows.columns]
                ...
                for (ts_code, _freq, trade_time), group in grouped:
                    ...
                    status = "duplicate_conflict_payload" if payload_distinct_count > 1 else "duplicate_same_payload"
                    records.append(
                        _issue_record(
                            gate="G5",
                            issue_type=status,
                            status="failed" if status == "duplicate_conflict_payload" else "duplicate_same_payload",
                            ...
                            evidence_dataset="research.stk_mins_by_date_clean",
                            evidence_ref=evidence_ref,
                            action="block" if payload_distinct_count > 1 else "repair_required",
                            reason="clean 分区存在重复 key。",
                        )
                    )

        if {"ts_code", "trade_time"}.issubset(frame.columns):
            expected_bar_count = INTRADAY_EXPECTED_BAR_COUNT.get(partition.freq)
            if expected_bar_count:
                valid_time_mask = (
                    trade_time_values.notna()
                    & ~invalid_price
                    & ~((vol_values < 0) | (amount_values < 0))
                    & ts_codes.isin(known_latest_codes)
                    & ~delisted_mask
                    & ~before_list_mask
                )
                valid_time_frame = frame.loc[valid_time_mask].copy()
                if not valid_time_frame.empty:
                    valid_time_frame["__trade_time"] = trade_time_values.loc[valid_time_frame.index]
                    ...
                    for ts_code, bar_count_value in bar_counts.items():
                        ...
                        if bar_count == expected_bar_count:
                            continue
                        status = "failed" if bar_count > expected_bar_count else "needs_review"
                        issue_type = "extra_intraday_bar" if bar_count > expected_bar_count else "missing_intraday_bar"
                        records.append(
                            _issue_record(
                                gate="G7",
                                ...
                                evidence_dataset="research.stk_mins_by_date_clean",
                                evidence_ref=evidence_ref,
                                action="block" if status == "failed" else "repair_required",
                                reason="clean 分区日内 bar 数与 A 股常规交易时段不一致。",
                            )
                        )
        return records
    ...
    def _to_clean_frame(
        ...
        *,
        ...
        clean["trade_date"] = partition.trade_date
        return clean.loc[:, list(CLEAN_STK_MINS_FIELDS)].sort_values(["ts_code", "trade_time"]).reset_index(drop=True)

    def _write_clean_partition(self, *, run_id: str, partition: StkMinsPartition, frame: Any, expected_rows: int) -> None:
        if frame is None:
            raise RuntimeError(
                f"clean 分区没有可写入 DataFrame：freq={partition.freq} trade_date={partition.trade_date.isoformat()}"
            )
        tmp_partition = (
            self.lake_root
            / "_tmp"
        ...
        final_partition = self._clean_partition(freq=partition.freq, trade_date=partition.trade_date)
        backup_root = self.lake_root / "_tmp" / run_id / "_backup" / "research" / "stk_mins_by_date_clean" / f"freq={partition.freq}"
        if tmp_partition.exists():
            shutil.rmtree(tmp_partition)
        tmp_partition.mkdir(parents=True, exist_ok=True)
        ...
        if validated != expected_rows:
            raise RuntimeError(
                f"clean rebuild 写入校验失败：freq={partition.freq} trade_date={partition.trade_date.isoformat()} "
    ...
    def _write_clean_issue_ledger(self, *, issue_records: list[dict[str, Any]]) -> dict[str, Any]:
        ledger_file = self.lake_root / CLEAN_ISSUE_LEDGER_RELATIVE_PATH
        ...
        LakeRootService(self.lake_root).require_ready_for_write()
        run_id = _run_id("stk-mins-clean-issue-ledger")
        tmp_file = self.lake_root / "_tmp" / run_id / CLEAN_ISSUE_LEDGER_RELATIVE_PATH
        backup_root = self.lake_root / "_tmp" / run_id / "_backup" / CLEAN_ISSUE_LEDGER_RELATIVE_PATH.parent
    ...
    def _discover_partitions(
        ...
        *,
        ...
        if layer == "raw_tushare":
            ...
            root = self.lake_root / "research" / "stk_mins_by_date_clean"
        else:
    ...
    def _clean_partition(self, *, freq: int, trade_date: date) -> Path:
        return self.lake_root / "research" / "stk_mins_by_date_clean" / f"freq={freq}" / f"trade_date={trade_date.isoformat()}"
...
def _empty_clean_frame(pd: Any) -> Any:
    return pd.DataFrame(columns=list(CLEAN_STK_MINS_FIELDS))
