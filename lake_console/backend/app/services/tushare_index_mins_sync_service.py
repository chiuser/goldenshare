...
from lake_console.backend.app.services.index_mins_common import (
    format_tushare_window_end,
    format_tushare_window_start,
    normalize_index_mins_freqs,
    normalize_index_mins_row,
)
...
INDEX_MINS_PAGE_LIMIT = 8000


class TushareIndexMinsSyncService:
    def __init__(
        self,
        *,
        lake_root: Path,
    ...
    def sync(
        ...
        *,
        ...
        run_id = _run_id("index-mins-tushare")
        ...
        if not trade_dates:
            raise RuntimeError("index_mins 在目标范围内没有可同步的开市交易日。")
        ...
        self.progress(
            f"[index_mins:tushare] start run_id={run_id} dates={len(trade_dates)} freqs={','.join(normalized_freqs)} "
        ...
        for current_freq in normalized_freqs:
            for unit_index, current_ts_code in enumerate(universe.ts_codes, start=1):
                request_count += 1
                effective_start_date, effective_end_date = universe.effective_windows_by_code[current_ts_code]
                start_at = format_tushare_window_start(effective_start_date)
                end_at = format_tushare_window_end(effective_end_date)
                offset = 0
                page = 1
                while True:
                    page_rows = self.client.index_mins(
                        fields=INDEX_MINS_FIELDS,
                        ts_code=current_ts_code,
                        freq=current_freq,
                        start_date=start_at,
                        end_date=end_at,
                        limit=INDEX_MINS_PAGE_LIMIT,
                        offset=offset,
                    )
                    fetched_total += len(page_rows)
                    self.progress(
                        f"[index_mins:tushare] freq={current_freq} unit={unit_index}/{len(universe.ts_codes)} "
                        f"ts_code={current_ts_code} page={page} offset={offset} fetched_page={len(page_rows)} "
                    ...
                    if len(page_rows) < INDEX_MINS_PAGE_LIMIT:
                        break
                    offset += INDEX_MINS_PAGE_LIMIT
                    page += 1
        ...
        self.progress(
            f"[index_mins:tushare] done dates={len(trade_dates)} freqs={len(normalized_freqs)} "
            f"requests={request_count} fetched={fetched_total} written={written_total} rejected={rejected_total} "
