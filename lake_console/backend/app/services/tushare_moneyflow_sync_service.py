...
from lake_console.backend.app.catalog.datasets.moneyflow import MONEYFLOW_FIELDS, MONEYFLOW_VOLUME_FIELDS
...
MONEYFLOW_PAGE_LIMIT = 6000


class TushareMoneyflowSyncService:
    def __init__(
        self,
        *,
        lake_root: Path,
    ...
    def sync(
        ...
        *,
        ...
        if trade_date and (start_date or end_date):
            raise ValueError("moneyflow 的 trade_date 与 start/end date 不能同时传。")
        if (start_date is None) != (end_date is None):
            raise ValueError("moneyflow 的 start-date 和 end-date 必须同时传入，或同时省略。")
        if trade_date is None and start_date is None:
            raise ValueError("moneyflow 必须传 --trade-date 或 --start-date/--end-date。")
        if start_date is not None and end_date is not None and end_date < start_date:
            raise ValueError("moneyflow 的 end-date 不能早于 start-date。")

        started_at = datetime.now(timezone.utc)
        started = time.monotonic()
        run_id = _run_id("moneyflow")
        ...
        self.progress(
            f"[moneyflow] start run_id={run_id} dates={len(trade_dates)} "
        ...
        self.progress(
            f"[moneyflow] done dates={len(trade_dates)} fetched={fetched_total} "
    ...
    def _sync_trade_date(
        ...
        *,
        ...
        page = 1
        while True:
            page_rows = self.client.moneyflow(
                fields=MONEYFLOW_FIELDS,
                trade_date=_format_tushare_date(trade_date),
                ts_code=ts_code,
                limit=MONEYFLOW_PAGE_LIMIT,
                offset=offset,
            )
            fetched_total += len(page_rows)
            self.progress(
                f"[moneyflow] unit={unit_index}/{unit_total} trade_date={trade_date.isoformat()} "
                f"page={page} offset={offset} fetched_page={len(page_rows)} fetched_total={fetched_total}"
            )
            for row in page_rows:
                normalized = _normalize_moneyflow_row(row, expected_trade_date=trade_date)
                if normalized is None:
                    rejected += 1
                    continue
                rows.append(normalized)
            if len(page_rows) < MONEYFLOW_PAGE_LIMIT:
                break
            offset += MONEYFLOW_PAGE_LIMIT
            page += 1

        if not rows:
            raise RuntimeError(
                f"moneyflow {trade_date.isoformat()} 未获取到任何有效记录，拒绝覆盖本地资金流向分区。"
            )

        tmp_dir = self.lake_root / "_tmp" / run_id / "raw_tushare" / "moneyflow" / f"trade_date={trade_date.isoformat()}"
        tmp_file = tmp_dir / "part-000.parquet"
        final_dir = self.lake_root / "raw_tushare" / "moneyflow" / f"trade_date={trade_date.isoformat()}"
        final_file = final_dir / "part-000.parquet"
        backup_root = self.lake_root / "_tmp" / run_id / "_backup" / "moneyflow" / f"trade_date={trade_date.isoformat()}"

        rows.sort(key=lambda item: str(item.get("ts_code") or ""))
        self.progress(f"[moneyflow] writing trade_date={trade_date.isoformat()} rows={len(rows)} output={tmp_file}")
...
def _normalize_moneyflow_row(row: dict[str, Any], *, expected_trade_date: date) -> dict[str, Any] | None:
    ...
    for field in MONEYFLOW_FIELDS:
        if field in {"ts_code", "trade_date"}:
            continue
        value = row.get(field)
        if field in MONEYFLOW_VOLUME_FIELDS:
            normalized[field] = _normalize_int64(value, field=field)
        else:
            normalized[field] = None if _is_nan(value) else value
    return normalized


def _normalize_int64(value: Any, *, field: str) -> int | None:
    ...
    try:
        ...
        raise RuntimeError(f"moneyflow 字段 {field} 不是整数格式：{value!r}") from exc
    if numeric != numeric.to_integral_value():
        raise RuntimeError(f"moneyflow 字段 {field} 不是整数格式：{value!r}")
    return int(numeric)
...
def _write_and_validate(*, rows: list[dict[str, Any]], tmp_file: Path) -> int:
    ...
    if validated != written:
        raise RuntimeError(f"moneyflow Parquet 校验失败：written={written} validated={validated} file={tmp_file}")
    return written
