...
> **2026-09-10 新任务：**股票分钟线按 **2014 年起、Raw → Silver → Gold → 指标**恢复，完整方案见 [§14](#minute-gap-recovery-2014)，**含准备工作的整体耗时见 §14.9，工作包见 [§14.12](#minute-gap-execution-work-packages)，执行硬约束与开跑条件见 [§14.13](#minute-gap-execution-constraints)**。**B0/B1 已完成，B2 首批20窗口取源已完成，结果及耗时见 [§14.17](#minute-gap-b2-first20)**；两项取源优化已实现并通过隔离验证，见 [§14.19](#minute-gap-source-execution-optimization)；冻结清单及命令见 [§14.16](#minute-gap-b0-b1-delivery)。管理员已明确接受 Raw 保留来源代码、Silver 起统一新身份。B2剩余批次、B3及下游尚未执行，正式数据和事件未写入；不沿用 P7/P8 的全市场重建范围。
...
方案日期：2026-09-10。当前状态：**B0/B1 已完成，B2首批20窗口已完成；B2剩余批次、B3及下游尚未执行**。本次取源结果见 §14.17；冻结清单、实现、隔离验收与命令单见 §14.16。§14.14/§14.15 保留此前停点和只读核验记录；用户后续明确接受 Raw 来源代码、Silver 起统一新身份，无需再做业务决策。B0/B1新增离线恢复入口和测试；B2首批仅写源缓存和进度到staging，正式数据、数据库、Dagster运行状态和事件未修改。
...
**源站优先。**目标频率返回有效记录就采用目标频率的源数据。原始响应逐窗口缓存；合并只替换缺失/获准修复的业务键，保存新增与替换统计，不覆盖整只股票全历史。当前确定缺口全部为整日无行，因此主要操作是追加该股票日，不是改写已有行情。

**只有目标频率源端确认缺失时才聚合。**候选必须比目标分钟数小且整除目标，必须完整覆盖被恢复日所需时间网格，并通过量价校验。建议确定性顺序：
...
这是待评审的聚合来源顺序，理由是优先用最近的合格频率减少计算，并固定选择。整股票日只采用一种更细频来源；同等条件优先源站原生记录，派生链必须能追到最终原生输入。未满足完整性不得逐字段拼接或插值、前值填充。全部候选仍缺时保留缺口，不造行情。
...
聚合是 Raw 来源合同的扩展，不能把生成记录声称为 Tushare 直接返回。推荐不增加行情业务列，使用随批次永久保留的逐股票日来源清单，记录 `origin=source/aggregated`、源频率、父输入键/校验值、聚合规则版本、目标文件校验值；正式资产 metadata 引用该清单，catalog 的来源说明和相关检查同步更新。聚合载荷与原生响应分开保存，不伪造源响应。来源清单作为解释正式数据的证据不能随临时缓存清理；未经该合同及消费者校准，不提升聚合 Raw。
...
| B3a 条件性聚合 | source-empty/partial 的准确缺失时点 → 按 §14.6 更细频候选生成整日合格数据 → aggregate-ready/残留 | 复用时间网格与 OHLCV 聚合基础，补 Raw 来源追溯和辅助列约定；只有确需时进入 | 先每市场两日、跨午休/竞价/零成交正反例；无法形成完整候选的单元留残留。请求错误不能进入此分支 | 2—4 小时，仅发生时 | 0.5—2 小时，仅发生时 |
