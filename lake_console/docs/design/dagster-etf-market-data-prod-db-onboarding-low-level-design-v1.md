...
2. ETF 分钟从 Prod DB 按“交易日范围 + 频率”整批读取到 staging；Prod SQL 不按 Basic 删代码，候选通过导出完整性、字段/分区合同和最新 `silver_etf_basic` 身份校验后先落正式 Raw。Raw 的存储连续性只看文件/身份门禁；应覆盖缺口、五频覆盖和分钟网格由 DuckDB 审计本地 Raw，并通过独立的 Silver admission 合同决定能否生成 Silver。
...
本 LLD 已经把推荐实现写完整。N3 的执行顺序已经确认：单个分区的 N3 decision 不阻断通过稳定导出/身份门禁的 Raw 物理 Bootstrap，也不阻断后续 Raw 日期继续推进；它只阻断不合格分区进入 Silver、对应 green admission/check 事件和 Silver 自动化。全局 N3 policy 尚未冻结时，P10 日常 Sensors 仍不得启用。当前状态如下：

| 编号 | 结论 | 状态与阻断范围 |
| --- | --- | --- |
| N1 | Basic 使用 `snapshot_id=<Raw内容hash>/part-000.parquet` 的 content-addressed 不可变版本，不建 `current` 文件；hash 是 DG Raw 自身可复算内容身份，不要求与 Prod 一致；只从 Dagster 最新 Silver materialization 开始并同时验证其引用的 Raw/checks，失败或不新鲜不回退 | 已确认；Basic path、writer、latest-only selector 可按本文合同开发 |
...
| N3 | 分钟 Raw 完整性审计拆成 N3A 观察/profile 与 N3B policy/decision | 执行顺序已确认；N3A 的规则建议不生效，N3B 待管理员看过真实报告后拍板；Raw 存储 continuity 不受 blocked admission 影响，Silver fail-closed |
...
当前没有需要立即补充拍板的架构口径，但这不等于授权开发。获准后只能先做 P0；真实 Tushare 分页、分钟 exchange 映射和 Prod 小批 profiling 未关闭前不得进入 P1。首次分钟 Raw 物理写入必须使用 P6 plan 动态冻结的 N4 水位，并执行 N5 冲突策略。N3 固定拆为 P7A observation/profile 和 P7B policy freeze/decision 两步；P7A 完成但 P7B 尚未确认期间，不得生成 `silver_eligible`、写 Silver、补 green admission/check event 或启用 Silver Sensor。
...
-> silver_etf_mins_* exact admission copy
```
...
| `defs/tushare_api_io.py` | 显式字段、`limit/offset` 短页分页、空结果阻断、小快照写 Parquet | 普通 full-file helper 不能直接覆盖一个固定正式文件；ETF Basic 要先写 staging 再按 hash 定位 |
...
| `defs/bootstrap/*mins*` | plan / Raw apply / Raw observe / Raw decide / Silver apply / event 分段授权、20 日批次、checkpoint、最近 20 日 check event | 不照搬现有 Raw/Silver 同一 apply；不读取旧 Lake、不用 Kopia、不把 runless event 混进物理写入 |
...
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_plan.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_plan_cli.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_raw_apply.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_raw_apply_cli.py
src/orchestrator/defs/bootstrap/etf_mins_raw_observe.py
src/orchestrator/defs/bootstrap/etf_mins_raw_observe_cli.py
src/orchestrator/defs/bootstrap/etf_mins_raw_decide.py
src/orchestrator/defs/bootstrap/etf_mins_raw_decide_cli.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_silver_apply.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_silver_apply_cli.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_partitions.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_partitions_cli.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_events.py
src/orchestrator/defs/bootstrap/etf_mins_bootstrap_events_cli.py
```
...
src/orchestrator/defs/sensors/readiness.py
```
...
Raw factory 固定 `partitions_def=cn_a_etf_mins_trade_days`、`deps=["silver_etf_basic"]` 和该 asset 的分钟频率；Silver factory 使用同一 partitions definition，并只依赖对应 Raw asset。`EtfMinsRawConfig`、`EtfMinsSilverConfig` 和所有 reference 使用 Pydantic 可序列化的小对象，不允许嵌入代码全集，也不允许携带 Dagster `storage_id`：
...
class EtfMinsRawConfig(Config):
    ...
    source_method: Literal["prod_db_readonly"] = "prod_db_readonly"

class EtfMinsSilverConfig(Config):
    partition_key: str
    admission_reference: EtfMinsDailyAdmissionReference
```

`EtfMinsDailyAdmissionReference` 是同日五频共享 N3 评估的 run-scoped 小引用，不是新 asset，也不新增第 13 个资产。它至少包含日期、五个当前 Raw URI/hash、五个 Raw materialization identity 的稳定 fingerprint、`gap_policy_version`、五频 decision/reason codes、评估时间和 `admission_fingerprint`；不含 storage id、完整代码集合或完整 observation。
...
def build_etf_mins_silver_run_config(
    *,
    partition_key: str,
    admission_reference: EtfMinsDailyAdmissionReference,
) -> dict[str, object]: ...
...
Raw builder 为五个 op 生成同一 Basic reference 和同一次 Sensor 五频 coverage reference；Silver builder 为五个 op 生成同一 admission reference。测试必须证明各自五份 config 的 fingerprint、partition key 和观测时间一致，且没有 `storage_id`、`ts_codes`、SQL 或可改写频率的字段。Raw asset 只校验 reference 结构、Basic hash 和目标日期，不重新请求 Prod coverage；Silver asset 必须回读当前五频 Raw 文件并复核 hash、policy version 和 admission fingerprint，不能只相信 Sensor。
...
ETF_MINS_PROD_STATEMENT_TIMEOUT_MS = <P0_SAMPLE_APPROVED_INTEGER>
...
`ETF_MINS_PROD_STATEMENT_TIMEOUT_MS` 不是环境变量，也不能在 Launchpad 改写。P0 真实受控样本验收后才能把整数写入代码；P0 未关闭时保持文档占位，禁止以猜测值进入实现。
...
| `EtfBasicSilverSnapshotReference` | latest-only selector 从最新 Silver materialization、Raw/Silver checks 和文件复算生成；只随单次分钟/Bootstrap 计划持久化 | 分钟 Raw、Bootstrap plan/apply | 含任务启动时 `eligibility_as_of`；最新失败/不新鲜不回退 | metadata/报告显示两个 hash、日期和 requestable count/hash；禁止完整代码集合 |
...
| `EtfMinsDailyAdmissionReference` | 同日五频 N3 evaluator 生成；只随 Silver run config 持久化 | 五个 `bar_domain` checks、Silver Sensor/assets | 绑定五个当前 Raw hash/materialization fingerprint 和当前 policy version | UI 显示 decision/reason/policy；reference 或 policy 漂移 fail-closed，禁止 storage id |
| `EtfMinsRawConfig` | Raw Sensor builder 生成 | 五个 Raw assets | `partition_key + Basic reference + coverage reference + 固定 source_method` | 频率不能配置；静态测试拒绝 SQL、代码全集、storage id |
| `EtfMinsSilverConfig` | Silver Sensor builder 生成 | 五个 Silver assets | `partition_key + admission_reference` | 手工启动也必须 revalidate，不能绕过 admission |
| `ETF_MINS_PROD_STATEMENT_TIMEOUT_MS` | P0 样本批准后写入 `run_contracts/etf_mins.py` 的正整数常量 | coverage 与 DuckDB attach 明细查询 | 两条查询路径用同一值；不新增 env/Launchpad 参数 | 日志只显示 timeout 数值与是否触发，不显示 conninfo；fake 和有界真实测试 |
| `ETF_MINS_BOOTSTRAP_BATCH_TRADE_DAY_LIMIT=20` | 代码常量 | plan/query budget/raw-apply | 单频单批最多 20 个冻结交易日 | plan 报告显示批次/查询数；超限拒绝 |
...
Bootstrap 的 frozen plan、checkpoint、候选文件清单、decision/admission 文件和 `actual_changed_raw_manifest.json` 都留在对应 operation 目录。它们不是正式 asset，不得被下游当作 Lake 事实源。
...
“最新已验收版本”只从 Dagster 最新一条 `silver_etf_basic` materialization 开始检查，并要求它自身及其引用的 Raw materialization 的 blocking checks 都精确绑定且通过；任一层失败或最新 Silver 不新鲜立即 fail-closed，绝不向前搜索旧成功版本。分钟任务也不得通过 `max(snapshot_id)`、目录 mtime 或文件创建时间选版本。
...
本地源文档声明单页上限 5,000 且支持 `limit/offset`；当前 `tushareMcp` 包装没有暴露这两个参数，因此 P0 必须用实际 `TushareResource` 做一次受控 `limit=5000, offset=0/5000` 边界验证。单元测试先用 fake client 覆盖整页、短页、第二页失败、跨页重复和列漂移。
...
## 11. 最新 ready Basic 的冻结合同

新增 `select_latest_ready_etf_basic_snapshot(...)`，只做有界 Dagster 状态读取和单文件校验。
...
1. 有界读取 `silver_etf_basic` 最新一条 materialization；不得扫描并回退到更早成功版本。
2. 要求其上海本地 `observed_at` 与 `eligibility_as_of` 同日。
3. selector 内部使用最新 Silver materialization 的 storage id，要求全部 Basic Silver blocking checks 精确绑定它且全部通过；再从 Silver metadata 定位它引用的 Raw materialization，要求三个 Basic Raw blocking checks 也精确绑定该 Raw 并全部通过。storage id 只在本次有界状态读取中使用，不进入输出 reference。
4. 从 Silver metadata 取得 `raw_uri/raw_snapshot_hash`，分别回读 Raw 和 Silver 文件，复算 Raw/Silver hash，并要求两者绑定同一 `snapshot_id`。
5. 用第 12 节条件计算 requestable targets 和 hash。
6. 构造不可变 reference；后续 run config 只传 reference，不传完整代码数组。
...
DuckDB 使用现有 `ProdPostgresResource.duckdb_connection_string()` 取得基础 conninfo，再由 `build_prod_etf_mins_duckdb_conninfo(base_conninfo, timeout_ms)` 这个纯 helper 注入经正整数校验的 PostgreSQL `statement_timeout` options，并以 `TYPE POSTGRES, READ_ONLY` attach。conninfo 只在内存中的 attach 语句使用，不进入远端查询、日志、异常文本、run config 或 metadata；测试用脱敏 fake conninfo 验证，不快照真实凭据。

### 13.3 Prod Raw 五频代码物理覆盖

`defs/prod_db/etf_mins.py` 复用 `stk_mins` 当前的索引友好形状：把冻结 Basic 计算出的 `expected(D)` 代码数组和五个源频率数组传给 `unnest`，构造 `expected_code × freq`，再对 `raw_tushare.etf_minute_bar` 用主键前缀条件做 `EXISTS ... LIMIT 1`。单次查询返回每个频率的 expected/present/missing 数量和最多 20 个缺失代码，不能返回分钟明细。

coverage/watermark 走 psycopg 只读事务，数组、日期和样本上限全部使用绑定参数；事务开始后先执行 `SELECT set_config('statement_timeout', %s, true)`，值来自 `ETF_MINS_PROD_STATEMENT_TIMEOUT_MS`，结束无论成功失败都 rollback。它与第 13.2 节的 DuckDB 明细 attach 是两种明确分开的执行策略，不能把 `%s` 参数合同误套到 `postgres_query` 远端字符串，也不能为了统一形式增加重复查询。
...
频率由 asset definition 固定，不允许 config 改写。`partition_key` 必须与 Dagster partition 一致。完整 Basic 代码列表和 Prod SQL 不进入 config。
...
N3A 输出全部放在本次 `/Volumes/datasource/data_lake_staging/etf_mins_raw_observe/operation_id=<id>/`：
...
N3B policy 冻结前，Raw 物理文件可以存在，文件/身份事实也可以在最终报告中验收，但不能宣称 `silver_admission_ready`；Silver writer、历史 green admission/check event 和 P10 日常 Sensors 必须 fail closed。policy 冻结后，单个分区的 blocked decision 只阻断该日 Silver，不取消 `raw_storage_ready`，也不阻断后续 Raw。不能先用一个宽泛 `quality_check` 掩盖未拍板语义。

N3B policy 冻结后，日常链不再运行整套历史 profile，而是调用同一规则合同的日常 evaluator：

```python
def evaluate_etf_mins_daily_admission(
    *,
    partition_key: str,
    raw_references: tuple[EtfMinsRawMaterializationReference, ...],
    approved_policy_version: str,
    duckdb: DuckDBResource,
) -> EtfMinsDailyAdmissionResult: ...
```

执行合同固定为：

1. 同日五个 Raw 都已完成 `file_contract/request_scope` checks 后才执行。
2. 一个 evaluator、一个 DuckDB connection 集中读取五个文件；按规则需要可发少量集合 SQL，但不能由五个 check 各自重扫一遍。
3. 结果生成一个 run-scoped `EtfMinsDailyAdmissionReference`，并由一个共享 check factory/`multi_asset_check` 等价实现发出五个 `bar_domain` evaluations。它不是新 asset，不写正式 Lake，也不把大 observation 塞进 Dagster event。
4. `green` 和 policy 明确允许的 `warn` 令对应 check `passed=True`；`blocked` 令 check `passed=False`。每条 check 都记录当前 Raw file hash/materialization fingerprint、policy version、decision、reason codes、计数和最多 20 个样本。
5. `bar_domain` 相对 Raw asset 设置 `blocking=False`，只为避免 blocked admission 阻断 Raw 连续性；Silver 的准入代码必须把“当前五个 bar_domain checks 已绑定、policy 仍是当前版本、全部 passed”当成强制条件。
6. evaluation 或 check event 写入失败不回滚 Raw 文件；该日保持 Silver not-admitted，修复观测链后可以只重跑 admission evaluation/check，不重新导出 Prod 或覆盖 Raw。

因此日常状态必须分成：

```text
raw_storage_ready
  = 当前 Raw materialization + file_contract passed + request_scope passed

silver_admission_ready
  = 同日五频 raw_storage_ready
  + 当前 policy 的 EtfMinsDailyAdmissionReference 可复算
  + 五个 bar_domain checks 绑定当前 Raw 且 passed
```

Raw Sensor 只消费第一种状态，Silver Sensor/asset 消费第二种状态。任何实现若把 `bar_domain` 加回 Catalog 的 Raw `blocking_check_names`，或让 Raw Sensor 因 N3 blocked 停在该日，静态测试必须失败。
...
Silver 只接受同日五频 `silver_admission_ready` 的 `raw_etf_mins_*` materialization。每个 Silver op 接收同一个 `EtfMinsDailyAdmissionReference`，开始时必须自行：

1. 回读五个当前 Raw path/hash，确认与 reference 完全一致。
2. 确认每个 Raw 的 `file_contract/request_scope` blocking checks 精确绑定当前 materialization 且通过。
3. 确认 reference 的 `gap_policy_version` 等于代码当前批准版本，重新计算 `admission_fingerprint`。
4. 确认五个 `bar_domain` admission checks 精确绑定当前 Raw，且 policy 允许的 green/WARN 均 `passed=True`、blocked 为 `passed=False`。
5. 当前频率在 reference 中 `silver_eligible=true`。

任一条件失败，Silver 写入前停止且不触碰正式目标。不得用文件存在、Raw row count、Sensor 已发起 run 或操作者手工提供 `passed=true` 代替这些验证。
...
Silver 使用 `build_materialization_metadata(...)` 返回 `dagster/uri`、`dagster/row_count`、`goldenshare/observed_columns`，并在 `goldenshare/*` 下记录 `partition_key/source_freq/code_count/raw_uri/raw_sha256/silver_sha256/write_disposition`、`basic_raw_snapshot_hash/basic_silver_content_hash/basic_reference_fingerprint/eligibility_as_of/requestable_code_hash`，以及 `gap_policy_version/admission_fingerprint/admission_decision/admission_reason_codes`。这样后续审计可以从 Silver 反查当时使用的 Basic 与 N3 准入基线，而无需猜测“今天最新版本”；不得写裸 metadata key 或 Dagster storage id。
...
- `bar_domain`：在 N3B policy 冻结后统一判定价格空值、OHLC、成交量/额、分钟网格、内部空洞和时间边界；`blocked` 失败，获准 WARN 的分区通过但写清 reason code。五个 evaluation 必须由一个共享 `@multi_asset_check` 或行为等价的单次 evaluator 产生，在同一个 DuckDB connection 中读取同日五频并复用 `EtfMinsDailyAdmissionReference`，不能由五个 check 各自扫描，也不能依赖 op 的偶然执行顺序。

blocking 语义固定为：

```text
file_contract  blocking=True   -> Raw 存储 readiness
request_scope  blocking=True   -> Raw 存储 readiness
bar_domain     blocking=False  -> 强制 Silver admission，不阻断 Raw 连续性
```

Catalog 的 Raw `blocking_check_names` 只登记前两类；`bar_domain` 仍是正式 active check，但在 Catalog notes/performance contract 中登记为 mandatory Silver admission check。Silver readiness helper 必须显式读取它，不能因为它不在 Raw blocking list 就忽略。
...
五频 Raw 的 `file_contract/request_scope` 和所有 Silver checks 必须写：
...
五频 `bar_domain` 的 check specs 则固定 `blocking=False`，仍必须绑定目标 Raw asset 和同一 partitions definition。测试断言全部 check 的 definition、`AssetCheckEvaluation.partition` 和 check execution partition 都等于目标日期，并断言 Raw Catalog blocking list 没有混入 `bar_domain`。
...
- 正式 checks：文件/身份两类 blocking checks 绑定 Raw materialization并供 Raw readiness 消费；`bar_domain` admission check 绑定同一 Raw、复用已冻结 N3 policy，并由 Silver readiness 强制消费。
- Silver writer 的写前 revalidation 只验证已冻结 admission reference 与当前 Raw/policy 一致，不在 writer 内重新发明质量规则。
- 四者的共同字段、分类和 SQL contract 必须从同一 helper 派生，不能维护多套不同规则。
...
分钟 Job selection 不包含 Basic assets。Basic 是 `deps=["silver_etf_basic"]` 的 lineage/readiness 依赖，并由 run config 中的冻结 reference 指向具体版本；分钟 Job 不顺手重跑 Basic。
...
- Readiness 名称不能再使用无边界的 `raw_ready`：`batch_etf_mins_raw_storage_readiness(...)` 只复刻文件/身份 checks；`batch_etf_mins_silver_admission_readiness(...)` 批量复核五频文件、当前 policy 和 admission check 绑定。
...
-> 10日 Raw storage batch readiness（只含 file_contract/request_scope）
-> select earliest actionable date
-> latest same-day ready Basic reference
-> one bounded Prod Raw five-frequency code-coverage probe
-> build one RunRequest for five Raw assets
```

Sensor 不读取 TaskRun，也不为 N3 对 Prod 做价格、网格或内部空洞预审计。它只用冻结 Basic 的 `expected(D)` 对 Prod Raw 做一次五频代码存在性检查；任一 expected code/freq 缺失都 skip，避免上游仍在写时启动。Raw asset 开始时只重新验证冻结 Basic，并校验 config 携带的 coverage reference 与该 Basic/日期一致；每频只做一次明细导出，再由本地候选完成范围校验，不重复 coverage 或 fingerprint 查询。文件存在但 `file_contract/request_scope` 失败时不得自动提交覆盖 run；只有 `bar_domain` blocked 时，该 Raw 仍是 storage-ready，Sensor 可以继续推进后续 Raw 日期。Basic 不新鲜、物理覆盖不全或全局 N3 policy 尚未冻结，都返回人能看懂的 SkipReason。

### 18.6 分钟 Silver Sensor

只读最近 10 日 Raw/Silver Lake readiness 和有界的当前 admission check 绑定，不访问 Prod/Tushare。其 batch helper 在一个 DuckDB connection 中复核同日五频文件/hash/policy，并用小范围 Dagster 状态读取确认五个 `bar_domain` checks 精确绑定当前 Raw materializations；禁止逐日深扫 event history。只有五频 `silver_admission_ready` 时才构造一个共享 `EtfMinsDailyAdmissionReference`，提交该日五个 Silver assets。最早日期 admission 缺失、过期或 blocked 时停在该日，不越过它推进后续 Silver；它不影响 Raw Sensor 继续处理更晚日期。
...
etf_mins_daily_admission_missing
etf_mins_daily_admission_blocked
etf_mins_daily_admission_stale
...
每个分钟 Raw entry 的 `blocking_check_names` 只登记对应 `file_contract_check` 和 `request_scope_check`。`bar_domain_check` 仍是正式 active check，但因它只决定 Silver admission、不能阻断 Raw 日期连续性，所以不进入该字段；Catalog notes 与 performance contract 必须明确 `mandatory_silver_admission_check=<bar_domain_name>`。静态测试既要证明这个 check 存在，也要证明它没有被错误登记为 Raw blocking check。
...
| `raw_etf_mins_<freq>m` | 按交易日保存 Prod Raw 中 `<freq>` ETF 分钟物理事实；文件/身份通过后可继续推进 Raw，价格与网格由后续 admission 判断。 |
...
| `raw_etf_mins_update_job` | 为一个交易日批量导出五个原生频率 ETF 分钟 Raw，并执行文件/身份门禁和同日 N3 admission 评估；冲突不覆盖，可按原分区重跑。 |
| `silver_etf_mins_update_job` | 为一个已通过五频 admission 的交易日生成五个 Silver 分区；任何 Raw/policy/reference 漂移时写前停止，修复准入状态后可重跑。 |
...
| `raw_etf_mins_update_job_sensor` | 在运行窗口内检查最早 Raw 存储缺口、最新 Basic 和一次 Prod 五频覆盖，满足后触发五频 Raw；N3 blocked 不阻断更晚 Raw，默认停止。 |
| `silver_etf_mins_update_job_sensor` | 检查最早未完成日期的五频 Raw 与当前 N3 admission，全部准入后触发 Silver；blocked 时停在该日且不访问 Prod，默认停止。 |
...
- 分钟 Raw `bar_domain`：说明“同日五频共享 N3 policy 的 Silver admission 结果；blocked 不删除 Raw，但禁止 Silver”；不得写成普通 WARN 或 Raw 存储门禁。
...
etf_mins_daily_admission_evaluated
...
| --- | --- | --- | --- |
| Sensor 为什么没触发 | Sensor cursor / SkipReason | target date、reason code、阻断组件、短 summary、next action | 等运行窗口/Basic/Prod，还是处理已有失败文件/admission |
| Asset run 为什么失败 | Run stdout/stderr | 最后一个里程碑、asset、partition/freq、关键数量、失败动作 | 失败在源读取、候选校验、冲突、admission 还是 promote |
| Raw 文件/身份 check 为什么失败 | check metadata | failed rule、数量、最多 20 个样本、URI、next action | 修源/合同或人工解释新代码；不能靠覆盖重跑 |
| N3/Silver admission 为什么失败 | `bar_domain` metadata + observation/decision 报告 | policy version、decision、reason codes、计数、报告路径 | policy 允许 WARN、修复观测链，或回到 Prod 处理数据事实 |
...
| 这次实际写了什么 | materialization metadata / final report | URI、row count、hash、write disposition、Basic/admission fingerprint | 对账新增、复用和零行文件，不从少量 Dagster event 推全历史 |
...
silver-apply  只消费 N3 decision manifest，经显式确认后写 Silver，不访问 Prod、不写 event
partitions    物理验收后注册已批准的历史动态分区，不读 Prod、不写 Lake/event
events        物理对账后单独补 materialization/check event，不写 Lake
```

禁止把写开关藏在只读入口。`raw-apply`、`silver-apply`、`partitions`、`events` 分别要求 `--confirm-raw-lake-write`、`--confirm-silver-lake-write`、`--confirm-partition-write`、`--confirm-event-write`。本 LLD 只设计入口，不授权执行。

对应调用形状固定为：

```text
python -m orchestrator.defs.bootstrap.etf_mins_bootstrap_plan_cli \
  --start-date YYYY-MM-DD --end-date YYYY-MM-DD \
  --report-path <explicit-json-path>

python -m orchestrator.defs.bootstrap.etf_mins_bootstrap_raw_apply_cli \
  --plan-path <explicit-json-path> \
  --checkpoint-path <explicit-json-path> \
  --confirm-raw-lake-write

python -m orchestrator.defs.bootstrap.etf_mins_raw_observe_cli \
  --raw-report-path <explicit-json-path> \
  --output-dir <explicit-operation-dir>

python -m orchestrator.defs.bootstrap.etf_mins_raw_decide_cli \
  --observation-summary-path <explicit-json-path> \
  --approved-policy-version <registered-version> \
  --output-dir <explicit-operation-dir>

python -m orchestrator.defs.bootstrap.etf_mins_bootstrap_silver_apply_cli \
  --raw-decision-summary-path <explicit-json-path> \
  --decision-manifest-path <explicit-parquet-path> \
  --checkpoint-path <explicit-json-path> \
  --confirm-silver-lake-write

python -m orchestrator.defs.bootstrap.etf_mins_bootstrap_partitions_cli \
  --final-report-path <explicit-json-path> \
  --confirm-partition-write

python -m orchestrator.defs.bootstrap.etf_mins_bootstrap_events_cli \
  --final-report-path <explicit-json-path> \
  --confirm-event-write
```

所有报告和 checkpoint 必须放在本次 operation 的 staging 目录；CLI 要求绝对路径并校验其位于 `/Volumes/datasource/data_lake_staging`。plan/raw-observe/raw-decide CLI 没有正式 Lake/event 写参数，raw-apply 没有 Silver/event 参数，silver-apply 没有 Prod/event 参数，partitions 没有 source/Lake/event 参数，events 没有 source/Lake/partition write 参数。`raw-decide` 只接受代码中已登记且已有正反测试的 policy version，不能从命令行临时传阈值。历史专项入口另加 `--protect-from-date 2026-01-01`，其值只允许等于合同常量，不能由操作者改成更晚日期。

首次 Direct Bootstrap 发生在分钟日常 Sensors 启用前，并由一个 `operation_id` 在单一进程内串行执行各频率和日期批次。它不是长期与日常链并行的第二条写路径，因此不新增跨路径 concurrency pool 或外部锁；日常 Sensors 必须保持 `STOPPED`，直到 Bootstrap、分区、事件和最终验收分别完成并获准启用。
...
`execution_watermark_date` 不能写成配置常量。plan 在提交写入审批前，使用当次最新 ready Basic，对操作者请求上界向前最多 10 个 SSE 开市日做一条批量五频代码覆盖查询：取其中最新一个五频完整日作为水位；10 日内没有完整日就 fail-closed，不继续向历史深扫。`execution_watermark_coverage_fingerprint` 只是这次 coverage 查询输入、聚合结果和观测时间的稳定摘要，不是分钟明细内容 hash。报告必须同时展示操作者请求上界、实际水位和被水位裁掉的日期。
...
7. 报告请求范围、水位后实际范围及被裁日期；日期、预算、冲突、Basic、水位或保护清单任一不关闭时 `should_stop=true`。N3 尚未执行不是 Raw plan 的停止条件。
...
Raw apply 完成后只生成 Raw final report，不生成 Silver，不补 Dagster event，也不宣称 `silver_admission_ready`；历史 `raw_storage_ready` 只能在后续正式 materialization 与两类 blocking checks 被正确补录后成立。

### 20.5 N3A Raw observation/profile

raw-observe 必须先验证 frozen plan、Raw final report 和正式 Raw 文件集合/hash 一致，然后在一个或少量 DuckDB connection 中完成第 14.4 节的本地批量观察。
...
只处理 `silver_eligible=true` 的 Raw 分区；`blocked` 分区不写 Silver，WARN 只有在冻结 policy 明确为非阻断时才能准入。每个 Silver 文件仍与对应 Raw 做 11 字段双向 `EXCEPT ALL`，不能修值、删行、去重或补 bar。
...
动态分区是 Dagster 的可寻址索引，不是数据存在证明。注册某日不代表该日 `raw_storage_ready` 或 `silver_admission_ready`；物理文件、N3 decision/admission 和 checks 仍分别决定每层状态。

### 20.9 完成对账

主验收使用集合和聚合，不为几千分区逐个深扫 Dagster readiness：

- Frozen plan 目标文件集合与正式 Raw 文件集合差异。
- Raw final report 的 source/staging/Raw 行数、范围和 hash 对账。
...
- 所有 candidate/staging/checkpoint 是否已结案。
- 少量首、中、尾日期的完整 blocking semantics 抽样。
...
- Raw target missing 才能新增。
- Raw target 等价则 `reused`，不进入 changed manifest。
- Raw target 冲突立即停止。
- Silver 只消费 `actual_changed_raw_manifest ∩ silver_eligible decision manifest`；不 glob 2026，不全范围重建。
- Event helper 只消费实际新增且最终对账通过的文件。
...
事件入口只消费 Bootstrap final report、已注册动态分区集合和实际正式文件，不调用 source、不写 Lake、不写动态分区、不运行 job/sensor。若任一目标 partition key 尚未注册，整个 event apply fail-closed。

建议规则：

- Raw materialization 只补已经进入 Raw final report 且被 N3 decision manifest 覆盖的分区；Silver materialization 只补实际生成且 `silver_eligible=true` 的分区。
- 正式 checks 只补最近 20 个 `cn_a_etf_mins_trade_days`；包括 Raw 两类 blocking checks、一类 Silver admission check，以及 Silver 两类 blocking checks。更早质量事实由最终物理审计报告保存。
...
单个 frozen plan 的 Raw materialization 候选上限等于 `target_file_count`，Silver materialization 候选不超过 `target_file_count`，因此 materialization event 总上限为 `2 × target_file_count`。正式 checks 只覆盖最近 20 个交易日，五频 Raw 的 2 blocking + 1 admission 加五频 Silver 的 2 blocking，最多 `20 × (5×3 + 5×2) = 500` 条 check event。blocked Raw 可以补 `bar_domain passed=False` 作为真实 admission 事实，但不得补 Silver materialization 或伪造 green。dry-run 超过这两个预算中的任一个就停止，不靠放宽上限执行。
...
| 分钟 Raw 日常 | Sensor 对最早候选日 1 条五频 coverage SQL；5 个 Raw asset 各 1 条单日单频明细 SQL，Raw 内不重查 coverage/fingerprint；随后五频 admission 共用 1 个 DuckDB connection | 最多 5 个 Raw 文件 + 5 个 admission evaluations | 每个自然日最多 6 条 Prod SQL；不允许 ETF×频率 N+1；五个 bar_domain 不得五次重扫 |
| 分钟 Silver 日常 | 在一个 admission reference 下读 5 个同日 Raw 文件，各 1 次 COPY/对账 | 最多 5 个 Silver 文件 | 不访问 Prod，不跨日期扫描；asset 内重新验证五频 hash/policy |
| Raw Sensor tick | 10 日期 batch Raw storage readiness；最早候选日最多 1 次五频代码物理覆盖 query，不为 N3 预扫 Prod 价格/网格/空洞 | 0/1 RunRequest，小 cursor | 只看 file/scope checks；N3 blocked 不堵后续 Raw；coverage/Basic 异常只 skip |
| Silver Sensor tick | 10 日期、Raw/Silver 最多 100 个目标路径的 true batch admission readiness + 有界当前 check 绑定 | 0/1 RunRequest | 1 DuckDB connection；禁止逐日深扫 event history；最早 blocked 日不越过 |
...
| Bootstrap Silver apply | 只读 `silver_eligible` Raw 文件和冻结 decision manifest | 每日/频率独立 Silver staging 和原子提升 | 不访问 Prod；blocked 分区零写入；Raw/Silver 双向 `EXCEPT ALL` |
| Partition registration | final report + Silver 交易日历 + 当前动态分区集合 | 只新增获批缺失 partition keys | 必须先于 event；禁止按有数据日期推导、禁止越水位或顺带补历史 |
...
`statement_timeout` 不新增环境变量；先通过经批准的小样本导出冻结为代码常量。若无法在安全时间内完成一个 20 日批次，缩小批次，不靠全量 Prod profiling 或放大 timeout 解决。N3 的主性能证据来自本地 Raw DuckDB 扫描。
...
- latest-ready 只检查最新一条同日 Silver materialization、其绑定 Silver checks 和它引用的 Raw materialization/checks，不按目录/mtime 选版本；最新任一层失败或不新鲜时不回退旧成功版本。
...
- coverage 使用 psycopg 绑定参数、rollback 和事务内 `statement_timeout`；明细远端 SQL 使用验证后的 literals + `postgres_query`，禁止保留 `%s` 占位符。DuckDB conninfo timeout helper 拒绝非法值且测试不泄露凭据。
- 物理 coverage 使用冻结 Basic expected codes × 五频，一次索引友好查询返回 expected/present/missing 和有界样本。
...
- 同日五频 admission 只建立一个 DuckDB connection/共享 evaluation，发出五个 `bar_domain blocking=False` checks；green/WARN passed，blocked failed，metadata 都绑定当前 Raw hash 和 policy。
- 某日 N3 blocked 后，该日 Raw 保留且下一日 Raw 仍可推进；该日及其后的 Silver 不越过最早 blocked 日。
- Silver 手工 Launchpad/CLI 缺 admission reference、reference 指向旧 Raw、policy 过期或任一 check 未绑定/失败时，在写候选前停止。
...
- 文件缺失可选；文件存在但 `file_contract/request_scope` 失败只 blocked，不自动覆盖；只有 `bar_domain` 失败时 Raw storage readiness 仍通过、Silver admission readiness 失败。
...
- cursor 小于 8 KB。
- Raw/Silver 分别调用 `batch_etf_mins_raw_storage_readiness` 与 `batch_etf_mins_silver_admission_readiness`；静态测试禁止恢复一个含混的 `raw_ready` helper。

### 24.5 Bootstrap/保护门禁

```text
tests/test_etf_mins_bootstrap_plan.py
tests/test_etf_mins_bootstrap_raw_apply.py
tests/test_etf_mins_raw_observe.py
tests/test_etf_mins_raw_decide.py
tests/test_etf_mins_bootstrap_silver_apply.py
tests/test_etf_mins_bootstrap_partitions.py
tests/test_etf_mins_bootstrap_events.py
...
- grid profile 先观察再分类；P7A 与 P7B 之间所有分区为 `unclassified`，只阻断 Silver，不回滚 Raw。
...
- Silver 只消费 `actual_changed_raw_manifest`。
...
- event apply 只消费 final report，不写 Lake，checks 只限最近 20 日。
...
- schema、path、partition model、checks 与 Definitions 一致；分钟 Raw Catalog 只登记两类 blocking checks，`bar_domain` 必须存在且 `blocking=False`、由 Silver readiness 强制消费。
...
完成条件：N1/N2/N4/N5 已按本文冻结；N6 只保留为 P10 启用门禁；N3A 输入输出与 N3B 拍板边界已确认；Tushare 第 4 满页 fail-closed 行为、exchange 比较映射、两类 Prod 查询策略、statement timeout 和 batch 行数都有 fake + 有界真实样本证据。P0 未完成不得进入 P1。
...
实现 Tushare staging、全页校验、Raw hash、不可变提升、三个 Raw checks 和 Raw job；Sensor 统一留到 P10。
...
完成条件：`.OF` 精确过滤、D/P 保留、日期/数值标准化、Raw/Silver hash 和 freshness 全部有正反测试。

### P4：Prod Raw 物理覆盖、SQL 和稳定 Raw validator

实现只读单表 allowlist、纯 SQL builder、Sensor 单次五频代码 coverage/reference、单次明细 relation 的本地传输对账、六类集合 SQL，以及只阻断传输/合同/身份污染的 Raw validator。网格只生成诊断，不在本切片先写 blocking 结论。
...
先实现只读 frozen plan、目标冲突/空间/查询预算和 2026 保护清单；单独授权后按频率/20 日批次写 staging、执行稳定门禁、提升 Raw 并逐文件 checkpoint。

完成条件：每个 20 日单频批次只有一次明细查询，source/staging/Raw 行数、范围、hash 和文件集合闭合，每个 frozen 日期/频率都有普通或显式零行 Raw；N4 截止日确定，2026 保护零变化；不写 Silver，不补事件，不宣称 `silver_admission_ready`。
...
在 N3 policy 冻结后实现五频 Raw/Silver asset factories、Raw 两类 blocking checks + 一类非 blocking 的强制 admission check、两类 Silver blocking checks、两个 jobs、同日五频单次 daily admission evaluator、Silver 写前 reference revalidation，以及只消费 `silver_eligible=true` manifest 的历史入口。

完成条件：11 字段两个方向 `EXCEPT ALL=0`，没有修值或删行；blocked/未知 decision 分区零 Silver 写入但不阻断后续 Raw；五个 admission evaluations 共用一次扫描，且手工启动不能绕过 reference/policy/check 绑定。

### P9：历史动态分区与 Runless events

物理验收后先单独 dry-run/授权注册历史动态分区，再单独 dry-run/授权补 materialization 和最近 20 日 checks，最后做 post-audit。任一目标分区未注册时不得补事件。
...
在 P6-P9 主链验收后，使用第 21 节保护合同单独计划、授权和执行；只消费 `actual_changed_raw_manifest`，并证明 2026 年及以后文件写前写后零变化。
...
3. 最新 Basic 不是当天，Raw/Silver 任一层 checks 没有绑定对应当前 materialization，或运行中 reference 漂移。
4. Prod SQL 需要 Basic join/代码过滤才能跑完，或 20 日批次超时/超空间。
5. 实现试图读取任何 Prod `ops.*`、Serving 表，或用执行状态代替 Prod Raw 物理覆盖。
6. 进入 Silver、补 green admission/check event 或启用 Silver Sensor 前，N3 仍未解释内部空洞、停牌/空结果和部分频率缺失；该条件不回滚已经通过稳定门禁的 Raw 文件，也不阻断 policy 已冻结后的后续 Raw 日期。
...
1. 12 个 assets、对应 checks、4 个 update jobs、5 个 Sensors、专属 partition 和 catalog 全部与本文一致；Raw storage readiness 与 Silver admission readiness 是两套明确合同。
2. Basic 无业务过滤、14 字段、不可变 Raw/Silver 版本和 latest-ready 冻结可独立复算。
3. 每次分钟 run 都记录并复核固定 Basic 两个 hash、eligibility date 和 requestable hash。
4. Prod 明细从 `prod-raw-db.raw_tushare.etf_minute_bar` 无 Basic 过滤读取；日常启动前做五频代码物理覆盖，候选提升前完成导出稳定性、六类集合和 Raw 前合同校验，且不访问任何 `ops.*`。
5. N3A 已用本地 Raw DuckDB 产出真实 observation/profile，N3B 的 blocking/WARN 经管理员批准并版本化；日常同日五频只做一次 admission evaluation，五个 `bar_domain` checks 绑定当前 Raw，Silver asset 重新验证 reference/policy。Prod 只承担 Sensor/plan 的单次有界 coverage、受控单次明细导出和异常有界回查，不做重复 fingerprint 或全量深审计。
