...
- M2 已实现 spec/helper 与临时目录测试；M3A 已实现受控 CLI/helper，但正式迁移、`cn_a_stock_mins_trade_days` 注册和 runless event 补录仍需按 dry-run、样本、全量步骤单独审批执行。
...
- M3A 已提供 `report-identity-map-events` 入口补录 `silver_stock_identity_map` 的 runless materialization 与 9 个 blocking check events；正式执行前仍需单独审批。
