...
- 版本：v1
- 状态：G1 已完成；G2 第一轮真实数据只读验证已执行，详见 G2 记录
- 更新时间：2026-08-08
- 适用范围：通用、因果、周期无关的波浪识别研究内核
- 依据：[通用波浪识别 G0 冻结合同 v1](./index-wave-g0-generic-contract-v1.md)
- 后续验收：[通用波浪识别 G2 真实数据只读验证与概率校准记录 v1](./index-wave-g2-readonly-real-data-validation-v1.md)
...
G1 阶段本身没有读取 Lake，没有新增 Dagster asset，没有写数据库或 Parquet，也没有训练真实概率。后续 G2 已在独立研究包中只读接入真实数据，但仍没有实现四浪专项、API、Wealth 或正式界面。因此，G1 完成只表示“算法合同可执行、可测试”；G2 的样本结果也不表示“波浪理论已经被 A 股历史证明有效”。
...
G2 真实数据性能审计发现，原实现虽然逐项 `yield`，`IncrementalWaveReplay` 内部仍重复保留了历史快照。现已在不改变默认增量对象和 `replay_wave` 返回语义的前提下，让流式入口使用 `retain_snapshots=false`。该修复把 8,542 根 120 分钟真实 K 线的单序列进程峰值降回门禁内。
...
G1 只实现合同和评估 harness。后续 G2 使用真实历史回答了第一轮门禁：日线固定分箱模型通过；120 分钟模型因 Brier 和 Log Loss 差于训练先验基线而失败。该结论只适用于 G2 记录中的模型、周期、结果空间和数据快照。
...
用户已批准并完成 G2 第一轮主要指数日线和 120 分钟只读运行：
...
详细结果见 [G2 记录](./index-wave-g2-readonly-real-data-validation-v1.md)。日线 10 个序列通过；120 分钟有 1 个价格跳空导致的 detector/swing 合同失败，且概率校准未过门禁。不得静默调参或吞掉失败样本。

30/60 分钟竞价行口径已经冻结：直接分析时过滤 `09:30`，不生成独立 Canonical Bar，也不并入第一根常规 K 线。对应 adapter 和真实数据验收尚未实现；后续扩展不得修改 G1 周期无关内核。四浪专项继续保持独立，不进入通用验收。
