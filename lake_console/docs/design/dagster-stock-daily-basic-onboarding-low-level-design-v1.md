...
状态：待评审，未实现。更新：2026-09-14。唯一上层目标见 [技术方案](dagster-stock-daily-basic-onboarding-plan-v1.md)。本文的“新增”均为计划代码，不是现有入口。P0 未通过不得把建议常量当成已批准实现。
...
下表为拟定 Parquet schema。小数位需 P0 全日样本验证，任何超精度值必须拒绝发布并先修订合同，不静默舍入。
...
| write_mode | 默认拒绝冲突；受控 replace 必须明确指定 | 本族 run config | 只有 writer 解析，不新增环境变量 |
...
source probe 的键字段分页属于拟新增实现，现有 18 字段样本不能替代其 P0 实测。若 keys-only 响应与完整字段请求键集不一致，停止该优化，修订请求方案。

最低代码覆盖的局限与批准门禁严格沿用方案 §6：只证明有行情股票没漏，不保证所有应有指标代码均已发布。**这条业务关系未验证前不得实现为正式门禁。** P0 必须列两向差集，并覆盖停牌、新股和退市样本；未能解释则等待裁定。
...
6. 正式提升前再次检查输入未变化和目标未被并发写入；同内容可幂等跳过，不同内容无 replace 授权则停止。每个分区使用已有互斥模式；若当前仓库没有可用的分区互斥，P1 先明确最小本族锁设计，不靠“先检查不存在”假装原子防并发。
...
| sensor 选中日 | 一次 keys-only 分页；当前样本应 1 页 | 再测 keys-only，典型 tick 目标 <10 秒；预算失败 skip |
...
全量 plan/apply 前必须填实：磁盘需求=导出 chunk + 年度候选 + 正式增量 + spill 余量，及导出/重核/构建总耗时和 prod 可接受 IO。当前未测传输/压缩字节，故不填假的 GB 数或完工时间；这项不通过不能进入 P3。
...
尚未达到开发准入：最低覆盖关系与缺少无行情股票的风险需要确认；完整日数值精度和 keys-only 需要验证；历史截止/独立事务来源稳定策略及全程成本需要冻结；建议对象、时间、Raw-only 和状态发布口径待评审。只有这些项通过，才能将本文从待评审改为可实施。
