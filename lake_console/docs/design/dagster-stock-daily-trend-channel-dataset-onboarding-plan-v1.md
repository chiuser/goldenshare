...
状态：方案已确认；LLD R10.1 专属业务模块、薄 CLI 与专属隔离测试、R10.2 完整本地门禁及 R10.3 正式只读计划均已完成。materialization/check 两阶段事件写入均未执行，当前停在第一次批准前。
