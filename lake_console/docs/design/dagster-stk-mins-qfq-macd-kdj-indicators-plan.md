...
| 日常 tick 额外查询量 | target date 直接 qfq lake readiness（同一真实 as-of 公式）+ bounded run-status 查询；不按历史 partition 扩张 |
