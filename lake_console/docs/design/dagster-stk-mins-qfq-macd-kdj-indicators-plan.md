...
> 2026-08-07 后续修正：90m/120m 上游 QFQ 集合竞价窗口合同错误，因此这两个频率的 indicator 和递推 state 必须在 QFQ 修复后从历史基线顺序重建。算法、上下游范围、文件重建和事件恢复以[统一修复与重建 LLD](./dagster-derived-minute-bars-90-120-contract-rebuild-low-level-design.md)为准；1m/5m/15m/30m/60m 不受影响。
