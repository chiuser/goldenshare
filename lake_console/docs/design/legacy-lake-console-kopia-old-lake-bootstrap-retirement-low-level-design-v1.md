...
状态：M1–M7 已提交（各阶段见 §11） / M8 获批 111 项删除记录已提交 `db4c3137`（§16.15） / 2026-09-06 独立获批的 5 项旧 Console 本机残留已移入废纸篓，记录已归档（§16.16） / 文档处置矩阵 165 份，控制文档另列 3 份 / 清单外对象保留
...
> 本文现为已完成专项的实施与执行证据，不是新的清退操作手册。上位方案保存理由和决策，本文保留历史文件矩阵、CLI 冻结、回归、删除清单与恢复映射。旧代码位置与“待执行”措辞以当时基线理解，不能机械套用到今天。

### 阅读导航（2026-09-10 整理）

- **原设计与防误删依据：** §1–10 的文件、符号、矩阵和约束按审计时点保留，不代表这些旧路径仍存在。
- **代码实施记录：** §11 保存各阶段结果，后续章节保存验收和风险边界；不是要求再跑一遍。
- **数据结果：** [§16.15](#retirement-m8-results) 是获批 111 项删除记录；[§16.16](#retirement-local-env-results) 是独立五项本机残留及废纸篓恢复映射。数量、路径、hash 与清单未因本轮整理改写。
- **停牌后续：** §16.11 的 TODO 已由独立 S0–S5 关闭；“保留 CSV”仅是清退当时的防误删结论，最终以[停牌 S5 对账](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-stock-suspend-confirmed-facts-low-level-design-v1.md#s5-final-closeout)为准，事故记录仍保留在其 §17。
- **授权与现状：** 已批准专项范围已收口；本次不再删任何文件或数据，也未重验生产、物理目录或废纸篓。清单外对象仍须单独审计、单独确认。
...
上述结论记录清退设计时的范围；已批准代码与数据清单现已完成，结果按顶部导航追溯。本文自身不授权新的代码/文档删除、数据或 Dagster event 写入、物理目录清理；范围变化必须重新确认。
...
<a id="suspend-confirmed-followup"></a>
...
用户已确认：不回写 Tushare Raw，不冻结历史 Raw；将现有历史全日停牌修正持久化为一份独立、固定、
正式登记的 Silver 输入资产，由现有 `silver_stock_suspend_daily` 合成最终事实。所有需要停牌事实的
本地 DG 消费者统一读取该最终 Silver，不直接合并修正规则；原有 Prod 读取链路及远程部署不在范围内。
不新增维护后台、数据库表或自动更新任务。数据湖本身允许读取正式 Parquet；退出的是未登记的运行时
CSV 旁路依赖，不是禁止一切文件存储。

具体资产归属、合并规则、逐文件影响面、迁移顺序、性能与验收统一落在
[本地 DG 停牌历史确认事实持久化技术方案](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-stock-suspend-confirmed-facts-technical-plan-v1.md)。
函数/SQL、CLI、发布续跑与测试的实施细则见
...
**执行边界**：这是用户确认另行推进的后续治理项，不自动扩成清退 M0–M8 的新实施内容或前置门禁。
当前文档轮不改 CSV、Python、SQL、正式停牌数据或其他业务数据。替代方案经确认、实现验收完成且具体删除
