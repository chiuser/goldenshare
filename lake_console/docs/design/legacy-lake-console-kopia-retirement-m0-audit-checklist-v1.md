...
状态：2026-08-28 M0 历史基线；M1–M6 已提交见 §22–28，M7 已提交 `3b94c48a` 见 §29–30；M8 111 项删除记录已提交 `db4c3137` 见 §31；2026-09-06 独立获批的 5 项旧 Console 本机残留已移入废纸篓，记录随本次提交归档见 §32；清单外对象保留
...
> 原“物理旧湖另轮处理”和 5 分钟硬门禁已被上位方案及 LLD 新口径替代；M5 当前矩阵为 165 份，不能再用
> 历史 145/154/156 份或当时未归类为 0 证明当前覆盖完整。物理数据的最新边界与结果见 §21 和 LLD §16.14。
...
后续独立需求已完成S0–S5（2026-09-08）：S2全范围零差异、S3单文件及3条事件发布证据保留于独立LLD §18.28–18.30；S4四历史日正式job等价复用、20项检查通过，恢复唯一Silver sensor后正常生成9月7日10行，5项检查及实际消费readiness通过，见[§18.31](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-stock-suspend-confirmed-facts-low-level-design-v1.md#s4-formal-daily-acceptance)。S5旧模块/CSV和专项临时产物已清理，运行器修正及156个逻辑用例通过，TODO-SUSPEND-001关闭，见[LLD §18.35](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-stock-suspend-confirmed-facts-low-level-design-v1.md#s5-final-closeout)。正式数据及发布记录未变；上述M0记录保持历史语境，不能据此恢复旧CSV读取。
...
## 27. 2026-09-05 M5 文档迁移与清退实施对账

本节是用户单独授权“继续推进 M5”后的执行记录，基线 `dev-interface@5f834b02`；不是原 M0 的只读审计成果。

- [x] 按文档治理 skill 的现行/混合/纯旧分类迁移，正式模板 7A/14 与性能治理 §6.4 承接有效源契约、类型、预算、只读、流式和安全写入检查；不照搬一刀切字段继承、日期类型或 raw-only 读取限制。
- [x] 正式初始化与旧分钟事故/重建/两次修复/代码集合差异收敛到唯一历史总账；保留必要数字和未解释差异，不复制旧命令，不把历史数据当本轮实测。
- [x] 全仓复扫补入 9 份矩阵外文档（6 保留、3 局部修改），另将 3 份仍有旧模板链接的保留项改为仅切换链接。矩阵 165 个唯一路径；删除目标仍为 86+3，不扩大范围。
- [x] 删除前确认引用切换及目标与基线一致，精确删除 89 份文档；无 archive/tombstone，全文可从 Git 历史恢复。总计修改 46 份文档/规则/skill，完整硬口径对账和 M6 延后项见 LLD §11 M5。
- [x] 待开发涨跌停方案只撤回旧湖来源与旧预算；字段/业务键、日常来源、审计指标、事件目标绑定/幂等/计数门禁保留。全历史来源与成本待该数据集开工前单独实测，不在 M5 假定已有替代方案。
- [x] 固定删除白名单、矩阵分类、全仓反向引用与候选归类通过；25 份保留文档字节不变，3 份模板链接之外正文不变，263 个旧产品文件和正式运行保护目录不变。
- [x] 修改文档的本地链接、10 份 HTML 受检结构/唯一 id、文档完整性与 diff 检查通过。CodeGraph 配合代码核验恢复/只读资源/源 adapter/catalog，根索引 sync/status 正常；未改变依赖矩阵。
...
- [x] 用户要求提交 M6 并进入 M7；已按 292 文件白名单提交 `63be03af`（263 删除、28 修改、1 新增），未推送。提交后工作区干净，M7 全量验收单独记录。
  正式 definitions 检查另行授权；M8 数据清理必须确认具体清单。本轮无新增设计拍板项。

## 29. 2026-09-05 M7 首轮全量验收与差异复核

本节记录用户授权“提交修改，然后进入 M7”后的首轮历史，基线 `dev-interface@63be03af`。当时未
全部通过；以下未勾选项的当前结果以 §30 为准，不再视为当前未决事项。详细失败证据见 LLD §11 M7。

- [x] 系统层禁止网络及移动盘写入，正式 Lake/staging/旧湖/Dagster home 不作测试对象；临时 spill、fake 资源与临时测试文件不接入正式链路。
- [x] orchestrator 全量运行结束：3,010 通过、3 失败、1,137 子例通过。原始失败未隐藏，也没有用选择性通过冒充全量全绿。
- [x] 两项九转内存失败已归因于整个测试进程的历史峰值；该文件在独立进程中 18 项全部通过。生产内存门禁保持 1 GiB，不增大、不关闭。
- [ ] DuckDB 默认 spill 测试与安全隔离冲突：建议只重构测试工厂替身及无参委托断言；默认配置和已有临时目录真实连接测试保留。两项临时委托/清理验证通过，但原测试尚未修改。
- [x] 根架构/六 Reader/API/snapshot service：242 通过、1 失败；Ops snapshot 查询/CLI/worker/API 补充 40 项通过。
- [ ] 根既有扫描误报：`execution_canceled` 子串命中当前 TaskRun 取消函数。建议仅修测试的语义识别并补正反例；不删当前取消代码，不整文件放行。相关修改需确认，本轮未做。
- [x] frontend 全量 149 项、Wealth 定向 60 项及两边 typecheck/build 通过；是 mock 与构建验证，不是线上 smoke。
- [x] 按 LLD 格式要求整理 5 份专项 Python 和 1 份专项测试的导入顺序；5 份 AST 相同，另 1 份导入集合/其余 AST 相同。复跑 184 项/246 子例和清退护栏 13 项均通过，未改 21 命令 fixture。
- [x] 六份修改 Python 的 Ruff/formatter、编排全体致命静态检查、compileall、ClickHouse 两个 bin 语法通过。36 个专项存续 Python 对比无新增 lint 诊断；既有 12 条 lint、19 份格式债务未整文件治理，不宣称全仓风格全绿。
- [x] M6 的 263 删除与原白名单一致；2,140 保护文件中 2,138 字节一致、2 个 CLI 仅格式改变且 AST 相同；旧 Console 11,692 ignored 文件元数据未变。各专项提交未混入根业务代码/前端/reports 的并行变更。
- [x] 文档复扫覆盖 165 处置对象 + 3 个显式登记的专项控制文档；发现并改正 LLD 中 3 处已删源码链接为 Git 历史位置。修正后 76 保留文档/3 控制文档无本地死链、12 HTML 结构与 id 通过、89 删除路径及入链清零、25 KEEP 与 M5 前基线字节相同。未发现新的未归类产品文档。
- [ ] 正式 `dg check defs` / `dg list defs` 未运行，等待单独授权；完整命令、cwd、DAGSTER_HOME、影响和退出边界已在 LLD §11 M7 列明。
- [x] 未执行 Kopia、生产读写、正式 Dagster 操作、服务停启、物理删除或部署。当前字段、日常链路、CSV 停牌规则和 Ops snapshot 保留。
- [ ] M7 新增记录及格式整理尚未提交；待测试问题与 Definitions 检查闭环后单独验收，不以 M6 已提交代替 M7 完成。

## 30. 2026-09-05 M7 续轮修正与技术验收收口

- [x] 用户明确同意两处测试修正及此前列明的 Definitions 操作；修正验收轮未提交、未推送、未进入 M8，后续提交授权见本节末项。
- [x] DuckDB 测试不再打开正式默认 spill，验证工厂无参委托、正常/错误清理与错误传播；三个既有默认配置/临时连接/错误目录测试保留，文件 6 项通过。
- [x] 旧模型扫描的 24 个原禁止项仍完整保留，只有取消事件名改为完整名称匹配；新增 23 个正反例，文件 64 项通过，其余 41 个顶层函数 AST 不变。未修改业务取消函数或放行整份源码文件。
- [x] 编排全量分两组独立进程：2,997 + 18 = 3,015 项全部通过，另有 1,137 子例通过；node ID 并集等于全量收集集合，遗漏/重复/失败/错误/跳过均为 0。九转正式 1 GiB 内存门禁不变。
- [x] 根架构/6 Reader/当前 API/Ops snapshot 联合回归 306 项全部通过；前端沿用首轮 149/60 项和构建证据，本续轮前端源码未变。
- [x] 正式 `dg check defs` 与 `dg list defs --json` 均退出 0；159 assets、364 checks、75 jobs、88 sensors、8 resources、1 schedule。资产 key 与当前 catalog 全等，各类无重复，返回清单无退役定义引用；未启动业务执行或补事件。
- [x] 两份测试完整 Ruff、编排 compileall/致命 Ruff 通过；38 个专项存续 Python 无新增 lint 诊断。原 12 条 lint、20 文件历史格式债务不在本轮整治范围，不宣称全仓风格全绿。
- [x] 263 删除项、2,140 保护文件与 11,692 ignored 文件再次对账通过；仅首轮两个 CLI 有格式差异、AST 不变。业务取消、正式 DuckDB 设置、九转恢复和 21 命令 fixture 字节不变。
- [x] 165 处置对象 + 3 控制文档/12 HTML、文档完整性和 diff 检查通过；CodeGraph 覆盖当前依赖并同步根索引，无新跨子系统依赖。
- [x] M7 技术验收通过，工作区精确 11 份修改：3 文档 + 6 份首轮格式整理 + 2 份已批准测试修正；无新增/删除文件、物理数据或 ignored 配置变更。
- [x] M7 已按 11 文件白名单提交 `3b94c48a`，未推送；该阶段未进入 M8，后续物理数据批准和执行结果见 §31。

结果与证据位置见 LLD §11“续轮收口”；未做生产恢复、线上 UI smoke、服务停启或部署，不能把本轮
技术验收解释成这些操作已经完成。

## 31. 2026-09-05 M8 获批物理清单执行与对账

- [x] 用户在 review LLD §16.14 的 106 个旧湖对象、D01–D05 和保留边界后明确确认。执行基线 `dev-interface@3b94c48a`，开工时工作区干净，不另开分支。
- [x] CodeGraph `explore` 和当前源码复核覆盖正式 Lake root、Raw staging、Silver 同次回滚、WMT 恢复输入；3,007 个程序/配置文件复扫，24 个命中均为拒绝规则或测试，没有新增业务消费者。
- [x] 111 个绝对路径唯一且互不嵌套；canonical path、符号链接、同设备、普通文件/目录检查通过。仅枚举元数据，不读湖数据内容、日期、完整性或恢复 manifest，不查询数据库/Dagster。
- [x] 删除前重新确认 108 个非 Git 对象无打开文件/相关进程命中；三份报告仅有 CodeGraph MCP 索引只读句柄，非业务消费者。未停启服务、未杀索引进程。
- [x] 一次性删除程序通过 9 个正反例测试及系统写白名单隔离测试。两次额外隔离预检失败均发生在任何删除之前；最终删除进程只允许 108 个获批路径、临时证据目录和系统空设备写入。
- [x] 108 个非 Git 对象逐项删除，约 56.5 秒；D01–D03 通过精确文件删除处理。合计 111/111 目标已不存在，普通文件 297,740 个、逻辑字节 509,841,740,142，未把逻辑大小说成实测释放空间。
- [x] 18 个保留锚点、64 个非目标同级对象和 27,240 个 ignored 文件对账通过；3,007 个程序/配置文件内容指纹不变。正式 2026-07-27 五频 Raw/Silver 共 10 文件仍在，停牌 CSV、身份映射 seed 和本机配置保留。
- [x] 旧湖根、六个共享家族目录和清单外 5 个 `.DS_Store` 保留；未清空 reports、quarantine、正式 raw/silver/gold、staging，不扩大到有效恢复输入。
- [x] 三份报告可从 `3b94c48a` 取回；其余非 Git 数据不承诺可恢复。没有新备份、快照或 Kopia 调用。
- [x] 结果与 111 行绝对路径清单落到 LLD §16.15；同步专项方案、主索引和三份历史文档的已清退位置标注。两份 Phase 3 HTML 只增加状态注释，原历史设计正文不变。
- [x] 根清退护栏 13 项、编排适配器退出/保留模块 17 项隔离回归通过；165 文档对象、3 控制文档、12 HTML、111 行绝对路径结果、文档完整性与 diff 检查通过。23 份 KEEP 字节不变，另 2 份仅追加历史位置注释；CodeGraph sync/status 通过。23:27 +08:00 再次物理边界对账通过。
- [x] 用户已另行要求提交本批执行记录。本次 Git 白名单为 7 份文档修改 + 3 份旧报告删除；提交前复核 staged diff，提交结果以 Git 记录为准。不推送，不再次操作物理数据，不继续处理 ignored 环境或其它目录。

原始临时证据在 `/private/tmp/lake-retirement-m8-20260905.SjIlR9`，稳定结果以 LLD §16.15 为准。
未运行正式 DG 命令、业务恢复、数据库查询或写入、job/sensor/check、物化、补事件、服务停启或部署。

## 32. 2026-09-06 五项旧 Console 本机残留独立清理

- [x] 用户先要求只读核用途，再明确确认删除；不沿用 M8 的数据授权。精确原路径及去向见 LLD §16.16，基线 `dev-interface@db4c3137`，开工时工作区干净。
- [x] 用途核验覆盖现行源码、CodeGraph 资源/Web 入口影响面、Shell/自启动/定时任务、Python 安装路径、前端目录和进程；无需要先迁移的现行消费者，配置历史来源注释不当作运行时读取。
- [x] 06:41 +08:00 刷新五项路径、数量、符号链接及占用；均为 ignored 且无当前占用。6 项隔离机械测试通过。
- [x] 五项已逐项移入私有废纸篓容器，原路径不存在，目标元数据一致；共 11,445 普通文件、430,452,648 逻辑字节和 16 个链接。没有永久擦除、跟随链接处理共享解释器、清空父目录或复制备份，不宣称已释放磁盘空间。
- [x] 16 个保护锚点与 2,968 个程序/配置文件指纹对账通过；清退护栏 13 项通过。当前 orchestrator、ClickHouse、根/Wealth 前端与共享 Miniconda 保留，无资源/字段/CLI/依赖矩阵变更。
- [x] 只同步专项方案、LLD、本清单和两处现行索引/说明；执行当轮未提交、未推送。用户随后要求提交，本次仅归档这五份文档，不推送、不自动继续删除或轮换凭据。

M0–M8 原章节中的“ignored 未处理”保留为当时事实；本批后续授权和结果以本节及 LLD §16.16 为准。废纸篓清空前可按映射恢复，之后不承诺恢复；不把恢复旧环境解释成恢复旧产品的许可。
