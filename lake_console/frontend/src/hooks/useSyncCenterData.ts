...
import { loadSyncCurrentRun, loadSyncLock, loadSyncProfiles, loadSyncRunDetail, loadSyncRunEvents } from "../services/lakeApi";
import type { SyncCurrentRun, SyncLock, SyncProfileSummary, SyncRunDetail, SyncRunEvent } from "../types";
...
function isFinishedRunStatus(status: string): boolean {
  return ["success", "failed", "backup_failed", "cancelled"].includes(status);
}
