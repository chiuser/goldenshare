"""make suspend d a protected raw-backed serving view

Revision ID: 20260828_000155
Revises: 20260828_000154
...
revision = "20260828_000155"
down_revision = "20260828_000154"
branch_labels = None
depends_on = None
...
BEGIN
    ...
    WHERE n.nspname = 'core_serving'
      AND c.relname = 'equity_suspend_d';

    IF raw_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected raw_tushare.suspend_d to be a physical table, found relation kind %',
            raw_kind;
    END IF;
    IF serving_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected core_serving.equity_suspend_d to be a physical table, found relation kind %',
            serving_kind;
    END IF;
    ...
    IF raw_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION
            'raw_tushare.suspend_d must remain on SSD pg_default, found %',
            raw_tablespace;
    END IF;
END
$migration$;
...
_LOCK_SOURCE_RELATIONS = (
    "LOCK TABLE raw_tushare.suspend_d IN SHARE MODE",
...
DO $migration$
DECLARE
    raw_oid oid := 'raw_tushare.suspend_d'::regclass;
    serving_oid oid := 'core_serving.equity_suspend_d'::regclass;
    raw_signature text[];
    serving_signature text[];
    raw_primary_key text[];
    serving_primary_key text[];
    raw_row_hash_index_columns text[];
    raw_row_hash_index_tablespace text;
    raw_trade_date_index_columns text[];
    raw_trade_date_index_tablespace text;
    raw_code_date_index_columns text[];
    raw_code_date_index_tablespace text;
    serving_row_hash_index_columns text[];
    serving_trade_date_index_columns text[];
    serving_code_date_index_columns text[];
BEGIN
    ...
    IF raw_signature IS DISTINCT FROM ARRAY[
        ...
        'suspend_timing|character varying(128)|NULL',
        'suspend_type|character varying(16)|NULL',
        'api_name|character varying(32)|NOT NULL',
        'fetched_at|timestamp with time zone|NOT NULL',
        'raw_payload|text|NULL'
    ]::text[] THEN
        RAISE EXCEPTION 'Unexpected raw_tushare.suspend_d column contract: %', raw_signature;
    END IF;
    ...
    IF serving_signature IS DISTINCT FROM ARRAY[
        ...
        RAISE EXCEPTION 'Unexpected core_serving.equity_suspend_d column contract: %', serving_signature;
    END IF;
    ...
    IF raw_primary_key IS DISTINCT FROM ARRAY['id']::text[] THEN
        RAISE EXCEPTION 'Unexpected raw primary key: %', raw_primary_key;
    END IF;
    IF serving_primary_key IS DISTINCT FROM ARRAY['id']::text[] THEN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on raw_tushare.suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
    ...
    SELECT pg_catalog.array_agg(a.attname ORDER BY key_column.ordinality),
           COALESCE(object_ts.spcname, database_ts.spcname)
    INTO raw_row_hash_index_columns, raw_row_hash_index_tablespace
    ...
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'uq_raw_tushare_suspend_d_row_key_hash'
    ...
    IF raw_row_hash_index_columns IS DISTINCT FROM ARRAY['row_key_hash']::text[] THEN
        RAISE EXCEPTION 'Required raw row-key-hash unique index is missing or invalid: %', raw_row_hash_index_columns;
    END IF;
    IF raw_row_hash_index_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION 'Required raw row-key-hash index must remain on SSD pg_default, found %', raw_row_hash_index_tablespace;
    END IF;
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_suspend_d_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
    ...
    SELECT pg_catalog.array_agg(a.attname ORDER BY key_column.ordinality),
           COALESCE(object_ts.spcname, database_ts.spcname)
    INTO raw_code_date_index_columns, raw_code_date_index_tablespace
    FROM pg_catalog.pg_index index_row
    JOIN pg_catalog.pg_class index_relation ON index_relation.oid = index_row.indexrelid
    JOIN pg_catalog.pg_database current_db ON current_db.datname = current_database()
    JOIN pg_catalog.pg_tablespace database_ts ON database_ts.oid = current_db.dattablespace
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_suspend_d_ts_code_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    IF raw_code_date_index_columns IS DISTINCT FROM ARRAY['ts_code', 'trade_date']::text[] THEN
        RAISE EXCEPTION 'Required raw ts-code/trade-date index is missing or invalid: %', raw_code_date_index_columns;
    END IF;
    IF raw_code_date_index_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION 'Required raw ts-code/trade-date index must remain on SSD pg_default, found %', raw_code_date_index_tablespace;
    END IF;

    SELECT pg_catalog.array_agg(a.attname ORDER BY key_column.ordinality)
    INTO serving_row_hash_index_columns
    FROM pg_catalog.pg_index index_row
    JOIN pg_catalog.pg_class index_relation ON index_relation.oid = index_row.indexrelid
    CROSS JOIN LATERAL pg_catalog.unnest(index_row.indkey)
        WITH ORDINALITY AS key_column(attnum, ordinality)
    ...
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'uq_equity_suspend_d_row_key_hash'
      AND index_row.indisvalid
      AND index_row.indisready
      AND index_row.indisunique
      AND NOT index_row.indisprimary
    ...
    IF serving_row_hash_index_columns IS DISTINCT FROM ARRAY['row_key_hash']::text[] THEN
        RAISE EXCEPTION 'Unexpected serving row-key-hash unique index contract: %', serving_row_hash_index_columns;
    END IF;
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_equity_suspend_d_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    SELECT pg_catalog.array_agg(a.attname ORDER BY key_column.ordinality)
    INTO serving_code_date_index_columns
    FROM pg_catalog.pg_index index_row
    JOIN pg_catalog.pg_class index_relation ON index_relation.oid = index_row.indexrelid
    CROSS JOIN LATERAL pg_catalog.unnest(index_row.indkey)
        WITH ORDINALITY AS key_column(attnum, ordinality)
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_equity_suspend_d_ts_code_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    IF serving_code_date_index_columns IS DISTINCT FROM ARRAY['ts_code', 'trade_date']::text[] THEN
        RAISE EXCEPTION 'Unexpected serving ts-code/trade-date index contract: %', serving_code_date_index_columns;
    END IF;
    IF (
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on raw_tushare.suspend_d';
    END IF;
    IF (
        SELECT pg_catalog.count(*)
        FROM pg_catalog.pg_index index_row
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on core_serving.equity_suspend_d';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected foreign-key dependency on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected user trigger on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected column-level ACL on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected RLS contract on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected view dependency on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_depend dependency_row
        ...
        RAISE EXCEPTION 'Unexpected function dependency on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected rewrite rule on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected extended statistics on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected security label on core_serving.equity_suspend_d';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected logical-publication contract on core_serving.equity_suspend_d';
    END IF;
END
$migration$;
"""
...
DECLARE
    ...
    raw_window_id_count bigint;
    serving_window_id_count bigint;
    raw_window_hash_count bigint;
    serving_window_hash_count bigint;
...
BEGIN
    ...
    FROM core_serving.equity_suspend_d;
    ...
    IF raw_count IS DISTINCT FROM serving_count
       ...
       OR raw_max_date IS DISTINCT FROM serving_max_date THEN
        RAISE EXCEPTION
            'suspend_d raw/serving range mismatch: raw_count=%, serving_count=%, raw_range=%..%, serving_range=%..%',
    ...
    WHILE window_start <= raw_max_date LOOP
        ...
        SELECT pg_catalog.count(*), pg_catalog.count(DISTINCT id),
               pg_catalog.count(DISTINCT row_key_hash)
        INTO raw_window_count, raw_window_id_count, raw_window_hash_count
        FROM raw_tushare.suspend_d
        ...
        SELECT pg_catalog.count(*), pg_catalog.count(DISTINCT id),
               pg_catalog.count(DISTINCT row_key_hash)
        INTO serving_window_count, serving_window_id_count, serving_window_hash_count
        FROM core_serving.equity_suspend_d
        WHERE trade_date >= window_start
          AND trade_date < window_end;
        ...
        IF raw_window_count > max_rows_per_month OR serving_window_count > max_rows_per_month THEN
            RAISE EXCEPTION
                'suspend_d monthly reconciliation exceeds safety cap: window=%..%, raw=%, serving=%, cap=%',
                window_start, window_end, raw_window_count, serving_window_count, max_rows_per_month;
        END IF;

        WITH raw_only AS (
            SELECT id, row_key_hash, ts_code, trade_date, suspend_timing, suspend_type
            FROM raw_tushare.suspend_d
            WHERE trade_date >= window_start
              AND trade_date < window_end
            EXCEPT ALL
        ...
        IF raw_window_count IS DISTINCT FROM serving_window_count
           OR raw_window_id_count IS DISTINCT FROM raw_window_count
           OR serving_window_id_count IS DISTINCT FROM serving_window_count
           OR raw_window_hash_count IS DISTINCT FROM raw_window_count
           OR serving_window_hash_count IS DISTINCT FROM serving_window_count
           OR raw_only_count <> 0
           OR serving_only_count <> 0 THEN
            RAISE EXCEPTION
                'suspend_d monthly mismatch: window=%..%, raw_count=%, serving_count=%, raw_id_count=%, serving_id_count=%, raw_hash_count=%, serving_hash_count=%, raw_only=%, serving_only=%',
                window_start, window_end, raw_window_count, serving_window_count,
                raw_window_id_count, serving_window_id_count,
                raw_window_hash_count, serving_window_hash_count,
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-owner DML grant on core_serving.equity_suspend_d';
    END IF;
    ...
    EXECUTE 'DROP TABLE core_serving.equity_suspend_d';
    EXECUTE $view$
        CREATE VIEW core_serving.equity_suspend_d AS
        SELECT
            id,
            row_key_hash,
            ts_code,
    ...
    EXECUTE pg_catalog.format(
        'ALTER VIEW core_serving.equity_suspend_d OWNER TO %I',
        serving_owner
    );
    EXECUTE 'REVOKE ALL ON core_serving.equity_suspend_d FROM PUBLIC';
    ...
    LOOP
        ...
        IF grantee_name = 'PUBLIC' THEN
            grant_sql := 'GRANT SELECT ON core_serving.equity_suspend_d TO PUBLIC';
        ELSE
            grant_sql := pg_catalog.format(
                'GRANT SELECT ON core_serving.equity_suspend_d TO %I',
                grantee_name
            );
        END IF;
    ...
    IF relation_comment IS NOT NULL THEN
        EXECUTE pg_catalog.format(
            'COMMENT ON VIEW core_serving.equity_suspend_d IS %L',
            relation_comment
        );
    END IF;
    ...
    LOOP
        EXECUTE pg_catalog.format(
            'COMMENT ON COLUMN core_serving.equity_suspend_d.%I IS %L',
            comment_entry.key,
            comment_entry.value
        );
    END LOOP;
    ...
    FROM pg_catalog.pg_class relation_row
    WHERE relation_row.oid = 'core_serving.equity_suspend_d'::regclass;
    ...
    FROM pg_catalog.pg_attribute attribute_row
    WHERE attribute_row.attrelid = 'core_serving.equity_suspend_d'::regclass
      AND attribute_row.attnum > 0
      AND NOT attribute_row.attisdropped;
    ...
    EXECUTE $trigger$
        CREATE TRIGGER trg_equity_suspend_d_reject_dml
        INSTEAD OF INSERT OR UPDATE OR DELETE
        ON core_serving.equity_suspend_d
        FOR EACH ROW
...
BEGIN
    ...
    JOIN pg_catalog.pg_roles owner_role ON owner_role.oid = relation_row.relowner
    WHERE namespace_row.nspname = 'core_serving'
      AND relation_row.relname = 'equity_suspend_d';

    IF view_kind IS DISTINCT FROM 'v' THEN
        RAISE EXCEPTION 'Expected core_serving.equity_suspend_d to be a view, found %', view_kind;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE trigger_row.tgrelid = view_oid
          AND trigger_row.tgname = 'trg_equity_suspend_d_reject_dml'
          AND NOT trigger_row.tgisinternal
          AND trigger_row.tgenabled = 'O'
    ) THEN
        RAISE EXCEPTION 'Missing enabled DML rejection trigger on core_serving.equity_suspend_d';
    END IF;
...
def downgrade() -> None:
    raise RuntimeError(
        "Recreating a physical core_serving.equity_suspend_d table requires an explicit approved "
