"""make margin a protected raw-backed serving view

Revision ID: 20260826_000152
Revises: 20260825_000151
...
revision = "20260826_000152"
down_revision = "20260825_000151"
branch_labels = None
depends_on = None
...
BEGIN
    ...
    WHERE n.nspname = 'core_serving'
      AND c.relname = 'equity_margin';

    IF raw_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected raw_tushare.margin to be a physical table, found relation kind %',
            raw_kind;
    END IF;
    IF serving_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected core_serving.equity_margin to be a physical table, found relation kind %',
            serving_kind;
    END IF;
    ...
    IF raw_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION
            'raw_tushare.margin must remain on SSD pg_default, found %',
            raw_tablespace;
    END IF;
END
$migration$;
...
_LOCK_SOURCE_RELATIONS = (
    "LOCK TABLE raw_tushare.margin IN SHARE MODE",
...
DO $migration$
DECLARE
    raw_oid oid := 'raw_tushare.margin'::regclass;
    serving_oid oid := 'core_serving.equity_margin'::regclass;
    raw_signature text[];
    ...
    raw_primary_key_tablespace text;
...
BEGIN
    ...
    IF raw_signature IS DISTINCT FROM ARRAY[
        ...
        'exchange_id|character varying(8)|NOT NULL',
        'rzye|numeric(20,4)|NULL',
        'rzmre|numeric(20,4)|NULL',
        'rzche|numeric(20,4)|NULL',
        'rqye|numeric(20,4)|NULL',
        'rqmcl|numeric(20,4)|NULL',
        'rzrqye|numeric(20,4)|NULL',
        'rqyl|numeric(20,4)|NULL',
        'api_name|character varying(32)|NOT NULL',
        'fetched_at|timestamp with time zone|NOT NULL',
        'raw_payload|text|NULL'
    ]::text[] THEN
        RAISE EXCEPTION 'Unexpected raw_tushare.margin column contract: %', raw_signature;
    END IF;
    ...
    IF serving_signature IS DISTINCT FROM ARRAY[
        ...
        RAISE EXCEPTION 'Unexpected core_serving.equity_margin column contract: %', serving_signature;
    END IF;

    -- margin is a bounded full-table contract. Both relations are SHARE-locked
    -- before this block runs, so the cap makes both full-set equality checks
    -- fit the fixed 16 MB work_mem budget without requiring temp_file_limit.
    SELECT pg_catalog.count(*) INTO raw_row_count
    FROM raw_tushare.margin;
    SELECT pg_catalog.count(*) INTO serving_row_count
    FROM core_serving.equity_margin;

    IF raw_row_count > max_relation_rows OR serving_row_count > max_relation_rows THEN
        RAISE EXCEPTION
            'margin relation size exceeds migration safety cap: raw=%, serving=%, cap=%',
    ...
    IF raw_primary_key IS DISTINCT FROM ARRAY['trade_date', 'exchange_id']::text[] THEN
        RAISE EXCEPTION 'Unexpected raw primary key: %', raw_primary_key;
    END IF;
    IF serving_primary_key IS DISTINCT FROM ARRAY['trade_date', 'exchange_id']::text[] THEN
        RAISE EXCEPTION 'Unexpected serving primary key: %', serving_primary_key;
    END IF;

    SELECT COALESCE(object_ts.spcname, database_ts.spcname)
    INTO raw_primary_key_tablespace
    FROM pg_catalog.pg_constraint constraint_row
    ...
    IF raw_primary_key_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION 'Raw primary-key index must remain on SSD pg_default, found %', raw_primary_key_tablespace;
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on raw_tushare.margin';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
    ...
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_margin_trade_date'
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_margin_exchange_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    IF raw_entity_index_columns IS DISTINCT FROM ARRAY['exchange_id', 'trade_date']::text[] THEN
    ...
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_equity_margin_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_equity_margin_exchange_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    IF serving_entity_index_columns IS DISTINCT FROM ARRAY['exchange_id', 'trade_date']::text[] THEN
    ...
    IF (
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on raw_tushare.margin';
    END IF;
    IF (
        SELECT pg_catalog.count(*)
        FROM pg_catalog.pg_index index_row
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on core_serving.equity_margin';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected foreign-key dependency on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected user trigger on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected column-level ACL on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected RLS contract on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected view dependency on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_depend dependency_row
        ...
        RAISE EXCEPTION 'Unexpected function dependency on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected rewrite rule on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected extended statistics on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected security label on core_serving.equity_margin';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected logical-publication contract on core_serving.equity_margin';
    END IF;
END
$migration$;
"""
...
BEGIN
    SELECT pg_catalog.count(*), pg_catalog.count(DISTINCT (trade_date, exchange_id))
    ...
    IF raw_count IS DISTINCT FROM serving_count
       ...
       OR serving_only_count <> 0 THEN
        RAISE EXCEPTION
            'margin raw/serving mismatch: raw_count=%, serving_count=%, raw_identity_count=%, serving_identity_count=%, raw_only=%, serving_only=%',
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-owner DML grant on core_serving.equity_margin';
    END IF;
    ...
    EXECUTE 'DROP TABLE core_serving.equity_margin';
    EXECUTE $view$
        CREATE VIEW core_serving.equity_margin AS
        SELECT
            trade_date,
            exchange_id,
            rzye,
    ...
    EXECUTE pg_catalog.format(
        'ALTER VIEW core_serving.equity_margin OWNER TO %I',
        serving_owner
    );
    EXECUTE 'REVOKE ALL ON core_serving.equity_margin FROM PUBLIC';
    ...
    LOOP
        ...
        IF grantee_name = 'PUBLIC' THEN
            grant_sql := 'GRANT SELECT ON core_serving.equity_margin TO PUBLIC';
        ELSE
            grant_sql := pg_catalog.format(
                'GRANT SELECT ON core_serving.equity_margin TO %I',
                grantee_name
            );
        END IF;
    ...
    IF relation_comment IS NOT NULL THEN
        EXECUTE pg_catalog.format(
            'COMMENT ON VIEW core_serving.equity_margin IS %L',
            relation_comment
        );
    END IF;
    ...
    LOOP
        EXECUTE pg_catalog.format(
            'COMMENT ON COLUMN core_serving.equity_margin.%I IS %L',
            comment_entry.key,
            comment_entry.value
        );
    END LOOP;
    ...
    FROM pg_catalog.pg_class relation_row
    WHERE relation_row.oid = 'core_serving.equity_margin'::regclass;
    ...
    FROM pg_catalog.pg_attribute attribute_row
    WHERE attribute_row.attrelid = 'core_serving.equity_margin'::regclass
      AND attribute_row.attnum > 0
      AND NOT attribute_row.attisdropped;
    ...
    EXECUTE $trigger$
        CREATE TRIGGER trg_equity_margin_reject_dml
        INSTEAD OF INSERT OR UPDATE OR DELETE
        ON core_serving.equity_margin
        FOR EACH ROW
...
BEGIN
    ...
    JOIN pg_catalog.pg_roles owner_role ON owner_role.oid = relation_row.relowner
    WHERE namespace_row.nspname = 'core_serving'
      AND relation_row.relname = 'equity_margin';

    IF view_kind IS DISTINCT FROM 'v' THEN
        RAISE EXCEPTION 'Expected core_serving.equity_margin to be a view, found %', view_kind;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE trigger_row.tgrelid = view_oid
          AND trigger_row.tgname = 'trg_equity_margin_reject_dml'
          AND NOT trigger_row.tgisinternal
          AND trigger_row.tgenabled = 'O'
    ) THEN
        RAISE EXCEPTION 'Missing enabled DML rejection trigger on core_serving.equity_margin';
    END IF;
...
def downgrade() -> None:
    raise RuntimeError(
        "Recreating a physical core_serving.equity_margin table requires an explicit approved "
