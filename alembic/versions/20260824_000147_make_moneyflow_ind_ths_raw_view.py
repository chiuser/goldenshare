"""make industry moneyflow ths a protected raw-backed serving view

Revision ID: 20260824_000147
Revises: 20260824_000146
...
revision = "20260824_000147"
down_revision = "20260824_000146"
branch_labels = None
depends_on = None
...
BEGIN
    ...
    WHERE n.nspname = 'raw_tushare'
      AND c.relname = 'moneyflow_ind_ths';
    ...
    WHERE n.nspname = 'core_serving'
      AND c.relname = 'industry_moneyflow_ths';

    IF raw_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected raw_tushare.moneyflow_ind_ths to be a physical table, found relation kind %',
            raw_kind;
    END IF;
    IF serving_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected core_serving.industry_moneyflow_ths to be a physical table, found relation kind %',
            serving_kind;
    END IF;
    ...
    IF raw_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION
            'raw_tushare.moneyflow_ind_ths must remain on SSD pg_default, found %',
            raw_tablespace;
    END IF;
END
$migration$;
...
_LOCK_SOURCE_RELATIONS = (
    "LOCK TABLE raw_tushare.moneyflow_ind_ths IN SHARE MODE",
...
DO $migration$
DECLARE
    raw_oid oid := 'raw_tushare.moneyflow_ind_ths'::regclass;
    serving_oid oid := 'core_serving.industry_moneyflow_ths'::regclass;
    raw_signature text[];
...
BEGIN
    ...
    IF raw_signature IS DISTINCT FROM ARRAY[
        ...
        'industry|character varying(128)|NULL',
        ...
        RAISE EXCEPTION 'Unexpected raw_tushare.moneyflow_ind_ths column contract: %', raw_signature;
    END IF;
    ...
    IF serving_signature IS DISTINCT FROM ARRAY[
        ...
        RAISE EXCEPTION 'Unexpected core_serving.industry_moneyflow_ths column contract: %', serving_signature;
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
    ...
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_moneyflow_ind_ths_trade_date'
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_moneyflow_ind_ths_ts_code_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND index_row.indpred IS NULL
    GROUP BY object_ts.spcname, database_ts.spcname;
    ...
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_industry_moneyflow_ths_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND index_row.indpred IS NULL;
    ...
    JOIN pg_catalog.pg_attribute a
     ...
     AND a.attnum = key_column.attnum
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_industry_moneyflow_ths_ts_code_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND index_row.indpred IS NULL;
    ...
    IF (
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on core_serving.industry_moneyflow_ths';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected foreign-key dependency on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected user trigger on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected column-level ACL on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected RLS contract on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected view dependency on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_depend dependency_row
        ...
        RAISE EXCEPTION 'Unexpected function dependency on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected rewrite rule on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected extended statistics on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected security label on core_serving.industry_moneyflow_ths';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected logical-publication contract on core_serving.industry_moneyflow_ths';
    END IF;
END
$migration$;
"""
...
BEGIN
    ...
    IF raw_count IS DISTINCT FROM serving_count
       ...
       OR raw_max_date IS DISTINCT FROM serving_max_date THEN
        RAISE EXCEPTION
            'moneyflow_ind_ths raw/serving range mismatch: raw_count=%, serving_count=%, raw_range=%..%, serving_range=%..%',
    ...
    WHILE window_start <= raw_max_date LOOP
        ...
        IF raw_window_count > max_rows_per_month OR serving_window_count > max_rows_per_month THEN
            RAISE EXCEPTION
                'moneyflow_ind_ths monthly reconciliation exceeds safety cap: window=%..%, raw=%, serving=%, cap=%',
        ...
        IF raw_window_count IS DISTINCT FROM serving_window_count
           ...
           OR serving_only_count <> 0 THEN
            RAISE EXCEPTION
                'moneyflow_ind_ths monthly mismatch: window=%..%, raw_count=%, serving_count=%, raw_identity_count=%, serving_identity_count=%, raw_only=%, serving_only=%',
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-owner DML grant on core_serving.industry_moneyflow_ths';
    END IF;
    ...
    EXECUTE 'DROP TABLE core_serving.industry_moneyflow_ths';
    EXECUTE $view$
        CREATE VIEW core_serving.industry_moneyflow_ths AS
        SELECT
            trade_date,
            ts_code,
            industry,
    ...
    EXECUTE pg_catalog.format(
        'ALTER VIEW core_serving.industry_moneyflow_ths OWNER TO %I',
        serving_owner
    );
    EXECUTE 'REVOKE ALL ON core_serving.industry_moneyflow_ths FROM PUBLIC';
    ...
    LOOP
        ...
        IF grantee_name = 'PUBLIC' THEN
            grant_sql := 'GRANT SELECT ON core_serving.industry_moneyflow_ths TO PUBLIC';
        ELSE
            grant_sql := pg_catalog.format(
                'GRANT SELECT ON core_serving.industry_moneyflow_ths TO %I',
                grantee_name
            );
        END IF;
    ...
    IF relation_comment IS NOT NULL THEN
        EXECUTE pg_catalog.format(
            'COMMENT ON VIEW core_serving.industry_moneyflow_ths IS %L',
            relation_comment
        );
    END IF;
    ...
    LOOP
        EXECUTE pg_catalog.format(
            'COMMENT ON COLUMN core_serving.industry_moneyflow_ths.%I IS %L',
            comment_entry.key,
            comment_entry.value
        );
    END LOOP;
    ...
    FROM pg_catalog.pg_class relation_row
    WHERE relation_row.oid = 'core_serving.industry_moneyflow_ths'::regclass;
    ...
    FROM pg_catalog.pg_attribute attribute_row
    WHERE attribute_row.attrelid = 'core_serving.industry_moneyflow_ths'::regclass
      AND attribute_row.attnum > 0
      AND NOT attribute_row.attisdropped;
    ...
    EXECUTE $trigger$
        CREATE TRIGGER trg_industry_moneyflow_ths_reject_dml
        INSTEAD OF INSERT OR UPDATE OR DELETE
        ON core_serving.industry_moneyflow_ths
        FOR EACH ROW
...
BEGIN
    ...
    JOIN pg_catalog.pg_roles owner_role ON owner_role.oid = relation_row.relowner
    WHERE namespace_row.nspname = 'core_serving'
      AND relation_row.relname = 'industry_moneyflow_ths';

    IF view_kind IS DISTINCT FROM 'v' THEN
        RAISE EXCEPTION 'Expected core_serving.industry_moneyflow_ths to be a view, found %', view_kind;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE trigger_row.tgrelid = view_oid
          AND trigger_row.tgname = 'trg_industry_moneyflow_ths_reject_dml'
          AND NOT trigger_row.tgisinternal
          AND trigger_row.tgenabled = 'O'
    ) THEN
        RAISE EXCEPTION 'Missing enabled DML rejection trigger on core_serving.industry_moneyflow_ths';
    END IF;
...
def downgrade() -> None:
    raise RuntimeError(
        "Recreating a physical core_serving.industry_moneyflow_ths table requires an explicit approved "
