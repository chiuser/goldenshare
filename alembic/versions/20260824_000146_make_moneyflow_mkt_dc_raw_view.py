"""make market moneyflow dc a protected raw-backed serving view

Revision ID: 20260824_000146
Revises: 20260823_000145
...
revision = "20260824_000146"
down_revision = "20260823_000145"
branch_labels = None
depends_on = None
...
BEGIN
    ...
    WHERE n.nspname = 'raw_tushare'
      AND c.relname = 'moneyflow_mkt_dc';
    ...
    WHERE n.nspname = 'core_serving'
      AND c.relname = 'market_moneyflow_dc';

    IF raw_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected raw_tushare.moneyflow_mkt_dc to be a physical table, found relation kind %',
            raw_kind;
    END IF;
    IF serving_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected core_serving.market_moneyflow_dc to be a physical table, found relation kind %',
            serving_kind;
    END IF;
    ...
    IF raw_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION
            'raw_tushare.moneyflow_mkt_dc must remain on SSD pg_default, found %',
            raw_tablespace;
    END IF;
END
$migration$;
...
_LOCK_SOURCE_RELATIONS = (
    "LOCK TABLE raw_tushare.moneyflow_mkt_dc IN SHARE MODE",
...
DO $migration$
DECLARE
    raw_oid oid := 'raw_tushare.moneyflow_mkt_dc'::regclass;
    serving_oid oid := 'core_serving.market_moneyflow_dc'::regclass;
    raw_signature text[];
...
BEGIN
    ...
    IF raw_signature IS DISTINCT FROM ARRAY[
        ...
        'close_sh|numeric(18,4)|NULL',
        'pct_change_sh|numeric(10,4)|NULL',
        'close_sz|numeric(18,4)|NULL',
        'pct_change_sz|numeric(10,4)|NULL',
        'net_amount|numeric(24,4)|NULL',
        'net_amount_rate|numeric(10,4)|NULL',
        'buy_elg_amount|numeric(24,4)|NULL',
        'buy_elg_amount_rate|numeric(10,4)|NULL',
        'buy_lg_amount|numeric(24,4)|NULL',
        'buy_lg_amount_rate|numeric(10,4)|NULL',
        'buy_md_amount|numeric(24,4)|NULL',
        'buy_md_amount_rate|numeric(10,4)|NULL',
        'buy_sm_amount|numeric(24,4)|NULL',
        'buy_sm_amount_rate|numeric(10,4)|NULL',
        'api_name|character varying(32)|NOT NULL',
        'fetched_at|timestamp with time zone|NOT NULL',
        'raw_payload|text|NULL'
    ]::text[] THEN
        RAISE EXCEPTION 'Unexpected raw_tushare.moneyflow_mkt_dc column contract: %', raw_signature;
    END IF;
    ...
    IF raw_primary_key IS DISTINCT FROM ARRAY['trade_date']::text[] THEN
        RAISE EXCEPTION 'Unexpected raw primary key: %', raw_primary_key;
    END IF;
    IF serving_primary_key IS DISTINCT FROM ARRAY['trade_date']::text[] THEN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
    ...
    SELECT pg_catalog.array_agg(a.attname ORDER BY key_column.ordinality),
           COALESCE(object_ts.spcname, database_ts.spcname)
    INTO raw_index_columns, raw_index_tablespace
    ...
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_moneyflow_mkt_dc_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND index_row.indpred IS NULL
    GROUP BY object_ts.spcname, database_ts.spcname;

    IF raw_index_columns IS DISTINCT FROM ARRAY['trade_date']::text[] THEN
        RAISE EXCEPTION 'Required raw trade-date index is missing or invalid: %', raw_index_columns;
    END IF;
    IF raw_index_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION 'Required raw trade-date index must remain on SSD pg_default, found %', raw_index_tablespace;
    END IF;

    SELECT pg_catalog.array_agg(a.attname ORDER BY key_column.ordinality)
    INTO serving_index_columns
    FROM pg_catalog.pg_index index_row
    JOIN pg_catalog.pg_class index_relation ON index_relation.oid = index_row.indexrelid
    CROSS JOIN LATERAL pg_catalog.unnest(index_row.indkey)
        WITH ORDINALITY AS key_column(attnum, ordinality)
    ...
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_market_moneyflow_dc_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND index_row.indpred IS NULL;

    IF serving_index_columns IS DISTINCT FROM ARRAY['trade_date']::text[] THEN
        RAISE EXCEPTION 'Unexpected serving trade-date index contract: %', serving_index_columns;
    END IF;
    IF (
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on core_serving.market_moneyflow_dc';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected foreign-key dependency on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected user trigger on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected column-level ACL on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected RLS contract on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected view dependency on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_depend dependency_row
        ...
        RAISE EXCEPTION 'Unexpected function dependency on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected rewrite rule on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected extended statistics on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected security label on core_serving.market_moneyflow_dc';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected logical-publication contract on core_serving.market_moneyflow_dc';
    END IF;
END
$migration$;
"""
...
DECLARE
    ...
    raw_identity_count bigint;
    serving_identity_count bigint;
    raw_only_count bigint;
    serving_only_count bigint;
BEGIN
    SELECT pg_catalog.count(*), pg_catalog.count(DISTINCT trade_date)
    INTO raw_count, raw_identity_count
    FROM raw_tushare.moneyflow_mkt_dc;

    SELECT pg_catalog.count(*), pg_catalog.count(DISTINCT trade_date)
    INTO serving_count, serving_identity_count
    FROM core_serving.market_moneyflow_dc;
    ...
    IF raw_count IS DISTINCT FROM serving_count
       OR raw_identity_count IS DISTINCT FROM raw_count
       OR serving_identity_count IS DISTINCT FROM serving_count
       OR raw_only_count <> 0
       OR serving_only_count <> 0 THEN
        RAISE EXCEPTION
            'moneyflow_mkt_dc raw/serving mismatch: raw_count=%, serving_count=%, raw_identity_count=%, serving_identity_count=%, raw_only=%, serving_only=%',
            raw_count, serving_count, raw_identity_count, serving_identity_count, raw_only_count, serving_only_count;
    END IF;
END
$migration$;
"""
...
DO $migration$
BEGIN
    IF pg_catalog.to_regprocedure('core_serving.reject_raw_direct_serving_view_dml()') IS NOT NULL THEN
        RAISE EXCEPTION 'Unexpected existing function core_serving.reject_raw_direct_serving_view_dml()';
    END IF;
END
$migration$;
"""
...
CREATE FUNCTION core_serving.reject_raw_direct_serving_view_dml()
RETURNS trigger
LANGUAGE plpgsql
SECURITY INVOKER
...
BEGIN
    RAISE EXCEPTION USING
        ...
        MESSAGE = pg_catalog.format(
            'DML is forbidden on raw-backed serving view %I.%I',
            TG_TABLE_SCHEMA,
            TG_TABLE_NAME
        ),
        DETAIL = pg_catalog.format('operation=%s', TG_OP),
        HINT = 'Write through DatasetWriter to the raw_tushare table.';
    RETURN NULL;
END
$function$;
"""
...
REVOKE ALL ON FUNCTION core_serving.reject_raw_direct_serving_view_dml() FROM PUBLIC
"""
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-owner DML grant on core_serving.market_moneyflow_dc';
    END IF;
    ...
    EXECUTE 'DROP TABLE core_serving.market_moneyflow_dc';
    EXECUTE $view$
        CREATE VIEW core_serving.market_moneyflow_dc AS
        SELECT
            trade_date,
            close_sh,
            pct_change_sh,
    ...
    EXECUTE pg_catalog.format(
        'ALTER VIEW core_serving.market_moneyflow_dc OWNER TO %I',
        serving_owner
    );
    EXECUTE 'REVOKE ALL ON core_serving.market_moneyflow_dc FROM PUBLIC';
    ...
    LOOP
        ...
        IF grantee_name = 'PUBLIC' THEN
            grant_sql := 'GRANT SELECT ON core_serving.market_moneyflow_dc TO PUBLIC';
        ELSE
            grant_sql := pg_catalog.format(
                'GRANT SELECT ON core_serving.market_moneyflow_dc TO %I',
                grantee_name
            );
        END IF;
    ...
    IF relation_comment IS NOT NULL THEN
        EXECUTE pg_catalog.format(
            'COMMENT ON VIEW core_serving.market_moneyflow_dc IS %L',
            relation_comment
        );
    END IF;
    ...
    LOOP
        EXECUTE pg_catalog.format(
            'COMMENT ON COLUMN core_serving.market_moneyflow_dc.%I IS %L',
            comment_entry.key,
            comment_entry.value
        );
    END LOOP;
    ...
    FROM pg_catalog.pg_class relation_row
    WHERE relation_row.oid = 'core_serving.market_moneyflow_dc'::regclass;
    ...
    FROM pg_catalog.pg_attribute attribute_row
    WHERE attribute_row.attrelid = 'core_serving.market_moneyflow_dc'::regclass
      AND attribute_row.attnum > 0
      AND NOT attribute_row.attisdropped;
    ...
    EXECUTE $trigger$
        CREATE TRIGGER trg_market_moneyflow_dc_reject_dml
        INSTEAD OF INSERT OR UPDATE OR DELETE
        ON core_serving.market_moneyflow_dc
        FOR EACH ROW
...
BEGIN
    ...
    WHERE namespace_row.nspname = 'core_serving'
      AND relation_row.relname = 'market_moneyflow_dc';

    IF view_kind IS DISTINCT FROM 'v' THEN
        RAISE EXCEPTION 'Expected core_serving.market_moneyflow_dc to be a view, found %', view_kind;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE trigger_row.tgrelid = view_oid
          AND trigger_row.tgname = 'trg_market_moneyflow_dc_reject_dml'
          AND NOT trigger_row.tgisinternal
          AND trigger_row.tgenabled = 'O'
    ) THEN
        RAISE EXCEPTION 'Missing enabled DML rejection trigger on core_serving.market_moneyflow_dc';
    END IF;
...
def upgrade() -> None:
    ...
    op.execute(_PREFLIGHT_REJECT_FUNCTION)
    op.execute(_CREATE_REJECT_FUNCTION)
    op.execute(_REVOKE_REJECT_FUNCTION_PUBLIC)
...
def downgrade() -> None:
    raise RuntimeError(
        "Recreating a physical core_serving.market_moneyflow_dc table requires an explicit approved "
