"""add balancesheet dataset on cold HDD storage

Revision ID: 20260830_000164
Revises: 20260830_000163
...
def _assert_postgresql() -> None:
    if op.get_bind().dialect.name != "postgresql":
        raise RuntimeError("资产负债表 HDD migration 只允许在 PostgreSQL 执行")
...
def _assert_hdd_tablespace() -> None:
    ...
    if not exists:
        raise RuntimeError(f"资产负债表要求 PostgreSQL tablespace `{_TABLESPACE}`，禁止回退到默认 SSD")
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        *(sa.Column(field_name, sa.Numeric(), nullable=True) for field_name in _DECIMAL_FIELDS),
        ...
        sa.Column(
            ...
            sa.String(length=32),
            nullable=False,
            server_default=sa.text("'balancesheet_vip'"),
        ),
    ...
    op.execute("ALTER INDEX raw_tushare.pk_raw_tushare_balancesheet SET TABLESPACE gs_raw_cold_hdd")
    op.execute(
        "CREATE INDEX idx_raw_tushare_balancesheet_ann_report_code "
        "ON raw_tushare.balancesheet (ann_date, report_type, ts_code) TABLESPACE gs_raw_cold_hdd"
    )
    op.execute(
        "CREATE INDEX idx_raw_tushare_balancesheet_serving_rank ON raw_tushare.balancesheet "
    ...
    op.execute(
        f"""
        CREATE VIEW core_serving.equity_balancesheet AS
...
def downgrade() -> None:
    raise RuntimeError("资产负债表保存业务事实，不支持自动 downgrade 删除数据。")
