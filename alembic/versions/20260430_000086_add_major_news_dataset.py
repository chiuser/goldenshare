"""add major news dataset

Revision ID: 20260430_000086
Revises: 20260430_000085
...
revision = "20260430_000086"
down_revision = "20260430_000085"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    if not inspector.has_table("major_news", schema="raw_tushare"):
        op.execute(
            """
    ...
    op.execute(
        """
        CREATE UNIQUE INDEX IF NOT EXISTS uq_raw_tushare_major_news_row_key_hash
        ON raw_tushare.major_news (row_key_hash)
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_raw_tushare_major_news_src_time
        ON raw_tushare.major_news (src, pub_time DESC)
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_raw_tushare_major_news_time
