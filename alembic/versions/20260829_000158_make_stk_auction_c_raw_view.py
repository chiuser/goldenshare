"""make stk auction close a protected raw-backed serving view

Revision ID: 20260829_000158
Revises: 20260829_000157
...
revision = "20260829_000158"
down_revision = "20260829_000157"
branch_labels = None
depends_on = None
...
BEGIN
    ...
    WHERE n.nspname = 'core_serving'
      AND c.relname = 'equity_auction_close';

    IF raw_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected raw_tushare.stk_auction_c to be a physical table, found relation kind %',
            raw_kind;
    END IF;
    IF serving_kind IS DISTINCT FROM 'r' THEN
        RAISE EXCEPTION
            'Expected core_serving.equity_auction_close to be a physical table, found relation kind %',
            serving_kind;
    END IF;
    ...
    IF raw_tablespace IS DISTINCT FROM 'pg_default' THEN
        RAISE EXCEPTION
            'raw_tushare.stk_auction_c must remain on SSD pg_default, found %',
            raw_tablespace;
    END IF;
END
$migration$;
...
_LOCK_SOURCE_RELATIONS = (
    "LOCK TABLE raw_tushare.stk_auction_c IN SHARE MODE",
    "LOCK TABLE core_serving.equity_auction_close IN SHARE MODE",
)
...
DO $migration$
DECLARE
    raw_oid oid := 'raw_tushare.stk_auction_c'::regclass;
    serving_oid oid := 'core_serving.equity_auction_close'::regclass;
    raw_signature text[];
...
BEGIN
    ...
    IF raw_signature IS DISTINCT FROM ARRAY[
        ...
        RAISE EXCEPTION 'Unexpected raw_tushare.stk_auction_c column contract: %', raw_signature;
    END IF;
    ...
    IF serving_signature IS DISTINCT FROM ARRAY[
        ...
        RAISE EXCEPTION 'Unexpected core_serving.equity_auction_close column contract: %', serving_signature;
    END IF;
    ...
    IF raw_primary_key IS DISTINCT FROM ARRAY['ts_code', 'trade_date']::text[] THEN
        RAISE EXCEPTION 'Unexpected raw primary key: %', raw_primary_key;
    END IF;
    IF serving_primary_key IS DISTINCT FROM ARRAY['ts_code', 'trade_date']::text[] THEN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on raw_tushare.stk_auction_c';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
        ...
        RAISE EXCEPTION 'Unexpected non-primary-key constraint on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_constraint constraint_row
    ...
    WHERE index_row.indrelid = raw_oid
      AND index_relation.relname = 'idx_raw_tushare_stk_auction_c_trade_date'
    ...
    WHERE index_row.indrelid = serving_oid
      AND index_relation.relname = 'idx_equity_auction_close_trade_date'
      AND index_row.indisvalid
      AND index_row.indisready
      AND NOT index_row.indisunique
      AND index_row.indpred IS NULL
    ...
    IF (
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on raw_tushare.stk_auction_c';
    END IF;
    IF (
        SELECT pg_catalog.count(*)
        FROM pg_catalog.pg_index index_row
        ...
        RAISE EXCEPTION 'Unexpected secondary-index count on core_serving.equity_auction_close';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected foreign-key dependency on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected user trigger on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected column-level ACL on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected RLS contract on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected view dependency on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        FROM pg_catalog.pg_depend dependency_row
        ...
        RAISE EXCEPTION 'Unexpected function dependency on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected rewrite rule on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected extended statistics on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected security label on core_serving.equity_auction_close';
    END IF;
    IF EXISTS (
        SELECT 1
        ...
        RAISE EXCEPTION 'Unexpected logical-publication contract on core_serving.equity_auction_close';
    END IF;
END
$migration$;
"""
...
BEGIN
    ...
    IF raw_count IS DISTINCT FROM serving_count
       ...
       OR raw_max_date IS DISTINCT FROM serving_max_date THEN
        RAISE EXCEPTION
            'stk_auction_c raw/serving range mismatch: raw_count=%, serving_count=%, raw_range=%..%, serving_range=%..%',
    ...
    WHILE window_start <= raw_max_date LOOP
        ...
        SELECT pg_catalog.count(*), pg_catalog.count(DISTINCT (ts_code, trade_date))
        ...
        IF raw_window_count > max_rows_per_month OR serving_window_count > max_rows_per_month THEN
            RAISE EXCEPTION
                'stk_auction_c monthly reconciliation exceeds safety cap: window=%..%, raw=%, serving=%, cap=%',
                window_start, window_end, raw_window_count, serving_window_count, max_rows_per_month;
        END IF;

        WITH raw_only AS (
            SELECT ts_code, trade_date, close, open, high, low, vol, amount, vwap
            FROM raw_tushare.stk_auction_c
            WHERE trade_date >= window_start
              AND trade_date < window_end
            EXCEPT ALL
        ...
        IF raw_window_count IS DISTINCT FROM serving_window_count
           ...
           OR serving_only_count <> 0 THEN
            RAISE EXCEPTION
                'stk_auction_c monthly mismatch: window=%..%, raw_count=%, serving_count=%, raw_identity_count=%, serving_identity_count=%, raw_only=%, serving_only=%',
...
LOCK TABLE core_serving.equity_auction_close IN ACCESS EXCLUSIVE MODE;
"""
...
BEGIN
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Unexpected non-owner DML grant on core_serving.equity_auction_close';
    END IF;
    ...
    EXECUTE 'DROP TABLE core_serving.equity_auction_close';
    EXECUTE $view$
        CREATE VIEW core_serving.equity_auction_close AS
        SELECT
            ts_code,
            trade_date,
            close,
    ...
    EXECUTE pg_catalog.format(
        'ALTER VIEW core_serving.equity_auction_close OWNER TO %I',
        serving_owner
    );
    EXECUTE 'REVOKE ALL ON core_serving.equity_auction_close FROM PUBLIC';
    ...
    LOOP
        ...
        IF grantee_name = 'PUBLIC' THEN
            grant_sql := 'GRANT SELECT ON core_serving.equity_auction_close TO PUBLIC';
        ELSE
            grant_sql := pg_catalog.format(
                'GRANT SELECT ON core_serving.equity_auction_close TO %I',
                grantee_name
            );
        END IF;
    ...
    IF relation_comment IS NOT NULL THEN
        EXECUTE pg_catalog.format(
            'COMMENT ON VIEW core_serving.equity_auction_close IS %L',
            relation_comment
        );
    END IF;
    ...
    LOOP
        EXECUTE pg_catalog.format(
            'COMMENT ON COLUMN core_serving.equity_auction_close.%I IS %L',
            comment_entry.key,
            comment_entry.value
        );
    END LOOP;
    ...
    FROM pg_catalog.pg_class relation_row
    WHERE relation_row.oid = 'core_serving.equity_auction_close'::regclass;
    ...
    FROM pg_catalog.pg_attribute attribute_row
    WHERE attribute_row.attrelid = 'core_serving.equity_auction_close'::regclass
      AND attribute_row.attnum > 0
      AND NOT attribute_row.attisdropped;
    ...
    EXECUTE $trigger$
        CREATE TRIGGER trg_equity_auction_close_reject_dml
        INSTEAD OF INSERT OR UPDATE OR DELETE
        ON core_serving.equity_auction_close
        FOR EACH ROW
...
BEGIN
    ...
    JOIN pg_catalog.pg_roles owner_role ON owner_role.oid = relation_row.relowner
    WHERE namespace_row.nspname = 'core_serving'
      AND relation_row.relname = 'equity_auction_close';

    IF view_kind IS DISTINCT FROM 'v' THEN
        RAISE EXCEPTION 'Expected core_serving.equity_auction_close to be a view, found %', view_kind;
    END IF;
    ...
    IF NOT EXISTS (
        ...
        WHERE trigger_row.tgrelid = view_oid
          AND trigger_row.tgname = 'trg_equity_auction_close_reject_dml'
          AND NOT trigger_row.tgisinternal
          AND trigger_row.tgenabled = 'O'
    ) THEN
        RAISE EXCEPTION 'Missing enabled DML rejection trigger on core_serving.equity_auction_close';
    END IF;
...
def downgrade() -> None:
    raise RuntimeError(
        "Recreating a physical core_serving.equity_auction_close table requires an explicit approved "
