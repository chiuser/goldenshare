...
class WealthSectorRelativeRotationDaily(Base):
    __tablename__ = "wealth_sector_relative_rotation_daily"
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            name="fk_wealth_sector_rotation_batch_date",
        ),
        CheckConstraint("period IN (5, 10, 20, 30)", name="wealth_sector_rotation_period_allowed"),
        CheckConstraint("minimum_group_size = 3", name="wealth_sector_rotation_min_group_fixed"),
        CheckConstraint(COMPARISON_IDENTITY_CHECK_SQL, name="wealth_sector_rotation_comparison_identity"),
        Index(
            "idx_wealth_sector_rotation_trade_scope_period_status",
            "trade_date", "comparison_scope", "comparison_key", "period", "rotation_status", "sector_code",
        ),
        {"schema": "core_serving"},
    )
    ...
    comparison_trade_date: Mapped[date] = mapped_column(Date, nullable=False)
    ...
    comparison_return_pct: Mapped[Decimal | None] = mapped_column(Numeric(18, 6))
    comparison_strength_rank: Mapped[int | None] = mapped_column(Integer)
    comparison_rankable_count: Mapped[int | None] = mapped_column(Integer)
    comparison_percentile: Mapped[Decimal | None] = mapped_column(Numeric(8, 4))
    percentile_delta_5d: Mapped[Decimal | None] = mapped_column(Numeric(8, 4))
    ...
    group_interpretation: Mapped[str] = mapped_column(String(32), nullable=False)
    current_missing_reason: Mapped[str | None] = mapped_column(String(64))
    comparison_missing_reason: Mapped[str | None] = mapped_column(String(64))
