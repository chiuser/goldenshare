...
class IndexNineTurnDaily(Base):
    __tablename__ = "index_nineturn_daily"
    __table_args__ = (
        CheckConstraint("close > 0 AND close < 1e308", name="close_positive"),
        CheckConstraint(
            "up_count >= 0 AND down_count >= 0", name="counts_non_negative"
        ),
        CheckConstraint(
            "NOT (up_count > 0 AND down_count > 0)", name="single_direction"
        ),
        CheckConstraint(
            "nine_up_turn IS NULL OR nine_up_turn = '+9'", name="up_signal_allowed"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "nine_up_turn IS NULL OR up_count >= 9", name="up_signal_count"
        ),
        CheckConstraint(
            "nine_down_turn IS NULL OR down_count >= 9", name="down_signal_count"
        ),
        CheckConstraint(
        ...
        Index("idx_index_nineturn_daily_trade_code", "trade_date", "ts_code"),
        {"schema": "core_serving"},
    )
