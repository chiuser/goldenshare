...
def _fund_div_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    del request
    del enum_values
    if anchor_date is None:
        raise ValueError("基金分红维护缺少公告日期锚点")
...
def _fina_indicator_vip_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    del request
    del enum_values
    if anchor_date is None:
...
def _bak_basic_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    if anchor_date is None:
        raise ValueError("缺少日期锚点")
    params: dict[str, Any] = {"trade_date": anchor_date.strftime("%Y%m%d")}
    ts_code = request.params.get("ts_code")
...
def _news_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    del request
    if anchor_date is None:
        raise ValueError("新闻快讯缺少日期")
    src = str(enum_values.get("src") or "").strip()
    if not src:
        raise ValueError("新闻快讯缺少新闻来源")
    date_text = anchor_date.isoformat()
    return {
        "src": src,
        "start_date": f"{date_text} 00:00:00",
...
def _etf_sz_cons_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    ts_code = str(enum_values.get("ts_code") or "").strip().upper()
    if not ts_code:
        raise ValueError("深市 ETF 持仓组合缺少 ETF 代码")
    if request.run_profile == "point_incremental":
        target_date = anchor_date or request.trade_date
        if target_date is None:
            raise ValueError("深市 ETF 持仓组合单日维护缺少交易日期")
        return {"ts_code": ts_code, "trade_date": target_date.strftime("%Y%m%d")}
    if request.run_profile == "range_rebuild":
        start_date = enum_values.get("start_date", request.start_date)
        end_date = enum_values.get("end_date", request.end_date)
        if start_date is None or end_date is None:
            raise ValueError("深市 ETF 持仓组合区间维护必须同时填写开始日期和结束日期")
        return {
            "ts_code": ts_code,
            "start_date": _format_yyyymmdd(start_date),
            "end_date": _format_yyyymmdd(end_date),
        }
    raise ValueError(f"深市 ETF 持仓组合不支持该运行模式：{request.run_profile}")
...
def _cyq_chips_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    ts_code = str(enum_values.get("ts_code") or "").strip().upper()
    if not ts_code:
        raise ValueError("每日筹码分布缺少股票代码")
    if request.run_profile == "point_incremental":
        target_date = anchor_date or request.trade_date
        if target_date is None:
...
def _margin_detail_params(request, anchor_date: date | None, enum_values: dict[str, Any]) -> dict[str, Any]:  # type: ignore[no-untyped-def]
    if anchor_date is None:
        raise ValueError("缺少日期锚点")
    params: dict[str, Any] = {"trade_date": anchor_date.strftime("%Y%m%d")}
    ts_code = request.params.get("ts_code")
