...
class SyncMoneyflowMktDcService(BaseSyncService):
    ...
    def execute(self, run_type: str, **kwargs: Any) -> tuple[int, int, date | None, str | None]:
        ...
        if run_type == "INCREMENTAL":
            ...
            fetched, written = self._sync_trade_date(trade_date=trade_date, execution_id=execution_id)
            self._update_progress(
                execution_id=execution_id,
                current=1,
                total=1,
                message=f"moneyflow_mkt_dc: 1/1 trade_date={trade_date.isoformat()} fetched={fetched} written={written}",
            )
        ...
        if explicit_trade_date is None and start_date is None and end_date is None:
            raise ValueError("sync_history.moneyflow_mkt_dc requires explicit time params: trade_date or start_date+end_date")
        ...
        if start_date is None or end_date is None:
            raise ValueError("sync_history.moneyflow_mkt_dc requires both start_date and end_date when trade_date is not provided")
        ...
        self._update_progress(
            ...
            current=0,
            total=total,
            message=f"准备按 {total} 个交易日同步市场资金流向（DC）。",
        )
        for index, trade_date in enumerate(trade_dates, start=1):
            ...
            self._update_progress(
                ...
                current=index,
                total=total,
                message=f"moneyflow_mkt_dc: {index}/{total} trade_date={trade_date.isoformat()} fetched={fetched} written={written}",
            )
        return total_fetched, total_written, end, f"trade_dates={total}"

    def _sync_trade_date(self, *, trade_date: date, execution_id: int | None) -> tuple[int, int]:
        ...
        while True:
            ...
            self.dao.raw_moneyflow_mkt_dc.bulk_upsert(normalized)
            written_total += self.dao.market_moneyflow_dc.bulk_upsert(normalized)
