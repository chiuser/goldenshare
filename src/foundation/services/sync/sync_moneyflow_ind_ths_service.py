...
from src.foundation.services.sync.fields import MONEYFLOW_IND_THS_FIELDS
from src.ops.models.ops.job_execution import JobExecution
from src.utils import coerce_row, parse_tushare_date


class SyncMoneyflowIndThsService(BaseSyncService):
    ...
    def execute(self, run_type: str, **kwargs: Any) -> tuple[int, int, date | None, str | None]:
        ...
        if run_type == "INCREMENTAL":
            ...
            self._update_progress(
                ...
                current=1,
                total=1,
                message=f"moneyflow_ind_ths: 1/1 trade_date={trade_date.isoformat()} fetched={fetched} written={written}",
            )
        ...
        if explicit_trade_date is None and start_date is None and end_date is None:
            raise ValueError("sync_history.moneyflow_ind_ths requires explicit time params: trade_date or start_date+end_date")
        ...
        if start_date is None or end_date is None:
            raise ValueError("sync_history.moneyflow_ind_ths requires both start_date and end_date when trade_date is not provided")
        ...
        self._update_progress(
            ...
            current=0,
            total=total,
            message=f"准备按 {total} 个交易日同步行业资金流向（THS）。",
        )
        for index, trade_date in enumerate(trade_dates, start=1):
            ...
            self._update_progress(
                ...
                current=index,
                total=total,
                message=f"moneyflow_ind_ths: {index}/{total} trade_date={trade_date.isoformat()} fetched={fetched} written={written}",
            )
    ...
    def _sync_trade_date(self, *, trade_date: date, ts_code: str | None, execution_id: int | None) -> tuple[int, int]:
        ...
        while True:
            ...
            self.dao.raw_moneyflow_ind_ths.bulk_upsert(normalized)
            written_total += self.dao.industry_moneyflow_ths.bulk_upsert(normalized)
