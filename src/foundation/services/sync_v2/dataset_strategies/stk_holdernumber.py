...
def build_stk_holdernumber_units(request, contract, dao, settings, session):
    if request.run_profile == "snapshot_refresh":
        ...
        if explicit_ann_date is not None:
            ...
            if request.start_date is None or request.end_date is None:
                raise RuntimeError("stk_holdernumber range_rebuild requires start_date and end_date")
            anchors = _expand_natural_dates(request.start_date, request.end_date)
    else:
        raise RuntimeError(f"stk_holdernumber unsupported run_profile: {request.run_profile}")
