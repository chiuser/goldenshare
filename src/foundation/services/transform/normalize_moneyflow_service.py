...
class NormalizeMoneyflowService:
    ...
    def to_std_from_tushare(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "buy_sm_vol": self._to_decimal(row.get("buy_sm_vol")),
            "buy_sm_amount": self._to_decimal(row.get("buy_sm_amount")),
            "sell_sm_vol": self._to_decimal(row.get("sell_sm_vol")),
            "sell_sm_amount": self._to_decimal(row.get("sell_sm_amount")),
            "buy_md_vol": self._to_decimal(row.get("buy_md_vol")),
            "buy_md_amount": self._to_decimal(row.get("buy_md_amount")),
            "sell_md_vol": self._to_decimal(row.get("sell_md_vol")),
            "sell_md_amount": self._to_decimal(row.get("sell_md_amount")),
            "buy_lg_vol": self._to_decimal(row.get("buy_lg_vol")),
            "buy_lg_amount": self._to_decimal(row.get("buy_lg_amount")),
            "sell_lg_vol": self._to_decimal(row.get("sell_lg_vol")),
            "sell_lg_amount": self._to_decimal(row.get("sell_lg_amount")),
            "buy_elg_vol": self._to_decimal(row.get("buy_elg_vol")),
            "buy_elg_amount": self._to_decimal(row.get("buy_elg_amount")),
            "sell_elg_vol": self._to_decimal(row.get("sell_elg_vol")),
    ...
    def to_std_from_biying_raw(self, row: dict[str, Any]) -> dict[str, Any]:
        ...
        buy_sm_vol = self._to_decimal(row.get("zmbxdcjl"))
        buy_sm_amount = self._to_decimal(row.get("zmbxdcje"))
        sell_sm_vol = self._to_decimal(row.get("zmsxdcjl"))
        sell_sm_amount = self._to_decimal(row.get("zmsxdcje"))
        buy_md_vol = self._to_decimal(row.get("zmbzdcjl"))
        buy_md_amount = self._to_decimal(row.get("zmbzdcje"))
        sell_md_vol = self._to_decimal(row.get("zmszdcjl"))
        sell_md_amount = self._to_decimal(row.get("zmszdcje"))
        buy_lg_vol = self._to_decimal(row.get("zmbddcjl"))
        buy_lg_amount = self._to_decimal(row.get("zmbddcje"))
        sell_lg_vol = self._to_decimal(row.get("zmsddcjl"))
        sell_lg_amount = self._to_decimal(row.get("zmsddcje"))
        buy_elg_vol = self._to_decimal(row.get("zmbtdcjl"))
        buy_elg_amount = self._to_decimal(row.get("zmbtdcje"))
        sell_elg_vol = self._to_decimal(row.get("zmstdcjl"))
        sell_elg_amount = self._to_decimal(row.get("zmstdcje"))

        net_mf_vol = self._subtract(
            self._sum(self._to_decimal(row.get(field)) for field in self._BUY_VOL_FIELDS),
            self._sum(self._to_decimal(row.get(field)) for field in self._SELL_VOL_FIELDS),
        )
