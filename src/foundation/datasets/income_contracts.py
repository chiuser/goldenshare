...
INCOME_SOURCE_FIELDS = tuple(
    ...
    oth_impair_loss_assets total_opcost amodcost_fin_assets oth_income asset_disp_income
    continued_net_profit end_net_profit update_flag
    """.split()
)
INCOME_DECIMAL_FIELDS = INCOME_SOURCE_FIELDS[7:-1]
INCOME_DATE_FIELDS = ("ann_date", "f_ann_date", "end_date")

if len(INCOME_SOURCE_FIELDS) != 94:
    raise RuntimeError(f"利润表源字段必须为 94 个，实际为 {len(INCOME_SOURCE_FIELDS)}")
if len(INCOME_DECIMAL_FIELDS) != 86:
    raise RuntimeError(f"利润表数值字段必须为 86 个，实际为 {len(INCOME_DECIMAL_FIELDS)}")
if INCOME_SOURCE_FIELDS[:7] != (
    "ts_code", "ann_date", "f_ann_date", "end_date", "report_type", "comp_type", "end_type"
):
    raise RuntimeError("利润表前置身份字段顺序发生漂移")
if INCOME_SOURCE_FIELDS[-1] != "update_flag":
    raise RuntimeError("利润表末尾字段必须为 update_flag")


__all__ = ["INCOME_DATE_FIELDS", "INCOME_DECIMAL_FIELDS", "INCOME_SOURCE_FIELDS"]
