...
BALANCESHEET_SOURCE_FIELDS = tuple(
    """
    ...
    use_right_assets lease_liab contract_assets contract_liab accounts_receiv_bill
    accounts_pay oth_rcv_total fix_assets_total update_flag
    """.split()
)
BALANCESHEET_DECIMAL_FIELDS = BALANCESHEET_SOURCE_FIELDS[7:-1]
BALANCESHEET_DATE_FIELDS = ("ann_date", "f_ann_date", "end_date")

if len(BALANCESHEET_SOURCE_FIELDS) != 158:
    raise RuntimeError(f"资产负债表源字段必须为 158 个，实际为 {len(BALANCESHEET_SOURCE_FIELDS)}")
if len(BALANCESHEET_DECIMAL_FIELDS) != 150:
    raise RuntimeError(f"资产负债表数值字段必须为 150 个，实际为 {len(BALANCESHEET_DECIMAL_FIELDS)}")
if BALANCESHEET_SOURCE_FIELDS[:7] != (
    "ts_code", "ann_date", "f_ann_date", "end_date", "report_type", "comp_type", "end_type"
):
    raise RuntimeError("资产负债表前置身份字段顺序发生漂移")
if BALANCESHEET_SOURCE_FIELDS[-1] != "update_flag":
    raise RuntimeError("资产负债表末尾字段必须为 update_flag")
