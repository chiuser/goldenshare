...
CASHFLOW_SOURCE_FIELDS = tuple(
    ...
    beg_bal_cash_equ update_flag
    """.split()
)
CASHFLOW_DECIMAL_FIELDS = CASHFLOW_SOURCE_FIELDS[7:-1]
CASHFLOW_DATE_FIELDS = ("ann_date", "f_ann_date", "end_date")

if len(CASHFLOW_SOURCE_FIELDS) != 97:
    raise RuntimeError(f"现金流量表源字段必须为 97 个，实际为 {len(CASHFLOW_SOURCE_FIELDS)}")
if len(CASHFLOW_DECIMAL_FIELDS) != 89:
    raise RuntimeError(f"现金流量表数值字段必须为 89 个，实际为 {len(CASHFLOW_DECIMAL_FIELDS)}")
if CASHFLOW_SOURCE_FIELDS[:7] != (
    "ts_code", "ann_date", "f_ann_date", "end_date", "comp_type", "report_type", "end_type"
):
    raise RuntimeError("现金流量表前置身份字段顺序发生漂移")
if CASHFLOW_SOURCE_FIELDS[-1] != "update_flag":
    raise RuntimeError("现金流量表末尾字段必须为 update_flag")


__all__ = ["CASHFLOW_DATE_FIELDS", "CASHFLOW_DECIMAL_FIELDS", "CASHFLOW_SOURCE_FIELDS"]
