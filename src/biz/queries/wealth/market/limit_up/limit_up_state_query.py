...
class LimitUpTradingDayContext:
    market: str
...
@dataclass(frozen=True, slots=True)
class LimitUpSourceState:
    observed_trade_date: date | None


class LimitUpStateQuery:
    """Resolve expected trading day and limit-up source observed date."""
    ...
    def resolve_trading_day(
        ...
        *,
        ...
        return LimitUpTradingDayContext(
            market=market,
    ...
    def load_source_state(self, session: Session) -> LimitUpSourceState:
        observed_trade_date = session.scalar(select(func.max(LimitListThs.trade_date)))
        return LimitUpSourceState(observed_trade_date=observed_trade_date)
