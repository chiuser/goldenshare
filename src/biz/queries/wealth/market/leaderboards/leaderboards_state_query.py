...
class LeaderboardsTradingDayContext:
    market: str
...
@dataclass(frozen=True, slots=True)
class LeaderboardsSourceState:
    daily_source_date: date | None
    hot_source_date: date | None


class LeaderboardsStateQuery:
    ...
    def resolve_trading_day(
        ...
        *,
        ...
        return LeaderboardsTradingDayContext(
            market=market,
    ...
    def load_source_state(self, session: Session) -> LeaderboardsSourceState:
        daily_source_date = session.scalar(select(func.max(EquityDailyBar.trade_date)))
        hot_source_date = session.scalar(select(func.max(DcHot.trade_date)))
        return LeaderboardsSourceState(
            daily_source_date=daily_source_date,
            hot_source_date=hot_source_date,
        )
