...
class MarketStyleTradingDayContext:
    market: str
...
@dataclass(frozen=True, slots=True)
class MarketStyleSourceState:
    index_source_date: date | None
    equity_source_date: date | None

    @property
    def observed_trade_date(self) -> date | None:
        dates = [self.index_source_date, self.equity_source_date]
...
class MarketStyleStateQuery:
    ...
    def resolve_trading_day(
        ...
        *,
        ...
        return MarketStyleTradingDayContext(
            market=market,
    ...
    ) -> MarketStyleSourceState:
        normalized_codes = tuple(code.strip().upper() for code in index_codes if code.strip())
        index_source_date: date | None = None
        if normalized_codes:
            index_source_date = session.scalar(
                select(func.max(IndexDailyServing.trade_date)).where(IndexDailyServing.ts_code.in_(normalized_codes))
            )
        equity_source_date = session.scalar(select(func.max(EquityDailyBar.trade_date)))
        return MarketStyleSourceState(index_source_date=index_source_date, equity_source_date=equity_source_date)
