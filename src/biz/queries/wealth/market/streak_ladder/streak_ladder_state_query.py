...
class StreakLadderTradingDayContext:
    market: str
...
@dataclass(frozen=True, slots=True)
class StreakLadderSourceState:
    observed_trade_date: date | None


class StreakLadderStateQuery:
    """Resolve trading day context and observed source date for streak ladder."""
    ...
    def load_source_state(self, session: Session) -> StreakLadderSourceState:
        observed_trade_date = session.scalar(
            select(func.max(EquityLimitList.trade_date)).where(EquityLimitList.limit_type == "U")
        )
        return StreakLadderSourceState(observed_trade_date=observed_trade_date)
