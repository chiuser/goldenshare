...
from src.biz.schemas.wealth.market.trading_assistant.scopes import AccountCreateScope, AccountFeesScope, AccountLedgerScope, RuleScope, RuleCreateScope, NotificationScope
...
def create_trading_assistant_router(*, auth_dependency, dependencies_dependency):
    ...
    async def pending(request: Request, scopeType: Literal["ACCOUNT_CREATE","ACCOUNT_FEES","ACCOUNT_LEDGER","RULE","RULE_CREATE","NOTIFICATION"], accountId: EntityId | None = None,
                      ruleType: Literal["PLAN", "ALERT"] | None = None, ruleId: EntityId | None = None, tsCode: StockCode | None = None,
                      notificationId: EntityId | None = None, owner_id: int = auth, deps: TradingAssistantDependencies = services):
        values = dict(request.query_params)
        if scopeType == "RULE_CREATE" and "accountId" not in values:
            values["accountId"] = None
        try:
            scope = TypeAdapter(AccountCreateScope | AccountFeesScope | AccountLedgerScope | RuleScope | RuleCreateScope | NotificationScope).validate_python(values)
        except ValidationError as error:
            raise WriteProtocolConflict("TA_REQUEST_INVALID") from error
