...
def get_news_reader_item(
    ...
    try:
        return NewsReaderQueryService().build_news_reader_item(session, news_id=normalized_news_id)
        ...
        logger.exception("news reader query failed", extra={"news_id": normalized_news_id})
        raise WebAppError(
            status_code=500,
