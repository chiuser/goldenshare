...
8. 长任务进度只有在对应业务持久化边界完成后才能递增；进度必须单调，心跳、当前阶段或内存中的临时计数不得冒充已完成量。
9. TaskRun 与当前活动 `task_run_node` 在成功、失败、取消时必须进入一致终态；观察状态写失败仍不得回滚已提交业务数据。
10. 历史回补、大范围 PLAN 和其他长任务必须明确其既有执行入口及串行／并发影响；不得仅因任务耗时较长就新增 worker lane、Worker、systemd unit 或队列，确需新增基础设施时必须另立范围并取得批准。
11. Ops 页面必须能看到阶段、当前对象或窗口、完成量、总量、百分比和最后更新时间；ETA 不可靠时明确显示无法估算，不得伪造倒计时。
