...
def test_worker_factory_reuses_one_existing_session_factory_and_registers_heat_executor() -> None:
    ...
    assert set(worker.dispatcher.maintenance_executors) == {
        "wealth_sector_heat",
        "wealth_sector_analysis_daily",
        "news_stock_linking",
    }
...
def test_worker_factories_share_dispatcher_assembly_and_select_lane(factory, lane) -> None:
    ...
    expected = {"wealth_sector_heat", "news_stock_linking"}
    if lane is WorkerLane.GENERAL:
        expected.add("wealth_sector_analysis_daily")
    assert set(worker.dispatcher.maintenance_executors) == expected
