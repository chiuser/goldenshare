...
def test_index_daily_strategy_builds_trade_date_unit_without_ts_code_by_default() -> None:
    contract = get_sync_v2_contract("index_daily")
    request = RunRequest(
        request_id="req-index-daily-strategy-point",
    ...
    units = build_index_daily_units(validated, contract, dao=dao, settings=settings, session=None)

    assert len(units) == 1
    assert units[0].request_params["trade_date"] == "20260417"
    assert units[0].request_params.get("ts_code") is None
    assert units[0].page_limit == 8000


def test_index_daily_strategy_range_rebuild_fans_out_trade_date_units() -> None:
    contract = get_sync_v2_contract("index_daily")
    request = RunRequest(
        request_id="req-index-daily-strategy-range",
        dataset_key="index_daily",
    ...
    assert len(units) == 3
    assert {unit.request_params["trade_date"] for unit in units} == {"20260401", "20260402", "20260403"}
