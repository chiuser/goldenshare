...
def test_sync_cyq_perf_incremental_requires_trade_date(mocker) -> None:
    session = mocker.Mock()
    mocker.patch("src.foundation.services.sync.sync_cyq_perf_service.TushareHttpClient", return_value=mocker.Mock())
    service = SyncCyqPerfService(session)

    with pytest.raises(ValueError, match="trade_date is required"):
        service.execute("INCREMENTAL")


def test_sync_cyq_perf_incremental_paginates_and_writes(mocker) -> None:
    session = mocker.Mock()
    client = mocker.Mock()
    ...
    mocker.patch("src.foundation.services.sync.sync_cyq_perf_service.TushareHttpClient", return_value=client)
    service = SyncCyqPerfService(session)
    raw_upsert = mocker.patch.object(service.dao.raw_cyq_perf, "bulk_upsert", return_value=1)
    core_upsert = mocker.patch.object(service.dao.equity_cyq_perf, "bulk_upsert", return_value=1)
    ...
    client.call.assert_called_once_with(
        "cyq_perf",
        params={"trade_date": "20260416", "limit": 5000, "offset": 0},
        fields=service.fields,
    )
    raw_row = raw_upsert.call_args.args[0][0]
    assert raw_row["trade_date"] == date(2026, 4, 16)
    assert raw_row["cost_50pct"] == Decimal("12.33")
    assert raw_row["winner_rate"] == Decimal("0.5821")
    core_upsert.assert_called_once()
    assert progress.call_count == 1


def test_sync_cyq_perf_history_requires_explicit_time_params(mocker) -> None:
    session = mocker.Mock()
    mocker.patch("src.foundation.services.sync.sync_cyq_perf_service.TushareHttpClient", return_value=mocker.Mock())
    service = SyncCyqPerfService(session)
...
def test_sync_cyq_perf_history_fans_out_by_trade_calendar(mocker) -> None:
    session = mocker.Mock()
    mocker.patch("src.foundation.services.sync.sync_cyq_perf_service.TushareHttpClient", return_value=mocker.Mock())
    service = SyncCyqPerfService(session)
    mocker.patch.object(
...
def test_sync_cyq_perf_history_clips_to_available_start_date(mocker) -> None:
    session = mocker.Mock()
    mocker.patch("src.foundation.services.sync.sync_cyq_perf_service.TushareHttpClient", return_value=mocker.Mock())
    service = SyncCyqPerfService(session)
    ...
    assert fetched == 5
    assert written == 5
    assert result_date == date(2018, 1, 10)
    assert message == "trade_dates=1"
    get_open_dates.assert_called_once_with("SSE", date(2018, 1, 1), date(2018, 1, 10))
