...
MIGRATION_PATH = (
    REPO_ROOT / "alembic/versions/20260829_000158_make_stk_auction_c_raw_view.py"
)
SOURCE_FIELDS = (
    "ts_code",
    "trade_date",
...
def _load_migration():  # type: ignore[no-untyped-def]
    spec = importlib.util.spec_from_file_location(
        "migration_20260829_000158", MIGRATION_PATH
    )
...
def test_stk_auction_c_definition_changes_only_storage_delivery_contract(
    mocker,
) -> None:
    definition = get_dataset_definition("stk_auction_c")

    assert definition.source.api_name == "stk_auction_c"
    assert definition.source.source_fields == SOURCE_FIELDS
    assert definition.source.request_builder_key == "_stk_auction_c_params"
    ...
    assert definition.storage.raw_dao_name == "raw_stk_auction_c"
    assert definition.storage.core_dao_name == "raw_stk_auction_c"
    assert definition.storage.target_table == "raw_tushare.stk_auction_c"
    assert definition.storage.raw_table == "raw_tushare.stk_auction_c"
    assert definition.storage.serving_table == "core_serving.equity_auction_close"
    ...
    assert plan.writing.raw_dao_name == "raw_stk_auction_c"
    assert plan.writing.core_dao_name == "raw_stk_auction_c"
    assert plan.writing.target_table == "raw_tushare.stk_auction_c"
...
def test_stk_auction_c_preserves_ts_code_filter_and_rejects_unknown_filter(
    mocker,
) -> None:
    plan = DatasetActionResolver(mocker.Mock()).build_plan(
        DatasetActionRequest(
...
def test_stk_auction_c_models_have_identical_business_contract_and_one_date_index() -> (
    None
):
    raw_table = RawStkAuctionC.__table__
    serving_table = EquityAuctionClose.__table__
    ...
    assert {index.name: tuple(index.columns.keys()) for index in raw_table.indexes} == {
        "idx_raw_tushare_stk_auction_c_trade_date": ("trade_date",),
    }
    assert {
        index.name: tuple(index.columns.keys()) for index in serving_table.indexes
    } == {
        "idx_equity_auction_close_trade_date": ("trade_date",),
    }


def test_stk_auction_c_is_not_registered_for_serving_publish_bypass() -> None:
    assert get_target_dao_attr("stk_auction_c") is None


def test_stk_auction_c_raw_target_is_registered_for_freshness_and_completeness() -> None:
    assert get_model_by_table_name("raw_tushare.stk_auction_c") is RawStkAuctionC


def test_stk_auction_c_migration_is_independent_atomic_and_fail_closed() -> None:
    source = MIGRATION_PATH.read_text(encoding="utf-8")
    uppercase = source.upper()

    assert 'revision = "20260829_000158"' in source
    assert 'down_revision = "20260829_000157"' in source
    ...
    assert "raw_tushare.stk_auction_c must remain on SSD pg_default" in source
    assert "idx_raw_tushare_stk_auction_c_trade_date" in source
    assert "idx_equity_auction_close_trade_date" in source
    ...
    assert (
        "Unexpected non-primary-key constraint on raw_tushare.stk_auction_c" in source
    )
    ...
    assert "LOCK TABLE raw_tushare.stk_auction_c IN SHARE MODE" in source
    assert "LOCK TABLE core_serving.equity_auction_close IN SHARE MODE" in source
    assert (
        "LOCK TABLE core_serving.equity_auction_close IN ACCESS EXCLUSIVE MODE" in source
    )
    assert uppercase.count("EXCEPT ALL") == 2
    assert "DROP TABLE core_serving.equity_auction_close" in source
    assert "DROP TABLE core_serving.equity_auction_close CASCADE" not in source
    assert "DROP TABLE raw_tushare.stk_auction_c" not in source
    assert "CREATE VIEW core_serving.equity_auction_close AS" in source
    ...
    assert "ALTER TABLE raw_tushare.stk_auction_c" not in source
    assert "INSERT INTO raw_tushare.stk_auction_c" not in source
    assert "UPDATE raw_tushare.stk_auction_c" not in source
    assert "DELETE FROM raw_tushare.stk_auction_c" not in source

    raw_lock_at = source.index("LOCK TABLE raw_tushare.stk_auction_c IN SHARE MODE")
    ...
    drop_at = source.index("DROP TABLE core_serving.equity_auction_close")
    create_view_at = source.index("CREATE VIEW core_serving.equity_auction_close AS")
    assert (
        raw_lock_at
        < serving_lock_at
        < equality_at
...
def test_stk_auction_c_migration_renders_complete_postgresql_sql() -> None:
    ...
    sql = output.getvalue()
    assert "CREATE VIEW core_serving.equity_auction_close AS" in sql
    assert "CREATE TRIGGER trg_equity_auction_close_reject_dml" in sql
    ...
    assert "DROP TABLE core_serving.equity_auction_close CASCADE" not in sql
    assert "DROP TABLE raw_tushare.stk_auction_c" not in sql


def test_stk_auction_c_migration_forbids_automatic_downgrade() -> None:
