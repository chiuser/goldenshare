...
def test_derive_range_uses_local_trading_calendar(tmp_path, monkeypatch):
    for trade_date in ("2026-04-24", "2026-04-27"):
        source = tmp_path / "raw_tushare" / "stk_mins_by_date" / "freq=30" / f"trade_date={trade_date}"
        source.mkdir(parents=True)
        (source / "part-000.parquet").write_text("fake", encoding="utf-8")
    ...
    monkeypatch.setattr(
        ...
        lambda *, lake_root, start_date, end_date: [date(2026, 4, 24), date(2026, 4, 27)],
    )
    monkeypatch.setattr(
        derived_module,
        "read_parquet_files",
    ...
    summary = StkMinsDerivedService(lake_root=tmp_path, progress=lambda message: None).derive_range(
        start_date=date(2026, 4, 24),
        end_date=date(2026, 4, 27),
        targets=[90],
    )

    assert summary["operation"] == "derive_stk_mins_range"
    assert summary["trade_dates"] == ["2026-04-24", "2026-04-27"]
    assert summary["trade_date_count"] == 2
    assert summary["written_rows"] == 2
    assert (tmp_path / "derived" / "stk_mins_by_date" / "freq=90" / "trade_date=2026-04-24" / "part-000.parquet").exists()
    assert (tmp_path / "derived" / "stk_mins_by_date" / "freq=90" / "trade_date=2026-04-27" / "part-000.parquet").exists()


def test_derive_range_preflights_sources_before_writing(tmp_path, monkeypatch):
    source = tmp_path / "raw_tushare" / "stk_mins_by_date" / "freq=30" / "trade_date=2026-04-24"
    source.mkdir(parents=True)
    (source / "part-000.parquet").write_text("fake", encoding="utf-8")
    ...
    try:
        StkMinsDerivedService(lake_root=tmp_path, progress=lambda message: None).derive_range(
            start_date=date(2026, 4, 24),
            end_date=date(2026, 4, 27),
            targets=[90],
        )
        ...
        assert "trade_date=2026-04-27" in str(exc)
    else:
        raise AssertionError("derive_range should fail when any source partition is missing")
