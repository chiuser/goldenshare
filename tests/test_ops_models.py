...
def test_ops_control_plane_models_expose_primary_keys_and_indexes() -> None:
    assert [column.name for column in OpsSchedule.__table__.primary_key.columns] == ["id"]
    assert {index.name for index in OpsSchedule.__table__.indexes} == {
        "idx_ops_schedule_target_type_target_key",
        "idx_ops_schedule_status_next_run_at",
    }
