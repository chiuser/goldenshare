...
MIGRATION_PATH = (
    REPO_ROOT / "alembic/versions/20260829_000159_make_moneyflow_ths_raw_view.py"
)
SOURCE_FIELDS = (
    "trade_date",
    "ts_code",
...
def _load_migration():  # type: ignore[no-untyped-def]
    spec = importlib.util.spec_from_file_location(
        "migration_20260829_000159", MIGRATION_PATH
    )
...
def test_moneyflow_ths_definition_changes_only_storage_delivery_contract(
    mocker,
) -> None:
    definition = get_dataset_definition("moneyflow_ths")

    assert definition.source.api_name == "moneyflow_ths"
    assert definition.source.source_fields == SOURCE_FIELDS
    assert definition.source.request_builder_key == "_moneyflow_ths_params"
    ...
    assert definition.planning.page_limit == 6000
    ...
    assert definition.storage.raw_dao_name == "raw_moneyflow_ths"
    assert definition.storage.core_dao_name == "raw_moneyflow_ths"
    assert definition.storage.target_table == "raw_tushare.moneyflow_ths"
    assert definition.storage.raw_table == "raw_tushare.moneyflow_ths"
    assert definition.storage.serving_table == "core_serving.equity_moneyflow_ths"
    ...
    assert plan.writing.raw_dao_name == "raw_moneyflow_ths"
    assert plan.writing.core_dao_name == "raw_moneyflow_ths"
    assert plan.writing.target_table == "raw_tushare.moneyflow_ths"
...
def test_moneyflow_ths_preserves_existing_ts_code_filter_and_rejects_new_filters(
    mocker,
) -> None:
    plan = DatasetActionResolver(mocker.Mock()).build_plan(
        DatasetActionRequest(
...
def test_moneyflow_ths_raw_model_covers_serving_business_contract_and_index() -> None:
    raw_table = RawMoneyflowThs.__table__
    serving_table = EquityMoneyflowThs.__table__
...
def test_moneyflow_ths_is_not_registered_for_serving_publish_bypass() -> None:
    assert get_target_dao_attr("moneyflow_ths") is None


def test_moneyflow_ths_raw_target_is_registered_for_freshness_and_completeness() -> (
    None
):
    assert get_model_by_table_name("raw_tushare.moneyflow_ths") is RawMoneyflowThs


def test_moneyflow_ths_migration_is_independent_atomic_and_fail_closed() -> None:
    source = MIGRATION_PATH.read_text(encoding="utf-8")
    uppercase = source.upper()

    assert 'revision = "20260829_000159"' in source
    assert 'down_revision = "20260829_000158"' in source
    ...
    assert "max_rows_per_month constant bigint := 150000" in source
    ...
    assert "raw_tushare.moneyflow_ths must remain on SSD pg_default" in source
    assert "idx_raw_tushare_moneyflow_ths_trade_date" in source
    assert "idx_raw_tushare_moneyflow_ths_ts_code_trade_date" in source
    assert "idx_equity_moneyflow_ths_trade_date" in source
    assert "idx_equity_moneyflow_ths_ts_code_trade_date" in source
    assert "Unexpected column-level ACL" in source
    assert (
        "Unexpected non-primary-key constraint on raw_tushare.moneyflow_ths" in source
    )
    ...
    assert "LOCK TABLE raw_tushare.moneyflow_ths IN SHARE MODE" in source
    assert "LOCK TABLE core_serving.equity_moneyflow_ths IN SHARE MODE" in source
    assert (
        "LOCK TABLE core_serving.equity_moneyflow_ths IN ACCESS EXCLUSIVE MODE"
        in source
    )
    assert uppercase.count("EXCEPT ALL") == 2
    assert "DROP TABLE core_serving.equity_moneyflow_ths" in source
    assert "DROP TABLE core_serving.equity_moneyflow_ths CASCADE" not in source
    assert "DROP TABLE raw_tushare.moneyflow_ths" not in source
    assert "CREATE VIEW core_serving.equity_moneyflow_ths AS" in source
    ...
    assert "board moneyflow dc" not in source
    ...
    assert "ALTER TABLE raw_tushare.moneyflow_ths" not in source
    assert "INSERT INTO raw_tushare.moneyflow_ths" not in source
    assert "UPDATE raw_tushare.moneyflow_ths" not in source
    assert "DELETE FROM raw_tushare.moneyflow_ths" not in source

    raw_lock_at = source.index("LOCK TABLE raw_tushare.moneyflow_ths IN SHARE MODE")
    ...
    drop_at = source.index("DROP TABLE core_serving.equity_moneyflow_ths")
    create_view_at = source.index("CREATE VIEW core_serving.equity_moneyflow_ths AS")
    assert (
        raw_lock_at
        < serving_lock_at
        < equality_at
...
def test_moneyflow_ths_migration_renders_complete_postgresql_sql() -> None:
    ...
    sql = output.getvalue()
    assert "CREATE VIEW core_serving.equity_moneyflow_ths AS" in sql
    assert "CREATE TRIGGER trg_equity_moneyflow_ths_reject_dml" in sql
    assert "name|character varying(64)|NULL" in sql
    assert "latest|numeric(18,4)|NULL" in sql
    assert "net_d5_amount|numeric(24,4)|NULL" in sql
    assert "buy_sm_amount_rate|numeric(10,4)|NULL" in sql
    ...
    assert "DROP TABLE core_serving.equity_moneyflow_ths CASCADE" not in sql
    assert "DROP TABLE raw_tushare.moneyflow_ths" not in sql


def test_moneyflow_ths_migration_forbids_automatic_downgrade() -> None:
