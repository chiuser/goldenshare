...
MIGRATION_PATH = (
    REPO_ROOT / "alembic/versions/20260829_000161_make_moneyflow_dc_raw_view.py"
)
SOURCE_FIELDS = (
    "trade_date",
    "ts_code",
...
def _load_migration():  # type: ignore[no-untyped-def]
    spec = importlib.util.spec_from_file_location(
        "migration_20260829_000161", MIGRATION_PATH
    )
...
def test_moneyflow_dc_definition_changes_only_storage_delivery_contract(
    mocker,
) -> None:
    definition = get_dataset_definition("moneyflow_dc")

    assert definition.source.api_name == "moneyflow_dc"
    assert definition.source.source_fields == SOURCE_FIELDS
    assert definition.source.request_builder_key == "_moneyflow_dc_params"
    ...
    assert definition.storage.raw_dao_name == "raw_moneyflow_dc"
    assert definition.storage.core_dao_name == "raw_moneyflow_dc"
    assert definition.storage.target_table == "raw_tushare.moneyflow_dc"
    assert definition.storage.raw_table == "raw_tushare.moneyflow_dc"
    assert definition.storage.serving_table == "core_serving.equity_moneyflow_dc"
    ...
    assert plan.writing.raw_dao_name == "raw_moneyflow_dc"
    assert plan.writing.core_dao_name == "raw_moneyflow_dc"
    assert plan.writing.target_table == "raw_tushare.moneyflow_dc"
...
def test_moneyflow_dc_preserves_existing_ts_code_filter_and_rejects_new_filters(
    mocker,
) -> None:
    plan = DatasetActionResolver(mocker.Mock()).build_plan(
        DatasetActionRequest(
...
def test_moneyflow_dc_raw_model_covers_serving_business_contract_and_index() -> None:
    raw_table = RawMoneyflowDc.__table__
    serving_table = EquityMoneyflowDc.__table__
...
def test_moneyflow_dc_is_not_registered_for_serving_publish_bypass() -> None:
    assert get_target_dao_attr("moneyflow_dc") is None


def test_moneyflow_dc_raw_target_is_registered_for_freshness_and_completeness() -> (
    None
):
    assert get_model_by_table_name("raw_tushare.moneyflow_dc") is RawMoneyflowDc


def test_moneyflow_dc_migration_is_independent_atomic_and_fail_closed() -> None:
    source = MIGRATION_PATH.read_text(encoding="utf-8")
    uppercase = source.upper()

    assert 'revision = "20260829_000161"' in source
    assert 'down_revision = "20260829_000160"' in source
    ...
    assert "max_rows_per_month constant bigint := 170000" in source
    ...
    assert "raw_tushare.moneyflow_dc must remain on SSD pg_default" in source
    assert "idx_raw_tushare_moneyflow_dc_trade_date" in source
    assert "idx_raw_tushare_moneyflow_dc_ts_code_trade_date" in source
    assert "idx_equity_moneyflow_dc_trade_date" in source
    assert "idx_equity_moneyflow_dc_ts_code_trade_date" in source
    assert "Unexpected column-level ACL" in source
    assert (
        "Unexpected non-primary-key constraint on raw_tushare.moneyflow_dc" in source
    )
    ...
    assert "LOCK TABLE raw_tushare.moneyflow_dc IN SHARE MODE" in source
    assert "LOCK TABLE core_serving.equity_moneyflow_dc IN SHARE MODE" in source
    assert (
        "LOCK TABLE core_serving.equity_moneyflow_dc IN ACCESS EXCLUSIVE MODE"
        in source
    )
    assert uppercase.count("EXCEPT ALL") == 2
    assert "DROP TABLE core_serving.equity_moneyflow_dc" in source
    assert "DROP TABLE core_serving.equity_moneyflow_dc CASCADE" not in source
    assert "DROP TABLE raw_tushare.moneyflow_dc" not in source
    assert "CREATE VIEW core_serving.equity_moneyflow_dc AS" in source
    ...
    assert "ALTER TABLE raw_tushare.moneyflow_dc" not in source
    assert "INSERT INTO raw_tushare.moneyflow_dc" not in source
    assert "UPDATE raw_tushare.moneyflow_dc" not in source
    assert "DELETE FROM raw_tushare.moneyflow_dc" not in source

    raw_lock_at = source.index("LOCK TABLE raw_tushare.moneyflow_dc IN SHARE MODE")
    ...
    drop_at = source.index("DROP TABLE core_serving.equity_moneyflow_dc")
    create_view_at = source.index("CREATE VIEW core_serving.equity_moneyflow_dc AS")
    assert (
        raw_lock_at
        < serving_lock_at
        < equality_at
...
def test_moneyflow_dc_migration_renders_complete_postgresql_sql() -> None:
    ...
    sql = output.getvalue()
    assert "CREATE VIEW core_serving.equity_moneyflow_dc AS" in sql
    assert "CREATE TRIGGER trg_equity_moneyflow_dc_reject_dml" in sql
    assert "name|character varying(64)|NULL" in sql
    assert "close|numeric(18,4)|NULL" in sql
    assert "net_amount_rate|numeric(10,4)|NULL" in sql
    assert "buy_elg_amount|numeric(24,4)|NULL" in sql
    assert "buy_elg_amount_rate|numeric(10,4)|NULL" in sql
    ...
    assert "DROP TABLE core_serving.equity_moneyflow_dc CASCADE" not in sql
    assert "DROP TABLE raw_tushare.moneyflow_dc" not in sql


def test_moneyflow_dc_migration_forbids_automatic_downgrade() -> None:
