...
MIGRATION_PATH = (
    REPO_ROOT / "alembic/versions/20260824_000148_make_moneyflow_cnt_ths_raw_view.py"
)
SOURCE_FIELDS = (
    "trade_date",
    "ts_code",
...
def _load_migration():  # type: ignore[no-untyped-def]
    spec = importlib.util.spec_from_file_location(
        "migration_20260824_000148", MIGRATION_PATH
    )
...
def test_moneyflow_cnt_ths_definition_changes_only_storage_delivery_contract(
    mocker,
) -> None:
    definition = get_dataset_definition("moneyflow_cnt_ths")

    assert definition.source.api_name == "moneyflow_cnt_ths"
    assert definition.source.source_fields == SOURCE_FIELDS
    assert definition.source.request_builder_key == "_moneyflow_cnt_ths_params"
    ...
    assert definition.storage.raw_dao_name == "raw_moneyflow_cnt_ths"
    assert definition.storage.core_dao_name == "raw_moneyflow_cnt_ths"
    assert definition.storage.target_table == "raw_tushare.moneyflow_cnt_ths"
    assert definition.storage.raw_table == "raw_tushare.moneyflow_cnt_ths"
    assert definition.storage.serving_table == "core_serving.concept_moneyflow_ths"
    ...
    assert plan.writing.raw_dao_name == "raw_moneyflow_cnt_ths"
    assert plan.writing.core_dao_name == "raw_moneyflow_cnt_ths"
    assert plan.writing.target_table == "raw_tushare.moneyflow_cnt_ths"
...
def test_moneyflow_cnt_ths_preserves_existing_ts_code_filter_and_rejects_new_filters(mocker) -> None:
    plan = DatasetActionResolver(mocker.Mock()).build_plan(
        DatasetActionRequest(
            dataset_key="moneyflow_cnt_ths",
            action="maintain",
...
def test_moneyflow_cnt_ths_raw_model_covers_serving_business_contract_and_index() -> (
    None
):
    raw_table = RawMoneyflowCntThs.__table__
    serving_table = ConceptMoneyflowThs.__table__
...
def test_moneyflow_cnt_ths_is_not_registered_for_serving_publish_bypass() -> None:
    assert get_target_dao_attr("moneyflow_cnt_ths") is None


def test_moneyflow_cnt_ths_migration_is_independent_atomic_and_fail_closed() -> None:
    source = MIGRATION_PATH.read_text(encoding="utf-8")
    uppercase = source.upper()

    assert 'revision = "20260824_000148"' in source
    assert 'down_revision = "20260824_000147"' in source
    ...
    assert "raw_tushare.moneyflow_cnt_ths must remain on SSD pg_default" in source
    assert "idx_raw_tushare_moneyflow_cnt_ths_trade_date" in source
    assert "idx_raw_tushare_moneyflow_cnt_ths_ts_code_trade_date" in source
    assert "idx_concept_moneyflow_ths_trade_date" in source
    assert "idx_concept_moneyflow_ths_ts_code_trade_date" in source
    ...
    assert "LOCK TABLE raw_tushare.moneyflow_cnt_ths IN SHARE MODE" in source
    assert "LOCK TABLE core_serving.concept_moneyflow_ths IN SHARE MODE" in source
    assert (
        "LOCK TABLE core_serving.concept_moneyflow_ths IN ACCESS EXCLUSIVE MODE" in source
    )
    assert uppercase.count("EXCEPT ALL") == 2
    assert "DROP TABLE core_serving.concept_moneyflow_ths" in source
    assert "DROP TABLE core_serving.concept_moneyflow_ths CASCADE" not in source
    assert "DROP TABLE raw_tushare.moneyflow_cnt_ths" not in source
    assert "CREATE VIEW core_serving.concept_moneyflow_ths AS" in source
    ...
    raw_lock_at = source.index("LOCK TABLE raw_tushare.moneyflow_cnt_ths IN SHARE MODE")
    ...
    drop_at = source.index("DROP TABLE core_serving.concept_moneyflow_ths")
    create_view_at = source.index("CREATE VIEW core_serving.concept_moneyflow_ths AS")
    assert (
        raw_lock_at
        < serving_lock_at
        < equality_at
...
def test_moneyflow_cnt_ths_migration_renders_complete_postgresql_sql() -> None:
    ...
    sql = output.getvalue()
    assert "CREATE VIEW core_serving.concept_moneyflow_ths AS" in sql
    assert "CREATE TRIGGER trg_concept_moneyflow_ths_reject_dml" in sql
    assert "name|character varying(128)|NULL" in sql
    assert "close_price|numeric(18,4)|NULL" in sql
    assert "industry_index|numeric(24,4)|NULL" in sql
    ...
    assert "DROP TABLE core_serving.concept_moneyflow_ths CASCADE" not in sql
    assert "DROP TABLE raw_tushare.moneyflow_cnt_ths" not in sql


def test_moneyflow_cnt_ths_migration_forbids_automatic_downgrade() -> None:
