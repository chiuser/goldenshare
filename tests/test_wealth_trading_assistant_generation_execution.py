...
def test_committed_unit_with_lost_reply_is_not_repeated(interruptions_db, published):
    ...
    if published:
        with Session(interruptions_db) as session, session.begin():
            ...
            for expected in ("VALUATION_END", "DATE_INPUT"):
                assert steps.prepare_next_inputs(session, lease, generation_id=generation,
                    business_date=DAY, fee_version_id=fee, valuation_at=AT, deadline=deadline()) == expected
        for _ in range(40):
            with Session(interruptions_db) as session, session.begin():
    ...
    with Session(interruptions_db) as session, session.begin():
        ...
        if published:
            ...
            for expected in ("VALUATION_END", "DATE_INPUT"):
                assert GenerationSteps(inputs.execution).prepare_next_inputs(session, resumed, generation_id=generation,
                    business_date=DAY, fee_version_id=fee, valuation_at=AT, deadline=deadline()) == expected
    if not published:
        assert GenerationExecution(inputs.execution, sessionmaker(interruptions_db)).run(
