...
from src.foundation.ingestion.codebook import INGESTION_ERROR_CODEBOOK, INGESTION_REASON_CODEBOOK
...
def test_ingestion_error_codes_in_runtime_must_exist_in_codebook() -> None:
    ...
    codebook_error_codes = {entry.code for entry in INGESTION_ERROR_CODEBOOK}
    missing = sorted(produced_error_codes - codebook_error_codes)
    assert not missing, (
        "检测到维护运行链路中出现未登记的 error_code。"
        "请先补齐 src/foundation/ingestion/codebook.py:\n" + "\n".join(missing)
    )
...
def test_ingestion_reason_codes_in_runtime_must_exist_in_codebook() -> None:
    ...
    assert not missing, (
        "检测到维护运行链路中出现未登记的 reason_code。"
        "请先补齐 src/foundation/ingestion/codebook.py:\n" + "\n".join(missing)
    )


def test_ingestion_codebook_codes_are_unique() -> None:
    error_codes = [entry.code for entry in INGESTION_ERROR_CODEBOOK]
    reason_codes = [entry.code for entry in INGESTION_REASON_CODEBOOK]
    assert len(error_codes) == len(set(error_codes)), "INGESTION_ERROR_CODEBOOK 存在重复 code"
    assert len(reason_codes) == len(set(reason_codes)), "INGESTION_REASON_CODEBOOK 存在重复 code"
