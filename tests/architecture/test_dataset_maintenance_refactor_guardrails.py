...
def test_supporting_validation_messages_do_not_emit_internal_field_tokens() -> None:
    paths = (
        ...
        REPO_ROOT / "src/foundation/ingestion/sentinel_guard.py",
        REPO_ROOT / "src/foundation/ingestion/writer.py",
        REPO_ROOT / "src/foundation/connectors/biying_connector.py",
        REPO_ROOT / "src/foundation/connectors/factory.py",
        REPO_ROOT / "src/foundation/dao/base_dao.py",
        REPO_ROOT / "src/foundation/dao/dividend_dao.py",
        REPO_ROOT / "src/foundation/dao/holdernumber_dao.py",
        REPO_ROOT / "src/foundation/dao/raw_multi_writer.py",
        REPO_ROOT / "src/foundation/datasets/definitions/_builder.py",
        REPO_ROOT / "src/foundation/resolution/policy_engine.py",
        REPO_ROOT / "src/foundation/services/transform/normalize_moneyflow_service.py",
        REPO_ROOT / "src/foundation/services/transform/normalize_security_service.py",
    ...
    forbidden_snippets = (
        ...
        "Biying source does not support",
        "BIYING_TOKEN is empty",
        "dm is required",
        "Only daily freq",
        "adj_type must be",
        ...
        "Unsupported raw multi route",
        "source_key_default must",
        "must declare",
        "must not be empty",
        ...
        "missing ts_code",
        "missing trade_date",
        "missing conflict key fields",
        "unsupported dataset action",
        "does not support action",
        "does not support point",
        "does not support range",
        "is not allowed for",
        "does not accept time anchors",
        "does not allow",
        "requires month",
        "requires point date",
        "unsupported date model",
        "forbidden business sentinel",
        "invalid date for",
        "invalid integer for",
        "invalid boolean for",
        "invalid enum for",
        "empty value is not allowed",
        "no active index codes found",
        "no ths_index board codes found",
        "unsupported universe_policy",
        "stock_basic unsupported",
        "biying_equity_daily invalid",
        "unsupported write_path",
        "unsupported dataset for index period serving",
        "stock_basic writer received",
        "User does not exist",
        "User is inactive",
