...
def test_quote_page_init_and_kline_for_stock(app_client, db_session) -> None:
    ...
    db_session.add_all(
        [
            EquityAdjFactor(ts_code="002245.SZ", trade_date=date(2026, 4, 1), adj_factor=Decimal("1.000000")),
            EquityAdjFactor(ts_code="002245.SZ", trade_date=date(2026, 4, 2), adj_factor=Decimal("2.000000")),
        ]
    )
    db_session.add(
        ThsMember(
...
def test_quote_kline_limit_bounds_are_respected(app_client, db_session) -> None:
    ...
    db_session.add_all(
        [
            EquityAdjFactor(ts_code="600000.SH", trade_date=date(2026, 4, 1), adj_factor=Decimal("1.000000")),
            EquityAdjFactor(ts_code="600000.SH", trade_date=date(2026, 4, 2), adj_factor=Decimal("1.000000")),
        ]
    )
    db_session.commit()
