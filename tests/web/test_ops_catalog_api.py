...
def test_ops_catalog_returns_dataset_actions_for_admin(app_client, user_factory) -> None:
    ...
    etf_sz_cons = actions["etf_sz_cons.maintain"]
    assert etf_sz_cons["target_display_name"] == "ETF 每日持仓组合（深市）"
    assert etf_sz_cons["group_key"] == "etf_fund"
    assert etf_sz_cons["group_label"] == "ETF基金"
    assert etf_sz_cons["freshness_policy"] == "continuous_open_day"
    ...
    assert sum(item["schedule_enabled"] for item in catalog_items) == 92
    assert all(
