...
@pytest.mark.parametrize("automatic_fees", [False, True])
def test_dispatch_resumes_entire_window_without_manual_stage_selection(interruptions_db, held, automatic_fees, monkeypatch):
    ...
    def unexpected_failure(*args, **kwargs):
        raise AssertionError("Unexpected interruption in successful dispatch scenario")
    monkeypatch.setattr(CalculationInterruptions, "record", unexpected_failure)
    ...
    positions = [dict(clientRowId=code, tsCode=code, openedOn=DAY.isoformat(),
        quantity=100, availableQuantity=100, costPrice="10.00") for code in ("601001.SH", "601002.SH")] if held else []
    protocol, _, saved = acceptance.create(interruptions_db, acceptance.create_command(positions))
    fee = UUID(saved.receipt["result"]["fees"]["feeVersionId"])
    inputs = CalculationInputs(RecalculationExecution(protocol.policy))
    with Session(interruptions_db) as session, session.begin():
        lease = inputs.execution.claim(session, executor_id="dispatch", deadline=deadline())
        assert str(lease.account_id) == saved.receipt["result"]["account"]["accountId"]
        generation = inputs.prepare_generation(session, lease, from_date=DAY, through_date=DAY,
        ...
        if held:
            for code in ("601001.SH", "601002.SH"):
                for day, price in ((DAY, "11.00"), (monday, "12.00")):
                    if session.get(EquityDailyBar, (code, day)) is None:
    ...
    def resolve(session, *, account_id, business_date, deadline):
        ...
        cutoff = AT + (business_date-DAY)
        return cutoff if automatic_fees else DayInputBasis(fee, cutoff)
    ...
    for _ in range(240):
        ...
        stage = (runner.step_with_valuation_cutoff(lease, generation_id=generation, resolve_valuation_at=resolve)
            if automatic_fees else runner.step(lease, generation_id=generation, resolve_day_inputs=resolve))
        stages.append(stage)
