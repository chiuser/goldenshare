...
### 5. 数据语义必须通过 ViewModel 收敛

`features` 层是 DTO -> ViewModel 的主要承接层之一。
...
1. feature 把原始 DTO 直接透传到页面组件并让页面自行解释。

### 6. 禁止兼容/临时分支长期化

当 feature 发现契约或模型不合理：

1. 先提出结构化改造方案并评审。
2. 再进入开发。

不允许以“先兼容一下”的方式把临时逻辑长期留在 feature 层。
...
1. 中等及以上 feature 需求必须先有技术方案并评审通过。
2. bug 修复必须写明根因与影响面，优先结构化修复而非局部补丁。
3. 涉及跨 feature 共享逻辑时，必须先定义边界再编码。

---

## 代码规模与抽象门禁（新增）

1. feature 单文件超过 `400` 行必须评估拆分。
2. feature 单文件超过 `600` 行必须拆分。
3. 相同流程逻辑出现 3 次以上，必须抽象为共享 hook/utility。
