...
describe("V2.1 数据集详情页", () => {
  ...
  it("详情页不再请求旧层级快照接口", async () => {
    ...
    expect(await screen.findByText("当前状态")).toBeInTheDocument();
    const requestedUrls = apiRequest.mock.calls.map(([url]) => String(url));
    const retiredPathSegment = ["layer", "snapshots"].join("-");
    expect(requestedUrls.every((url) => !url.includes(retiredPathSegment))).toBe(true);
  });
});
