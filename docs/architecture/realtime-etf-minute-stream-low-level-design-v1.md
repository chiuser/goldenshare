...
状态：静态架构与源接口事实已收口；R1A 可开发；R0B 下午盘与收盘验证已完成，早盘边界仍待补测。
...
1. 组合通配符携带全部显式字段返回 `2325` 行、无重复和身份字段缺失，完整命中 1395 代码池，普通响应约 `0.5-0.8s`。
...
2. 冻结持续旧时间代码的就绪判定，不能把传播中的旧行误当停牌事实，也不能永久要求 1395 行时间完全一致。
3. 结合有界 HTTP transport retry，冻结等待、重试、截止、限速、timeout 和 lease 数值。
4. R1A 完成后用真实 Redis 字段补充单批容量测试。
...
| R0B，进行中 | 时间与容量事实 | 2026-08-26 已完成下午盘、收盘、池命中、逐步传播、五频率共同边界和请求耗时验证；早盘、持续旧时间行规则、transport retry 与最终请求预算仍待冻结。 |
