...
<a id="historical-design-motives"></a>

## 附录：早期设计动机（历史参考，非新增规则）

2026-09-12 承接早期设计动机摘要。旧 foundation/web/core/dm 分层不再用于分配当前职责；旧稿从 Git 追溯。本附录只解释历史取舍，不定义目录、契约、权限或开发门禁。规则入口见本文 §1、§6 和主文档索引，不再复制导航表。
...
“源接口支持日期就必须暴露点/范围输入”“所有调度与观测属于 Foundation”“配置统一环境变量”等旧概括不能作为当前要求。历史想法如需采用，应在对应现行专题明确适用场景，不恢复整套旧规则。
