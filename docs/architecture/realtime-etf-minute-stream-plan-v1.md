...
状态：静态源接口与目标架构已确认；R0B 下午盘与收盘验证已完成，早盘边界仍待补测；可进入 R1A 静态能力开发。
...
2026-08-26 R0B 下午盘与收盘实测记录见：[ETF 实时分钟流 R0B 开市验证记录](/Users/congming/github/goldenshare/docs/architecture/realtime-etf-minute-r0b-open-market-validation-2026-08-26.md)。当前已确认组合通配符完整命中 1395 代码池，以及 `1MIN/5MIN/15MIN/30MIN/60MIN` 在下午盘和 `15:00` 的闭合与逐步传播行为；`09:30/09:35` 早盘边界仍待补测。
...
| R0B，进行中 | 时间与容量事实 | 2026-08-26 已完成下午盘、收盘、池命中、逐步传播、五频率共同边界和请求耗时验证；早盘 `09:30/09:35`、持续旧时间行规则及最终重试预算仍待冻结。 |
