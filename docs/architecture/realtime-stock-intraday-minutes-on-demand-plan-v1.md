...
关联方案：[A股实时分钟流架构方案 v1](/Users/congming/github/goldenshare/docs/architecture/realtime-market-data-stream-technical-plan-v1.md)
...
3. 单次请求耗时大多约 `40-60ms`，实测最高约 `202ms`；20 秒仅是 §7 候选配置，不是已部署值；这些开盘小样本不足以证明全天最坏墙钟预算。
...
右列是本方案待实现目标；当前没有按需序列 API、Redis 缓存或可发布配置对象。
...
历史进度截至 2026-06-02；本轮文档审计不重新请求源站。重新启动需求后，先补齐下表 M0，不能由凌晨空结果推断午休或收盘后必为空。
...
已完成的开盘样本、参数反例及字段/分页结果见 §2.3，不在此重复。剩余时段按 §8.1 顺序补测，逐项记录 §8.3 问题；本段不是自动调度或源请求授权。
...
当前仍有既有开工条件未满足：§7 配置持久化位置与配置合同须拍板，M0 的盘中、午休、下午盘和收盘后样本须补齐。没有完成前不进入实现。候选冷却/限速还须在 LLD 交代多进程并发范围，不能把单进程内存计时默认称为全局控制。本轮只校准文档状态，不代替这些决定。
