...
> **当前边界声明**：本文保留旧 Local Lake Console CLI/Planner/Engine 证据。文中的旧 Lake Root、旧同步引擎和 Parquet 写入链路不得作为当前 Dagster Lake 或新开发、迁移、历史补录、bootstrap、修复、写湖依据。当前正式 Lake 路径和安全规则以根目录 `AGENTS.md` 与 `lake_console/orchestrator/src/orchestrator/defs/paths.py` 为准；禁止新增或调用 Kopia。
