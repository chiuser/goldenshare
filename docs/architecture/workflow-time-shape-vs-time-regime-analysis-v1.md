...
1. `dividend` / `stk_holdernumber`
   - 继续用 `trade_date/start_date/end_date` 这一组公共槽位表达 `ann_date`

修正说明：

1. `namechange` 已重新判定为 no-time snapshot 型历史区间事实。
2. `st` 已重新判定为 no-time snapshot 型 ST 风险警示事件事实，不再用公共日期槽位表达 `pub_date`。
2. 它不再通过公共日期槽位映射 `ann_date`，正确维护方式是不传日期参数并分页拉全集。
...
1. `ManualActionQueryService` 会根据 `DatasetDefinition.date_model` 区分：
   ...
   - `trade_date_range`
   - `calendar_date_range`
2. `dividend` / `stk_holdernumber` 这类 `ann_date_or_start_end` 数据集，point 模式会标记 `date_field="ann_date"`
...
1. [基础数据自然日维护工作流方案 v1（历史归档）](/Users/congming/github/goldenshare/docs/ops/ops-workflow-reference-data-natural-day-maintenance-development-v1.md)
...
2. `st` 的 workflow 对接；`namechange` 已按 no-time snapshot 归入 `reference_data_refresh`
3. 五个新数据集的实际接入
