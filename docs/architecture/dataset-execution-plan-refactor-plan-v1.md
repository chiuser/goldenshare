...
| `index_daily` | 默认按 `index_daily_raw` 请求池逐指数请求，raw 全写本次返回，serving 按 `index_daily` active 池过滤；区间任务量可能较大 | 必须评估单个事务的写入量，做真实计算 |
...
1. `index_active_codes` 已退场；当前代码里仍存在 `dc_index_board_codes`、`ths_index_board_codes` 这类历史 selector。
...
| dataset | 旧入口 | 新 plan |
|---|---|---|
| `index_daily` 区间维护 | 指数区间旧入口 | `run_profile=range_rebuild`，默认按 `ops.index_series_active resource='index_daily_raw'` 拆 `ts_code` 请求；raw 写完整返回，serving 写入前按 `resource='index_daily'` active 池过滤 |
