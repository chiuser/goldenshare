...
3. 历史实施时未触碰 `index_daily` universe 规划逻辑；当前 `index_daily` 已在后续指数 raw/serving 分层收口中明确为：默认按 `index_daily_raw` 请求池拆 `ts_code` 请求，raw 全写本次返回，serving 再按 `index_daily` active 池过滤。
