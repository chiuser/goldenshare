...
2. 本机 ignored 依赖环境、构建产物和配置：不随 M0–M8 删除。2026-09-06 用途审计后，用户另行确认的五项旧 Console 残留已精确移入废纸篓，见 LLD §16.16；其他环境和配置仍保留。此处不是把所有 ignored 业务数据一律排除，数据仍按第 1、3 项用途审计。
...
2026-09-06 更新：下列 ignored 五类已完成独立用途审计；实际编译缓存仅 `tsconfig.tsbuildinfo` 一个文件。用户确认后五项均已移入废纸篓，精确原路径、对应去向和验收见 LLD §16.16。它们不再是待核用途项，也不属于 M8 当时的 111 项删除记录。
