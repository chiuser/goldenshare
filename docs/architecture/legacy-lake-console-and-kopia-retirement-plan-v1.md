...
状态：2026-09-05 M1 已提交 `3007cc0e` / M2A 已提交 `0cc84004` / M2B 已提交 `e8e2abf9` / M3 完成、本次提交归档 / M4 未开始 / 旧湖已批量分类、其余具体删除待确认
...
清退边界、Raw 恢复工具重构和 86 份旧文档删除口径已经全部拍板。当前已完成至 M3，本次按用户要求提交归档，不自动进入 M4；后续从 M4 Raw 恢复工具重构开始，按 M4 → M7 分轮推进，再按 M8 精确清退已证实无代码用途的数据。M3 不包含 M4 适配器、旧 Console、正式执行或物理数据清理。目录分组及五项无引用候选已列 LLD §16.14；不可清空共享根。
