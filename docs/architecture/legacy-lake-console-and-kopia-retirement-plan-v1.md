...
状态：2026-09-05 M1 已提交 `3007cc0e` / M2A 已提交 `0cc84004` / M2B 已提交 `e8e2abf9` / M3 已提交 `1b0deb63` / M4 已提交 `68f97744` / M5 文档清退已实施并通过校验，随本次提交归档 / 其余具体数据删除待确认
...
M4 已提交 `68f97744`；M5 文档迁移与精确清退已完成，用户要求按 135 文件白名单提交，不推送。提交前 HEAD 为 `58d316c0`，其中并行板块分析改动不纳入本次提交。下一阶段 M6 才原子删除旧 Console frontend/backend、专属测试和入口，并同步规则/架构最终时态；不自动进入 M6。当前字段、`vwap` 差异和日常链路保持不变，无新增待拍板项。涨跌停全历史来源待该数据集开工前另审计，不要求本专项代做源实测。之后仍按 M6 → M7 分轮验收，再执行经用户确认的 M8 精确数据清退；目录分组及五项无引用候选见 LLD §16.14，不可清空共享根。
