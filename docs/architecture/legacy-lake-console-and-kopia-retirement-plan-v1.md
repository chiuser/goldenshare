...
状态：2026-09-05 M1 已提交 `3007cc0e` / M2A 已提交 `0cc84004` / M2B 已提交 `e8e2abf9` / M3 已提交 `1b0deb63` / M4 已提交 `68f97744` / M5 已提交 `3ed4c6ca` / M6 旧产品原子清退已通过定向回归，随本次提交归档 / 其余具体数据删除待确认
...
- M6 已完成；用户已要求提交并进入 M7，本次按 292 文件白名单归档，不推送。M7 结果另行留证，不进入 M8。无新增设计待拍板项。
...
M5 已提交 `3ed4c6ca`；M6 已按 263 文件清单完成旧产品同轮清退并通过定向验证，当前未提交。下一步 M7 全量验收与差异复核，正式 DG definitions 命令仍须单独授权；本轮不自动推进。当前字段、`vwap` 差异和日常链路保持不变，无新增待拍板设计项。涨跌停全历史来源待该数据集开工前另审计。M8 仍需用户确认具体数据清单，目录分组见 LLD §16.14，不可清空共享根或捎带删除 ignored 环境。
