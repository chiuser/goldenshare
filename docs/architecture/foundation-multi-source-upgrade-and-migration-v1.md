...
为避免“新表已建但历史数据未迁移”的遗漏，后续每个数据集迁移必须满足以下硬性规则：

1. 两段式迁移必须成对出现
- 结构迁移：创建分源 raw / std / serving 目标表与索引。
- 数据迁移：将 legacy 历史数据回填到新表（`INSERT ... SELECT ... ON CONFLICT ...`）。
...
3. 发布前阻断检查
- 行数对账：新表行数不能低于 legacy 表（有过滤时需在迁移说明写明规则）。
- 主键覆盖对账：核心主键覆盖率必须达标。
- 日期范围对账：起止日期必须符合预期。
