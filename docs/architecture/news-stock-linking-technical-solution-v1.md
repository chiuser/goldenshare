...
本文记录已实现主链合同及后续改造决策。物化窗口已经收敛为 `news_time + manual_range/scheduled_incremental + cursor_end`；旧
`fetched_at + full/incremental + overlap` 只作为历史契约和负向拒绝样本保留。原开发轮次不负责部署、生产 Schedule 和生产回填；
2026-09-01 用户确认关联物化主链结案后，这些事项不再作为本文的开放需求跟踪。2026-09-05 生产页面审计确认股票详情页会把
同一事件的多来源转载逐条展示，用户已拍板由 Biz 查询层在返回前按事件合并；第 13 节已按该口径实现，并于 2026-09-09 完成验收。
...
| 详情页事件合并 | raw 新闻和关联表保留全部来源事实；Biz 查询层把同一股票、相近时间内属于同一事件的多来源报道合并后返回；前端不自行去重 |
...
3. 候选查询每批最多 500 条，使用 `news_time + row_key_hash` keyset 继续读取；单次请求最多扫描 10,000 条候选。API 的 `limit` 只在事件合并和最终排序后执行。
...
`match_method` 只在 `debug=1` 时序列化到响应；普通响应不泄漏识别内部字段。当前实现由 Biz 事件合并器识别同一事件的多来源转载，
不使用标题 `DISTINCT`，也不会仅因公司名称相同或发生在同一天就合并独立事件，具体边界见第 13 节。
...
关联物化主链没有未决业务口径：5 分钟推荐间隔、最小 3 分钟、运营自行配置、上海自然日截止日期包含整天、按 `news_time` 推进、晚到旧时间新闻手动补跑、每批删除后重建均已确认。

本地验证结果已记录在第 12.3 节。关联物化主链于 2026-09-01 结案；股票详情新闻事件合并于 2026-09-09 验收结案，本文没有待开发或待验收事项。

## 13. 股票详情新闻事件合并增强

### 13.1 问题事实与根因

2026-09-05 对股票 `600021.SH` 的生产只读样本确认：页面在 `2026-08-28` 展示的 3 条半年报新闻实际属于同一事件，
在 `2026-08-18` 展示的 8 条项目投产新闻实际也属于同一事件。11 条数据库记录都有不同的 `row_key_hash`，但用户真正需要看到的是
2 个事件。

这不是数据库重复写入，也不是前端重复渲染：

1. `raw_tushare.news` 的 `row_key_hash` 包含来源、发布时间、标题、正文、频道和评分；不同媒体转载同一事件时生成不同新闻身份是正确的源事实语义。
2. `core_serving.news_stock_link` 的主键 `(news_id, ts_code)` 只防止同一源新闻重复关联同一股票，不能把多家媒体的转载识别成一个事件。
3. 改造前的 `StockDetailNewsQuery` 按源新闻身份逐条返回，且在事件合并前执行 `limit`；当前实现已改为事件合并后再应用 `limit`。
4. Wealth 前端只映射并展示 API 数组，没有制造重复，也不应承担业务事实合并。

根因是改造前的股票详情接口把“源站新闻记录身份”直接当成了“用户可见事件身份”。当前实现已经分开这两层语义。

### 13.2 当前实现口径

```text
raw_tushare.news                        保留全部来源新闻事实
        +
core_serving.news_stock_link            保留每条来源新闻与股票的关联
        │
        ▼
StockDetailNewsQuery 候选读取
        │
        ▼
Biz 新闻事件合并器                      请求内计算，不落数据库
        │
        ▼
按事件排序、按事件应用 limit
        │
        ▼
股票详情新闻 API                       一个事件只返回一条代表新闻
        │
        ▼
Wealth 新闻 Tab                        原样展示 API 结果
```

硬边界：

1. 不删除、不修改、不覆盖 raw 新闻，不改变 `row_key_hash` 生成规则。
2. 不改变 `news_stock_link` 表结构、主键、物化算法或自动任务。
3. 不新增新闻事件表、聚合表、migration、后台清洗任务或持久化事件指纹。
4. 事件合并只属于 `src/biz` 的读取模型；不得下沉到 `foundation`，也不得交给 Wealth 前端实现。
5. 不使用大模型或外部语义服务；V1 必须是可测试、可复现的确定性规则。
6. API 路径和响应字段保持不变；`newsId` 使用最终代表新闻的真实 `row_key_hash`。

### 13.3 事件合并规则

事件合并不能退化为简单的“标题相同就去重”。同一事件可能出现无标题正文、书名括号标题、标题截断和小幅改写；反过来，
相似标题也可能描述不同进展。当前规则采用保守的两阶段判断：

1. **候选范围**：只比较同一 `ts_code`、发布时间相近的新闻；跨越事件时间窗口的记录不得合并。
2. **文本归一化**：复用当前 `build_news_display_title()` 的标题提取规则，并统一全半角、空白、常见标点和尾部省略号；同时从正文提取首个完整事实句作为辅助签名。
3. **确定性直连**：归一化标题相同、正文事实句相同，或明确被省略号截断的标题与另一条完整标题构成安全前缀关系时，判为同一事件。
4. **保守近似判断**：标题有轻微改写时，必须同时满足相近发布时间、标题主体高度重合和正文事实证据一致；只满足其中一项不得合并。
5. **数字冲突保护**：标题或事实句中的日期、金额、比例、项目编号等关键数字存在冲突时，不得合并。
6. **分组稳定性**：输入顺序变化不得改变最终分组；相同候选集合必须得到相同事件集合和相同代表新闻。

相近时间窗口、文本相似度阈值和最短安全前缀长度不得凭经验直接写死。本轮已用上海电力生产只读样本、独立事件反例和现有新闻测试样本完成校准，
并在 LLD 中冻结为具名常量及正反例。无法用确定性证据区分时默认保留两条，避免错误合并独立事件。

### 13.4 代表新闻与排序

同一事件内只选择一条代表新闻，选择顺序必须确定且可解释：

1. 优先使用标题非空且没有明显截断的记录。
2. 再优先选择标题和正文信息更完整的记录。
3. 信息质量相同时，选择发布时间较新的记录。
4. 仍相同时，以 `row_key_hash ASC` 作为稳定 tie-breaker。

最终事件列表按代表新闻的完整 `news_time DESC` 排序，时间相同再按代表新闻的 `row_key_hash ASC`。前端继续严格保持 API 顺序。
`debug=1` 时只返回代表新闻对应关系的 `matchMethod`，不新增事件指纹、相似度或内部合并证据字段。

### 13.5 候选读取与 limit 语义

`limit` 从“最多返回多少条源新闻”改为“最多返回多少个新闻事件”。因此不能在第一次 SQL 查询中直接用 `LIMIT :limit` 截断候选源新闻。

当前读取方式：

1. 按 `news_time DESC, row_key_hash ASC` 分批读取明确字段，不使用 `SELECT *`。
2. 每批加入事件合并器，直到已经得到 `limit` 个事件，并继续越过最后一个入选事件的合并时间窗口，或查询范围已经耗尽。
3. 完成事件合并和代表新闻选择后，再排序并截取前 `limit` 个事件。
4. 候选批大小、单次请求最大候选扫描量及默认/最大 `limit` 下的查询耗时必须在 LLD 和性能测试中明确；在没有边界前不得实现无上限全范围加载。

`meta.count` 表示实际返回的事件数量，`meta.limit` 表示事件上限。现有响应结构无需新增字段。
...
| 层 | 目标改动 |
|---|---|
| Biz 共享新闻能力 | 收敛展示标题归一化，并新增无数据库依赖的确定性事件合并器 |
| 股票详情查询 | 分批读取候选新闻，调用事件合并器，按事件执行排序与 limit |
| API schema/route | 保持路径和字段不变，只修正 `meta.count/limit` 的事件语义 |
| Wealth 前端 | 不改去重逻辑，继续原样消费 API 数组；只保留防止前端二次去重的回归测试 |
| 文档和测试 | 替换旧“不去重”口径，增加生产问题样本、反误合并样本和性能边界 |

明确不影响新闻 ingestion、`NewsStockLinkingService`、TaskRun、Schedule、数据库模型、DAO、迁移和其他股票详情模块。
市场总览已有按展示标题完全相等去重的实现，但本轮不能直接复用其 SQL 就宣称问题解决；可以复用标题归一化事实，是否共用事件合并器必须单独核对市场总览契约，不能顺手改变其行为。

### 13.7 测试门禁

实现前后至少覆盖：

1. 同一股票、相近时间、不同来源、完全相同标准标题，合并为一个事件。
2. 无标题但正文以 `【标题】` 开头，与有标题记录合并。
3. 明确截断标题与完整标题合并。
4. 上海电力生产问题的脱敏固定样本：3 条半年报转载合并为 1 个事件，8 条项目投产转载合并为 1 个事件，合计返回 2 个事件。
5. 同一股票、相近时间、标题相似但关键数字不同，必须保留为两个事件。
6. 标题相同但发布时间跨越合并窗口，必须保留为两个事件。
7. 同一事件内代表新闻选择稳定，输入顺序变化不影响结果。
8. `limit` 在事件合并后执行；前若干条源新闻均属同一事件时，仍能继续读取并返回后续事件。
9. `news_time DESC + row_key_hash ASC` 的最终排序、时区、debug、404/400/500 和前端原样顺序保持不变。
10. 默认 50 和最大 2000 事件场景完成查询计划与响应耗时验证，候选读取不得无界增长。

### 13.8 已完成里程碑

1. **D0 LLD 与样本冻结**：逐代码复核查询、标题归一化、API 和前端消费者；把时间窗口、相似度边界、候选批次与扫描上限冻结到 LLD。
2. **D1 纯事件合并器**：实现无数据库依赖的归一化、分组和代表新闻选择，并先完成正反例单测。
3. **D2 查询接入**：改造 `StockDetailNewsQuery` 的候选读取、事件 limit 和响应映射；不改 route/schema 字段。
4. **D3 消费者回归**：验证 Wealth 不做二次去重，股票详情其他 Tab、市场总览新闻和新闻关联物化主链行为不变。
5. **D4 验收收口**：用脱敏生产样本、最大 limit 性能测试和文档检查完成对账；2026-09-09 用户确认实际效果正确，需求关闭。

### 13.9 完成标准

1. 股票详情页对同一事件的多来源转载只展示一条代表新闻。
2. 上海电力问题样本从 11 条源记录稳定收敛为 2 个事件。
3. 独立事件不会因标题相似、公司名称相同或发生在同一天而被合并。
4. raw 新闻和新闻—股票关联事实完整保留，现有数据库不需要清理或重建。
5. 前端没有新增事实拼装或去重逻辑，API 继续是唯一展示事实来源。

### 13.10 实现与验收结果

2026-09-05 已完成 D0 至 D4 的本地开发与回归：

1. 新增 Biz 纯事件合并器，使用 10 分钟窗口、三元字符包含率、数字冲突保护、截断保护和确定性代表记录选择；不依赖数据库或前端。
2. `StockDetailNewsQuery` 改为按 `news_time DESC, row_key_hash ASC` 分批读取候选，完成事件合并后再应用 `limit`；API route、schema 和前端类型均未改变。
3. 上海电力脱敏样本稳定从 11 条源记录收敛为 2 个事件；关键数字冲突、跨时间窗口和短通用文本均不会误合并。
4. 10,000 条独立候选的本地纯合并验证耗时约 0.09 秒；查询仍受每批 500 条、单次最多 10,000 条候选的硬边界约束。
5. 后端事件/API 定向测试、市场新闻与依赖边界回归、Wealth 新闻适配和列表组件回归均通过。未执行部署、生产写入或数据库变更。
6. 2026-09-09 用户确认部署后的股票详情新闻事件合并效果正确，本需求正式关闭。
