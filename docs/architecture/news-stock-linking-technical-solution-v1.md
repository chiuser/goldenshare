...
- 文档类型：技术方案
- 当前状态：关联物化主链已结案；股票详情新闻按事件合并已完成本地开发与回归，待部署验收
...
同一事件的多来源转载逐条展示，用户已拍板由 Biz 查询层在返回前按事件合并；第 13 节已按该口径完成本地实现，生产效果仍以部署后验收为准。
...
3. 当前代码在上述 `ORDER BY` 后直接执行 `LIMIT`；事件合并增强落地后，`limit` 必须作用于合并后的事件列表，不能继续截断源新闻行。
...
`match_method` 只在 `debug=1` 时序列化到响应；普通响应不泄漏识别内部字段。当前代码未做展示去重，因此多来源转载会逐条返回；
该旧行为已被 2026-09-05 的事件合并决策取代。目标实现不得只按标题做 `DISTINCT`，也不得把两个确实独立的事件仅因标题接近而合并，
具体边界见第 13 节。
...
本地验证结果已记录在第 12.3 节。2026-09-01 用户确认的是关联物化主链结案；2026-09-05 新增的股票详情新闻事件合并改造按第 13 节继续推进，不得再把整份新闻需求描述为没有待开发事项。
...
3. 当前 `StockDetailNewsQuery` 按源新闻身份逐条返回，且在事件合并前执行 `limit`。
4. Wealth 前端只映射并展示 API 数组，没有制造重复，也不应承担业务事实合并。

根因是股票详情接口把“源站新闻记录身份”直接当成了“用户可见事件身份”。本次改造必须分开这两层语义。
...
5. **D4 验收收口**：用脱敏生产样本、最大 limit 性能测试和文档检查对账，确认没有数据库或生产数据变更后再更新为已实现。
