...
- 状态：`index_weight`、`stk_mins`、`biying_*`、`index_mins`、`dc_member`、`ths_member` 对象池语义收口已完成；`none`、`index_active_codes`、`dc_index_board_codes`、`ths_index_board_codes` 已清零（2026-05-17）
- 初始范围：只处理 `index_weight`
- 第一轮语义清理范围：把已确认“不按对象池展开”的数据集从 `none` 收口到 `no_pool`
- 第一轮后续处理：`stk_mins` 与 `biying_*` 已完成对象池语义迁移；`dc_member` 已迁移为 `pool + core_dc_index_by_trade_date` 显式来源；`ths_member` 已迁移为 `pool + core_ths_index_snapshot` 显式来源；`index_daily` 仍保持现有特殊对象池语义，不在本轮改动。
- 关联代码：
