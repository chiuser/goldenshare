...
|---|---|---|---|---|---|---|
| `adj_factor`<br/>复权因子 | equity_market<br/>股票行情 | `tushare.adj_factor` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string) | `core.equity_adj_factor` | universe=none; enum=-; pagination=none; max_units=- |
...
| `cyq_perf`<br/>每日筹码及胜率 | equity_market<br/>股票行情 | `tushare.cyq_perf` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string) | `core_serving.equity_cyq_perf` | universe=none; enum=-; pagination=offset_limit; max_units=- |
| `daily`<br/>股票日线 | equity_market<br/>股票行情 | `tushare.daily` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string) | `core_serving.equity_daily_bar` | universe=none; enum=-; pagination=none; max_units=- |
...
| `dc_hot`<br/>东方财富热榜 | board_theme<br/>板块 / 题材 | `tushare.dc_hot` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string)<br/>market(多选枚举:默认 A股市场/ETF基金/港股市场；`TUSHARE_ENABLE_US_HOT_MARKETS=true` 时追加美股市场)<br/>hot_type(多选枚举:人气榜/飙升榜)<br/>is_new(单选枚举:Y) | `core_serving.dc_hot` | universe=none; enum=market, hot_type, is_new; pagination=none; max_units=- |
...
| `fund_daily`<br/>基金日线行情 | index_fund<br/>指数 / ETF | `tushare.fund_daily` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string) | `core_serving.fund_daily_bar` | universe=none; enum=-; pagination=offset_limit; max_units=- |
...
| `index_daily`<br/>指数日线行情 | index_fund<br/>指数 / ETF | `tushare.index_daily` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string) | `core_serving.index_daily_serving` | universe=no_pool; enum=-; pagination=offset_limit; max_units=- |
| `index_daily_basic`<br/>指数每日指标 | index_fund<br/>指数 / ETF | `tushare.index_dailybasic` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string) | `core_serving.index_daily_basic` | universe=none; enum=-; pagination=offset_limit; max_units=- |
...
| `index_weight`<br/>指数成分权重 | index_fund<br/>指数 / ETF | `tushare.index_weight` | month_window/month_window_has_data/start_end_month_window | index_code(string) | `core_serving.index_weight` | universe=pool(ops_index_series_active -> core_index_basic_active); enum=-; pagination=offset_limit; max_units=- |
...
| `stk_period_bar_adj_month`<br/>股票月线行情（复权） | equity_market<br/>股票行情 | `tushare.stk_week_month_adj` | natural_day/month_last_calendar_day/trade_date_or_start_end | ts_code(string) | `core_serving.stk_period_bar_adj` | universe=none; enum=-; pagination=none; max_units=- |
| `stk_period_bar_adj_week`<br/>股票周线行情（复权） | equity_market<br/>股票行情 | `tushare.stk_week_month_adj` | natural_day/week_friday/trade_date_or_start_end | ts_code(string) | `core_serving.stk_period_bar_adj` | universe=none; enum=-; pagination=none; max_units=- |
| `stk_period_bar_month`<br/>股票月线行情 | equity_market<br/>股票行情 | `tushare.stk_weekly_monthly` | natural_day/month_last_calendar_day/trade_date_or_start_end | ts_code(string) | `core_serving.stk_period_bar` | universe=none; enum=-; pagination=none; max_units=- |
| `stk_period_bar_week`<br/>股票周线行情 | equity_market<br/>股票行情 | `tushare.stk_weekly_monthly` | natural_day/week_friday/trade_date_or_start_end | ts_code(string) | `core_serving.stk_period_bar` | universe=none; enum=-; pagination=none; max_units=- |
...
| `ths_hot`<br/>同花顺热榜 | board_theme<br/>板块 / 题材 | `tushare.ths_hot` | trade_open_day/every_open_day/trade_date_or_start_end | ts_code(string)<br/>market(多选枚举:默认 热股/ETF/可转债/行业板块/概念板块/期货/港股/热基；`TUSHARE_ENABLE_US_HOT_MARKETS=true` 时追加美股)<br/>is_new(单选枚举:Y) | `core_serving.ths_hot` | universe=none; enum=market, is_new; pagination=none; max_units=- |
