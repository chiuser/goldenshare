...
- [本地 DG 停牌历史确认事实持久化与统一消费技术方案 v1（S0–S2完成并提交；S3固定文件已发布，事件登记待批准）](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-stock-suspend-confirmed-facts-technical-plan-v1.md)
- [本地 DG 停牌历史确认事实持久化与统一消费 LLD v1（§18.29D：确认修正后发布成功、既有数据未变及事件边界；保留首次失败证据）](/Users/congming/github/goldenshare/lake_console/docs/design/dagster-stock-suspend-confirmed-facts-low-level-design-v1.md#s3-confirmed-file-publication)
