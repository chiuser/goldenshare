...
scraped_at: "2026-04-19 23:19:34"
api_name: "income"
---
...
- 分类：股票数据 / 财务数据
- 原始链接：[https://tushare.pro/document/2?doc_id=33](https://tushare.pro/document/2?doc_id=33)
...
> 实测差异（2026-08-30）：虽然源文档把 `end_type` 标为默认显示字段，但 `income_vip` 请求 `ts_code=601112.SH, ann_date=20260109, report_type=1` 时，报告期 `20240930` 的真实返回行中 `end_type=NULL`。同公司其他报告期及 `600000.SH / 20240930` 交叉验证确认：`0331/0630/0930/1231` 分别对应 `1/2/3/4`，所以该缺失值应规范化为 `3`。Goldenshare 在内容指纹计算前完成补齐；源站非空值与日期矛盾时拒绝。`end_type` 可由 `end_date` 唯一推导，不进入 raw 身份。
