...
> 实测差异（2026-08-30）：虽然源文档把 `end_type` 标为默认显示字段，但 `income_vip` 请求 `ts_code=601112.SH, ann_date=20260109, report_type=1` 时，报告期 `20240930` 的真实返回行中 `end_type=NULL`。Goldenshare 必须保留该空值，不得伪造默认值；`end_type` 不作为 raw 唯一身份字段。
