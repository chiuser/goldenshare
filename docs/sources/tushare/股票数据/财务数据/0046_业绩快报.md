...
title: "业绩快报"
doc_id: 46
source_url: "https://tushare.pro/document/2?doc_id=46"
category_path: ["股票数据", "财务数据"]
scraped_at: "2026-04-19 23:19:35"
api_name: "express"
---
...
- 分类：股票数据 / 财务数据
- 原始链接：[https://tushare.pro/document/2?doc_id=46](https://tushare.pro/document/2?doc_id=46)
...
| remark | str | 备注 |
| update_flag | str | 更新标识（当前实测可通过 `fields` 显式请求，原页未列出） |

## 2026-08-10 实测补充

> 本节只记录当前源端行为，不替代 Tushare 官方 SLA。

- `express` 仍要求按单只股票请求；全市场请求使用参数一致的 `express_vip`。
- 不传 `fields` 时当前只返回约 15 个默认字段；显式请求本文 33 个字段时，33 列均会返回。
- `update_flag` 是当前可返回的第 33 个字段；因原页未记录它，使用时必须显式写入 `fields`。
- `is_audit` 实测出现 `0`、`1`、`2`；原页只解释 `0/1`，因此 `2` 的业务含义仍属上游未说明值，不应在本地强制为布尔值。
- `ann_date=20250408/20250409/20250410` 分别返回 `14/11/9` 行；同一闭区间请求返回 34 行，业务键集合与三个单日请求并集相同。
- `period=20241231` 返回 1,409 行；使用 `limit=500` 的分页为 `500/500/409`，合并后业务键集合与不分页的同范围基线一致。
- 无业务参数的宽范围拉取曾在不同字段组合下出现分页重叠差异，原因未证明；不应用它作为上线维护的完整性基线。
