...
- 分类：股票数据 / 财务数据
- 原始链接：[https://tushare.pro/document/2?doc_id=79](https://tushare.pro/document/2?doc_id=79)
...
| update_flag | str | N | 更新标识过滤；`fina_indicator_vip` 实测可用，源站页面当前未列出该输入项 |
| limit | int | N | 单次返回数据长度 |
| offset | int | N | 请求数据的开始位移量 |

> Goldenshare 于 2026-08-29 实测确认：`fina_indicator_vip` 接受 `update_flag` 作为可选过滤参数。该参数属于源接口能力，不代表运营维护入口必须开放此参数。
