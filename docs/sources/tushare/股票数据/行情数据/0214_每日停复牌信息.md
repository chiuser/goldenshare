...
> 2026-09-12 字段核验：当前官方 doc_id 214 已明确上述空值语义；旧抓取版只写“日内停牌时间段”，本次按官方说明补齐。Tushare MCP 对 000016.SZ、20260907—20260911 的默认字段及显式字段请求均返回 5 行，suspend_type 为 S、suspend_timing 为 null。需同时核对类型与日期，不能将任意空值或行情空结果认定为全天停牌。该证据不单独证明无公司行动影响，也不证明源分页覆盖。
