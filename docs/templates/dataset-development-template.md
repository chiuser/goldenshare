...
> - 本文档按第 1 节分阶段完成：开工前完成事实核验与完整设计，实现后补测试证据，获准执行后补真实验收结果。未完成对应阶段前置项，不得编码、发版或远程同步；设计完成不代表数据集已实现或已验收。
...
开工前必须完成 0.3.0 的源接口实测、0.3.1 的语义设计、0.3.2 的当前消费者审计、0.3.3 的源字段核验与目标字段设计，并在 0.3.4 列清改动和验收计划；满足长任务条件时，0.3.5 的全部执行合同也必须设计完成。现有代码、现有表和源接口事实不能用计划代替核验。

尚未实现的代码、尚未执行的迁移及后续测试结果，明确标为“拟新增 / 拟修改 / 待验证”，写清目标、验收方法和所属阶段；不留空，也不预填“已验证”。这只区分设计与执行证据，不豁免任何验收项。
...
| 代表性小范围的完整基准与分页合并对账 |  |  | 是 / 否 |  | 范围、过滤条件和数据版本一致时，唯一业务键及内容是否一致；说明基准为何完整 |

强约束：

1. 参数过滤、不传参数及分页行为必须用真实请求核验，不能只抄接口文档。源站测试页下载或原始 CSV 可以作为字段和值的样本，但不能单独证明请求参数、过滤或分页语义；记录样本来源及其能证明的范围。
...
7. 分页正确性与全量完整性按 6.3 分开验证。分页实测选取已证明基准完整且可验证多页的代表性小范围；不能把可能截断的无日期或宽区间请求当全量基准，也不能把小范围分页通过当作全量验收通过。
...
10. 本表是字段去向与落地状态的唯一对账表。开工前，源字段结论必须有实测依据，目标列区分已核验现状与拟实现设计；实现和获准迁移后，在同一行补实际文件、目标环境和核验结果。类型、含义引用源文档，转换及不落库原因引用 3.2，不另抄一份字段清单。
...
6. 提交前必须按追溯账本复核 `git diff`：设计提交核对本次设计文档及其引用的现有代码，拟实现文件不得当作已落地证据；实现提交中，每个声称已实现的文件要么出现在本次 / 已引用的前序提交中，要么明确说明已存在且给出验证证据。任何本阶段未覆盖行都是 blocker。
7. 开工前，“实现文件”“正向测试”“反向测试”“真实验证 / 浏览器路径”填写拟改位置、测试场景和验收方法，明确标注“计划”；完成后在原条目补实际证据并更新状态。设计阶段与实现 / 验收阶段分别编号，不能因设计通过就把尚未执行的测试或同步标为“已验证”。
8. 测试、命令输出及真实运行证据只在本账本维护一次，可链接已有报告；其他章节引用 `REQ-*`。引用必须能定位到具体场景、结果和对应代码版本；同一证据覆盖多条需求时可以共用链接，不能只写“见测试”。
...
| 合同项 | 本数据集答案或详述位置 | 关联追溯 ID（计划 / 实际证据） |
| --- | --- | --- |
| 是否为长任务及判断依据 |  |  |
...
本表统一判定长任务合同是否齐全，不重复抄写后文章节：unit 与规模引用 6.2，批次与内存引用 6.3，持久化、幂等、事务、失败与续跑引用 6.5，进度、取消及终态引用 7.3，入口、并发影响与执行方案引用第 9 节。每项必须有具体答案或可直接定位的完整设计；开工前先填验收方法，实际结果完成后通过 0.3.4 引用，不能以“后面再设计”通过开工检查。
...
| 阶段 | 必须完成的内容 | 该阶段不能宣称的结果 |
| --- | --- | --- |
| 开工前：事实与设计 | 0.3 的前置核验；第 2—7 节完整设计；第 8 节测试与验收计划；第 9 节执行、授权和回滚方案；适用时完整填写长任务合同 | 未写的代码、未执行的测试、未迁移的目标表均不能标成已验证 |
| 实现后：代码与测试 | 补实际代码和迁移文件；按 8.1、8.2 完成该阶段测试；更新字段对账和追溯账本；完成发版前要求的最小真实验证 | 本地测试或开发库结果不代表生产已部署、已同步或已验收 |
| 获准执行后：真实验收 | 在第 9 节记录实际执行环境、范围与授权；在 0.3.4 关联真实结果、行数和读回证据；按 8.3、8.4 对账，更新第 10 节 | 未执行的目标环境、缺失的验证或后续阶段不能算完成 |

分阶段填写不改变执行授权，也不把原本要求在发版或全量生产执行前完成的真实验证推迟到事后。第 8.3 节是最终验收清单，不要求在编码前全部勾选。

### 单处维护与引用

源请求实测保存在 0.3.0；字段去向与落地状态保存在 0.3.3；各项设计写在第 4—7 节对应位置，0.3.1 和 0.3.5 引用这些设计；测试及真实结果统一由 0.3.4 关联。8.4 只汇总阶段结论，第 10 节只汇总交付状态，不复制代码、测试或运行证据。

引用写到章节、字段或追溯 ID，不能只写“见前文”。设计变更时同步校准引用方；重复的检查提示可以保留，同一事实不要求重复填写。

### 执行顺序

1. 固定源站事实：官方文档、输入参数、输出字段、分页、限速、更新时间。
2. 按 0.3 完成开工前核验与设计，并判断是否触发 0.3.5 长任务门禁；实际实现及验收证据按上述阶段补入原表。
...
4. 完成本文档的全部设计项，明确 `DatasetDefinition` 完整事实合同和执行/落库/观测方案；Prod 数据集长任务同时明确内存、持久化、续跑、进度和取消。未执行的验证只填方法与计划阶段，不填成功结果。
...
字段类型与含义引用第 2 节的本地源文档；逐字段是否请求、落到哪层及落地状态统一填 0.3.3。这里只填写转换规则、源文档未说明的语义，以及本轮明确不落库的原因，不再列第二份全量字段清单。

| 字段名 | 类型转换 / 清洗规则 / 补充语义 | 不落库原因（如适用） |
| --- | --- | --- |

硬规则：

1. 本表按字段名供 0.3.3 引用；无特殊转换或例外时注明沿用源字段语义，不省略 0.3.3 的逐字段核验。
...
本节维护时间意图归一化的具体设计，0.3.1 的时间输入与执行语义引用本节，不重复填写。同一时间输入在三层中的职责如下：
...
- ORM 模型路径：
- 标准字段映射与清洗规则：引用 0.3.3 的字段去向和 3.2 的转换规则。
- 主键与索引：
...
1. 数值类型依据字段语义、取值范围和精度要求选择，不统一默认浮点或 `NUMERIC`。计数按范围选择整数类型；要求精确十进制保存的字段选择 `NUMERIC`，说明精度、小数位及舍入规则；允许近似计算的字段可选择 `DOUBLE PRECISION`，说明可接受误差。同类字段可合并说明，不必逐字段重复填写。已有字段继续遵守现行 Schema 和消费契约，不因模板调整批量修改类型、Normalizer 或历史数据；确需变更时另行审计影响并取得明确确认。
...
本节填写分页、批次与内存的执行设计，源请求参数、返回行数及分页实测证据引用 0.3.0；触发长任务门禁时，0.3.5 引用本节设计。
...
- 限速策略：
- 源端错误映射：
- 批次大小、单 unit 页数 / 行数上限、内存模式与上限：
- 全量覆盖验收方法与证据来源：说明实际任务范围、覆盖及漏项核验、重复与结束条件；有可信全量基准时列清对账方式，实际结果通过 0.3.4 关联。
...
3. 分开证明分页正确性和全量完整性：
   ...
   - 分页正确性：在代表性小范围内，通过项目实际 connector 验证多页合并与已证明完整的基准在唯一业务键及内容上均一致；范围、过滤条件和数据版本保持一致，记录参数、页序列及差异结果。
   - 全量完整性：按实际任务范围核验覆盖、漏项、重复及结束条件；有可信全量基准时进行对账，不强求源接口无法提供的“全量单次请求”。小范围分页通过或出现 short page，均不能单独证明全量完整。
   - 出现漏键、额外键或内容冲突时，阻断对应写入 / 上线并查明原因。缺少完整性证据时明确标为“尚未证明”，停下来确认替代验证办法；不能自行省略该验收或标成通过。
...
- 字段类型、日期、decimal/float 转换：引用 3.2 的具体字段规则。
- required 字段拒绝策略：
- row transform：
- reject reason code：
...
- 各阶段失败后的已提交事实、读回依据与重试 / 续跑判定（包括已提交 unit）：
- 取消后保留 / 清理边界：
...
合同值引用第 4 节的 `observability.audit_applicable`、`date_model.bucket_rule`、`date_model.observed_field` 及 `not_applicable_reason`；0.3.1 的 freshness / audit 语义引用这些字段和本节，不再填写第二套口径。
...
以下是覆盖检查清单，不另填第二份结果表。开工前将适用场景映射到 0.3.4 的测试计划；执行后在原追溯条目补结果，不适用项按 0.3.4 说明依据。
...
- Request builder：
  ...
  - 对分页接口，真实 connector 或等价测试替身覆盖第二页、short page、页合并业务键及内容对账；替身测试不能替代 0.3.0 的真实请求核验或 6.3 的全量覆盖验收
- Normalizer：
...
开工前确认命令、所需环境与执行阶段；执行后将实际命令、代码版本、结果及输出位置记录到 0.3.4，8.4 只引用。下面的命令清单不是已经执行通过的证据。
...
本清单用于最终验收，逐项引用 0.3.4 的证据；各里程碑按 8.4 核对当期范围。未到执行阶段的项目保持未勾选并注明计划阶段，不因此提前宣称整体完成；不适用项仍须有明确依据。
...
- [ ] 分页正确性已按 6.3 用项目实际 connector 在代表性小范围验证第二页、short page、业务键及内容一致；范围、过滤条件和数据版本一致，基准完整
- [ ] 全量完整性已按 6.3 的方法核验实际任务范围、覆盖、漏项、重复及结束条件，并在有可信全量基准时完成对账；未用可能截断的请求、小范围分页通过或 short page 代替全量完整性证据
...
| 阶段及验收目标 | 本阶段追溯 ID（含证据链接） | 未完成项 / 风险 | 结论 |
| --- | --- | --- | --- |
| M0 / 设计验证 |  |  | 未开始 / 部分完成 / 已完成 |

完成判定：只要本阶段任一追溯行未验证，结论必须是“部分完成”，并列出缺口与下一步；禁止写“阶段完成，后续再补前端 / 测试 / 实测”。

设计阶段只验收事实核验、设计与验证计划；其完成不改变后续实现和验收条目的状态。不得把原属本阶段的必验项移到后续阶段来宣称完成；范围变化须先确认。

---

## 9. 发布与回滚

开工前填写执行及回滚方案、验收 SQL 和所需授权；实际执行后补环境、范围、时间及授权记录。命令输出、真实行数和读回结果统一通过 0.3.4 关联，不在这里重复抄写；没有执行授权不得因方案已填完而自行执行。
...
引用 8.4 的阶段结论作简短汇总，不复制证据。未实现、未部署或未验收必须分别说明，不能以设计已完成代替交付完成。
