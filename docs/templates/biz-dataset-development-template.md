...
更新时间：2026-09-12；精简重复字段/验收填写，模型、性能、事务与授权门禁不变。
...
填写上方完整 Definition 一次即可，不再抄写第二份字段表。逐字段附代码/数据库证据；业务解释引用 §1–5。空值、query/policy 的适用范围和 producer 的实际存在性须明确，不能用 linter 通过替代真实查询验证。
...
每条只填一处实现、正反测试与验收证据，§10/§12 引用本账本，不重复列同一检查。未执行与不适用必须分别说明。

| ID | 必须验证的要求 | 代码 / 正反测试 / 真实验收证据 | 状态 |
| --- | --- | --- | --- |
| BIZ-001 | Definition 合法注册且对应观测查询已接通，API 返回卡片；未知 query/policy、非法控制/staging 注册被拒绝；新增映射和注册集合测试同步 |  | 未开始 |
| BIZ-002 | key/table 唯一，物理表/中文名/分组/顺序由唯一事实源投影；API 与页面一致，不在页面维护名单 |  | 未开始 |
| BIZ-003 | 业务日期、正式发布状态、构建时间、空表、新鲜度策略正确；单卡查询失败处理符合约定，不伪装正常 |  | 未开始 |
| BIZ-004 | 正式 maintenance action 存在、target_tables 包含本表、执行器注册；manual_enabled=True 可创建 TaskRun，False/无 action 无入口；链接类型正确，不伪造 Dagster 维护动作 |  | 未开始 |
| BIZ-005 | schedule_enabled=True 才可配置；参数、时间策略、去重、执行器及自动任务摘要来自后端，同一正式生产入口；禁用能力有反例测试 |  | 未开始 |
| BIZ-006 | TaskRun/观测失败不回滚业务数据；事务、取消、续跑按 §3；Ops 不反向依赖 Biz 或 orchestrator |  | 未开始 |
| BIZ-007 | 页面轮询不触发昂贵 count(*) 或无索引全表扫描；查询索引、规模与性能边界有证据 |  | 未开始 |

## 10. 验证组织

按定义/架构、查询/API、手动/自动任务、页面消费四层填写 §9 的测试路径与结果；覆盖正常、空值、失败及禁止项。不能只写“测试通过”而缺少样例和范围，也不把离线替身测试算成生产验收。没有手动/自动入口的只读资产，记录“不适用”和反向测试，不强行创建任务。
...
- [ ] 开发文档完成并评审，Definition、正式 producer/目标表及观测 query/policy 已对账。
- [ ] §9 的 BIZ-001～007 均有实现、正反测试及证据；手动/自动不适用项有理由。
- [ ] §11 的生产只读验收完成；未完成时不得写“已验收”。
- [ ] 文档完整性通过，索引与状态按真实结果更新。

不重复填写同一份测试/验收清单。新增配置、合同或 Prod 长任务继续遵守根 AGENTS，模板精简不降低性能、事务、权限和真实验收要求。
