...
### 1.6 2026-05-06 远端重审结论

在用户补齐 `namechange` 历史后，已再次对远端数据库做逐日重审，结论如下：

1. 之前阻塞早期缺失日的 `600793.SH` 已补齐历史区间：
   - `*ST宜纸`：`2008-02-01 ~ 2009-08-18`
   - `ST宜纸`：`2009-08-19 ~ 2016-11-20`
   - `宜宾纸业`：`2016-11-21 ~ now`
2. 当前这 `25` 个缺失交易日，候选池都能在 `namechange` 中选到有效记录：
   - `unresolved_candidate_count = 0`
3. 按正式判定规则重建后，预计补数总量为：
   - `3613` 行
4. 这 `25` 个缺失日里：
   - `21` 个日期属于稳定日
   - `4` 个日期属于边界日，需要把候选池中的非 ST 代码排除
5. 当前已识别的边界排除项为：
   - `2020-04-23`：排除 `600225.SH`
   - `2020-06-18`：排除 `300028.SZ`
   - `2020-08-24`：排除 `600365.SH`、`601777.SH`
   - `2021-03-16`：排除 `600255.SH`
6. 当前只有 `7` 个缺失日能拿到同日 `st` 事件作为辅助验证，说明：
   - `st` 仍然只能做辅助验证，不能作为主成员真值表
7. `namechange` 同日多条重叠记录在真实数据中非常常见，不能使用“命中任一 ST 区间即算 ST”的粗判法。
   必须先选出缺失日当天“最新有效的一条 `namechange` 记录”，再判断这条记录是否属于 ST。
...
3. 不在本方案内重复实现 `namechange` / `st` 数据集接入。
...
3. 若同日命中多条活动记录，按下面顺序只选一条“最新有效记录”：
   - `start_date DESC`
   - `ann_date DESC`
   - `end_date DESC`
   - `id DESC`
4. 对选中的这条记录做名称归一化判断：
   - 若归一化后匹配 ST-like，则该 `ts_code` 在 `D` 视为 ST 成员
   - 若归一化后不匹配 ST-like，则该 `ts_code` 在 `D` 视为非 ST，不能写入

这样做的原因是：真实 `namechange` 历史里经常存在同日重叠记录，例如旧的 `*ST` 区间尚未裁掉，但新的“撤销*ST”记录已在同一天生效。
如果只要命中任一 ST 区间就算 ST，会把边界日误判成 ST。

### 7.3 名称取值规则

写入 `stock_st.name` 时，优先级如下：

1. 若前后相邻快照都存在该 `ts_code`，且名字相同，且与选中的 `namechange` 记录不冲突，则直接用这个稳定名字
2. 否则用第 `7.2` 节选中的 `namechange` 活动名称
...
2. `namechange` 在 `D` 命中多条活动记录，但按“最新有效记录优先”仍无法稳定收敛到唯一判定
3. 相邻快照与最新有效 `namechange` 记录在边界上冲突，且 `st` 也无法给出单向解释
