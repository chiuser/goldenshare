...
审计启动时，`etf_sz_cons` 尚未进入 Goldenshare 的 DatasetDefinition、ingestion、ORM/DAO、Ops 目录或生产表。本轮现已完成代码接入、生产部署、数据库迁移、独立激活池初始化、区间同步与页面验收。
...
- 使用独立的 `ops.etf_series_active(resource='etf_sz_cons')` 作为深市 ETF 对象池。初始 seed 按 `.SZ + list_status='L'` 生成 726 个候选；对象池是运营可调整范围，不绑定固定行数。2026-08-22 源端复核并移除 6 个无返回代码后，当前池为 720 个。池为空或存在非法后缀时直接失败，不静默跳过、不回退到全市场深分页。
- 首期不加入任何既有工作流，不做专用 probe；自动任务由运营按源端发布时点自行配置。

代码部署、生产数据库迁移、后续数据同步和页面审计均由运营手动执行，不作为代码交付的自动动作；本轮这些生产步骤已于 2026-08-22 完成。
...
| 生产验收 | 已完成部署、迁移、初始 726 个 `.SZ` 候选 seed、区间同步、源端对象复核和页面验证；当前运营池为 720 个 |
...
对象池初始化口径已确认：`etf_basic(exchange='SZ')` 实测有 786 个 `.SZ` 代码，其中 L=726、P=14、D=46。`exchange='SZ'` 不是 `.SZ` 后缀门禁，当前列表还包含 `.OF`；因此 seed 必须先按 `ts_code` 后缀过滤 `.SZ`，再只保留 `list_status='L'`，初始候选为 726 个，不纳入 P/D。这个候选范围不等同于 `etf_sz_cons` 源端一定返回数据；对象池不设固定数量，需由运营结合源端可服务性维护。
...
- 当前 active 池为 720 个 code，单日约 720 个首次请求；后续以实际 active 池数量为准；
...
| 场景 | 当前 720 code active 池 |
| --- | ---: | ---: |
| 单日 | 720 units |
| 1 年区间（月窗口） | 8,640 units |
| 3 年区间（月窗口） | 25,920 units |
...
| M6 | 本地测试与交接 | 已完成。代表小/大 ETF 的 connector 验证和定向测试通过；生产验收记录见下文 |
| M7 | 运营方部署与验收 | 已完成。生产部署、迁移、对象池初始化、区间同步和页面验收均已完成 |
| M8 | 文档收口 | 已完成。开发文档与 LLD 已按实际代码和验证口径收口 |

## 10. 已确认口径

1. **深市对象池生命周期**：建立 `resource='etf_sz_cons'`，seed 只接收 `list_status='L'` 的 `.SZ` 代码，不放 P/D，不做无池全市场分页；对象池为运营可维护范围，当前为 720 个，不以 726 写死。
2. **采用 raw-only + view**：该接口是源站明细事实，view 直出可以避免第二份物理存储。
3. **首期不加入任何工作流**：日请求量约等于当前 active 池规模，且源站是盘前披露，由运营配置普通自动任务。
4. **首期不做专用 probe**：不新增探测运行时契约。
5. **部署后的同步与回补由运营方负责**：本方案不设计单独回补流程；本轮生产部署、迁移和同步已由运营完成。
...
## 12. 生产验收记录（2026-08-22）

- 数据库迁移 `20260822_000141_add_etf_sz_cons_dataset.py` 已在生产执行。
- 初始 seed 以 `.SZ + list_status='L'` 建立 726 个候选，非法后缀数量为 0；这只是基础信息候选集，不保证 `etf_sz_cons` 源端一定有记录。
- 区间任务 `TaskRun 9080` 已成功完成：处理范围为 `2026-01-05 ~ 2026-08-21`，共 5,808 个 unit；读取 11,251,932 行，写入 11,251,932 行，拒绝 0 行。
- `raw_tushare.etf_sz_cons` 与 `core_serving.etf_sz_cons` view 均为 11,251,932 行，覆盖 154 个交易日、717 个实际返回数据的 ETF，日期范围一致。
- 后续源端复核确认：9 个候选在本次同步区间均没有返回记录。其中 `158005.SZ`、`159043.SZ` 的上市日为 `2026-08-24`，`158000.SZ` 的上市日为 `2026-08-26`，均晚于本次任务截止日；其余 6 个 `list_status='L'` 代码 `158001.SZ`、`158008.SZ`、`158010.SZ`、`158017.SZ`、`158019.SZ`、`159096.SZ` 在 Tushare MCP 的无日期请求和本次区间请求中均返回空数组，已按运营决定从 active 池移除。
- 当前 active 池为 720 个：717 个已有源端历史记录的 ETF，加上 3 个待上市代码 `158000.SZ`、`158005.SZ`、`159043.SZ`。这次调整只改变后续维护对象，不删除已经写入的业务数据。
- 运营页面已按预期展示，未发现本轮接入相关问题。
