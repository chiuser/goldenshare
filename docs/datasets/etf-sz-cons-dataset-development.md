...
| 生产验收 | 已完成部署、迁移、726 个 `.SZ` 激活池初始化、区间同步和页面验证 |
...
- `ops.etf_series_active(resource='etf_sz_cons')` 已初始化 726 个当前上市 `.SZ` ETF，非法后缀数量为 0。
...
- 717 与当前激活池 726 的差异不代表本次同步失败：激活池表达当前维护对象，数据表表达本次历史任务实际返回的源站记录；V1 未把 ETF × 日期完整性审计纳入本数据集，因此不对未返回历史记录的 9 个当前激活代码作自动归因。
