...
> 状态：M0～M4 已完成；迁移 `20260818_000138` 已在 Prod 执行，分类 TaskRun `8714/8717` 与成员 TaskRun `8718/8719` 的生产发布及幂等重放已通过。M5 日行情发布进行中，OHLC 四舍五入取整校验已完成本地纠偏并等待部署。
...
M5 已按用户授权进入生产发布；分类和成员生产验收已完成，日行情 OHLC 取整口径已完成本地纠偏并等待部署。当前仍不授权新增迁移、历史回补、研究物化或排程启用。
