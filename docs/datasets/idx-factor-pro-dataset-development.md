...
| M4：自动任务页面 | 已完成 | 自动任务页仅为 `idx_factor_pro.maintain` 显示“源站已有指数技术因子”；默认选中该条件，并移除本地 freshness 条件与所有源接口参数输入。 |
| M5：全量验证与发布验收 | 本地完成，待部署验收 | Alembic head 已确认是 `20260801_000120`；定向后端回归 399 项、Ruff、Definition lint、前端测试/类型检查与文档检查均通过。迁移与真实单日同步必须在获得部署授权后执行。 |
