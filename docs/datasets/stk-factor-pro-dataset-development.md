...
状态：已落地。当前实现为 raw-only 写入、serving view 直出，并在默认单日维护时根据复权因子变化追加必要的单股历史区间重刷 unit。
...
- 第一阶段生成目标交易日 `trade_date` 请求，内部 `limit/offset` 分页直至无数据。
- 默认全市场单日维护会继续比较目标交易日 `T` 与上一交易日 `P` 的 `adj_factor`，只对复权因子变化且 raw 中已有历史起点的股票追加 `ts_code + start_date + end_date` 历史区间重刷 unit。
- 显式传入 `ts_code` 的单日维护只维护该股票当天数据，不触发全市场复权变化审计。
