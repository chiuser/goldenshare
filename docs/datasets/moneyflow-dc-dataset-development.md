...
- 来源：Tushare `moneyflow_dc`，doc_id=349；[本地接口说明](/Users/congming/github/goldenshare/docs/sources/tushare/股票数据/资金流向数据/0349_个股资金流向（DC）.md)。
...
| 维度 | 当前口径 |
| --- | --- |
| 维护入口 | `moneyflow_dc.maintain`；支持手动、定时与重试 |
...
- 唯一物理事实表及 `target_table`：`raw_tushare.moneyflow_dc`；写入 `raw_only_upsert`。
- Raw 主键：`(trade_date, ts_code)`。审计字段为 `api_name/fetched_at/raw_payload`；物理索引以 [Raw ORM](/Users/congming/github/goldenshare/src/foundation/models/raw/raw_moneyflow_dc.py)及实际数据库 catalog 为准。
- 读取出口：`core_serving.equity_moneyflow_dc` 普通视图，直接投影 Raw 业务字段，并将 `fetched_at` 投影为 `created_at/updated_at`。
...
- 历史切换实现：[revision 20260829_000161](/Users/congming/github/goldenshare/alembic/versions/20260829_000161_make_moneyflow_dc_raw_view.py)。原子迁移的依赖、权限、全字段对账和拒写护栏保留在迁移代码，不作为现在重建或回退表的授权。

## 5. 回归与运行边界

- 专项回归：[test_moneyflow_dc_raw_view_m1.py](/Users/congming/github/goldenshare/tests/test_moneyflow_dc_raw_view_m1.py)：存储合同、请求计划、writer、模型及迁移护栏。
...
### 历史阶段 11：`P1-B4-moneyflow_dc-M0` 只读复审结论（2026-08-29）
...
### 历史阶段 12：`P1-B4-moneyflow_dc-M1` 编码与离线验证结论（2026-08-29）
...
### 历史阶段 13：`P1-B4-moneyflow_dc-M2` 隔离 PostgreSQL 验收结论（2026-08-29）
...
### 历史阶段 14：`P1-B4-moneyflow_dc-M3a` 生产即时验收结论（2026-08-29）
...
### 历史阶段 15：`P1-B4-moneyflow_dc-M3b` 自然工作流验收与结案（2026-08-31）
