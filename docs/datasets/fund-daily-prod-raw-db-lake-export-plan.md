# 基金日线行情 Lake prod-raw-db 导出方案

状态：已落地（2026-05-06）

本文定义 `fund_daily` 数据集从生产 `raw_tushare.fund_daily` 只读导出到本地 Lake Parquet 的方案。该方案只覆盖 `prod-raw-db` 导出模式，不改变现有 Tushare API 下载链路。

## 1. 目标

把生产库中已经落在 `raw_tushare.fund_daily` 的 Tushare 原始基金日线行情数据，按 Lake Console 的按日分区布局导出成本地 Parquet。

核心目标：

- 导出的 Parquet 字段必须与 Tushare `fund_daily` 输出参数一致。
- 只访问生产库 `raw_tushare.fund_daily`。
- 禁止 `select *`。
...
| 项 | 值 |
| --- | --- |
| Lake dataset_key | `fund_daily` |
| 显示名 | 基金日线行情 |
| Tushare api_name | `fund_daily` |
| 源接口文档 | `docs/sources/tushare/ETF专题/0127_ETF日线行情.md` |
| 生产 raw 表 | `raw_tushare.fund_daily` |
| Lake 目标层 | `raw` |
| Lake 目标路径 | `raw_tushare/fund_daily/trade_date=YYYY-MM-DD/part-000.parquet` |
| 分区字段 | `trade_date` |
...
| 检查项 | 结果 | 说明 |
| --- | --- | --- |
| 表存在 | 是 | `raw_tushare.fund_daily` |
| schema | `raw_tushare` | 符合边界 |
| 行数 | `2443246` | 到 2026-04-30 |
| 日期范围 | `2016-01-04 ~ 2026-04-30` | 覆盖 ETF 历史主区间 |
...
- Lake 正式分区语义是某交易日全市场基金日线快照，不允许局部结果覆盖正式分区。
...
1. 总行数已达两百四十多万，不能使用 `fetchall`。
2. 支持单交易日点查，也支持区间只读事务 + 服务端游标流式读取。
3. 范围流式读取后按 `trade_date` 拆分写入分区，能复用首批 `daily_basic` 模板。
...
```text
raw_tushare/fund_daily/trade_date=YYYY-MM-DD/part-000.parquet
```
