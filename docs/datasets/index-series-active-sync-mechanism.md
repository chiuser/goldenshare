...
3. 只有成功完成全部源站分页后仍无该指数当月月线，且库内也无同月源站月线，才可进入派生判断。
4. 同一“指数 + 月份”只保留一条 serving 记录。源站覆盖派生，源站修订覆盖旧源站；本次空返回不删除或降级已有源站行。
5. 月份未结束、基础日线不全或源站请求失败时，不生成新派生；派生不写 raw，正常替换在 unit 事务内完成。

月末当天缺少源站月线的指数不会被当场派生，最早下月再次维护该月时处理。本次不新增调度，不自动重跑，也不清理历史表；旧的不完整派生必须经有效结果校正，不能视为已经满足新规则。
