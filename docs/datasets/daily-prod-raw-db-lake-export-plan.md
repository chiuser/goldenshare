...
| `trade_date` | `str`，`YYYYMMDD` | `date` | `string`，`YYYY-MM-DD` | 是 | 交易日期，导出时转成 ISO 日期字符串 |
...
| 差异 | 处理方式 |
| --- | --- |
| raw 表 `trade_date` 是 `date`，Tushare 文档是 `YYYYMMDD` 字符串 | Lake 统一写成 `YYYY-MM-DD` 字符串，便于与当前 Lake 分区和前端展示口径一致 |
...
区间导出不应一次性把全区间加载进内存。推荐执行方式是按日期逐日查询、逐日写入：
...
区间查询可以作为内部优化，但第一阶段不采用，避免大结果集造成内存和事务风险。
