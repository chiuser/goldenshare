...
> 状态：M0～M5 已完成；M6 历史事实与回补暂缓。迁移 `20260818_000138`、分类、成员和日行情生产发布均已通过；日行情已完整覆盖 2025-01-02～2026-08-19。
> 初版：2026-08-16；代码对账：2026-08-17；最终产品拍板：2026-08-18；M2/M3 纠偏验收与 M4 本地验收：2026-08-19；M5 生产验收与 M6 暂缓决策：2026-08-20。
...
以下三项由用户确认，已成为三份申万数据集 LLD 的共同硬约束，不再列为待拍板项：
...
3. `sw_daily` 当前只承担源事实保存，不在入库阶段判断 `low/open/close/high` 的业务大小关系，也不使用四舍五入、固定小数阈值或其他容差修正源值。OHLC 只要求字段存在、数值可解析且有限，并按源值原样保存；未来线上消费所需的清洗或可信度标记另行设计，不得反向改写本表原始事实。
...
- `open/low/high/close` 必须是可解析的有限数值，但不校验四者之间的大小关系，不修改源值；
...
- 空结果 no-op 不生成日期桶：目标原无该日记录时仍缺失；原已有该日记录时完整保留，不会因这次空响应变成缺口。Date completeness 读取目标 `trade_date` 桶，只证明日期存在，不证明当次源端已发布或代码集合完整；
...
M1 开工时，本地仓库唯一 head 为 `20260816_000137`；M1 已生成线性迁移 `20260818_000138`，其 `down_revision` 为 `20260816_000137`。M1 阶段只完成本地生成与测试；M5 于 2026-08-19 重新核验仓库与 Prod head 后执行迁移，当次确认 Prod head 为 `20260818_000138`，不是今日生产 head。
...
4. 每个交易日的全部页先合并、标准化、去重。源端 0 行时直接完成该 unit，读取/保存/拒绝均为 0，不解析目标 DAO、不执行 DELETE/INSERT/read-back，也不使 TaskRun 失败；该日期仍由完整性检查报告为缺失。非空结果中的任意 reject、日期越界、非有限数值、负量额或业务键冲突都在 DML 前使当日 unit 失败。OHLC 不做大小关系或取整关系判断，Tushare 返回的可解析有限数值必须原样保存；后续业务清洗不得污染本表源事实。
...
4. 按失败日期幂等重试和 read-back 报告范围。原专项没有引入 checkpoint/acquire/定点跳过语义；M6 已暂缓，未来恢复时须按届时根规则与数据集模板重新评审可恢复性，不能把历史限制视为长期通用禁令；
5. 首次回补窗口及上线所需最小历史长度。
...
1. 先发布分类快照并验收代码映射。
2. 再发布成员全集并验收分类闭包。
3. 先同步 `sw_daily` 最近一个有效交易日，核验源端/目标集合；本地幂等保留，生产验收按 §9.8 的最终决定，不追加生产幂等重放。
4. 批准历史窗口后再做按日回补。
5. 当前只支撑最新及最近 20/60 个交易日展示；历史回测须先恢复并验收 M6，不能由本次发布推定可用。页面能力不属于本 LLD。
...
| 日内质量 | 439 行可含 1 个空 pe；可解析且有限的 OHLC 原样保存，不判断四者大小关系 | 空值补 0、日期越界、修改 OHLC 源值、非有限数值或负量额时失败 |
...
本节及 §9.4 为 M5 关闭前的验收设计；日行情生产幂等重放要求已被 §9.8 的 2026-08-20 用户决定替代，不再作为待补门禁。本地幂等回归继续保留。
...
- M1～M4 申万专项与三组 Ops API 回归：53 passed；覆盖 2,000/2,001 行、60/61 units、439/414/25 保留、原空结果失败口径、部分 reject、日期越界、原 OHLC 关系门禁、负量额、同日替换、跨日保护和幂等重放。M5 空结果和 OHLC 最终口径按用户后续拍板另行纠偏，不回写为当时已通过。
...
M4 验收只证明本地代码契约和事务行为满足 LLD；生产迁移、最小同步与真实目标表 read-back 当时仍属于 M5，现已按 9.8 完成。

### 9.6 M5 OHLC 生产事件与最终口径（2026-08-19）

- TaskRun `8724` 请求 2026-07-01～2026-08-19 共 36 个交易日；前 15 日已各发布 439 行，共 6,585 行，2026-07-22 因 `801280.SI` 的 `high=2513.36/close=2513.37` 在当日 DML 前失败。
- 首轮曾按“十进制 `ROUND_HALF_UP` 四舍五入取整后判断关系”纠偏；该规则使 `801280.SI@20260722` 和 `801270.SI@20260817` 的 `0.01` 源端交叉可以保真发布。TaskRun `8732` 随后已将 2026-07-01～2026-08-18 共 35 个交易日全部发布，每日 439 行。
- TaskRun `8742` 请求 2025-02-05～2025-04-30 共 60 个交易日；前 24 日已保存 10,536 行，`2025-03-11` 因 `801260.SI`（申万消费）再次失败。源值为 `high=6403.49/close=6403.50`，取整后分别为 `6403/6404`，证明取整规则仍会因小数边界阻断源事实保存。
- 用户最终拍板：删除 OHLC 大小关系校验和取整规则。`sw_daily` 当前仅保存 Tushare 原始源事实；只要 OHLC 字段存在、可解析且有限，就按原值入库。不得设置 `0.01` 阈值、不得四舍五入、不得修正 high/low，也不得在 Foundation 阶段生成“清洗后”替代值。
- 仍保留的入库门禁仅包括本轮未取消的结构性约束：必填字段、有限数值、SW2021 版本、unit 日期一致、代码/唯一键/替换范围、任意归一化 reject 和非负量额。未来线上使用时所需的 OHLC 清洗、异常标记或可信度分层必须使用独立业务/研究设计，不得回写源事实表。
- 本地验收：三类生产边界样本及明显倒置 OHLC 均按原值通过，`NaN/Inf` 仍被拒绝；申万 M1～M4 与三组 Ops API 共 55 passed，Foundation 最小门禁 160 passed，Ruff 与 `ingestion-lint-definitions` 均通过。
...
- 空结果 no-op 必须发生在 DAO 解析和任何 DML 前，TaskRun 正常完成。本次 2026-08-19 目标原为空，因此桶仍缺失；若重试日期已有数据，则保留原桶与原行，不能把空响应一律写成日期缺失。两种情况都不证明本次源端已发布。
- 正例：范围中最后一天空结果时，前序日期正常提交、最后一天 0 读取/0 保存/0 拒绝、总任务成功。反例：`fail_unit` 完整快照空结果仍报 `write.scope_empty`；任意 reject、日期错配、非有限 OHLC、跨日范围仍失败且不发生目标 DML。
...
### 9.8 M5 生产验收关闭（2026-08-20）

- TaskRun `8857` 以 60/60 单元成功完成 2025-02-05～2025-04-30，读取和保存均为 26,340 行，拒绝 0；此前因 OHLC 关系校验阻断的 2025-03-11 已正常发布。
- TaskRun `8858` 成功发布 2026-08-19，1/1 单元，读取和保存均为 439 行，拒绝 0。
- Prod read-back 确认 2025-01-02～2026-08-19 共 395 个开市日、173,405 行，逐日均为 439 行和 439 个唯一代码，交易日缺口与重复业务键均为 0。
- 按用户明确验收口径，本轮不追加日行情生产幂等重放；本地幂等正反例继续有效，生产验收以成功 TaskRun、全交易日覆盖、零拒绝、目标表 read-back 和唯一键为准。M5 至此关闭。
...
| SD-003 | 单次最多 60 个交易日 | planner/Manual Action | `max_units_per_execution=60` | 显示范围限制 | Definition、planner、Ops | 60 units 通过 | 61 units 拒绝 | 60 日 PLAN | M4/M6 | M4/M5 已通过；M6 暂缓 |
| SD-004 | 无 Raw/Lake/双写 | storage/card | direct-serving scope replace | 卡片展示服务表 | linter、writer、Ops query | 只解析 serving DAO | Raw DAO/表出现即失败 | source/target 对账 | M1/M4/M5 | M1/M4/M5 已通过 |
| SD-005 | 15 个源字段保真 | source/normalizer/ORM | 显式 source_fields | 不适用 | Definition、ORM、迁移 | 全字段 payload/落库 | 漏身份/OHLC 字段失败 | 20260814 样本 | M1/M4/M5 | M1/M4/M5 已通过 |
| SD-006 | 源码与业务码分离 | normalization/下游 | 共享 code contracts | 不以源码关联 | contracts、transform、ORM | 850412 保持 | 850401 未归一/840401 失败 | 关键码 read-back | M1/M4/M5 | M1/M4/M5 已通过 |
| SD-007 | 保存当日全部源行 | writer/read-back | 不在 Foundation 过滤分类外指数 | 不适用 | writer、ORM | 439 源行全入 | 擅自过滤 25 行失败 | 439/414/25 对账 | M4/M5 | M4/M5 已通过 |
| SD-008 | 单日空结果安全跳过；任意 reject 不发布；OHLC 原始事实保真且不做关系校验 | Definition/normalizer/writer/completeness | `empty_result_policy=allow`；空批次在任何 DML 前 no-op；非空批次只校验 OHLC 存在、可解析且有限，不判断大小关系、不修改源值 | 空结果任务成功；原无桶仍缺失，已有桶保留；结构性非法事实结构化失败 | Definition、normalizer、writer、completeness | 空日 0 读取/0 保存且既有数据不变；三类生产 OHLC 边界样本原值发布 | `fail_unit` 数据集空结果仍失败；日期越界、非有限数值、负量额或部分 reject 回滚 | TaskRun 8724/8732/8742/8857/8858 与源端样本对账 | M1/M4/M5 | 本地实现、部署与生产验收均已通过 |
| SD-009 | 同日精确替换且幂等 | writer/DAO | 限定 `trade_date` scope replace + read-back | 不适用 | writer、DAO | 同日重放摘要一致 | 无 where/跨日期/部分页发布失败 | 两次 APPLY/read-back | M1/M4/M5 | M1/M4 本地事务已通过；M5 按用户口径以生产唯一键和 read-back 验收，不追加生产幂等重放 |
| SD-010 | 日期完整性只证明桶存在 | completeness/freshness | date_bucket distinct date | 展示日期缺口，不宣称行数完整 | Definition、Ops audit | 开市日桶存在 | 非交易日误报/把部分行当集合验收失败 | 日期审计+独立 key set 报告 | M4/M5 | M4/M5 已通过，生产日期缺口为 0 |
...
| SD-013 | 历史覆盖不越权宣称 | 回补报告 | 每批最多 60 日、全代码覆盖后结论 | 不适用 | runbook/验收报告 | 获批窗口完整对账 | 单代码外推/超 60 unit 失败 | 分批 PLAN/APPLY/read-back | M6 | 暂缓；当前不补 2025-01-02 以前的历史 |

---

## 11. 三数据集统一开发里程碑与停止条件

阶段编号、生产验收和 M6/M7 状态统一维护在[分类 LLD 的 M0～M7 总表](index-classify-sw2021-low-level-design-v1.md#sw2021-milestones)。本文件只保留本数据集的实现、验收证据与下列停止条件，避免三份阶段表分别漂移。
...
后续阶段仍须独立评审和授权；历史成员有效期与回测能力不在本次已验收范围。
