...
状态：分层基线已实施；指数月线完整月份与覆盖修复已实现，待生产部署与验收
创建日期：2026-05-05
修订日期：2026-09-04
适用范围：`index_daily`、`index_weekly`、`index_monthly`

> 第 11 节的指数月线修复已完成本地实现与回归；本轮没有部署、重跑或修改生产数据。历史月线仍须在部署后由运营重新维护校正，不能把代码完成当成历史数据已经修复。日线、周线业务规则未扩大修改。
...
图中 raw 写入、serving 替换均在既有 unit 事务内，不表示 raw 已提前提交。空结果或不具备派生资格不授权整月清理；月线条件 upsert 使用同月唯一键，更新包含 `trade_date`，不再先删后插。
...
| `DatasetUnitPlanner._resolve_anchors` / `_resolve_index_monthly_anchors` | `month_last_open_day` 统一走完整月历规划；已移除截断式月末压缩方法；当前唯一 Definition 消费者是 `index_monthly` |
| `index_month_calendar.load_index_month_open_dates` | planner/writer 共用整月日历证据，要求所有自然日均有开闭市记录；不把日历缺失当休市 |
| `_index_monthly_params` | 继续只格式化计划中的日期与代码，不承接日期选择或月份完整性判断 |
| `DatasetWriter._write_index_period_serving` / 派生辅助方法 | 月线检查北京时间跨月、完整日历、已有 API、完整日线日期集合与非空有限数值；源站归一化存在拒绝时不把拒绝误判成源站缺失；周线保持原行为 |
| `_upsert_index_monthly_serving_rows` / `IndexMonthlyServing` | 按现有月份唯一键更新日期及行情，SQL 条件保护 API 不被派生降级；`RETURNING` 统计实际保存行数，不以冲突忽略替代修订 |
...
2026-09-04 已获得开发指令，按以下顺序实施；本轮不部署、不操作生产数据：
...
生产部署、历史月份重跑和数据清理均不属于本轮开发；没有自动清表或自动补跑授权。不满足新规则的旧派生数据仍保留原行，本文的“待校正”不是新增状态字段，不能宣称历史数据已修复。

### 11.8 编码与验证映射（2026-09-04）

| 硬口径 | 本轮编码与测试位置 |
| --- | --- |
| 完整月历、真实月末、截断范围与单日校验 | ingestion 内共用月历读取方法，检查整月每个自然日均有开闭市记录；planner 用真实月末筛选区间，非月末 point 明确失败。日历不全不能猜月末 |
| 跨月后、整月逐日完整才派生 | writer 的月线分支检查北京时间，SQL 同时比较日期集合和必要数值字段；每个指数独立判断，周线保持原规则 |
| 同月替换、源站优先 | 月线按现有 `(ts_code, period_start_date)` 执行条件 upsert；API 可覆盖所有来源，派生只能覆盖非 API，不删除空返回月份 |
| 原子性与可重跑 | 隔离 PostgreSQL 验证实际主键/唯一索引、更新截至日期、写入失败回滚、先前月份保留、取消后新 session 幂等重跑 |
| 源端错误不转派生 | 经真实 source client 的假 connector 注入首/后页失败，断言 writer 不执行，旧数据保留 |

执行边界：沿用一个月末全市场（或显式单代码）为一个 unit；源端 1000 行分页后合并，已有审计样本单月 1998 行，约 2 页。新校验只读一个自然月的日历（最多 31 行），逐指数读取该月日线，SQL 仅返回合格聚合行，不载入多月日线。raw 单月源站行与 serving 当月 active 行在原 unit 事务内一起提交，数据库写入仍使用现有批量大小。窗口增长只增加 unit，不合并多月事务。

本轮不改 Definition、Plan 合同、源接口参数、source client、执行器或 Ops。取消/进度仍走现有执行器 unit 边界；不新增分页提交、心跳或续跑状态。重跑依据是已提交业务行与同月幂等键，不自动跳过已完成月份。进程退出后已提交月份仍可读，未提交事务由数据库回滚；部署与生产重跑由运营决定。

源站复核：按本地 doc 172 显式请求 `000001.SH + trade_date=20260731` 的 11 个字段，返回 1 行；`trade_date=20260730` 默认字段请求返回 0 行。请求参数与字段不变，修正的是本地月份选择与 serving 写入。开发前 resolver/writer 基线 114 项通过。

### 11.9 开发验收记录

- CodeGraph 使用 `status/explore/callers/callees/impact`，核对 resolver、共享日期入口、writer、单元执行器及 Ops 调用方；开发后 `codegraph sync` 已确认索引同步。未增加跨子系统依赖。
- 已读手动预检、TaskRun dispatcher、自动任务月份策略、freshness、日期审计和月线来源统计消费者。它们不接收新字段，自动任务仍生成真实月末 point；页面时间选择规则与本次 planner 校验一致。freshness/日期审计规则不属于本次修改。
- `tests/test_index_monthly_calendar.py` 覆盖截断范围、point 非月末、月底周末/节假日、缺失日历和周线原行为。
- `tests/test_index_monthly_postgres.py` 使用全新临时 PostgreSQL 集群和各测试独立数据库，覆盖真实唯一约束、源站覆盖旧派生、修订重放、完整性、空返回保护、并发来源保护、首/后页失败、事务回滚、取消重跑和子进程异常退出；没有使用本机业务库或 Prod。
- 原 writer 文件中三项使用模拟插入/删除的月线测试已由上述真实数据库用例替代；日线、周线回归保留。
- 核心定向测试与架构门禁 251 项通过（含 39 项隔离 PostgreSQL 用例及周线实际 SQL 回归）；自动任务月末相关回归 4 项通过；Ruff、`ingestion-lint-definitions` 与文档完整性检查通过。
- 生产后续：部署后重新维护受影响的已结束月份。存在源站月线时覆盖旧派生；没有源站且日线不齐时保持旧行，不会假装补齐。无需迁移、清表或新配置。
