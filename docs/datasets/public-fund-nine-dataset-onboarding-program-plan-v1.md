...
状态：**B0/B1 已实现并通过隔离验证；B1 尚未生产迁移、未创建任务、未写入远程数据。B2 及后续批次待进入。**
确认日期：2026-08-05
...
| B1 | `fund_company`、`mkt_idx_bmk` | 小型无时间分页快照、观察版本、HDD 物理表与公募基金 Ops 条目 | B0 验收 | ✅ 项目 connector 实测全量/短页行为、全部字段、身份与版本、迁移、Ops/API、两次隔离真实同步对账闭环；待生产迁移、首次生产同步对账及运营手工创建 schedule；两表不做文本关联 |
...
- B0/B1 已完成实现与隔离验证；未经生产迁移、首次完整生产同步及五段对账授权，不得创建 B1 schedule，也不得进入 B2。
