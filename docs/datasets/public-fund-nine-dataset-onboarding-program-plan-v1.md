...
状态：**B0/B1 已实现并通过隔离与生产验收：B1 已完成生产迁移、首次完整同步及五段对账，尚未创建 schedule。B2 及后续批次待进入。**
确认日期：2026-08-05
...
| B2 | `fund_basic` | E/O 全市场分页主数据、25 字段 | B1 生产迁移与首次生产同步对账通过 | O 市场完整分页、E/O 无 `status` 过滤、观察版本与对象池结果对账；`fund_portfolio` 仅可使用该已验收对象池 |
...
- B0/B1 已完成实现、生产迁移与首次完整生产同步五段对账；B1 schedule 仍需运营明确给出频率与 cron/once 意图后手工创建。B2 可以在单独授权后进入其 LLD/实现阶段。
