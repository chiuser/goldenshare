...
状态：**九项中已有七项进入当前 DatasetDefinition/Ops/存储主链并完成生产 migration 与首次真实同步验收：`fund_company`、`mkt_idx_bmk`、`fund_basic`、`fund_manager`、`fund_share`、`fund_div`、`fund_portfolio`。B7-M3.1 季度内逐页实时进度已完成编码、本地自动化门禁并部署 Prod；本轮未以新的生产长分页 TaskRun 做页面运行验收。`fund_nav` 与 `fund_factor_pro` 仍只有发现审计，未进入 LLD、代码、建表或生产同步。历史回补与 schedule 继续按数据集独立授权。**
...
本表是截至确认日期的状态投影。数据集静态事实仍以 `src/foundation/datasets/definitions/public_fund.py` 为准；生产部署与真实同步属于发布事实，不能仅从代码存在反推。

| 数据集 | 当前代码事实 | 生产状态 | 尚未完成 |
| --- | --- | --- | --- |
| `fund_company` | Definition、18 fields、current/observation ORM/DAO、HDD migration、Ops Catalog 均存在 | TaskRun `#7401` 首次完整同步 204 行、reject 0 | 未创建 schedule；观察历史只能从接入日起积累 |
| `mkt_idx_bmk` | Definition、8 fields、current/observation ORM/DAO、HDD migration、Ops Catalog 均存在 | TaskRun `#7402` 首次完整同步 141 行、reject 0 | 未创建 schedule；不自动关联 `fund_basic.benchmark` |
| `fund_basic` | Definition、25 fields、current/observation ORM/DAO、HDD migration、Ops Catalog 均存在 | 17 页、32,342 行首次完整同步，reject 0 | 未创建 schedule |
| `fund_manager` | Definition、10 fields、current/observation ORM/DAO、HDD migration、Ops Catalog 均存在 | TaskRun `#7515` 首次完整同步 84,357 行、reject 0 | 未创建 schedule |
| `fund_share` | Definition、6 fields、按日期 current/observation ORM/DAO、HDD migration、Ops Catalog 均存在 | TaskRun `#7556` 对 `2026-07-07` 同步 1,673 行、reject 0 | 未历史回补或创建 schedule |
| `fund_div` | Definition、16 fields、不可变事实 ORM/DAO、HDD migration、Ops Catalog 均存在 | TaskRun `#7653/#7654` 完成首次与幂等同步，目标 74 行、reject 0 | 未历史回补或创建 schedule |
| `fund_portfolio` | Definition、8 fields、页流式 stage/final ORM/DAO、HDD migration、Ops Catalog 与 typed paged-unit progress 均存在 | M3 TaskRun `#7813/#7814` 完成 42 行首次与幂等同步；M3.1 代码已部署 Prod | 未历史回补或创建 schedule；M3.1 尚未用新的生产长分页运行做页面验收 |
| `fund_nav` | 当前代码、迁移、Ops Catalog 均无该数据集 | 未接入 | 仅发现审计；待 B5 LLD、编码、隔离与生产验收 |
| `fund_factor_pro` | 当前代码、迁移、Ops Catalog 均无该数据集 | 未接入 | 仅发现审计；待 B6 LLD、编码、隔离与生产验收 |
