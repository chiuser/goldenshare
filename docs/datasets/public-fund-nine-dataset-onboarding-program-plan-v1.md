...
状态：**B0/B1 已实现并通过隔离与生产验收；B1 尚未创建 schedule。B2 LLD 与 M1 本地实现门禁已通过，待 M2 隔离验证；B3 及后续批次待进入。**
确认日期：2026-08-05
...
| B2 | `fund_basic` | E/O 全市场分页主数据、25 字段 | ✅ B1 生产迁移与首次生产同步对账通过 | LLD 与 M1 本地实现已通过：单个无 market 完整分页 unit、E/O 批次防缺失、无 `status` 过滤；待 M2 隔离 migration、HDD placement、真实同步及观察版本对账；`fund_portfolio` 仅可使用该已验收对象池 |
...
- B0/B1 已完成实现、生产迁移与首次完整生产同步五段对账；B1 schedule 仍需运营明确给出频率与 cron/once 意图后手工创建。B2 已通过 LLD 与 M1 本地实现门禁；M2 隔离验证、生产迁移、生产同步和 schedule 仍须后续单独授权。
