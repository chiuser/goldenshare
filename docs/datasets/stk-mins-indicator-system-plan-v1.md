...
| 北交所 `920xxx.BJ` 分钟线首次出现 | 若 `ts_code` 存在于本地 `stock_basic`，且当前日期是该股票在本地分钟线源文件中的首次出现日期，视为分钟线口径下刚上市，从首根 K 线建立 state |
...
6. 北交所 `920xxx.BJ` 只在分钟线指标 state 初始化中走专项口径：先要求 `ts_code` 存在于本地 `stock_basic`，再以本地分钟线源文件中的首次出现日期作为有效初始化日。
7. `bse_mapping` 只能作为旧代码切换辅助参考，不是完整 `920xxx.BJ` 清单；像 `920002.BJ` 这类不在 `bse_mapping` 中、但存在于 `stock_basic` 且本地源分钟线真实出现的股票，也应按本地源首次出现日初始化。
8. `2022-07-15` 只是本地分钟线新代码的最早有效起点；真正允许初始化的日期必须是该股票在本地分钟线源文件中的首次出现日期。例如某只股票到 `2022-08-18` 才第一次出现，则只能在 `2022-08-18` 初始化。
9. 如果 `920xxx.BJ` 晚于自身首次出现日期才缺 state，必须拒绝并提示从该股票本地首次分钟线日期补起。
...
7. 北交所 `920xxx.BJ` 分钟线代码切换按 `stock_basic 存在 + 本地源分钟线首次出现日期` 判定有效首次出现日期；`bse_mapping` 只作为旧代码映射辅助参考，不是完整清单；`2022-07-15` 只是允许参与该专项口径的最早源数据起点。该口径只服务分钟线指标 state 初始化，不改写 `stock_basic` 全局事实。
