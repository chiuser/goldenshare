...
状态：**B3-M0、LLD、B3-M1 与 B3-M2 隔离 PostgreSQL 验收通过；尚未执行生产迁移/首次同步，未创建任务**
...
B3-M0、LLD、B3-M1 与 B3-M2 均已通过。无参单 unit 全量快照、5,000 行分页、批内任职身份唯一性和 B0 writer 容量/事务阈值均已落到代码、自动化和隔离 PostgreSQL 实测；下一门禁是生产 migration、生产 HDD 路径与首次生产五段对账。
...
- B3-M2 已完成；生产迁移与首次同步仍需后续单独授权。隔离临时 tablespace 只证明 relation placement 契约，不证明生产路径为真实机械盘。

下一步是在用户单独授权后进入 B3-M3：生产应用 migration，核验 `gs_raw_cold_hdd` 的真实机械盘路径和全部 relation 归属，再做首次无参完整同步及源端、归一化、写入、拒绝原因、current/observation 的完整对账；不得提前创建 schedule。
