...
- 版本：v1
- 状态：一期 12 项均已完成生产 raw 直出 M3a；P1-B0/P1-B1/P1-B2 与 `stk_auction_o` 已结案，`stk_auction_c`、`moneyflow_ths`、`moneyflow_dc`、`stk_limit` 的 M3b 待 `2026-08-31` 自然观察
- 更新时间：2026-08-30
...
本文定义一期实施合同，并记录已获授权完成的 P1-B0-M1～M3、P1-B0 市场资金补充 M3b、P1-B1 M0、行业/概念/`margin` 的 M1/M2/M3a/M3b、`P1-GATE-SSE-M1/M2/M3`，以及 P1-B2 三项与 `P1-B3-stk_auction_o` 的 M0/M1/M2/M3a/M3b。`moneyflow_ind_dc` 已完成只读生产基线、代码、自动化测试、隔离 PostgreSQL migration、切换后生产只读合同与自然运行验收；完整部署在未暂停 schedule/worker 时自动应用 revision 153 的发布顺序偏差继续作为历史事实保留，不因最终验收通过而删除。`dc_daily`、`suspend_d` 已按维护窗口顺序完成生产 revision 154/155、连接池回收、查询合同、最小 TaskRun 与自然双工作流验收。`stk_auction_o` 生产已为 revision 156，日期完整性门禁经普通 vacuum 和真实基准关闭，TaskRun `9726` 与自然工作流验收均通过；但标准完整部署在本轮正式暂停 schedule/worker 前自动应用 revision 156 的顺序偏差同样保留。上述四项均已结案。`stk_auction_c`、`moneyflow_ths` 与 `moneyflow_dc` 已分别完成生产 revision 158/159/161 的 raw 直出切换和受控 TaskRun `10111/10128/10149`；各自 M3b 待 `2026-08-31` 自然观察。`stk_limit` 已按正确维护顺序完成 M0/M1/M2/M3a，生产为 revision 162、Raw 唯一物理事实表和 0 B Serving view；TaskRun `10182` 即时验收通过，M3b 待 schedule #24/#2 两个自然父任务分别验收。
...
一期启动前，12 个数据集都使用 `DatasetWriter.write()` 的 `raw_core_upsert` 分支；`_write_raw_and_core()` 会把同一个 `NormalizedBatch` 分别按 raw/serving ORM 列过滤后写入两层，且 12 个 Definition 的 `serving_conflict_resolution_policy` 均为 `none`。截至 2026-08-30，12 项均已完成生产 raw-only 切换；`stk_auction_c`、`moneyflow_ths`、`moneyflow_dc` 与 `stk_limit` 的 M3b 待 `2026-08-31` 自然观察。
...
| P1-B3 | 9 | `stk_auction_c` | `raw_tushare.stk_auction_c` → `core_serving.equity_auction_close` | `(ts_code, trade_date)` | 372.19 MiB | 2,255,593 / 2,255,593 | M0/M1/M2/M3a 通过；生产 revision 158、0 B Serving view，已释放 390,266,880 B；M3b 待自然观察 |
| P1-B3 | 10 | `moneyflow_ths` | `raw_tushare.moneyflow_ths` → `core_serving.equity_moneyflow_ths` | `(trade_date, ts_code)` | 468.18 MiB | 2,077,033 / 2,077,033 | M0/M1/M2/M3a 已通过；生产 revision 159、0 B view；M3b 待 schedule #4 自然观察 |
| P1-B4 | 11 | `moneyflow_dc` | `raw_tushare.moneyflow_dc` → `core_serving.equity_moneyflow_dc` | `(trade_date, ts_code)` | 1,131,995,136 B（约 1.054 GiB） | 4,151,016 / 4,151,016 | Ops/freshness；Lake raw；M0/M1/M2/M3a 已通过，M3b 待自然观察 |
| P1-B4 | 12 | `stk_limit` | `raw_tushare.stk_limit` → `core_serving.equity_stk_limit` | `(ts_code, trade_date)` | 664,354,816 B（633.58 MiB） | 4,608,112 / 4,608,112 | M0/M1/M2/M3a 已通过；生产 revision 162、0 B Serving view，已释放 664,354,816 B；M3b 待自然观察 |
|  |  | **合计** |  |  | **3,548,766,208 B，约 3.305 GiB** |  |  |

合计仍是 2026-08-23 全名单同一时点基线；各数据集行中的后续 M0 当前值用于本项设计，不把不同时点数据重新相加伪装成同一容量快照。
...
18. `P1-B2-dc_daily-M3a/M3b` 已完成并结案：生产按“两个 schedule 同时暂停 → 通用 worker 停止 → maintenance migration → 连接池回收 → 查询合同 → 最小 TaskRun → schedule 原样恢复”的固定顺序切换至 revision 154 和 0 B raw-backed view；TaskRun `9704` 及自然 TaskRun `9747/9773` 均通过。
...
20. `P1-B2-suspend_d-M1` 已完成 raw-only Definition、独立 revision 155、20,000 行/月 fail-closed、六字段与 `id/row_key_hash` 双身份对账、显式 raw-backed view、拒写/权限合同及专项自动化测试。
21. `P1-B2-suspend_d-M2` 已在仅 Unix socket 可达的 PostgreSQL 18.4 隔离实例完成 revision 155、20,000/20,001 行边界、字段及 `id/row_key_hash` 双身份差异、未知依赖 fail-closed、ACL/comment、三类 DML、正式 writer、raw 即时可见、事务回滚和三类代表查询计划验收；该结论不改变生产 revision 154。
22. `P1-B2-suspend_d-M3a/M3b` 已完成并结案：生产按维护窗口完成 revision 155、0 B raw-backed view、三类 DML、真实消费者查询、连接池回收、TaskRun `9717` 和两个 schedule 原样恢复；自然 TaskRun `9747/9773` 通过，生产切换时 raw/view 均为 640,504 行，serving 原物理 relation 的 222,199,808 B 已释放。
23. `P1-B3-stk_auction_o-M0/M1/M2/M3a/M3b` 已完成并结案：20 个自然月、2,183,621 行和全部 9 个业务字段的切换前双向 `EXCEPT ALL` 为 0，两层唯一 `(ts_code, trade_date)` 数一致；Definition、独立 revision 156、160,000/160,001 行隔离边界、回滚、权限/DML、普通 vacuum、生产查询、TaskRun `9726` 与自然 TaskRun `9747/9773` 均通过。生产现为 Raw 唯一物理事实表和 0 B Serving view；标准完整部署提前应用 migration 的发布顺序偏差继续保留。
24. `P1-B3-stk_auction_c-M0/M1/M2/M3a` 已完成：生产 M0 证明两层各 2,255,593 行和唯一身份、20 个月九字段等价及代表查询门禁；M1/M2 完成 Raw-only Definition、revision 158 与隔离门禁；M3a 按“暂停两个 schedule → 停 scheduler/worker → 最终全量对账 → maintenance migration → 连接池 → 查询与 DML → TaskRun → 恢复 schedule”的顺序切换生产。TaskRun `10111` 单日读取/保存 5,551/5,551，reject/去重/重试为 0；生产现为 Raw 唯一物理表与 0 B Serving view，已释放 390,266,880 B。M3b 待下一次自然双工作流观察。
25. `P1-B3-moneyflow_ths-M0` 已完成：生产两层各 2,077,033 行、21 个自然月全部 13 个业务字段双向差集为 0，最大自然月 116,993 行；Raw 索引、已知消费者、每日资金工作流、查询性能和迁移容量合同已经独立复审。生产仍为 revision 158、两张物理表和现有双写代码；M0 不改变生产状态。
26. `P1-B3-moneyflow_ths-M1` 已完成：Definition storage、Raw ORM 两个既存索引 metadata、独立 revision 159、专项和共享回归均已落地；生产仍为 revision 158 和旧双写部署，下一阶段为独立授权的隔离 PostgreSQL M2。
27. `P1-B3-moneyflow_ths-M2/M3a` 已完成：隔离 PostgreSQL 七项场景通过；生产按维护顺序应用 revision 159，Raw OID/索引保留、Serving 成为 0 B view，TaskRun `10128`、查询、拒写、连接池、schedule #4 暂停/恢复和服务健康均闭环。M3b 待 schedule #4 下一次自然工作流观察。
28. `P1-B4-moneyflow_dc-M0/M1/M2/M3a` 已完成：生产 revision 161、Raw 唯一物理事实表、Serving 0 B view、TaskRun `10149` 与查询/拒写/连接池/schedule #4 均通过，已释放 1,131,995,136 B；M3b 待自然观察。
29. `P1-B4-stk_limit-M0/M1/M2/M3a` 已完成：生产 revision 162、Raw 唯一物理事实表和 0 B Serving view，Raw OID/索引未变；32 月五字段与审计投影对账、三类拒写、两类查询及 TaskRun `10182` 均通过，已释放 664,354,816 B。M3b 待 schedule #24/#2 的下一次自然父任务分别验收。

尚未完成：

1. 一期 12 项都已完成自身 M0 的全历史业务字段双向 `EXCEPT ALL` 和生产 M3a raw 直出切换；
2. `stk_limit` 已在生产 M3a 复核市场情绪 join 与日期存在性结果、Raw 索引下推、零临时块和时延，未超过 20% 停止线；
3. 仓库外 SQL、BI、人工脚本和依赖 relation catalog 的工具尚未完成签字；
4. `suspend_d` 的 `id` 逐行一致已证明；`suspend_type` 多选输入与 planner fan-out 不一致已在 M1 前独立修复，并随生产 commit `9d32b266` 生效；
5. P1-B0 已运行生产 Biz 查询服务，但未做带登录态的浏览器验收；行业 relation 当前没有 Biz/QTF/DG serving 读取，已按 Ops/freshness、真实 SQL 和 TaskRun 完成验收；`moneyflow_ind_dc` 已完成生产查询、Heat source/readiness、自然 TaskRun 与结果回读验收；`dc_daily`、`suspend_d` 与 `stk_auction_o` 均已完成代表查询、最小 TaskRun 和两个既有 workflow 的自然 M3b；`stk_auction_c` 与 `stk_limit` 已完成代表查询和最小 TaskRun，两个既有 workflow 的自然 M3b 待观察。
6. `anns_d` 在 TaskRun `9747/9773` 中出现 69/184 行批内 `row_key_hash` 冲突，已另列独立只读数据质量审计 TODO；该问题不属于一期 12 个 raw 直出对象，且未授权源端请求、代码或生产数据修改。

因此，一期 12 项均已完成生产 raw 直出切换；尚未触发的自然 M3b 统一在第 8 节台账观察，不创建额外任务或重复请求源端。

### 2.4 当前代码消费者清单

2026-08-29 `stk_limit` M0 复审时 CodeGraph 索引为 current，覆盖 2,865 个文件、50,920 个节点和 128,640 条边；本轮重新审计了 Definition、request builder、source pagination、writer/DAO、raw/serving ORM、ServingPublish、Ops workflow/freshness/date completeness、Biz/QTF、前端、Lake Console、DG、测试和脚本，并用精确代码搜索补齐动态 registry 与 schema 字符串。此前各项的历史 CodeGraph 快照仍保留在其专属记录中。
...
| --- | --- | --- | --- | --- | --- |
| RD-001 | raw 成为唯一物理事实表 | 对应 Definition、现有 `raw_only_upsert` | writer 仅调用 raw DAO | serving DAO 零调用 | 12 项均已完成生产切换；`stk_limit` TaskRun `10182` 只写 Raw |
| RD-002 | 原 serving 名称和业务列合同不变 | 每数据集独立 migration、原 serving ORM | ORM/SQL 查询结果一致 | 禁止改 Biz/QTF/DG relation 名 | `stk_limit` 生产同名 view、五业务列、全表对账和两类查询 hash 已通过 |
| RD-003 | serving 禁止写入 | Definition + DB INSTEAD OF reject trigger | SELECT 正常 | INSERT/UPDATE/DELETE 均失败 | `stk_limit` 生产三类 DML 均返回 SQLSTATE `55000`，测试行残留 0 |
| RD-004 | 全量事实无差异 | 生产维护窗口分块对账 | 两层差集均为 0 | 任一差异阻断 migration | 12/12 已完成各自 M0 的全业务字段有界双向差集；`stk_limit` 为 32 月五字段全 0 |
| RD-005 | raw 索引覆盖查询 | raw ORM index 声明、生产索引、计划基准 | 代表查询使用等价索引 | 计划/延迟退化阻断 | `stk_limit` 生产两类查询结果、Raw 下推和零临时块通过，时延未超过 20% 停止线 |
...
| RD-008 | 不改变源端请求 | Definition 只改 storage | connector/request 测试不变 | 禁止改 fields/分页/date model | 12 项代码均保持原请求合同；`stk_limit` M1 已测试五字段、逐交易日 unit、5,800 行逐页 fields 与短页结束 |
...
| RD-010 | owner/grant/comment 可追溯 | migration 动态快照与恢复 | 非 owner SELECT 权限一致 | 未知 grant/comment 阻断 | `stk_limit` 生产 owner、Raw reader、Serving ACL 与空 comments 已恢复 |
| RD-011 | Ops 观测改读 raw | `target_table` 派生、freshness registry | 新 TaskRun/freshness 读 raw | 历史 TaskRun 不回写 | `stk_limit` M1 已把 freshness/date completeness target 改为 `raw_tushare.stk_limit` 并通过共享测试 |
| RD-012 | 失败不破坏 raw | 同事务 DDL、禁止自动 downgrade | 提交前失败原子回滚 | 禁止清表/删 raw | `stk_limit` M2 已验证失败回滚，生产迁移一次成功且 Raw OID/索引不变 |
...
| 最大单表当前行数 | `stk_limit` 4,608,112 行；`moneyflow_dc` 4,151,016 行（分别为 2026-08-29 M0 快照） |
...
| `P1-B2-dc_daily-M3b` | `dc_daily` | `daily_market_close_maintenance` schedule #24（18:30）与 schedule #2（21:02）；目标日 `2026-08-28` | 分别核对两个父 TaskRun 与 `dc_daily` node 的目标日、状态、读取、保存、reject、去重和分页；raw/view 当日行数、唯一身份与业务字段一致；第二次执行幂等且不制造重复 | **通过并结案：TaskRun `9747/9773` 两个节点均为 1 页短页、读取/保存 `1,031/1,031`，reject/去重/重试为 0；最终 Raw/view 各 1,031 行和身份，逐字段双向差异为 0，最终 `fetched_at` 来自第二轮原位更新** |
| `P1-B2-suspend_d-M3b` | `suspend_d` | 与 `dc_daily` 相同的 schedule #24（18:30）与 schedule #2（21:02）；目标日 `2026-08-28` | 分别核对两个父 TaskRun 与 `suspend_d` node 的目标日、状态、单页短页、读取、保存、reject、去重；raw/view 当日行数、`id/row_key_hash` 与六字段一致，第二次执行不制造重复 | **通过并结案：TaskRun `9747/9773` 两个节点均为 1 页短页、读取/保存 `7/7`，reject/去重/重试为 0；最终 Raw/view 各 7 行，`row_key_hash` 与源事实均唯一，六字段双向差异为 0，最终 `fetched_at` 来自第二轮原位更新** |
| `P1-B3-stk_auction_o-M3b` | `stk_auction_o` | 与前两项相同的 schedule #24（18:30）与 schedule #2（21:02）；目标日 `2026-08-28` | 分别核对两个父 TaskRun 与 `stk_auction_o` node 的目标日、状态、单页短页、读取、保存、reject、去重和重试；raw/view 当日 9 字段与唯一身份一致；结合既有生产重跑证据判断幂等 | **通过并结案：TaskRun `9747` 的 18:30 节点为 `0/0` 短页，TaskRun `9773` 的 21:02 节点读取/保存 `5,508/5,508`，均无 reject/去重/重试；最终 Raw/view 各 5,508 行和身份，九字段双向差异为 0。自然双跑证明 18:30 尚未就绪、21:02 正常就绪；幂等由生产 TaskRun `9726` 对已有日期重跑且未增行的证据共同闭环** |
| `P1-B3-stk_auction_c-M3b` | `stk_auction_c` | schedule #24 与 #2；2026-08-29 生产只读复核的下一次分别为 `2026-08-31 18:30+08`、`21:02+08` | 分别核对两个父 TaskRun 与 `stk_auction_c` node 的目标日、状态、分页、读取、保存、reject、去重和重试；Raw/view 当日九字段与唯一身份一致；结合 M3a TaskRun `10111` 判断已有日期重跑幂等 | **待自然观察；两个 schedule 均为 active，不创建额外任务、不重复请求源端。尚未触发或尚未统一查看不阻塞后续数据集独立 M0/M1/M2/M3a** |
| `P1-B3-moneyflow_ths-M3b` | `moneyflow_ths` | `daily_moneyflow_maintenance` schedule #4；2026-08-29 生产只读复核的下一次为 `2026-08-31 20:00+08` | 核对父 TaskRun 与 `moneyflow_ths` node 的目标日、状态、分页最终短页、读取、保存、reject、去重和重试；Raw/view 当日 13 字段与唯一身份一致；结合 M3a TaskRun `10128` 判断已有日期重跑幂等 | **待自然观察；schedule #4 为 active，cron `0 20 * * 1-5`、时区 `Asia/Shanghai`。2026-08-29 为周六，不会自然触发；不创建额外任务、不重复请求源端，待触发不阻塞后续数据集独立 M0/M1/M2/M3a** |
| `P1-B4-moneyflow_dc-M3b` | `moneyflow_dc` | 与 `moneyflow_ths` 相同的 `daily_moneyflow_maintenance` schedule #4；下一次为 `2026-08-31 20:00+08` | 在同一父 TaskRun 中独立核对 `moneyflow_dc` node 的目标日、两页分页与最终短页、读取、保存、reject、去重和重试；Raw/view 当日 15 字段与唯一身份一致；结合 M3a TaskRun `10149` 判断已有日期重跑幂等 | **待自然观察；schedule #4 已原样恢复 active。可以和 `moneyflow_ths` 在同一个父 TaskRun 中一起查看，但两个 node 必须分别闭环；不创建额外任务、不重复请求源端** |
| `P1-B4-stk_limit-M3b` | `stk_limit` | `daily_market_close_maintenance` schedule #24（`2026-08-31 18:30+08`）与 schedule #2（`21:02+08`） | 分别核对两个父 TaskRun 与 `stk_limit` node 的目标日、两页分页、最终短页、读取、保存、reject、去重和重试；Raw/view 当日五字段与唯一身份一致；结合 M3a TaskRun `10182` 判断已有日期重跑幂等 | **待自然观察；#24/#2 已原样恢复 active，config revision `133/134` 留痕。两个自然入口必须分别闭环，不能相互替代；不创建额外任务、不重复请求源端** |

#### 独立数据质量 TODO：`anns_d` 批内身份键冲突

本次 M3b 只读验收同时发现两个父 workflow 虽均为 `success`，但其 reject 分别为 69 与 184 行，全部来自 `anns_d` 节点，reason code 均为 `write.duplicate_conflict_key_in_batch:row_key_hash`：TaskRun `9747` 为读取/保存/reject `6,358/6,289/69`，TaskRun `9773` 为 `6,616/6,432/184`。三个本期目标节点 reject 均为 0，因此该问题不重新打开 `dc_daily`、`suspend_d` 或 `stk_auction_o` 的结案状态，也不归因于 raw 直出 migration。

`anns_d` 当前 `row_key_hash` 由 `ann_date, ts_code, title, url, rec_time` 计算，源字段中的 `name` 不参与身份；现有证据只能证明同一批次发生相同身份键，不能在未比较全部六个 source fields 前把这些行称为“完全重复”。后续须另立只读数据质量审计，核对 reject sample 与已保存行的六字段、确认是否只有 `name` 差异或存在其它归一化碰撞，并评估应在归一化阶段计为完全重复去重、调整身份合同，还是继续 fail-closed。该 TODO 不授权请求 Tushare、修改 Definition/transform/writer 或改写生产数据。
...
`P1-B2-dc_daily-M3a` 据此通过。`P1-B2-dc_daily-M3b` 已于 2026-08-29 依据 TaskRun `9747/9773` 和目标日 `2026-08-28` 的生产只读对账关闭；M0/M1/M2/M3a/M3b 全部通过，本数据集结案。

#### 2026-08-28 `P1-B2-suspend_d-M0` 只读复审

本阶段只读取当前代码、CodeGraph、生产 catalog、受控 SQL 与既有 TaskRun 诊断；没有修改代码、请求 Tushare、创建 TaskRun、部署、执行 migration 或修改 schedule。生产部署 commit `4ef82716` 与本地 HEAD 在 `suspend_d` 的 Definition、ingestion、DAO/ORM、Ops 和 Biz 消费链上没有差异；CodeGraph 索引为 current，覆盖 2,784 个文件、48,998 个节点和 113,601 条边，并以精确搜索补齐动态 DAO registry、Lake raw 读取和 DG 独立资产。

1. 当前 Definition 显式请求 `ts_code, trade_date, suspend_timing, suspend_type` 四个源字段；point/range 输入由 SSE 交易日历展开为逐交易日 unit，单 unit 使用 `offset/limit=5,000` 分页、短页结束、无任意最大页数。存储仍为 `raw_core_upsert`，同一归一化批次先后写入 `raw_tushare.suspend_d` 与 `core_serving.equity_suspend_d`，冲突键为 `row_key_hash`；未来 M1 只能把本数据集切到既有 `raw_only_upsert`，不能修改共享 writer、request builder、日期/unit 或分页合同；
2. 两层均为 `pg_default` 普通物理表，业务列固定为 `id, row_key_hash, ts_code, trade_date, suspend_timing, suspend_type`。两层各 640,504 行、640,504 个唯一 `id` 和 640,504 个唯一 `row_key_hash`，日期范围 `2000-01-04..2026-08-27`；320 个自然月按全部 6 个业务字段做双向 `EXCEPT ALL`，两个方向差异月份和差异行均为 0，逐 hash 连接的 `id`/内容差异也均为 0。月峰值是 `2015-07` 的 17,074 行，因此后续独立 migration 的月容量门禁冻结为 20,000 行；执行前仍须重新采样，超过即在 serving DDL 前失败，不能放宽或截断；
3. raw 总大小 227,672,064 B，serving 总大小 222,199,808 B（211.91 MiB），后者是当前可释放毛量。两层都有等价的 `trade_date` 与 `(ts_code, trade_date)` 二级索引，全部 valid/ready 且位于 `pg_default`；raw 现有 `lake_raw_reader` SELECT 必须保持，不新增索引，不把 raw 移往 HDD；
4. owner 均为 `goldenshare_user`；raw 额外授予 `lake_raw_reader` SELECT，serving 仅 owner 权限。未发现外部 FK、依赖 view/materialized view、用户函数依赖、trigger/rule、RLS/policy、publication、继承、扩展统计、security label、relation/column comment 或列级 ACL。未来 migration 必须继续按自身白名单 fail-closed，禁止复用 `dc_daily` 的字段、主键、容量或 ACL 结论；
5. 已登记 serving 消费者包括 Wealth 指数详情、板块成员、连板梯队、有效 A 股池、板块热度源、市场情绪计算及 walk-forward 间接链；Lake Console 已直接读取 raw，DG 的正式 Lake 资产走独立源链，不读取该 serving relation。`2026-08-27` 的停牌点查、`BK0596.DC` 板块成员查询以及 `2026-03-06..2026-08-27` 日期范围 join 的 raw/serving 结果双向差异均为 0；日期范围样本包含 660,861 根日线、355 个停复牌连接命中；
6. 点查两层均使用各自 `trade_date` 索引，执行时间为 raw/serving `0.112/0.057 ms`；板块成员查询均使用 `trade_date` 索引，执行时间为 `8.641/6.587 ms`；真正消费连接结果的 120 个开市日范围查询均对停复牌子查询使用 `trade_date` 索引，返回 2,050 个唯一停复牌身份、2,063 个源行扫描，单次冷缓存诊断为 `7.966/8.599 s`。该范围查询主要成本来自 660,861 行日线扫描，本轮只冻结计划形态和结果基线，不能以单次冷热顺序宣称 raw 更快；M2/M3a 仍须做切换前后交错性能验收；
7. 两层 sequence 都处于 `642263`；当前 writer 的通用 upsert 会把未显式提供的 `id=excluded.id` 更新回两层，因此历史上两层物理 `id` 保持一致。未来 view 必须显式投影 raw `id`，M1 测试必须证明 `row_key_hash` 冲突写入后 raw/view 的 `id` 仍一致，但禁止为本项修改共享 `BaseDAO`。raw `fetched_at` 与 serving `updated_at` 全部相同，1,042 行历史 `created_at` 不同；已登记消费者不读取审计列，所以仍采用 `fetched_at AS created_at/updated_at`，不宣称历史 `created_at` 透明；
8. 自动写入口不是独立 dataset schedule，而是同一个 `daily_market_close_maintenance` workflow 的两个 active schedule：#24 在 18:30、#2 在 21:02。最近 `2026-08-27` 两个 `suspend_d` node 均成功，各为 1 unit、1 页短页、读取/保存 4 行、0 reject、0 去重、0 重试；M3a 必须同时暂停/恢复两个 schedule，并在开放 TaskRun、目标锁、长事务和磁盘门禁通过后才能停止 worker。M0 快照时开放 TaskRun、目标 relation 锁和超过 30 秒事务均为 0；根盘可用 52,380,954,624 B，HDD 可用 320,993,443,840 B，生产 Alembic 为 `20260828_000154`；
9. M0 发现一个与 raw 直出无关、但会破坏手动 filter 正确性的现存契约缺陷：Ops 把 `suspend_type` 暴露为多选，执行策略文档要求多值按枚举 fan-out，但 Definition 没有把它登记进 `enum_fanout_fields`，request builder 又直接执行 `str(value).upper()`。正式 resolver 证明单选列表会生成 `suspend_type="['S']"`，双选会生成 `suspend_type="['S', 'R']"`，而不是一个或两个合法的 `S/R` 请求；旧开发文档又写成单选，三处口径互相冲突。自动 workflow 不带 filter，因此既有自然任务未受影响；但 M1 的硬边界是“不改请求语义”，不能把缺陷混入 storage migration 顺手修复。

随后取得的修复授权只处理该缺陷。`tushareMcp` 对 `20260827` 的实测为：无类型 4 行、`S` 3 行、`R` 1 行，`S/R` 并集与无类型完整结果一致；当前错误字面量 `"['S', 'R']"` 被源端以 `50101` 拒绝。实现据此保留 Ops 多选，把 `suspend_type` 登记为 Definition 枚举 fan-out；未选保持一个无过滤 unit，单选保持一个合法单值 unit，多选按日期与去重后的 `S/R` 形成单值 unit。request builder 对任何未展开 list/tuple/set 直接 fail-closed，不再允许字符串化后调用源端。专项测试覆盖 point/range、无过滤、单选、多选、非法枚举和未展开列表拒绝；执行策略和数据集开发文档已统一。
...
#### 2026-08-28 `P1-B2-suspend_d-M2` 隔离 PostgreSQL 验证

M2 在本轮创建的 PostgreSQL 18.4 临时实例完成。实例只监听 `/private/tmp` 下独立 Unix socket，`listen_addresses=''`、应用连接的 `inet_server_addr()` 为空；应用 migration 的 `suspend_d_m2_app` 为非超级用户。每次 Alembic 调用前都使用只包含目标 URL 的独立 `GOLDENSHARE_ENV_FILE`，先以应用连接核对 URL、数据库、用户和恢复状态，再以临时实例管理员只读核对 socket、端口与 `data_directory`；没有给应用角色增加读取系统设置的额外权限。全程没有连接 Prod、请求 Tushare、部署、创建 TaskRun 或修改 schedule/workflow。

1. 受控正向库包含 `2026-07` 的精确 20,000 行和次月 4,000 行，revision 154→155 成功；raw relation OID 与四个 raw 索引 OID 全部保持，serving 变为 0 B 普通 view，24,000 行、24,000 个 `id`、24,000 个 `row_key_hash` 及双向 `EXCEPT ALL` 全部一致；
2. owner、raw reader SELECT、serving reader `SELECT WITH GRANT OPTION`、relation comment 和 column comment 均恢复；独立拒写 trigger 为 enabled，serving 的 `INSERT/UPDATE/DELETE` 分别以 SQLSTATE `55000` 拒绝；
3. raw 事务内新增行会立刻出现在 view，回滚后 raw/view 均无残留。正式 `DatasetWriter` 对现有 `row_key_hash` 做冲突更新后，raw/view 同时投影新 `id=24001` 与新内容；事务回滚后恢复原 `id=1` 和原内容，证明 raw-only writer 与 view 的身份一致及回滚合同；
4. 20,001 行单月库以 `monthly reconciliation exceeds safety cap` 在 DDL 前失败；业务字段差异、`id` 差异、`row_key_hash` 差异均以 `suspend_d monthly mismatch` 失败；未知 view 依赖以 `Unexpected view dependency` 失败。五组负向库的 revision 154、relation/index OID、relkind与行数快照均保持原样；
5. 另一个 revision 154 库在完成 preflight、锁和 serving 切换后注入事务错误，回滚后两张物理表、全部表/索引 OID、relkind、行数和 revision 与切换前完全一致；
6. 交易日点查、`ts_code+trade_date` 点查和最大日期三类查询的结果 SHA-256 在切换前后完全一致，并分别从 serving 的日期/复合索引等价下推到 raw 的 `idx_raw_tushare_suspend_d_trade_date` 或 `idx_raw_tushare_suspend_d_ts_code_trade_date`。总成本分别为 `494.56→572.75`、`8.31→8.31`、`0.38→0.39`；第一项受临时样本行宽和统计信息影响增加 15.8%，仍低于 20% 门禁，三项均无临时块读写。隔离微秒耗时只证明计划形态，不替代 M3a 对生产真实消费者的交错重复测量。

`P1-B2-suspend_d-M2` 据此通过，未发现需要修改 revision 155 或业务代码的问题。生产仍是 revision 154，raw/serving 仍为两张物理表；下一阶段只能在独立授权后进入 `P1-B2-suspend_d-M3a`，实时复核部署版本、Alembic head、640,504 行基线漂移、320 月全字段/双身份差异、磁盘/锁/长事务/开放 TaskRun，并同时暂停 schedule #24 与 #2、停止相关 worker、应用 migration、回收连接池及完成真实查询和最小 TaskRun 验收。

#### 2026-08-28 `P1-B2-suspend_d-M3a` 生产切换与即时验收

M3a 于 `13:08..13:21+08` 完成。切换前生产为 PostgreSQL 16.13、revision 154、远端干净 commit `4ef82716`；raw/serving 都是 `pg_default` 物理表，各 640,504 行和 640,504 个唯一 `id/row_key_hash`，日期范围 `2000-01-04..2026-08-27`。320 个自然月六字段双向差异为 0、月峰值 17,074；开放 TaskRun、目标锁、等待锁、超过 30 秒事务和外部 view 依赖均为 0。根盘/HDD 可用空间分别为 52,420,927,488 B/320,991,494,144 B。

1. schedule #2/#24 原为 active，cron 分别为 `2 21 * * 1,2,3,4,5` 和 `30 18 * * 1,2,3,4,5`。通过正式 `OperationsScheduleService` 逐个暂停并回查，配置审计 revision 109/110 分别记录 `paused`；随后停止通用 worker，再次完成相同全量对账和运行门禁；
2. 只把本项 M1 所需的 `9d32b266` 推到 `origin/dev-interface`；生产用 `--maintenance-migration` 从 `4ef82716` 快进到该 commit，跳过前端/Wealth 构建、seed、unit 同步和服务自动重启。revision 154→155 在 7 秒内成功，未触发 lock/statement timeout；
3. raw relation 与索引保持，serving 成为 0 B 普通 view，显式 8 列、owner、ACL 与独立拒写 trigger 正确。raw/view 都是 640,504 行、640,504 个唯一 `id/row_key_hash`，六字段全表双向差异为 0；原 serving 物理大小 222,199,808 B（211.91 MiB）即本项 catalog 毛释放量；
4. serving 的 `INSERT/UPDATE/DELETE` 以三个独立事务实测，均在数据变化前以 SQLSTATE `55000` 拒绝。Web、QTF、scheduler 与日期完整性 worker逐个重启并确认 active；generic worker在最小任务前启动，最终全部相关服务 active、两个健康端点为 200；
5. 点查、`BK0596.DC` 板块成员和市场情绪 `2026-03-06..2026-08-27` 日期范围 join 的 raw/view 结果校验值分别完全一致。三轮交错中位耗时 raw/view 为 `0.131/0.125 ms`、`30.837/32.712 ms`、`2415.554/1589.289 ms`；板块查询 view 慢 6.1%，其余无退化。三类计划均下推 `idx_raw_tushare_suspend_d_trade_date`，无临时块读写；市场情绪缓存顺序造成的加速不作为 SLA 收益宣称；
6. 通过正式 `ManualActionCommandService → resolver/planner → TaskRun` 创建唯一 TaskRun `9717`，目标日 `2026-08-27`。任务 1/1 unit 成功，worker 日志证明 `offset=0, limit=5000, page_rows=4, is_short_page=True`；读取/保存为 4/4，reject/去重为 0。目标日 raw/view 都是 4 行、4 个 `id`、4 个 `row_key_hash`，双向六字段差异为 0；全表仍为 640,504 行，证明重复同步幂等；
7. TaskRun 完成且开放任务回到 0 后，按正式 service 逐个恢复 schedule #2/#24；config revision 111/112 记录 `resumed`。cron、时区和 `last_triggered_at` 均未变，`next_run_at` 仍为当日 21:02/18:30。最终生产 commit 为 `9d32b266`、revision 155、远端工作区干净，目标锁和超过 30 秒事务为 0；根盘/HDD 可用空间分别为 52,642,947,072 B/320,991,494,144 B。

`P1-B2-suspend_d-M3a` 据此通过。`P1-B2-suspend_d-M3b` 已于 2026-08-29 依据 TaskRun `9747/9773` 和目标日 `2026-08-28` 的生产只读对账关闭；M0/M1/M2/M3a/M3b 全部通过，本数据集结案。

#### 2026-08-28 `P1-B3-stk_auction_o-M0` 只读复审与后续门禁

本阶段只读取当前代码、CodeGraph 和生产 PostgreSQL，没有请求 Tushare、创建 TaskRun、部署、migration、暂停 schedule 或修改业务数据。CodeGraph 索引为 current，覆盖 2,787 个文件、49,059 个节点和 124,757 条边；分析覆盖 Definition、request builder、writer、raw/serving ORM 与 DAO、workflow、freshness/date completeness、ServingPublish、Biz/QTF/DG/frontend/Lake 和测试，并用精确搜索补齐动态 registry 和 relation 字符串消费者。

1. Definition 当前显式请求 `ts_code, trade_date, close, open, high, low, vol, amount, vwap` 九字段；point/range 由 planner 按开市日生成单日 unit，request builder 每个 unit 只发 `trade_date` 和可选 `ts_code`，source client 使用 `limit/offset`、`page_limit=10000`。当前仍为 `raw_core_upsert`，同一归一化批次写两层；未发现 serving 专属转换、行过滤、聚合、冲突消解、ServingPublish mapping 或 serving DML 旁路。M1 禁止改变这些源端、日期、分页、planner、workflow 和共享 writer 合同；
2. 生产 PostgreSQL 16.13、revision `20260828_000155`。raw OID `808825`、serving OID `808845`，均为 `pg_default` 普通持久表，owner 为 `goldenshare_user`，无用户 trigger、RLS、publication、外部 view 依赖、security label 或扩展统计；raw 额外保留 `lake_raw_reader SELECT`。两层 9 个业务字段名称、类型、nullability 和主键 `(ts_code, trade_date)` 一致，日期索引签名等价且有效/ready。M0 快照时开放 TaskRun、开放目标 node、等待目标锁和超过 30 秒事务均为 0；根盘/HDD 可用空间分别为 52,590,292,992 B/320,990,502,912 B，仅作为只读水位证据，不替代 M3a 实时预检；
3. raw/serving 各 2,183,621 行、2,183,621 个唯一身份、0 个空身份，日期范围 `2025-01-02..2026-08-27`。20 个自然月逐月对九字段双向 `EXCEPT ALL` 的 raw-only/serving-only 都为 0；401 个交易日的日行数为最小 5,164、中位 5,456、P95 5,509、最大 5,535。月峰值是 `2026-07` 的 126,364 行，因此本项 M1 migration 的月容量门禁固定为 **160,000 行/层/月**，M2 必须证明 160,001 行在任何 serving DDL 前 fail-closed；
4. raw 总大小 411,148,288 B（392.10 MiB），serving 总大小 382,353,408 B（364.64 MiB）。serving 当前物理大小是本项 catalog 毛释放量；raw 及两个索引继续留 SSD，不新增索引。`raw.fetched_at = serving.updated_at` 对全部行成立，但 93,560 行的 `serving.created_at` 与最新同步时间不同；view 以后按既有一期合同把 `fetched_at` 投影为 `created_at/updated_at`，不承诺历史审计时间逐行透明，且当前未发现这些字段的业务消费者；
5. 已知仓库消费者只有 Ops Catalog、freshness、日期完整性和 TaskRun 观测；未发现 Biz、QTF、DG、frontend 或 Lake Console 对 serving 的直接读取。仓库外 SQL/BI/catalog 工具仍是执行前登记边界。自动入口是同一 `daily_market_close_maintenance` 的 schedule #24（18:30）与 #2（21:02）；最近自然节点中，18:30 每日为 0 行短页，21:02 为约 5,500 行单页短页且 reject/去重/重试为 0。未来 M3a 必须同时暂停/恢复两个 schedule，M3b 也必须按两个父 workflow 的目标 node 分别验收；
6. 三轮交错只读基准的 raw/serving 结果校验值完全一致：最大日期中位耗时 `0.112/0.135 ms`，单日 5,512 行 `11.551/11.129 ms`，单股全历史 401 行 `1.475/1.293 ms`，五个交易日全市场 27,506 行 `54.081/53.961 ms`，均无临时块读写且退化低于 20%；
7. 日期完整性查询同样返回一致的 81 个日期，但 raw/serving 中位耗时为 `75.683/54.636 ms`，raw 慢 38.5%。两层都走日期 index-only scan；raw 的 all-visible page 比例仅 80.18%、发生 456,208 次 heap fetch，serving 为 92.91% 和 188,095 次。两表约 9.9 万 dead tuple，默认 autovacuum `50 + 0.2 * reltuples` 阈值约 42 万行，当前不会自然触发及时 vacuum。该差异属于表维护状态，不是字段、索引或 view 计划缺陷，但已经超过固定性能门禁，不能忽略；
8. 因此 M0 结论为**条件通过**：M1/M2 可在独立授权后准备和验证 raw-only Definition、独立 migration、160,000/160,001 行、九字段逐月差异、依赖/ACL/comment、三类 DML、事务回滚、即时可见与代表查询；生产 M3a 暂不放行。未来 M3a 必须在 schedule #2/#24 已暂停、worker 已停止、开放任务/长事务/等待锁为 0 后，先执行经本阶段单独授权的普通 `VACUUM (ANALYZE) raw_tushare.stk_auction_o`，再交错重复日期完整性查询三轮。若 raw 仍比 serving 慢超过 20%、结果不一致、出现临时块或计划异常，必须在 migration 前停止，不得 drop serving 表。普通 vacuum 只改善可见性和计划统计，不计入空间释放量，也不得写进 Alembic migration。

#### 2026-08-28 `P1-B3-stk_auction_o-M1` 编码与自动化测试

M1 严格限定为 `stk_auction_o` 自身的 Definition storage contract、独立 migration、专项/回归测试和文档；没有连接数据库、请求 Tushare、部署、执行 migration、创建 TaskRun、修改 schedule/workflow 或运行 vacuum。CodeGraph 与精确引用审计覆盖 Definition、resolver/request builder、writer/DAO、raw/serving ORM、ServingPublish、freshness/date completeness、两个 workflow 入口及 Biz/QTF/DG/frontend/Lake 消费者；确认不需要修改共享主链。

1. Definition 只把 `core_dao_name/target_table/delivery_mode/layer_plan/write_path` 收敛为既有 `raw_stk_auction_o`、`raw_tushare.stk_auction_o`、`raw_with_serving_view`、`raw->serving_view` 和 `raw_only_upsert`。九个显式 source fields、开市日 point/range、可选 `ts_code`、`offset/limit`、`page_limit=10000`、manual/schedule/retry、freshness 与两个 `daily_market_close_maintenance` 入口不变；未知 filter 继续在 resolver 拒绝。
2. Raw ORM 已精确覆盖原 serving 的九个业务字段、`(ts_code, trade_date)` 主键和单列日期索引，因此没有 ORM、DAO factory、共享 writer、索引或 ServingPublish mapping 改动。参数化 writer 测试证明 Definition 进入现有 `raw_only_upsert` 后只写 raw DAO。
3. 独立 revision `20260828_000156` 接编码时唯一真实 head `20260828_000155`。migration 以 15 秒锁超时、120 秒语句超时、16 MiB work_mem 运行；在 serving DDL 前按 raw→serving 顺序取得 `SHARE` 锁，验证精确 relation/owner/raw SSD/列/主键/单列日期索引/ACL/依赖合同，并按自然月比较九字段双向 `EXCEPT ALL` 与两列身份唯一性。每层每月上限为 160,000 行，超限或任何差异都必须在 `ACCESS EXCLUSIVE` 和 `DROP TABLE` 前 fail-closed。
4. 切换仅删除 serving 物理表并创建显式 11 列普通 view；raw 不做 DDL/DML、不新建索引，禁止 `CASCADE` 和自动 downgrade。view 恢复原 owner、非 owner SELECT ACL、relation/column comments，并复用既有受控函数加独立 trigger 拒绝三类 DML。原 `created_at/updated_at` 按一期既定边界映射 raw `fetched_at`。
5. 自动化门禁覆盖 Definition/plan/filter、source fields/分页、raw-only writer、ORM/索引、ServingPublish 无旁路、migration 资源/对象/数据/锁顺序/离线 SQL/拒写/回滚边界，以及 registry、resolver、Ops workflow/freshness 和架构依赖回归。

`P1-B3-stk_auction_o-M1` 据此通过。生产仍为 revision 155 和两张物理表；下一阶段是独立授权的 M2，在隔离 PostgreSQL 真实应用 revision 156 并验证 160,000/160,001 行、权限恢复、三类 DML、事务回滚、raw 写入/view 即时可见和代表查询计划。日期完整性 raw 性能门禁仍只允许在未来 M3a 维护窗口内先 vacuum 后复测，M1/M2 不得提前修改生产。

#### 2026-08-28 `P1-B3-stk_auction_o-M2` 隔离 PostgreSQL 验收

M2 使用 `listen_addresses=''`、仅随机 Unix socket 可达的 PostgreSQL 18.4 临时实例。每个库在 Alembic 前都通过应用角色核对最终配置 URL、数据库、用户和 `inet_server_addr=NULL`，再由临时实例管理员核对 port/socket/data directory；没有连接生产、请求源端、部署、TaskRun、schedule 或生产 vacuum。所有场景完成后实例停止，成功数据目录删除。

1. 160,000 行边界库由 revision 155 成功应用 156。Raw relation OID `16391`、PK/date index OID `16402/16404` 保持不变，均 valid/ready 且在 `pg_default`；Serving 由物理表 OID `16405` 切为 view OID `16421`、0 B。Raw/view 各 160,000 行和身份，九字段、审计投影双向差异均为 0；owner、Raw/Serving SELECT ACL、grant option、relation/column comments 和独立 trigger 恢复。
2. Serving `INSERT/UPDATE/DELETE` 全部返回 SQLSTATE `55000`。Raw 插入/更新/删除即时反映到 view，事务回滚后无残留；正式 `DatasetWriter` 写入一个既有身份时只报告目标 `raw_tushare.stk_auction_o`，Raw/view 值和时间戳同步，回滚后原值恢复。
3. 160,001 行库在 DDL 前因容量上限失败；业务字段差异、身份差异和外部 view 依赖三个库分别在 DDL 前失败。四库的 revision、relation OID/类型、索引、行数、comments 和 trigger 快照均保持原状。
4. 回滚库在同一 migration transaction 完成 serving `DROP TABLE -> CREATE VIEW -> trigger` 后注入失败；回滚后 revision 155、两张物理表、两层 relation/索引 OID、索引定义与 valid/ready、ACL/comments、行数和零用户 trigger 与执行前一致。
5. 单日 5,714 行、单股单日、最大日期和 10 日日期完整性四类查询的结果 hash 一致，切换后分别下推 Raw date index/PK/date index，且无临时块。实测耗时前后为 `2.520/2.661 ms`、`0.005/0.007 ms`、`0.005/0.007 ms`、`8.306/8.758 ms`；最大正向退化约 5.4%，只作为隔离计划证据。

`P1-B3-stk_auction_o-M2` 据此通过，revision 156 无需修改。生产仍为 revision 155 和两张物理表。下一阶段只能是另行授权的 M3a：实时预检、暂停 #2/#24、停止目标 worker、对 Raw 执行普通 `VACUUM (ANALYZE)`，再交错重测生产日期完整性；退化不超过 20% 且结果/计划/临时块均通过后，才允许应用 156。M2 结果不能替代这项生产门禁。

#### 2026-08-28 `P1-B3-stk_auction_o-M3a` 生产即时验收

M3a 于 `14:56..15:05+08` 完成。实时预检发现，一次标准完整部署已在正式暂停 schedule/worker 之前拉取 commit `bbb3befc`、自动应用 revision 156，并于 `14:58` 重启 Web、scheduler 与 worker。该顺序违反第 5.4 节，不能补造迁移前门禁；本轮没有重复 migration。发现时远端代码已包含 raw-only Definition，Serving 已为 view，开放任务、目标 node、等待锁和长事务均为 0，没有观测到旧双写代码向 view 写入。

1. 生产 Raw OID `808825` 和原 PK/日期索引保持不变，索引 valid/ready 且位于 `pg_default`；Serving 为 OID `2032797`、0 B 普通 view。view 显式投影 9 个源字段和 `fetched_at AS created_at/updated_at`，owner、Raw `lake_raw_reader SELECT`、Serving ACL 和独立拒写 trigger 均存在；`INSERT/UPDATE/DELETE` 各以 SQLSTATE `55000` 拒绝。
2. schedule #2/#24 通过正式 service 暂停，config revision `113/114` 记录 `paused`；通用 worker 停止后再次确认开放 TaskRun、目标 node、目标锁、等待锁和超过 30 秒事务为 0。随后执行普通 `VACUUM (ANALYZE) raw_tushare.stk_auction_o`，未使用 `VACUUM FULL`、未改 relation/index/tablespace；Raw all-visible page 从 80.18% 提升到 100%，dead tuple 统计归零。
3. M0 留存的旧物理 Serving 日期完整性三轮中位数为 `54.636 ms`；vacuum 后 Raw 为 `50.751 ms`，约快 7.1%，20% 门禁关闭。当前 Raw/view 五类查询校验值全部一致、计划均下推 Raw 日期索引或 PK、临时块为 0；日期完整性 Raw/view 为 `50.751/53.614 ms`。因 Serving 已提前成为 view，本结论明确使用 M0 旧物理基线作跨时点对照，不伪装成同一时点迁移前测试。
4. Web、日期完整性与 TaskRun 收尾连接池回收，健康端点正常。通用 worker 启动后，正式 Manual Action 创建 TaskRun `9726`，只请求 `2026-08-27` 一个 point unit：`1/1` 成功，1 页短页读取/保存 `5,512/5,512`，reject、去重、重试为 0。目标日 5,512 行全部在任务时间窗刷新，Raw/view 各 5,512 行和唯一身份，9 字段双向差异为 0；全表各 2,183,621 行和唯一身份。
5. 开放任务归零后，schedule #2/#24 通过正式 service 恢复，config revision `115/116` 记录 `resumed`；cron、时区、下一次执行和上次触发时间不变。最终相关服务全部 active，健康端点正常，开放任务、目标锁、等待锁和长事务均为 0。

`P1-B3-stk_auction_o-M3a` 据此**通过，但保留标准部署提前应用 migration 的发布顺序偏差**。原 Serving relation 的 `382,353,408 B（364.64 MiB）` 已释放，Raw 继续留 SSD。`P1-B3-stk_auction_o-M3b` 已于 2026-08-29 依据 TaskRun `9747/9773` 和目标日 `2026-08-28` 的生产只读对账关闭；结合 TaskRun `9726` 的已有日期重跑证据，M0/M1/M2/M3a/M3b 全部通过，本数据集结案。

#### 2026-08-29 `P1-B3-stk_auction_c-M0` 只读复审与后续门禁

本阶段只读取当前代码、CodeGraph、生产 catalog、受控只读 SQL 与既有 TaskRun 诊断；没有修改代码、请求 Tushare、创建 TaskRun、部署、执行 migration、暂停 schedule 或写生产数据库。CodeGraph 索引为 current，覆盖 2,835 个文件、50,143 个节点和 127,454 条边；分析覆盖 Definition、resolver/request builder、writer、raw/serving ORM 与 DAO、Ops workflow/freshness/date completeness、ServingPublish、Biz/QTF/DG/frontend/Lake 和测试，并用精确搜索补齐动态 registry 与 relation 字符串消费者。

1. 当前 Definition 仍是 9 个显式 source fields、交易日 point/range、每个开市日一个 unit、`limit/offset` 10,000 行分页和可选 `ts_code`；storage 仍为 `raw_core_upsert`。`DatasetWriter._write_raw_and_core()` 把同一归一化批次分别按两个 ORM 投影后 upsert，业务主键均为 `(ts_code, trade_date)`，无 serving 冲突消解。
2. 生产 Alembic 为 `20260828_000156`。Raw/Serving 均为 `pg_default` 物理表，大小分别为 424,484,864 B 与 390,266,880 B；两边各 2,255,593 行、2,255,593 个唯一身份，日期范围均为 `2025-01-02..2026-08-28`。列类型、非空合同、主键和交易日索引一致且索引 valid/ready；没有外键、外部 view/materialized view、用户 trigger、RLS、publication、扩展统计或安全标签依赖。
3. 对 20 个自然月逐月比较全部 9 个业务字段，双向 `EXCEPT ALL` 均为 0；月峰值为 132,619 行。`fetched_at -> updated_at` 全表一致；93,769 行的 serving `created_at != updated_at`，因此未来 view 的历史 `created_at` 会收敛到 Raw `fetched_at`，但当前仓库内未发现审计时间消费者。
4. M1 容量上限固定为每层每月 160,000 行：它是本数据集 132,619 行峰值加约 20.6% 余量，并经 97 B 最大业务行宽与 16 MiB `work_mem` 下的 132,619 行单向差集实测支撑；M2 必须独立验证 160,000 行通过、160,001 行在任何 serving DDL 前 fail-closed，不能只引用 `stk_auction_o` 的相同数值。
5. 生产查询结果/hash 一致，Raw/Serving 三轮中位数分别为：单日完整字段 11.516/11.041 ms、单股票历史 1.162/1.236 ms、5 日范围 58.333/56.401 ms、日期完整性 61.834/54.941 ms；Raw 最慢相对退化 12.55%，低于 20% 停止阈值，计划均使用 Raw 对应主键或交易日索引，临时块为 0。Raw all-visible 页比例为 95.21%，M0 不要求生产 vacuum；未来 M3a 仍要实时复测。
6. 仓库内未发现 Biz/QTF/DG/frontend/Lake 对 Serving 的直接读取，也未发现 ServingPublish 或显式 serving DML 旁路。已知消费者为 Ops Catalog、freshness、日期完整性和 TaskRun；自动写入口为 `daily_market_close_maintenance` schedule #24（18:30）与 #2（21:02）。最近自然节点表现为 18:30 空短页、21:02 约 5,550 行，后续必须把两者当成源端就绪时序分别验收。
7. M0 时 Web、scheduler、worker 等正式服务 active，开放 TaskRun、目标 node、目标锁与超过 30 秒事务均为 0；根盘可用 48.58 GiB，HDD 可用 298.82 GiB。该快照只证明 M0 当时环境安全，不能替代未来 M3a 的实时预检。
...
#### 2026-08-29 `P1-B3-stk_auction_c-M2` 隔离 PostgreSQL 验收

M2 使用 `listen_addresses=''`、仅随机 Unix socket 可达的 PostgreSQL 18.4 临时实例。每个数据库在 Alembic 前都由应用角色核对最终 `get_settings()` URL、数据库名、用户和 `inet_server_addr=NULL`，并由实例管理员核对端口、socket 与 data directory；没有连接生产、请求源端、部署、TaskRun、schedule 或生产 DDL。全部场景完成后实例已停止，成功数据目录已删除。

1. 160,000 行边界库从 revision 157 成功应用 158。Raw relation OID `16391`、PK/date index OID `16402/16404` 保持不变，索引 valid/ready 且位于 `pg_default`；Serving 由 OID `16405` 物理表切为 OID `16421`、0 B view。Raw/view 各 160,000 行和身份，九字段与审计投影双向差异均为 0；owner、Raw/Serving SELECT ACL、grant option、relation/column comments 和独立 trigger 恢复。
2. Serving `INSERT/UPDATE/DELETE` 全部返回 SQLSTATE `55000`。Raw 插入、更新、删除即时反映到 view，事务回滚后无残留；正式 `DatasetWriter` 写既有身份时只报告目标 `raw_tushare.stk_auction_c`，Raw/view 值与时间戳同步，回滚后原值恢复。
3. 160,001 行库在 DDL 前因容量上限失败；业务字段差异、身份差异和外部 view 依赖三个数据库也分别在 DDL 前失败。四库的 revision、relation OID/类型、索引、行数、comments 和 trigger 快照均保持原状。
4. 回滚库在同一 migration transaction 完成 Serving `DROP TABLE -> CREATE VIEW -> trigger` 后注入失败；回滚后 revision 157、两张物理表、两层 relation/索引 OID、索引定义与 valid/ready、ACL/comments、行数和零用户 trigger 与执行前一致。
5. 单日、单股票、最大日期和 10 日日期完整性四类查询切换前后结果 hash 一致，计划下推 Raw 日期索引或 PK，且无临时块。单日 `2.653 -> 2.577 ms`、日期完整性 `8.419 -> 8.757 ms`；两个点查询均低于 `0.01 ms`，只作为索引形态证据，不用其微秒级抖动代替生产性能判断。

`P1-B3-stk_auction_c-M2` 据此**通过**，revision 158 无需修改。生产仍为 revision 156 和两张物理表，尚未释放 390,266,880 B。下一阶段只能是另行授权的 M3a：重新执行生产只读预检，暂停 schedule #2/#24，证明开放 TaskRun、目标 node、锁和长事务为 0，停止目标 worker，实时复测 M0 代表查询后再安装 Raw-only 代码并显式应用 revision 158；随后回收连接池、执行最小 TaskRun、完成对账并恢复 schedule。M2 不授权任何生产操作。

#### 2026-08-29 `P1-B3-stk_auction_c-M3a` 生产即时验收

M3a 于 `16:26..16:32+08` 按本 LLD 的维护顺序完成，没有发生前几项曾出现的“标准部署在暂停前自动应用 migration”偏差：

1. 实时预检确认生产 revision 157、两层各 2,255,593 行和唯一身份、日期 `2025-01-02..2026-08-28`。20 个自然月九字段双向 `EXCEPT ALL` 全为 0、月峰值 132,619；开放任务、目标 node、锁、长事务与 catalog 依赖门禁均为 0。切换前五类查询 hash 一致、命中等价索引且无临时块，Raw 最大正向退化为单日查询约 7.14%，低于 20% 门禁；95.21% all-visible 已足够，本项没有机械复制开盘竞价的 vacuum。
2. schedule #2/#24 通过正式服务暂停，config revision `123/124` 留痕；scheduler 与 generic worker 停止后再次完成全量对账。只用 `--maintenance-migration` 安装 commit `3030524987a15740333240c4bf4edf49df4ff383` 并应用 revision 157→158，该模式没有构建、seed、unit 同步、隐式任务或服务重启。
3. Raw OID `808835`、PK/date index 保持不变且继续位于 SSD；Serving 成为 OID `2038214`、0 B 普通 view。显式九字段与审计时间投影、owner、Raw `lake_raw_reader SELECT`、Serving ACL 和独立拒写 trigger 正确；三类 DML 均返回 SQLSTATE `55000`。原 Serving 390,266,880 B 已由 catalog 确认释放。
4. Web、日期完整性和 TaskRun 收尾连接池回收，健康端点为 200；QTF 没有该 relation 消费者，未做无关重启。切换后五类 Raw/view hash 一致、view 计划全部下推 Raw 索引、临时块为 0；view 相对 Raw 最大中位数开销仅 `0.004 ms`，批量查询最大约 1.2%。
5. 正式 TaskRun `10111` 请求 `2026-08-28` 一个 point unit：父任务 1/1 成功，目标 node 1 页短页读取/保存 `5,551/5,551`，reject、去重、重试为 0。目标日全部 5,551 行在任务窗口刷新，Raw/view 行数、唯一身份和九字段一致，全表仍为 2,255,593 行。raw upsert 以 `rows_saved` 和 `fetched_at` 验收，不使用不适用的 immutable-fact 插入/匹配诊断字段。
6. schedule #2/#24 通过正式服务恢复 active，config revision `125/126` 记录 resumed，cron、时区、next/last timing 不变；Web、generic worker、scheduler、日期完整性、TaskRun 收尾与 QTF worker 均 active，两个健康端点为 200，开放任务、目标 node、目标锁和长事务均为 0。最终 Raw/view 全表仍各为 2,255,593 行及同数唯一身份；根盘最终可用 `51,782,873,088 B`，确定性收益只认 catalog 释放量，不把 `df` 噪声冒充收益。

`P1-B3-stk_auction_c-M3a` 据此**通过**。M0/M1/M2/M3a 已闭环，生产为 Raw 唯一物理事实表和读取透明的 0 B Serving view。M3b 已登记到统一夜间台账，只待两个既有 schedule 的下一次自然工作流；它不创建额外请求，也不阻塞 `moneyflow_ths` 的独立 M0。

#### 2026-08-29 `P1-B3-moneyflow_ths-M0` 只读复审与 M1 准入合同

本阶段只读取当前代码、CodeGraph、生产 PostgreSQL catalog/数据/执行计划、服务状态和磁盘水位；没有修改代码或生产配置，没有部署、migration、暂停 schedule、创建 TaskRun、调用 Tushare 或写业务数据。CodeGraph 与精确引用审计覆盖 Definition、resolver/request builder、writer/DAO、raw/serving ORM、ServingPublish、Ops workflow/freshness/date completeness、Biz/QTF、frontend、Lake Console、DG、测试和脚本。

1. Definition 固定显式请求 13 个业务字段：`trade_date, ts_code, name, pct_change, latest, net_amount, net_d5_amount, buy_lg_amount, buy_lg_amount_rate, buy_md_amount, buy_md_amount_rate, buy_sm_amount, buy_sm_amount_rate`。point/range 输入按开市日逐日生成单日 unit，request builder 每个 unit 固定发送 `trade_date` 和可选 `ts_code`；source client 保持 `offset/limit`、`page_limit=6000`、无任意最大页数。M1 禁止修改字段、日期/unit、filter、分页、manual/schedule/retry 或源端请求语义；
2. 当前 storage 仍是 `raw_moneyflow_ths + equity_moneyflow_ths + raw_core_upsert`。`DatasetWriter._write_raw_and_core()` 将同一个 normalized batch 仅按两套 ORM 列过滤后分别 upsert，未发现 serving 专属转换、过滤、聚合、冲突消解、ServingPublish mapping 或旁路 DML。M1 只允许切到既有 `raw_only_upsert + raw_with_serving_view`，不修改共享 writer、normalizer、resolver、planner、source client 或 Ops contract；
3. 生产 PostgreSQL 16.13、Alembic revision `20260829_000158`。Raw/Serving OID 为 `22825/22890`，均为 owner `goldenshare_user`、`pg_default` 普通持久表；总大小分别为 518,840,320 B 与 490,921,984 B（Serving 约 468.18 MiB）。两层 13 个业务列、类型、nullability 和主键 `(trade_date, ts_code)` 一致；两边日期索引与 `(ts_code, trade_date)` 索引签名等价且 valid/ready。Raw 额外有 `lake_raw_reader SELECT`；两表均无 inheritance/partition、外键、用户 trigger、列 ACL、RLS/policy、外部 view/materialized view、function dependency、rewrite rule、扩展统计、security label、publication 或 comments；
4. Raw/Serving 各 2,077,033 行，日期范围均为 `2024-12-19..2026-08-28`，身份空值为 0，Raw `api_name` 异常为 0。21 个自然月逐月比较全部 13 个业务字段，raw-only/serving-only 均为 0；`raw.fetched_at = serving.updated_at` 对全部行成立。Serving 有 35,990 行 `created_at != updated_at`，仓库内未发现审计时间消费者；一期 view 仍按固定合同投影 `fetched_at AS created_at/updated_at`，因此只承诺业务字段读取透明，不承诺这 35,990 行历史 `created_at` 值不变；
5. 409 个交易日的日行数最小 2、中位 5,091、P95 5,214、最大 5,222；最近五个 schedule #4 自然节点均成功，单日读取/保存 5,209～5,211，1 页短页结束、0 重试、0 reject、0 去重、未截断。自然月峰值是 `2025-12` 的 116,993 行；13 字段业务行实测最大 134 B、平均 118.59 B。`work_mem=16MB` 下 21 月完整双向逐月对账约 25.6 秒，最大月单向差集会使用约 25 MiB PostgreSQL 临时文件但在约 1 秒内完成。M1 migration 固定每层每月 **150,000 行** 上限，较峰值保留约 28.2% 余量；M2 必须独立证明 150,000 行通过、150,001 行在任何 Serving DDL 前 fail-closed；
6. 五类代表查询的 Raw/Serving 结果校验值全部一致，且均使用对应日期或复合索引、没有查询临时块：最大日期中位 `0.116/0.106 ms`，单日 5,211 行 `10.819/11.384 ms`，五日 26,049 行 `66.925/68.479 ms`，单股全历史 406 行 `1.482/1.490 ms`，近 80 个日期完整性 `61.274/59.575 ms`。Raw 最大正向退化约 9.43%，低于 20% 门禁；Raw all-visible 为 89.32%、Serving 为 93.51%，当前不需要复制 `stk_auction_o` 的生产 vacuum。M3a 仍须在维护窗口实时交错复测，结果不一致、退化超过 20% 或计划异常即停止；
7. 仓库内 Serving 直接引用只剩 Definition、ORM/DAO registry 与通用 Ops 事实；未发现 Biz、QTF、frontend、DG 或脚本直接读取 `core_serving.equity_moneyflow_ths`。Lake Console 已通过 `prod-raw-db` 直接读取 Raw 的同一 13 字段，排序键为 `ts_code`，不需要改数据源。仓库外 SQL/BI/catalog 工具仍无法由代码审计穷尽；依赖 OID、relkind、约束、审计时间或 Serving DML 的外部消费者不属于透明承诺；
8. 唯一自动入口是 active schedule #4 `daily_moneyflow_maintenance`，`20:00` 工作日触发；M3a 必须暂停/恢复 #4 并停止会领取该 workflow 的 generic worker，不得复制竞价数据集的 #2/#24 双入口。M0 快照时开放 TaskRun、目标开放 node、目标 relation 锁、等待锁和超过 30 秒事务均为 0；Web、generic worker、scheduler、date-completeness 与 task-completion 服务 active，健康端点为 200。根盘/HDD 可用空间为 51,779,489,792 B/319,074,136,064 B，只是 M0 水位，不替代 M3a 实时预检；
9. Raw ORM 当前漏声明生产已存在的两个二级索引。M1 只补 `idx_raw_tushare_moneyflow_ths_trade_date` 与 `idx_raw_tushare_moneyflow_ths_ts_code_trade_date` metadata，migration 不得重建索引。独立 migration 必须接编码时真实 Alembic head，按 Raw→Serving 顺序持 `SHARE` 锁，在锁内用 16 MiB work_mem 重做 21 月有界双向差集与身份门禁；恢复 owner/SELECT ACL/comments，复用并验证现有拒写函数，创建本 view 独立 trigger，禁止 `CASCADE`、Raw DDL、共享函数重建和自动 downgrade。

`P1-B3-moneyflow_ths-M0` 据此**通过**。下一阶段只能是另行授权的 M1：修改本数据集 Definition storage、Raw ORM 索引 metadata、新增独立 revision 与专项/回归测试，并把上述 150,000/150,001、13 字段、依赖、权限、DML、回滚和查询门禁写成可执行合同。生产当前仍是双物理表和双写代码，M0 不授权 M2 或 M3a。

#### 2026-08-29 `P1-B3-moneyflow_ths-M1` 实施与代码门禁

1. 本项只把 `moneyflow_ths` storage contract 改为 `raw_moneyflow_ths` 双 DAO 名、`raw_tushare.moneyflow_ths` target、`raw_with_serving_view`、`raw->serving_view` 与 `raw_only_upsert`；13 个显式 source fields、交易日逐 unit、可选 `ts_code`、6,000 行 offset/limit 分页、手动/定时/重试能力及 schedule #4 均未改变；
2. Raw ORM 新增的两个 Index 只是生产既存索引 metadata。独立 revision `20260829_000159` 接编码时真实 head `20260829_000158`，不包含 `CREATE INDEX`、Raw DDL、Raw DML、共享 writer/resolver/source client 改动；
3. revision 159 在任何 Serving DDL 前逐项检查 relation 类型、SSD tablespace、列序/类型/nullability、`(trade_date, ts_code)` 主键、两个二级索引的列序/唯一性/表达式/有效性/ready/tablespace、额外约束、FK/trigger/RLS/ACL/rewrite/依赖/扩展统计/security label/publication。锁序固定为 Raw `SHARE` → Serving `SHARE` → 对账 → Serving `ACCESS EXCLUSIVE`；
4. migration 在 16 MiB `work_mem` 下逐自然月检查两层各不超过 150,000 行、身份数等于行数、13 个业务字段双向 `EXCEPT ALL` 为 0；M2 必须证明精确 150,000 行通过而 150,001 行在 drop 前失败，M1 的静态与离线 SQL 测试不能替代该数据库证据；
5. view 显式投影 13 个业务字段及 `fetched_at AS created_at/updated_at`，恢复原 owner、非 owner SELECT ACL/grant option、relation/column comments，并只验证复用共享拒写函数、创建本 view 独立 trigger；禁止 `CASCADE`、共享函数重建和自动 downgrade；
6. 专项及共享回归已覆盖 Definition/plan/filter、ORM 契约、Raw-only writer、freshness/date-completeness Raw target、ServingPublish 旁路、migration 顺序/禁止项与 PostgreSQL offline SQL render。M1 未连接 PostgreSQL、请求 Tushare、部署、创建 TaskRun、修改 schedule 或执行 migration。

`P1-B3-moneyflow_ths-M1` 据此**通过**。生产仍是 revision 158、双物理表和旧双写部署；下一阶段只能是另行授权的 M2，且必须使用隔离 PostgreSQL 完成正负容量、原子回滚、权限、三类 DML、正式 writer 即时可见和查询计划验收。

#### 2026-08-29 `P1-B3-moneyflow_ths-M2` 隔离 PostgreSQL 验收

M2 在本轮新建的 PostgreSQL 18.4 一次性实例执行。实例 `listen_addresses=''`，只允许随机 Unix socket 连接；应用 migration 的角色为非超级用户。每次 Alembic 调用前同时核对最终 `get_settings()` URL、数据库、用户、socket、端口、`inet_server_addr=NULL`、恢复状态与 data directory，临时 `gs_raw_cold_hdd` 路径也必须位于本轮数据根。全程没有连接 Prod、请求 Tushare、部署、创建 TaskRun、修改 schedule #4 或执行任何生产 DDL。

150,000 行/月正向场景从 revision 158 成功应用 159。Raw relation OID `16392`、主键和两个二级索引的 OID/定义/valid/ready 状态全部保持不变且继续位于 `pg_default`；Serving 从 OID `16407` 的物理表切换为新的 0 B 普通 view。Raw/view 各 150,000 行和同数唯一身份，13 个业务字段双向差异为 0。owner、Raw/Serving SELECT ACL、grant option、relation/column comments 与本 view 独立拒写 trigger 均恢复；Serving 三类 DML 全部返回 SQLSTATE `55000`。直接 Raw DML 与正式 `DatasetWriter` 的写入由 view 即时可见，writer 目标明确为 `raw_tushare.moneyflow_ths`，事务回滚后均无残留。

150,001 行、业务字段差异、身份差异、未知外部 view 依赖、缺失 Raw `(ts_code, trade_date)` 索引五个场景全部在 Serving DDL 前失败并保持完整对象快照；另一个场景在完成 `DROP TABLE -> CREATE VIEW -> trigger` 后注入异常，整个 migration transaction 回滚到 revision 158 和原两张物理表。单日、单股票、股票区间、最大日期、日期完整性五类查询的结果行数与 hash 前后一致，计划从等价 Serving 索引切换到 Raw 索引且无临时读写块。

首次测试脚本把“单股票点查必须命中主键”写成过窄断言，而 PostgreSQL 合理选择了等价的 `(ts_code, trade_date)` 索引；migration 已成功但脚本主动判失败，该实例随即停止，结果未被记为通过。修正断言后从全新数据目录完整复跑七项场景并全部通过；最终报告为 `/private/tmp/goldenshare_moneyflow_ths_m2_report.json`，成功实例已经停止且数据目录删除。

`P1-B3-moneyflow_ths-M2` 据此**通过**，revision 159 与业务代码无需修改。生产仍是 revision 158、两张物理表和旧双写部署；下一阶段只能是另行授权的 M3a，必须实时复核生产身份、head、schedule #4、worker、开放任务、锁、长事务、磁盘、全量差异和代表查询后再决定是否切换。

#### 2026-08-29 `P1-B3-moneyflow_ths-M3a` 生产即时验收

M3a 于 `18:47..18:57+08` 按本 LLD 的维护顺序完成，没有发生标准部署提前应用 migration 的偏差：

1. 实时预检确认生产 commit `fe3caa3b`、revision 158，Raw/Serving 各 2,077,033 行和同数唯一身份，日期范围 `2024-12-19..2026-08-28`。21 个自然月十三字段双向 `EXCEPT ALL` 全为 0、月峰值 116,993；开放任务、目标 node、锁、长事务与 catalog 依赖门禁均为 0。两层都在 SSD，Raw/Serving 大小为 518,840,320/490,921,984 B。
2. 切换前五类查询 hash 一致、命中等价索引且无临时块。三轮样本中的亚毫秒“最大日期”查询出现 0.037 ms 抖动，补做 21 轮交错复测后 Raw/Serving 中位数为 `0.109/0.121 ms`；其余有实际数据量的查询 Raw 相对 Serving 为约 `-9.2%..+6.0%`，无需 vacuum。
3. schedule #4 经正式服务暂停，config revision 127 留痕，cron `0 20 * * 1-5`、时区、next/last timing 保持不变；scheduler 与 generic worker 分别停止后再次完成全量对账。`--maintenance-migration` 只安装 commit `754f6c78` 并应用 revision 158→159，没有构建、seed、unit 同步、任务创建或隐式服务重启。
4. Raw OID `22825` 保持不变，主键和两个二级索引仍 valid/ready 且位于 `pg_default`；Serving 由 OID `22890` 物理表变为 OID `2039022`、0 B 普通 view。显式十三字段与审计时间投影、owner、Raw `lake_raw_reader SELECT`、Serving ACL 和独立拒写 trigger 正确；三类 DML 均返回 SQLSTATE `55000`，原 Serving 490,921,984 B 已由 catalog 确认释放。
5. Web、日期完整性和 TaskRun 收尾连接池回收后，远端运行时 Definition 明确为 Raw-only。五类 Raw/view 查询继续 hash 一致、全部下推 Raw 索引且无临时块；批量查询最大相对开销低于 6%。QTF 没有该 relation 消费者，未重启无关服务。
6. 正式 TaskRun `10128` 请求 `2026-08-28` 一个 point unit：父任务 1/1 成功，目标 node 1 页短页读取/保存 `5,211/5,211`，reject、去重、重试为 0。目标日 5,211 行全部在任务窗口刷新，Raw/view 行数、身份和十三字段一致，全表最终仍各为 2,077,033 行及同数身份。raw upsert 以 `rows_saved` 和 `fetched_at` 验收，不使用不适用的 immutable-fact 插入/匹配诊断字段。
7. schedule #4 通过 config revision 128 原样恢复 active，scheduler 与 generic worker 恢复；Web、日期完整性和 TaskRun 收尾服务均 active，两个健康端点为 200，开放任务、目标锁和长事务均为 0。根盘最终可用 `52,283,146,240 B`；确定性收益只认 catalog 释放的 490,921,984 B，不用一次 `df` 差值代替 relation 收益。

`P1-B3-moneyflow_ths-M3a` 据此**通过**。生产为 revision 159、Raw 唯一物理事实表和读取透明的 0 B Serving view。M3b 已登记到统一夜间台账，只待 schedule #4 下一次自然工作流；不创建额外任务、不重复请求源端。

#### 2026-08-29 `P1-B4-moneyflow_dc-M0` 只读复审与 M1 准入合同

本阶段只读取当前代码、CodeGraph、生产 PostgreSQL catalog/数据/查询计划和既有 TaskRun 诊断；没有修改代码或生产配置，没有部署、migration、暂停 schedule、创建 TaskRun、调用 Tushare 或写业务数据。CodeGraph 索引为 current，覆盖 2,842 个文件和 50,545 个节点；分析覆盖 Definition、resolver/request builder、writer/DAO、raw/serving ORM、ServingPublish、Ops workflow/freshness/date completeness、Biz/QTF、frontend、Lake Console、DG、测试和脚本，并以精确字符串搜索补齐动态 registry。

1. Definition 当前显式请求全部 15 个源字段：`trade_date, ts_code, name, pct_change, close, net_amount, net_amount_rate, buy_elg_amount, buy_elg_amount_rate, buy_lg_amount, buy_lg_amount_rate, buy_md_amount, buy_md_amount_rate, buy_sm_amount, buy_sm_amount_rate`。point/range 由交易日历逐日形成单日 unit，request builder 每 unit 只发 `trade_date` 和可选 `ts_code`；source client 固定 `offset/limit`、`page_limit=6000`、无任意最大页数、短页才结束。既有 schedule #4 TaskRun `9760` 的 `moneyflow_dc` node 对 `2026-08-28` 真实执行两页：`6000 + 7 = 6007` 行，terminal offset 6,000、短页结束、无截断、无重试、reject 和去重均为 0，证明当前分页链能完整处理超过单页上限的数据；M0 没有额外请求 Tushare。
2. 当前 storage 是 `raw_moneyflow_dc + equity_moneyflow_dc + raw_core_upsert`。`DatasetWriter._write_raw_and_core()` 对同一个 normalized batch 只按两套 ORM 列过滤后分别 upsert，未发现 Serving 专属转换、过滤、聚合、冲突消解、ServingPublish mapping 或旁路 DML。M1 只允许切到既有 `raw_only_upsert + raw_with_serving_view`，不得修改共享 writer、DAO 类型、normalizer、resolver、planner、source client、日期/unit、分页、filter 或工作流合同。
3. 生产数据库为 `goldenshare`，用户 `goldenshare_user`，Alembic revision `20260829_000159`。Raw/Serving OID 分别为 `22836/22899`，均为 owner `goldenshare_user`、SSD `pg_default` 普通持久表；Raw 总大小 `1,188,683,776 B`，Serving 总大小 `1,131,995,136 B`（约 1.054 GiB），后者是当前可释放 catalog 毛量。两层 15 个业务列、类型、nullability、主键 `(trade_date, ts_code)` 和 `(trade_date)`、`(ts_code, trade_date)` 二级索引签名一致，全部 valid/ready；Raw 额外有 `lake_raw_reader SELECT`，Serving 只有 owner ACL，两层 relation/column comments 为空。
4. 未发现 inheritance/partition、非主键约束、外键、用户 trigger、列级 ACL、RLS/policy、外部 view/materialized view、function dependency、rewrite rule、扩展统计、security label 或 publication。M0 时开放 TaskRun、开放目标 node、目标 relation 锁和超过 5 分钟事务均为 0；schedule #4 为 active，工作日 `20:00` 触发，下一次为 `2026-08-31 20:00+08`。这些都是时点证据，M3a 必须在任何状态变更前重新核验。
5. Raw/Serving 各 `4,151,016` 行和同数唯一 `(trade_date, ts_code)`，日期范围均为 `2023-09-11..2026-08-28`；身份空值与 Raw `api_name` 异常均为 0。36 个自然月逐月比较全部 15 个业务字段，raw-only/serving-only 差异均为 0；最大业务行宽为 148 B。逐月对账在 `work_mem=16MB` 下总耗时约 115.7 秒，证明必须保持有界月份循环和较宽 statement timeout，禁止改成一次无界全表差集。
6. 717 个交易日的日行数最小 2,812、中位 5,807、P95 6,088、最大 6,106；自然月峰值为 `2025-07` 的 139,877 行。M1 独立 migration 的容量门禁固定为 **170,000 行/层/月**：由实测峰值上浮 20% 后向上取整，不复用 B3 的 150,000/160,000 阈值。M2 必须证明 170,000 行通过、170,001 行在任何 Serving DDL 前 fail-closed；执行前若生产月峰值已超过门禁，必须停止并重新评审，不能放宽或截断。
7. `raw.fetched_at = serving.updated_at` 对全部 4,151,016 行成立；2,350,240 行历史 `serving.created_at != raw.fetched_at`，最大差约 5 天 21 小时。仓库内未发现审计时间消费者，一期 view 仍按固定合同投影 `fetched_at AS created_at/updated_at`，从而保持 `updated_at` 全量不变；但只承诺已登记消费者的 15 个业务字段读取透明，不宣称这 2,350,240 行历史 `created_at`、relation OID/relkind 或约束 catalog 透明。
8. Raw/Serving all-visible 分别为 90.57%/88.18%，当前没有复制 `stk_auction_o` 普通 vacuum 的证据。最大日期、单日 6,007 行、单股全历史 715 行、2026-08 的 119,021 行 Lake 范围和 2026 年日期完整性五类查询均返回等价结果并使用日期或 `(ts_code, trade_date)` 索引。代表执行分别约为 Raw/Serving `0.123/0.107 ms`、`12.372/11.905 ms`、交错中位 `1.087/1.264 ms`、`244.904/240.316 ms`、交错中位 `132.094/119.787 ms`；Raw 最大有意义正向退化约 10.3%，低于本项 20% 停止门禁。冷热缓存导致的单次抖动不作为收益或失败，M2/M3a 仍须交错复测结果 hash、索引下推、临时块和中位时延。
9. 仓库内未发现 Biz、QTF、frontend 或 DG 直接读取 `core_serving.equity_moneyflow_dc`；Ops freshness/date completeness 由 Definition target 驱动，切换后会读 Raw。Lake Console 已通过 `prod-raw-db` 显式读取 Raw 的同一 15 字段，point/range 均按 `trade_date, ts_code` 排序，不需要修改 Lake source。仓库外 SQL、BI、人工脚本和 catalog 工具无法由代码审计穷尽，依赖 Serving DML、OID、relkind、约束或历史 `created_at` 的未登记消费者继续作为残余运营风险。
10. 唯一自动写入口是 `daily_moneyflow_maintenance` schedule #4。M3a 必须暂停/恢复 #4 并停止会领取该 workflow 的 generic worker，不得复制竞价数据集的 #2/#24 双入口；自然 M3b 必须读取父 TaskRun 内的 `moneyflow_dc` node，核对分页两页、最终短页、读取/保存、reject、去重、Raw/view 当日 15 字段和全表身份。若 `moneyflow_ths` 的 M3b 尚待同一次 schedule #4，可在同一父 TaskRun 分别验收两个 node，但任何一个 node 的结论都不能替代另一个。
11. Raw ORM 当前漏声明生产已经存在的 `idx_raw_tushare_moneyflow_dc_trade_date` 与 `idx_raw_tushare_moneyflow_dc_ts_code_trade_date`。M1 只补这两个 metadata，migration 禁止 `CREATE INDEX` 或其它 Raw DDL。独立 revision 必须接编码时真实 Alembic head，固定 `lock_timeout=15s`、`statement_timeout=300s`、`work_mem=16MB`，按 Raw→Serving 顺序持有 `SHARE` 锁，在锁内逐月验证 170,000 行上限、身份和 15 字段双向差集；全部通过后才取得 Serving `ACCESS EXCLUSIVE` 锁，原子替换为显式 view、恢复 owner/ACL/comments 并创建独立拒写 trigger。

`P1-B4-moneyflow_dc-M0` 据此**通过**，具备 M1 编码准入。M1 仍需单独授权，范围只包括本数据集 Definition storage、Raw ORM 索引 metadata、独立 migration、专项/回归测试和文档；不得连接数据库、请求 Tushare、创建 TaskRun、修改 schedule 或顺带启动 `stk_limit`。

#### 2026-08-29 `P1-B4-moneyflow_dc-M1` 编码与离线验证

M1 已按上述准入合同完成，且严格保持单数据集边界：

1. Definition 只把 storage 从 `raw_core_upsert + single_source_serving` 改为 `raw_only_upsert + raw_with_serving_view`，写入 DAO 与 target 均指向 `raw_tushare.moneyflow_dc`；15 个字段、point/range、交易日 unit、`ts_code` filter、6,000 行分页和 manual/schedule/retry 能力没有变化；
2. Raw ORM 只补生产既有的 `idx_raw_tushare_moneyflow_dc_trade_date` 与 `idx_raw_tushare_moneyflow_dc_ts_code_trade_date` metadata；DAO factory、core ORM、共享 writer/resolver/request builder/normalizer/source client、Ops、前端和 Lake 代码均未修改；
3. 独立 revision `20260829_000161` 接编码时唯一真实 head `20260829_000160`。迁移固定 15 秒锁等待、300 秒语句上限和 16 MiB `work_mem`，验证 Raw/Serving 的 SSD placement、显式列类型/nullability、复合主键、四个既有二级索引、owner/ACL/comments、未知依赖及共享拒写函数；
4. migration 按 Raw→Serving 获取 `SHARE` 锁，按自然月以 170,000 行/层上限执行身份与 15 字段双向 `EXCEPT ALL`；只有全部通过才获取 Serving `ACCESS EXCLUSIVE` 锁并在同一事务内执行无 `CASCADE` 的物理表→显式 view 切换，恢复 metadata 并创建本 relation 独立拒写 trigger。migration 不含 `CREATE INDEX`、Raw DDL/DML、共享函数重建或自动 downgrade；
5. 专项测试证明 Definition/Resolver/filter/pagination 未漂移，Raw/Serving ORM 与索引合同一致，ServingPublish 没有旁路，freshness target 为 Raw，writer 只调用 `raw_moneyflow_dc`，migration 顺序、禁止项与完整 PostgreSQL 离线 SQL 可验证。M1 未连接数据库、请求 Tushare、部署、应用 migration、创建任务或修改 schedule。

`P1-B4-moneyflow_dc-M1` 据此**通过**。生产仍为 revision 159、Raw/Serving 两张物理表且继续由已部署旧版本双写，1,131,995,136 B 尚未释放。下一阶段只能是独立授权的 M2：在隔离 PostgreSQL 应用 revision 161，真实验证 170,000/170,001 容量边界、三类 DML 拒绝、事务回滚、Raw/writer 即时可见、ACL/comments/tablespace 和代表查询计划；不得越级进入生产 M3a。

#### 2026-08-29 `P1-B4-moneyflow_dc-M2` 隔离 PostgreSQL 验收

M2 在本轮新建的 PostgreSQL 18.4 一次性实例执行。实例 `listen_addresses=''`，只允许随机 Unix socket 连接；应用角色为非超级用户，且没有建库、建角色或复制权限。每次 Alembic 前均核对独立 env 解析后的 URL、数据库、用户、socket、端口、`inet_server_addr=NULL`、恢复状态和 data directory；临时 `gs_raw_cold_hdd` 也位于本轮隔离根。全程没有连接 Prod、请求 Tushare、部署、创建 TaskRun、修改 schedule #4 或执行生产 DDL。

170,000 行/月正向库成功从 revision 160 升级到 161：Raw relation 及主键、日期、实体日期三个索引的 OID/定义/valid/ready 状态保持不变并留在 `pg_default`，Serving 成为 0 B 普通 view。Raw/view 行数、唯一身份、15 字段双向差异和审计时间投影完全一致；owner、Raw/Serving reader、SELECT grant option、relation/column comments 和独立拒写 trigger 均恢复。Serving INSERT/UPDATE/DELETE 全部返回 SQLSTATE `55000`；Raw DML 与正式 `DatasetWriter` 写入在同一事务内立即由 view 可见，回滚后没有残留，writer 目标明确为 `raw_tushare.moneyflow_dc`。

170,001 行、业务字段差异、身份差异、未知 view 依赖和缺失 Raw 实体日期索引五类负向场景都在 Serving DDL 前失败，revision、relation、OID、索引、行数、comments 和 trigger 快照保持不变。另一个数据库在 `DROP TABLE -> CREATE VIEW -> trigger` 后注入故障，migration transaction 完整回滚到 revision 160 与原两张物理表。单日、单代码、代码范围、最大日期和日期完整性五类查询的结果 hash 均不变，计划下推等价 Raw 索引且无临时块；单次隔离耗时均无明显退化，只作为计划门禁，不作为生产 SLA。

隔离实例已停止、临时数据根已删除，报告保留于 `/private/tmp/goldenshare_moneyflow_dc_m2_report.json`。`P1-B4-moneyflow_dc-M2` 据此**通过**，revision 161 与业务代码无需修改。生产仍为 revision 159、两张物理表和已部署旧版双写，1,131,995,136 B 尚未释放；下一阶段只能另行授权生产 M3a，并在执行前重新完成生产身份、head、schedule #4、worker、开放任务、锁、长事务、全量差异、查询和磁盘门禁。

#### 2026-08-29 `P1-B4-moneyflow_dc-M3a` 生产即时验收

M3a 于 `22:54..23:09+08` 按本 LLD 的维护顺序完成，没有发生标准部署提前应用 migration 的偏差：

1. 生产预检确认 PostgreSQL 16.13、revision 160、远端旧 commit `96b1a865`，待部署 head 为 `dc191135`；schedule #4 的目标、cron、时区、下一次与上次触发时间均符合合同。暂停后停 scheduler/generic worker，再次确认开放 TaskRun、目标 node、完整性审计 run、目标锁、等待锁和超过 30 秒事务均为 0；36 个月、4,151,016 行、4,151,016 个身份和 15 字段双向差异为 0，月峰值 139,877。
2. 迁移前性能门禁真实阻断过一次：日期完整性查询 20 轮 Raw/Serving 中位为 `167.036/98.824 ms`。根因是 Raw `trade_date` 统计陈旧使 planner 选择串行计划；经明确批准只执行目标列 ANALYZE 后，Raw/Serving 中位为 `98.261/96.454 ms`，差异约 1.87%，且执行计划恢复并行、结果不变。该门禁证明不能因索引存在就跳过生产统计与 planner 验收。
3. `--maintenance-migration` 只拉取 `dc191135`、安装后端并应用 revision `160→161`，没有构建、seed、自动创建任务或隐式恢复服务。migration 一次成功；Raw OID `22836` 与三个索引 OID `22843/22845/22846` 保持不变并继续位于 SSD `pg_default`，Serving 变为 OID `2041227` 的 0 B view，旧 OID `22899` 消失，释放 1,131,995,136 B。
4. view 17 列、owner/ACL、Raw reader、独立拒写 trigger 正确；Raw/view 全表行数、身份、日期范围、15 字段和审计时间投影一致，三类 Serving DML 全部以 SQLSTATE `55000` 拒绝且没有测试行残留。
5. 迁移后五类查询结果 hash 相同、下推等价 Raw 索引、临时块为 0。单日查询的三轮样本曾出现约 47% 抖动，按门禁补做 20 轮交替复测后 Raw/view 中位为 `13.602/13.371 ms`，差异约 -1.69%，证明不存在结构性视图退化。
6. Web 与 Ops 连接池回收后，创建任务前的两个执行失误都在副作用前失败：远端 `/private/tmp` 不可写，改用 `/tmp`；第一次 Python 进程错误使用仓库 `.env.web.local`，连接本机 `postgres` 时认证失败，未创建 TaskRun、未调用 Tushare。随后明确读取 systemd unit，使用其真实 `GOLDENSHARE_ENV_FILE=/etc/goldenshare/web.env`，再由正式主链创建唯一 TaskRun `10149`，目标日 `2026-08-28`。任务 success、`1/1/0`；源端两页 `6000+7`、terminal offset 6000、最终短页、无截断；读取/保存 `6007/6007`，reject/去重/重试为 0。Raw/view 当日均为 6,007 行和身份，15 字段差异为 0，全部行在节点窗口内刷新；全表仍为 4,151,016 行，已有日期重跑幂等。
7. schedule #4 已由正式 service 原样恢复 active，config revision `129/130` 分别记录暂停/恢复；五个常驻服务全部 active，两个健康端点正常。最终开放 TaskRun、开放目标 node、目标锁、等待锁与超过 30 秒事务均为 0；根盘可用 53,534,212,096 B、使用率 81%，但确定性收益只按已删除 relation 的 catalog 字节计。

`P1-B4-moneyflow_dc-M3a` 据此**通过**。生产为 revision 161、Raw 唯一物理事实表和读取透明的 0 B Serving view；M3b 已登记到统一夜间台账，等待 `2026-08-31 20:00+08` 的自然 schedule #4 父任务。该待观察项不阻塞 `P1-B4-stk_limit-M0`，且不允许创建额外任务或重复请求 Tushare。

#### 2026-08-29 `P1-B4-stk_limit-M0` 只读复审与 M1 准入合同

本阶段只读取当前代码、CodeGraph、生产 PostgreSQL catalog/汇总数据/查询计划和既有 TaskRun 诊断；没有修改代码或生产配置，没有部署、migration、暂停 schedule、创建 TaskRun、调用 Tushare 或写业务数据。CodeGraph 索引为 current，覆盖 2,865 个文件、50,920 个节点和 128,640 条边；影响面覆盖 Definition、resolver/request builder、source pagination、writer/DAO、raw/serving ORM、ServingPublish、Ops workflow/freshness/date completeness、市场情绪 Biz、QTF、frontend、Lake Console、DG、测试和脚本。

1. Definition 当前显式请求 `trade_date, ts_code, pre_close, up_limit, down_limit` 五字段；日期 point/range 经交易日历形成逐交易日 unit，request builder 每 unit 只发 `trade_date` 与可选 `ts_code`。source client 每页都携带完整 source fields，`page_limit=5800`、无任意最大页数、短页才结束。生产 schedule #24/#2 的 TaskRun `9747/9773` 对 `2026-08-28` 均实际完成 `5800+1968=7768` 两页，terminal offset 5,800、最终短页、无截断/重试/reject/去重；M0 没有额外请求源端。
2. 当前 storage 是 `raw_stk_limit + equity_stk_limit + raw_core_upsert`；writer 对同一 normalized batch 只按 ORM 列投影后双写。未发现 Serving 专属转换、过滤、聚合、冲突消解、ServingPublish mapping 或显式 Serving DML 旁路。M1 只允许把 storage delivery 切为现有 `raw_only_upsert + raw_with_serving_view`，不得修改共享 writer、DAO 类型、normalizer、resolver、planner、source client、日期/unit、filter、分页或工作流。
3. 生产为 PostgreSQL 16.13、revision `20260829_000161`。Raw/Serving OID `21604/21614`，均为 owner `goldenshare_user`、SSD `pg_default` 普通物理表；总大小分别为 `661,782,528/664,354,816 B`，后者约 633.58 MiB，是本项当前可释放 catalog 毛量。两层五业务列、nullability、主键 `(ts_code, trade_date)` 与日期索引签名一致，全部索引 valid/ready；Raw 的 `lake_raw_reader SELECT` 必须保留，Serving 只有 owner ACL，两层 comments 为空。
4. 未发现非主键约束、外键、用户 trigger、RLS/policy、外部 view/materialized view、function dependency 或仓库内旁路写入。M0 时开放 TaskRun、目标 relation 锁和超过 30 秒事务均为 0；这些只是时点证据，M3a 必须实时重查。
5. Raw/Serving 各 `4,608,112` 行和同数唯一身份，日期范围均为 `2024-01-02..2026-08-28`，空身份与异常 Raw `api_name` 为 0。32 个自然月逐月比较五业务字段，两个方向 `EXCEPT ALL` 全为 0；最大业务行宽 61 B。月峰值为 `2026-07` 的 177,009 行，因此独立 migration 容量门禁固定为 **220,000/220,001 行/层/月**，不得复制其它数据集阈值；锁内最终对账继续使用 16 MiB `work_mem`、300 秒 statement timeout 和自然月有界循环。
6. Raw `fetched_at = Serving.updated_at` 对全部行成立，但 514,328 行历史 `created_at != fetched_at`。仓库内没有 `EquityStkLimit.created_at/updated_at` 消费者，因此 view 继续固定投影 `fetched_at AS created_at/updated_at`；只承诺已登记消费者的五业务字段读取透明，不宣称历史 `created_at`、relation OID/relkind 或约束 catalog 透明。
7. 当前 Serving 业务读取只有 `MarketMoodCalculator` 的复合键外连接和 `MarketMoodWalkForwardValidationService` 的按交易日存在性检查；入口是 Ops CLI 走查与集成测试。未发现 frontend、QTF 或 DG 直接读取 Serving；Lake Console 已显式读取 Raw 五字段。20 日市场情绪 join 的 Raw/Serving 计划同形、Raw 单次约 `628.8 ms`、Serving 约 `657.1 ms`；64 日存在性查询交错热样本中位约 `0.849/0.770 ms`，Raw 退化约 10.3%，低于本项 20% 停止线。两层 all-visible 为 93.91%/94.28%，没有先 vacuum 的依据；M2/M3a 仍须复测结果 hash、索引下推、临时块和交错中位时延。
8. 自动入口是同一 `daily_market_close_maintenance` workflow 的两个 active schedule：#24 工作日 18:30、#2 工作日 21:02，`stk_limit` 是第 9 个 step。M3a 必须同时暂停并原样恢复两者，停止 scheduler 与 generic worker；M3b 必须分别验收切换后首个自然交易日两个父 TaskRun 内的 `stk_limit` node，不能相互替代。M0 快照时下一次为 `2026-08-31 18:30/21:02+08`。
9. Raw ORM 已准确声明生产现有 `(trade_date)` 索引，M1 不需要 ORM 改动，migration 也不得创建/重建/移动 Raw 表或索引。M1 只改本数据集 Definition storage、新增接编码时真实 Alembic head 的独立 revision、专项测试与必要共享回归；当前 head 161 只是 M0 快照，不能预写 `down_revision`。
10. migration 必须在 Serving DDL 前验证 relation/owner/SSD/列/主键/索引/ACL/comments/依赖与 220,000 行月上限，按 Raw `SHARE` → Serving `SHARE` → 32 月身份和五字段双向差集 → Serving `ACCESS EXCLUSIVE` 执行。view 显式投影五业务字段与 `fetched_at AS created_at/updated_at`，恢复 metadata 并挂本 view 独立 DML 拒写 trigger；禁止 `CASCADE`、Raw DDL/DML、共享函数重建和自动 downgrade。M2 必须证明 220,000 行通过、220,001 行与字段/身份/依赖/索引异常均在 drop 前失败，并验证回滚、正式 writer 即时可见和两类真实查询。

`P1-B4-stk_limit-M0` 据此**通过**，具备独立 M1 编码准入。M0 不授权隔离 PostgreSQL、生产 migration、TaskRun、schedule 修改或 Tushare 请求；仓库外 SQL、BI、人工脚本和依赖物理 catalog/历史 `created_at` 的工具继续作为残余运营风险。

#### 2026-08-30 `P1-B4-stk_limit-M1` 编码与自动化验证

M1 只修改本数据集代码、独立 migration、测试与本文，不连接 PostgreSQL、不应用 migration、不部署、不创建 TaskRun、不修改 schedule，也不请求 Tushare。

1. Definition storage 已精确改为 `raw_stk_limit/raw_stk_limit` 双 DAO 名、`target_table=raw_tushare.stk_limit`、`raw_with_serving_view / raw->serving_view / raw_only_upsert`；五个 source fields、`_stk_limit_params`、日期模型、point/range、可选 `ts_code`、5,800 行分页、无最大页数、capability 与 workflow 均未修改。freshness 和日期主体矩阵随 Definition 改读 Raw；两个市场情绪 Biz 消费者继续通过原 `EquityStkLimit` ORM 读取同名 Serving relation。
2. 新 revision `20260830_000162` 接编码时真实 head `20260829_000161`，没有修改 Raw ORM，因为现有 `idx_raw_tushare_stk_limit_trade_date` metadata 已与生产一致。migration 在任何 Serving DDL 前校验双方 relation kind、owner、非分区、列/nullability、PK、唯一一个日期二级索引、索引 valid/ready、Raw `pg_default`、Raw 精确 `lake_raw_reader SELECT`、Serving ACL/comment、trigger/RLS/view/function/rewrite/statistics/security-label/publication 依赖，以及每层每月 220,000 行上限。
3. 锁与对账顺序为 Raw `SHARE` → Serving `SHARE` → 32 月身份及五字段双向 `EXCEPT ALL` → Serving `ACCESS EXCLUSIVE`。切换 SQL 不含 `CASCADE`，不含 Raw DDL/DML，不创建/重建索引或共享函数；view 仅显式投影 `ts_code, trade_date, pre_close, up_limit, down_limit, fetched_at AS created_at, fetched_at AS updated_at`，恢复 owner/SELECT grant/comment，并挂 `trg_equity_stk_limit_reject_dml`。自动 downgrade 明确禁止。
4. 新专项测试覆盖 storage-only 变更、未知 filter 拒绝、Raw/Serving 五字段与索引、ServingPublish 旁路、freshness/completeness Raw target、`5800+1` 两页逐页完整 fields/短页结束、Raw-only writer 零 Serving DAO 调用、migration SQL 顺序/禁止项/offline rendering 和 downgrade 拒绝。共享回归继续覆盖 registry、resolver、runtime guardrails、Ops freshness/date completeness 与子系统依赖。

`P1-B4-stk_limit-M1` 据此**通过**，下一步只能单独授权 M2：在隔离 PostgreSQL 真实应用 revision 162，验证 220,000/220,001 行、列/身份/索引/依赖/ACL fail-closed、三类 DML、事务回滚、正式 writer 即时可见和两类市场情绪查询计划。M1 不改变生产 revision 161、双物理表或旧部署写入行为。

#### 2026-08-30 `P1-B4-stk_limit-M2` 隔离 PostgreSQL 验收

M2 在本轮创建的 PostgreSQL 18.4 一次性实例执行。实例 `listen_addresses=''`，只允许随机 Unix socket 连接，应用侧 `inet_server_addr()` 为空；应用角色 `stk_limit_m2_app` 为非超级用户，且没有建库、建角色或复制权限。每次 Alembic 前都通过只含目标 URL 的独立 `GOLDENSHARE_ENV_FILE` 核对最终 URL、数据库、用户、socket、端口、恢复状态和 data directory，未读取仓库 `.env.web.local`。全程没有连接 Prod、请求 Tushare、部署、创建 TaskRun 或修改 schedule/workflow。

1. 220,000 行单月正向库从 revision 161 成功升级到 162。Raw relation OID `16392` 与主键/日期索引 OID `16403/16405`、定义、valid/ready 状态保持不变，且全部位于 `pg_default`；Serving 变为 OID `16430`、0 B 普通 view。Raw/view 各 220,000 行和同数唯一身份，五业务字段双向差异与审计投影差异均为 0。
2. Raw `lake_raw_reader SELECT`、Serving reader 的 `SELECT WITH GRANT OPTION`、owner、relation/column comments 和独立 `trg_equity_stk_limit_reject_dml` 全部恢复。Serving `INSERT/UPDATE/DELETE` 均返回 SQLSTATE `55000`。Raw 插入/更新/删除在同一事务内立即反映到 view，回滚后无残留。
3. 正式 `DatasetWriter` 只报告目标 `raw_tushare.stk_limit` 并保存 1 行；Raw/view 在同一事务内得到相同 `up_limit` 与时间戳，回滚后恢复原值，证明已部署 M1 Definition 不会尝试写 Serving view。
4. 220,001 行、业务字段差异、身份差异、Raw 列类型漂移、外部 view 依赖、缺失 Raw 日期索引和额外 Raw ACL 七个负向数据库均在 Serving DDL 前失败；各自 revision、relation/OID、索引、列、ACL、comments、trigger 与行数快照不变。另一个数据库在完成 Serving `DROP TABLE -> CREATE VIEW -> trigger` 后注入除零错误，transaction 完整回滚到 revision 161 与原两张物理表。
5. 20 日市场情绪复合键 join 和按交易日存在性查询的切换前后行数/hash 一致，切换后分别下推 `stk_limit_pkey` 与 `idx_raw_tushare_stk_limit_trade_date`，全部计划零临时块。隔离样本时延约为 `0.214 -> 0.194 ms` 与 `0.069 -> 0.062 ms`；它们只证明结果、索引形态和 bounded work，不作为生产 SLA。

隔离实例已停止，临时数据根已删除；报告保留于 `/private/tmp/goldenshare_stk_limit_m2_report.json`。`P1-B4-stk_limit-M2` 据此**通过**，revision 162 与业务代码无需修改。生产仍为 revision 161、双物理表和旧双写部署，尚未释放 664,354,816 B；下一阶段只能另行授权 M3a，并在执行前重新完成生产版本/head、schedule #24/#2、worker、开放任务、锁、长事务、32 月全字段差异、两类查询和磁盘门禁。

#### 2026-08-30 `P1-B4-stk_limit-M3a` 生产即时验收

M3a 于 `06:50..06:59+08` 按正确维护顺序完成：

1. 生产预检为 PostgreSQL 16.13、revision 161、远端旧 commit `dc191135`。Raw/Serving 各 4,608,112 行和同数身份，日期 `2024-01-02..2026-08-28`；32 月五字段差异为 0、月峰值 177,009。开放 TaskRun、目标 node、完整性审计 run、目标锁、等待锁和超过 30 秒事务均为 0；根盘可用 53,501,075,456 B。
2. schedule #24/#2 通过正式服务暂停，config revision `131/132` 留痕；cron、时区和 next/last timing 未漂移。scheduler 与 generic worker 分别停止后，最终对账仍为 4,608,112/4,608,112、五字段差异 0。
3. `--maintenance-migration` 只拉取并安装 `da84a32a`、应用 revision 161→162，不构建、不 seed、不同步 unit、不创建任务或隐式恢复服务。Raw OID `21604` 和索引 OID `21611/21613` 保持不变并继续位于 SSD；Serving 成为 OID `2041913` 的 0 B view，确定性释放 664,354,816 B。
4. Raw/view 全表行数、身份、日期范围、32 月五字段及审计投影一致；owner、Raw reader、Serving ACL 和独立拒写 trigger 正确。Serving 三类 DML 均返回 `55000`，测试行残留 0。
5. 20 日市场情绪 join 与 64 日存在性查询的 Raw/view 结果 hash 一致，view 下推 Raw，均无临时块。切换后 view join 约 575 ms，相对切换前 Serving 约 567 ms 波动约 1.5%；存在性约 0.76 ms，未触发 20% 停止线。
6. Web、日期完整性、TaskRun 收尾与 generic worker 逐个回收连接池后，通过正式主链创建唯一 TaskRun `10182`，目标 point `2026-08-28`。第一次远端 Python 命令在解释阶段失败，服务未执行；回查无 TaskRun、无源请求后才正式创建。任务 `success/1/1/0`，源端 `5800+1968` 两页、最终短页、无截断/重试；读取/保存 `7,768/7,768`，reject/去重 0。Raw/view 当日各 7,768 行和身份、五字段差异 0，全部 `fetched_at` 位于 node `15895` 窗口，全表仍为 4,608,112 行。
7. schedule #2/#24 经 config revision `133/134` 原样恢复 active，下一次仍为 `2026-08-31 21:02/18:30+08`。scheduler 恢复后九个常驻服务 active，两个健康端点正常；开放任务、目标 node、完整性审计 run、目标锁、等待锁和长事务均为 0。终态根盘可用 55,182,610,432 B，文件系统水位不替代 catalog 释放量。
8. 维护期间远端另有一次 `06:56:04+08` fast-forward 到 `4e54dec8`。该提交是 `da84a32a` 的后继且只改 8 份财务数据集文档，不涉及代码、migration、Definition 或运行契约；并发事实保留，但不影响本项验收。

`P1-B4-stk_limit-M3a` 据此**通过**。生产为 revision 162、Raw 唯一物理事实表和 0 B Serving view。M3b 已登记到统一夜间台账，等待 schedule #24/#2 两个自然父任务分别验收；不得为此创建额外任务或重复请求 Tushare。
...
| 生产临时 Python 脚本误用仓库 `.env.web.local` | 手工进程不会自动继承 systemd unit 的 `EnvironmentFile`，仓库 env 在生产机上可指向不同数据库 | 生产命令在连接前必须先读目标 unit 的 `EnvironmentFile`，按 unit 的最终 env 启动并核对数据库身份；不得把仓库 env 文件名视为生产连接合同。连接失败且未创建任务时只修正环境，不重复业务 TaskRun | `moneyflow_dc-M3a` fail-before-side-effect 实证 |
...
| `stk_auction_o` | M0 已证 2,183,621 行、20 月九字段全量等价，月峰值 126,364；受每日收盘 schedule #24/#2 覆盖，当前无 Biz/QTF/DG/Lake 直读；M3a 普通 vacuum 后日期完整性门禁已关闭 | raw-only writer、显式 view、拒写函数、按月有界差集、单数据集 revision 与事务外普通 vacuum | 小表容量、只测点查、忽略日期完整性、把普通 vacuum 塞进 migration；M3a 必须在维护窗口独立执行和验收 vacuum |
| `stk_auction_c` | M0 已证 2,255,593 行、20 月九字段全量等价，月峰值 132,619；受每日收盘 schedule #24/#2 覆盖，当前无 Biz/QTF/DG/frontend/Lake 直读；Raw 日期完整性相对旧 Serving 退化 12.55%，当前无需 vacuum | raw-only writer、显式 view、拒写函数、按月有界差集、单数据集 revision；本数据集独立固定 160,000 行/月门禁 | 直接复制开盘竞价 migration/消费者/vacuum 结论；忽略 18:30 空结果与 21:02 就绪的双入口时序 |
| `moneyflow_ths` | M0 已证 2,077,033 行、21 月十三字段全量等价，月峰值 116,993；受每日资金 schedule #4 覆盖，Lake 已读 Raw，当前无 Biz/QTF/frontend/DG Serving 直读；Raw 日期完整性退化 2.85%，无需 vacuum | raw-only writer、显式 view、拒写函数、按月有界差集、单数据集 revision；本数据集独立固定 150,000 行/月门禁 | 竞价数据集的 160,000 上限、#2/#24 双入口、vacuum 结论或九字段 migration；忽略 35,990 行历史 `created_at` 不透明边界 |
| `moneyflow_dc` | M0/M1/M2/M3a 已通过；生产 revision 161、Raw 唯一物理事实表、0 B Serving view，TaskRun `10149` 通过并释放 1,131,995,136 B；M3b 待自然观察 | raw-only writer、显式 view、拒写函数、按月有界差集、单数据集 revision；本数据集独立固定 170,000 行/月与 300 秒 statement timeout | B3 的 150,000/160,000 上限、#2/#24、vacuum 结论或 13 字段 migration；忽略 2,350,240 行历史 `created_at` 不透明边界；把同一 schedule #4 内另一个 node 的 M3b 当成本 node 验收 |
| P1-B4 `stk_limit` | M0/M1/M2/M3a 已通过；生产 revision 162、Raw 唯一物理事实表、0 B Serving view，TaskRun `10182` 通过并释放 664,354,816 B；M3b 待 #24/#2 自然观察 | 只能复用阶段状态机、审计方法、Raw-only writer、受保护 view 与单数据集 revision 原则 | 不得用单个父 workflow 代替双入口 M3b；不得把文件系统瞬时水位或通用 writer 的不适用字段冒充本项释放/写入证据 |
...
10. `P1-B3-stk_auction_c-M0/M1/M2/M3a` 已通过：生产 M0 证明两层各 2,255,593 行、20 月九字段等价、月峰值 132,619 与查询门禁；M1/M2 完成 Raw-only Definition、独立 revision 158、160,000/160,001 行、权限、拒写、writer、回滚和查询计划；M3a 按维护窗口正确顺序应用 revision 158，TaskRun `10111` 与最终对账通过。生产现为 Raw 唯一物理事实表和 0 B Serving view，已释放 390,266,880 B；M3b 待统一夜间台账自然观察；
11. `P1-B3-moneyflow_ths-M0/M1/M2/M3a` 已通过：生产 M0 证明两层各 2,077,033 行、21 月十三字段等价与 150,000 行/月门禁；M1/M2 完成 Raw-only Definition、独立 revision 159、隔离容量/差异/权限/拒写/writer/回滚和查询门禁；M3a 按正确维护顺序应用 revision 159，TaskRun `10128` 与最终对账通过。生产现为 Raw 唯一物理事实表和 0 B Serving view，已释放 490,921,984 B；M3b 待 schedule #4 自然观察；
12. `P1-B4-moneyflow_dc-M0/M1/M2/M3a` 已通过：生产 revision 161、Raw OID/索引保持不变、Serving 为 0 B view，TaskRun `10149` 两页读取/保存 6,007/6,007 且 0 reject/去重/重试；已释放 1,131,995,136 B。M3b 待 schedule #4 的 `2026-08-31 20:00+08` 自然工作流观察；
13. `P1-B4-stk_limit-M0/M1/M2/M3a` 已通过：生产 revision 162、Raw OID/索引保持、Serving 0 B view，TaskRun `10182` 两页读取/保存 `7,768/7,768` 且 0 reject/去重/重试；已释放 664,354,816 B。M3b 待 schedule #24/#2 的 `2026-08-31 18:30/21:02+08` 两个自然父任务分别验收；
14. 未来任何生产操作仍须实时确认开放 TaskRun 为 0、目标 schedule/probe 已按本项契约暂停、worker 已停止、目标锁和磁盘水位满足门禁。扩容后的容量快照不是跳过这些检查的理由；
15. 仓库外 SQL/BI/人工脚本无法由仓库审计穷尽，若存在依赖 OID、relkind、约束 catalog、旧审计时间或 serving DML 的未登记消费者，仍是每项 migration 的残余运营风险。
