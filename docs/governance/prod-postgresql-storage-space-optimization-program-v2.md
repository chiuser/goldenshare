...
2026-08-29 `stk_auction_c` M3a 维护前后根盘可用空间分别为 51,424,526,336 B 与 51,782,873,088 B，使用率均为 82%。依赖安装、WAL 与运行噪声会影响 `df`，本项只以 PostgreSQL catalog 确认原 Serving 物理 relation 的 390,266,880 B 已释放。随后 `moneyflow_ths`、`moneyflow_dc` 与 `stk_limit` M3a 又分别由 catalog 确认释放 490,921,984 B、1,131,995,136 B 与 664,354,816 B；一期已确认的 catalog 毛释放量累计为 **3,586,457,600 B（约 3,420.31 MiB / 3.340 GiB）**。该累计使用各数据集切换时的真实 catalog 字节；第 4.5 节 3.305 GiB 仍是 2026-08-23 全名单同一时点基线，二者不能混写。`stk_limit` 完成时根盘可用 55,182,610,432 B，但 `df` 仍只作为水位证据，不替代 relation catalog 字节。
...
### 2.31 2026-08-29 `P1-B3-moneyflow_ths-M1/M2` 实施与隔离验收事实

M1 只把 `moneyflow_ths` 的 storage delivery contract 收敛到 `raw_tushare.moneyflow_ths + raw_only_upsert + raw_with_serving_view`，并为 Raw ORM 补齐生产已存在的日期与实体日期索引 metadata。13 个显式 source fields、交易日逐 unit、可选 `ts_code`、6,000 行分页、planner、手动/定时/重试和 schedule #4 均未改变。独立 revision `20260829_000159` 接真实 head 158，在任何 Serving DDL 前固定检查 150,000 行/月容量、13 字段双向差异、身份、对象/依赖/ACL/comment/索引/tablespace，并禁止 `CASCADE`、Raw DDL、共享函数重建和自动 downgrade。M1 的专项与共享自动化测试通过，提交为 `754f6c78`；该阶段没有连接数据库、请求 Tushare、部署、创建 TaskRun 或修改 schedule。

M2 在 `listen_addresses=''`、仅随机 Unix socket 可达的 PostgreSQL 18.4 临时实例，以非超级用户和独立 env 完成。每次 Alembic 前均核对数据库、用户、socket、端口、server address、恢复状态和 data directory；没有连接 Prod、请求 Tushare、部署、TaskRun、schedule 或生产 DDL。150,000 行正向场景成功应用 revision 159：Raw relation 及三个索引 OID/定义/valid/ready 状态保持不变并留在 `pg_default`，Serving 成为 0 B view；Raw/view 行数、唯一身份和 13 字段双向差异均一致。权限/grant option/comments/拒写 trigger 正确，三类 DML 均以 SQLSTATE `55000` 拒绝，Raw DML 与正式 writer 写入即时可见且回滚无残留。

150,001 行、业务字段差异、身份差异、未知依赖、缺失 Raw 实体日期索引五类负向场景都在 Serving DDL 前失败，另一个 DDL 后注入故障场景完整回滚；五类代表查询结果 hash 一致、计划切换到等价 Raw 索引且没有临时块。首次测试脚本曾把点查索引限定得过窄，migration 成功后测试主动判失败；实例已停止且没有把该结果算作通过。修正断言后从全新数据目录完整复跑七项场景并全部通过，最终实例已停止、数据目录已删除，报告位于 `/private/tmp/goldenshare_moneyflow_ths_m2_report.json`。

M2 据此通过，revision 159 无需修改。生产仍为 revision 158、双物理表和旧双写部署，约 468.18 MiB Serving 空间尚未释放；下一阶段只能单独授权 M3a，并在生产执行前实时复核身份、任务、schedule #4、worker、锁、长事务、全量差异、查询与磁盘水位。

### 2.32 2026-08-29 `P1-B3-moneyflow_ths-M3a` 生产切换事实

M3a 于 `18:47..18:57+08` 按维护合同完成。实时预检确认生产 commit `fe3caa3b`、revision 158，Raw/Serving 各 2,077,033 行和同数身份，21 个月十三字段双向差异全为 0，月峰值 116,993；开放任务、目标 node、锁、长事务和 catalog 依赖均无阻塞。切换前五类查询 hash 一致、索引等价、无临时块；对亚毫秒最大日期查询补做 21 轮交错复测后 Raw/Serving 中位数为 `0.109/0.121 ms`，其余有实际数据量的查询 Raw 相对 Serving 为约 `-9.2%..+6.0%`，无需 vacuum。

schedule #4 经正式服务暂停，config revision 127 留痕；scheduler 与 generic worker 停止后再次完成全量对账。`--maintenance-migration` 只安装 commit `754f6c78` 并应用 revision 158→159，没有构建、seed、unit 同步、任务创建或隐式服务重启。Raw OID `22825` 保持不变，三个索引继续 valid/ready 并位于 SSD；Serving 由 OID `22890` 物理表变为 OID `2039022`、0 B view。owner、Raw `lake_raw_reader SELECT`、Serving ACL 和拒写 trigger 正确，三类 DML 均以 SQLSTATE `55000` 拒绝，原 Serving 490,921,984 B 已释放。

连接池回收后，生产运行时 Definition 明确为 `raw_only_upsert`，Raw/view 五类查询继续 hash 一致并下推 Raw 索引，无临时块。正式 TaskRun `10128` 请求 `2026-08-28` 一个 unit，1 页短页读取/保存 `5,211/5,211`，reject、去重、重试为 0；目标日 5,211 行全部在任务窗口刷新，Raw/view 当日行数、身份和十三字段一致，全表最终仍各为 2,077,033 行和身份。

schedule #4 由 config revision 128 按原 cron、时区和 next/last timing 恢复 active；Web、generic worker、scheduler、日期完整性和 TaskRun 收尾服务均 active，两个健康端点为 200，开放任务、锁和长事务为 0。根盘最终可用 `52,283,146,240 B`，但确定性收益只认 catalog 释放量。M3a 据此通过；M3b 已登记 schedule #4 下一次自然工作流观察，不创建额外任务、不重复请求源端。

`2026-08-29 19:07+08` 再次只读核对生产调度：schedule #4 仍为 active，cron `0 20 * * 1-5`、时区 `Asia/Shanghai`，上次触发 `2026-08-28 20:00:21+08`，下一次 `2026-08-31 20:00+08`。当天为周六，不会出现自然 M3b；手工任务不能替代自然调度验收。同期 schedule #24/#2 的下一次分别为 `2026-08-31 18:30/21:02+08`，用于 `stk_auction_c-M3b`。三项待观察都不阻塞下一数据集只读 M0。

### 2.33 2026-08-29 `P1-B4-moneyflow_dc-M0` 只读复审事实

M0 只读取当前代码、CodeGraph、生产 PostgreSQL catalog/数据/查询计划和既有 TaskRun 诊断，没有部署、migration、schedule 变更、TaskRun 创建、Tushare 请求或业务数据写入。

1. 当前 Definition 继续显式请求 15 个源字段，交易日逐 unit，`limit/offset=6000`、短页结束且无任意最大页数。既有 schedule #4 TaskRun `9760` 对 `2026-08-28` 实际执行两页 `6000+7`，总读取/保存 6,007 行，最终短页、无截断、重试、reject 或去重。当前 storage 仍为 `raw_core_upsert`，writer 对同一 normalized batch 仅按 ORM 列过滤后双写，未发现 Serving 专属业务转换或写入旁路。
2. 生产 revision 159 下，Raw/Serving OID `22836/22899`，均为 owner `goldenshare_user`、SSD `pg_default` 物理表；Raw/Serving 总大小 `1,188,683,776/1,131,995,136 B`。两层列、主键 `(trade_date, ts_code)` 和两个二级索引签名一致且 valid/ready；Raw 的 `lake_raw_reader SELECT` 是必须保留的独立 ACL。未发现未知约束、外键、trigger、列 ACL、RLS、依赖 view/function、rewrite、扩展统计、security label、publication、继承或 comments。
3. 两层各 4,151,016 行和同数唯一身份，日期范围 `2023-09-11..2026-08-28`。36 个自然月全部 15 个业务字段双向 `EXCEPT ALL` 均为 0，最大业务行宽 148 B；月峰值为 `2025-07` 的 139,877 行。独立 migration 容量门禁据此固定为 170,000/170,001 行，不复制前项阈值；逐月对账必须在 16 MiB `work_mem` 和 300 秒 statement timeout 内完成，禁止无界全表差集。
4. Raw/Serving all-visible 为 90.57%/88.18%，五类代表查询都命中等价索引；交错样本中 Raw 最大有意义正向退化约 10.3%，低于本项 20% 停止门禁，当前不需要 vacuum。仓库内未发现 Biz/QTF/frontend/DG Serving 直读，Lake 已显式读取 Raw 的同一 15 字段；Ops freshness/date completeness 随 Definition target 改读 Raw。
5. Raw `fetched_at` 与 Serving `updated_at` 全量一致，但 2,350,240 行历史 `created_at` 不同。已登记消费者不读取审计时间，所以可继续使用一期 `fetched_at AS created_at/updated_at` view 合同；该变化、relation OID/relkind/catalog 以及仓库外未登记消费者仍是显式透明边界。
6. schedule #4 为本数据集唯一自动入口，M3a 必须暂停/恢复该 workflow 并停止 generic worker。M0 时开放 TaskRun、目标 node、锁和长事务均为 0；下一次自然运行是 `2026-08-31 20:00+08`。同一父 TaskRun 可以分别验收 `moneyflow_ths` 与 `moneyflow_dc` 的 M3b，但两个 node 必须各自闭环，不能互相替代。

M0 据此通过。下一步只能在独立授权后进入 M1：修改本数据集 Definition storage、补 Raw ORM 已存在索引的 metadata、新增接编码时真实 head 的独立 migration，并增加 170,000/170,001、15 字段、依赖、ACL、三类 DML、回滚、writer 和查询计划测试；不得顺带修改共享 ingestion/worker/schedule，也不得启动 `stk_limit`。

### 2.34 2026-08-29 `P1-B4-moneyflow_dc-M1` 编码事实

M1 已按 2.33 节冻结合同完成。Definition 只把 `moneyflow_dc` 改为 `raw_only_upsert + raw_with_serving_view`，写入 DAO 与 target 指向 `raw_tushare.moneyflow_dc`；15 个 source fields、交易日 point/range、`ts_code` filter、6,000 行分页、短页结束、manual/schedule/retry 和 workflow 均未改变。Raw ORM 只登记生产已存在的 `(trade_date)` 与 `(ts_code, trade_date)` 索引，不创建或移动物理对象；共享 writer、DAO factory、resolver、request builder、normalizer、source client、Ops、前端和 Lake 代码均未修改。

独立 revision `20260829_000161` 接编码时唯一真实 head `20260829_000160`。它固定 `lock_timeout=15s`、`statement_timeout=300s`、`work_mem=16MB`，要求 Raw/Serving 为 owner 当前角色的 `pg_default` 物理表，校验全部列、主键、四个二级索引、ACL/comments、未知依赖和共享拒写函数；按 Raw→Serving 锁序逐月执行 170,000 行/层容量、唯一身份和 15 字段双向差集，全部通过后才原子创建显式 view、恢复 metadata 并挂独立 DML 拒写 trigger。migration 禁止 `CASCADE`、Raw DDL/DML、`CREATE INDEX`、共享函数重建和自动 downgrade。

专项与共享回归覆盖 Definition/Resolver/filter/pagination 不漂移、ORM 类型和索引、ServingPublish 无旁路、Raw freshness target、writer 只写 Raw、migration 禁止项/顺序和完整 PostgreSQL 离线 SQL。M1 没有连接数据库、请求 Tushare、部署、应用 migration、创建 TaskRun 或修改 schedule。当前开发代码和 revision 161 已就绪，但生产仍为 revision 159、两张物理表和已部署旧版双写，1,131,995,136 B 尚未释放。下一步只能在独立授权后进入隔离 PostgreSQL M2，不得越级执行生产 M3a，也不得随本项启动 `stk_limit`。

### 2.35 2026-08-29 `P1-B4-moneyflow_dc-M2` 隔离验收事实

M2 使用本轮创建的 PostgreSQL 18.4 一次性实例，关闭 TCP 监听并只开放随机 Unix socket。Alembic 前逐次核对独立 env 最终解析的 URL、数据库、用户、socket、端口、`inet_server_addr=NULL`、恢复状态和 data directory；应用角色明确为非超级用户且无建库、建角色或复制权限，临时 `gs_raw_cold_hdd` 路径也位于隔离根。全程没有连接 Prod、请求 Tushare、部署、创建 TaskRun、修改 schedule #4 或执行生产 DDL。

170,000 行/月正向场景成功应用 revision 160→161。Raw relation 和三个索引 OID/定义/valid/ready 状态保持不变并继续位于 SSD `pg_default`；Serving 切换为 0 B 普通 view。Raw/view 均为 170,000 行、170,000 个身份，15 字段双向差异和审计时间投影差异为 0。owner、Raw/Serving reader、SELECT grant option、relation/column comments 与独立拒写 trigger 正确恢复；Serving 三类 DML 均返回 SQLSTATE `55000`。Raw DML 和正式 `DatasetWriter` 写入在同一事务内由 view 即时可见，回滚后无残留，writer 目标为 `raw_tushare.moneyflow_dc`。

170,001 行、业务字段差异、身份差异、未知 view 依赖、缺失 Raw 实体日期索引五类场景均在 Serving DDL 前失败，并保持 revision、两张物理表、OID、索引、行数和 metadata 快照。DDL 后注入故障场景完整回滚到 revision 160 和原物理对象。五类代表查询的结果 hash 一致，计划下推等价 Raw 索引且没有临时块；隔离单次时延没有明显退化，但不外推为生产 SLA。实例最终停止、临时数据根删除，报告保留在 `/private/tmp/goldenshare_moneyflow_dc_m2_report.json`。

M2 据此通过，revision 161 与业务代码无需修改。生产仍为 revision 159、两张物理表和旧版双写，1,131,995,136 B 尚未释放；下一阶段只能独立授权 M3a，重新执行生产身份、head、schedule #4、worker、开放任务、锁、长事务、全量差异、查询与磁盘门禁。`stk_limit` 不在本阶段范围。

### 2.36 2026-08-29 `P1-B4-moneyflow_dc-M3a` 生产验收事实

M3a 于 `22:54..23:09+08` 按正确维护顺序完成。生产预检为 PostgreSQL 16.13、revision 160，远端代码由 `96b1a865` 快进到 `dc191135`；schedule #4 暂停、scheduler/generic worker 停止后，开放 TaskRun、目标 node、完整性审计 run、目标锁、等待锁和超过 30 秒事务均为 0。36 月、4,151,016 行、唯一身份和 15 字段双向差异再次为 0，月峰值 139,877。

迁移前日期完整性查询因 Raw `trade_date` 统计陈旧触发真实性能阻断：20 轮 Raw/Serving 中位为 `167.036/98.824 ms`。经明确同意执行目标列 ANALYZE 后变为 `98.261/96.454 ms`，差异约 1.87%，planner 恢复并行计划；这一步只更新统计，不改业务数据。随后 maintenance migration `160→161` 一次成功：Raw OID `22836` 与索引 OID `22843/22845/22846` 保持不变并留在 SSD，Serving 变为 OID `2041227` 的 0 B view，原 1,131,995,136 B relation 被删除。Raw/view 全表行数、身份、15 字段与审计投影一致；三类 DML 均以 SQLSTATE `55000` 拒绝。

迁移后五类查询 hash、索引与临时块门禁通过；单日三轮样本的噪声以 20 轮交替复测关闭，Raw/view 中位为 `13.602/13.371 ms`。创建任务前，临时脚本曾因远端 `/private/tmp` 权限和误用仓库 `.env.web.local` 两次在副作用前失败，均未创建 TaskRun 或调用 Tushare；核对 systemd unit 后改用其真实 `/etc/goldenshare/web.env`。正式 TaskRun `10149` 对 `2026-08-28` 成功执行 `6000+7` 两页，读取/保存 `6007/6007`，reject、去重、重试均为 0；Raw/view 当日各 6,007 行和身份，15 字段差异为 0，全表仍为 4,151,016 行。schedule #4 通过 config revision `129/130` 暂停并原样恢复，cron、时区、下一次 `2026-08-31 20:00+08` 未漂移；五个常驻服务与健康端点正常，最终开放任务、锁和长事务为 0。

M3a 据此通过，确定性 catalog 毛释放量增加 1,131,995,136 B。`P1-B4-moneyflow_dc-M3b` 已登记到 `2026-08-31 20:00+08` 的统一自然任务台账，不创建额外任务、不重复请求 Tushare；该待观察项没有阻塞现已完成的 `P1-B4-stk_limit-M0`。

### 2.37 2026-08-29 `P1-B4-stk_limit-M0` 只读复审事实

M0 只读取当前代码、CodeGraph、生产 PostgreSQL catalog/汇总数据/查询计划和既有 TaskRun 诊断，没有部署、migration、schedule 变更、TaskRun 创建、Tushare 请求或业务数据写入。

1. 当前 Definition 显式请求 `trade_date, ts_code, pre_close, up_limit, down_limit` 五字段，交易日逐 unit，`limit/offset=5800`、短页结束且无任意最大页数。source client 每页继续传完整 fields。schedule #24/#2 的 TaskRun `9747/9773` 对 `2026-08-28` 都执行 `5800+1968=7768` 两页，最终短页、无截断、重试、reject 或去重。当前 storage 仍为 `raw_core_upsert`，writer 只对同一 batch 做 ORM 列投影后双写，未发现 Serving 专属业务转换或写入旁路。
2. 生产 revision 161 下，Raw/Serving OID `21604/21614`，均为 owner `goldenshare_user`、SSD `pg_default` 物理表；总大小为 `661,782,528/664,354,816 B`。两层五业务列、主键 `(ts_code, trade_date)` 和日期索引签名一致且 valid/ready；Raw 的 `lake_raw_reader SELECT` 是必须保留的独立 ACL。未发现非主键约束、FK、用户 trigger、RLS、外部 view/function 依赖或 comments。
3. 两层各 4,608,112 行和同数唯一身份，日期 `2024-01-02..2026-08-28`。32 个自然月全部五业务字段双向 `EXCEPT ALL` 为 0，最大业务行宽 61 B；月峰值为 `2026-07` 的 177,009 行。因此独立 migration 固定 220,000/220,001 行月门禁、16 MiB `work_mem` 和 300 秒 statement timeout，禁止无界全表差集。
4. Raw/Serving all-visible 为 93.91%/94.28%，无需先 vacuum。市场情绪 20 日复合键 join 与 64 日存在性两类代表查询使用等价索引和同形计划；Raw join 单次快于 Serving，存在性查询交错热样本中位约慢 10.3%，低于本项 20% 停止线。M2/M3a 仍须复测 hash、索引下推、临时块和交错中位时延。
5. Raw `fetched_at` 与 Serving `updated_at` 全量一致，但 514,328 行历史 `created_at` 不同。仓库内唯一业务读取为 `MarketMoodCalculator` 与 `MarketMoodWalkForwardValidationService`，均不读取审计时间；未发现 frontend/QTF/DG Serving 直读，Lake 已显式读取 Raw 五字段。因此沿用 `fetched_at AS created_at/updated_at` view 合同，但不承诺历史 `created_at`、OID/relkind 或约束 catalog 透明。
6. 自动入口是同一 `daily_market_close_maintenance` workflow 的 schedule #24（工作日 18:30）和 #2（工作日 21:02），`stk_limit` 是第 9 个 step。M3a 必须同时暂停/恢复两者并停止 scheduler/generic worker；M3b 必须分别验收两个自然父任务的目标 node。M0 时开放任务、目标锁和超过 30 秒事务均为 0；下一次自然触发为 `2026-08-31 18:30/21:02+08`。
7. Raw ORM 已准确声明生产日期索引，M1 不需要补 ORM metadata。M1 只允许修改本数据集 Definition storage、新增接编码时真实 head 的独立 migration、专项和必要共享回归；不得修改 source fields、分页、日期/unit、filter、workflow、共享 ingestion/worker/schedule，也不得连接数据库或执行迁移。

M0 据此通过。下一步只能在独立授权后进入 M1；M1 migration 必须在任何 Serving DDL 前完成 relation/owner/SSD/列/主键/索引/ACL/依赖/220,000 行月容量检查，按 Raw→Serving 锁序完成锁内 32 月五字段对账，再原子建立显式只读 view 与独立 DML 拒写 trigger。M0 不授权生产操作或源端请求。

### 2.38 2026-08-30 `P1-B4-stk_limit-M1` 编码与自动化验证

M1 已严格按 2.37 与一期 LLD 落地，且未连接任何 PostgreSQL、应用 migration、部署、创建 TaskRun、修改 schedule 或请求 Tushare。

1. Definition 只把 storage delivery 改为 `raw_stk_limit/raw_stk_limit`、`target_table=raw_tushare.stk_limit`、`raw_with_serving_view / raw->serving_view / raw_only_upsert`；源字段、日期/unit、`ts_code` filter、5,800 行分页、能力与工作流全部保持原样。freshness 与日期主体矩阵由同一 Definition 自动改读 Raw，Biz 仍使用原 Serving ORM。
2. 独立 revision `20260830_000162` 接真实 head `20260829_000161`。它在所有 Serving DDL 前校验双方物理关系、owner、列、PK、唯一日期索引、Raw SSD 与 `lake_raw_reader SELECT`、Serving ACL/comment、未知依赖和每层每月 220,000 行上限；使用 Raw `SHARE` → Serving `SHARE` → 月度身份/五字段双向差集 → Serving `ACCESS EXCLUSIVE` 锁序。
3. migration 只把原 Serving 表原子替换为显式同名 view，投影五业务字段及 `fetched_at AS created_at/updated_at`，恢复 metadata 并挂本 view 独立三类 DML 拒写 trigger。代码明确禁止 `CASCADE`、Raw DDL/DML、Raw 索引重建、共享函数重建和自动 downgrade。
4. 专项测试覆盖 storage-only、未知 filter、模型/索引、ServingPublish 旁路、`5800+1` 两页逐页完整 fields、Raw-only writer、migration 顺序/禁止项/offline SQL；共享测试同步覆盖 registry、freshness Raw target 和日期主体矩阵 Raw target。

M1 据此通过。生产仍为 revision 161、双物理表和旧双写部署，尚未释放 664,354,816 B；下一阶段只能单独授权 M2，在隔离 PostgreSQL 真实验证 migration、220,000/220,001 行容量、ACL/依赖/字段/身份/索引 fail-closed、三类 DML、事务回滚、writer 即时可见与市场情绪查询计划。

### 2.39 2026-08-30 `P1-B4-stk_limit-M2` 隔离 PostgreSQL 验收

M2 使用只监听随机 Unix socket 的 PostgreSQL 18.4 一次性实例，应用角色为无超级用户、建库、建角色或复制权限的 `stk_limit_m2_app`。每次 Alembic 前均通过独立 env 文件核对目标 URL、数据库、用户、socket、端口、恢复状态和 data directory，未读取仓库 `.env.web.local`。本轮没有连接 Prod、请求 Tushare、部署、创建 TaskRun 或修改 schedule/workflow。

1. 220,000 行单月正向库成功从 revision 161 升级到 162；Raw relation 与主键/日期索引 OID、定义、valid/ready 和 `pg_default` placement 均保持不变，Serving 成为 0 B view。Raw/view 行数、唯一身份、五字段双向差异和审计时间投影差异均为 0。
2. owner、Raw `lake_raw_reader SELECT`、Serving reader grant option、relation/column comments 和独立拒写 trigger 全部恢复；Serving 三类 DML 均返回 `55000`。Raw 三类 DML 和正式 `DatasetWriter` 写入均由 view 立即可见，回滚后无残留。
3. 220,001 行、业务字段差异、身份差异、Raw 列类型漂移、未知 view 依赖、缺失 Raw 日期索引和额外 Raw ACL 七类场景都在 Serving DDL 前失败并保持完整快照；切换后故障注入也完整回滚到 revision 161 与两张物理表。
4. 20 日市场情绪 join 和交易日存在性查询前后结果 hash 一致，切换后分别下推 Raw 主键和日期索引，临时块为 0；隔离时延只作为计划证据，不作为生产 SLA。

实例已停止，临时数据根已删除；报告位于 `/private/tmp/goldenshare_stk_limit_m2_report.json`。M2 据此通过，revision 162 与业务代码无需修改。生产仍为 revision 161、双物理表和旧双写部署，664,354,816 B 尚未释放；下一阶段只能独立授权生产 M3a，并实时复核版本/head、schedule #24/#2、worker、开放任务、锁、长事务、32 月全字段差异、查询与磁盘门禁。

### 2.40 2026-08-30 `P1-B4-stk_limit-M3a` 生产验收事实

M3a 于 `06:50..06:59+08` 按维护顺序完成。生产预检为 PostgreSQL 16.13、revision 161、远端旧 commit `dc191135`；Raw/Serving 各 4,608,112 行和身份，32 月五字段差异为 0、月峰值 177,009。schedule #24/#2 经 config revision `131/132` 分别暂停，scheduler/generic worker 停止后开放任务、目标 node、完整性审计 run、目标锁、等待锁和超过 30 秒事务均为 0。

`--maintenance-migration` 只拉取并安装 `da84a32a`、应用 revision 161→162，不构建、不 seed、不同步 unit、不创建任务或隐式恢复服务。Raw OID `21604` 与索引 OID `21611/21613` 保持不变并继续位于 SSD；Serving 成为 OID `2041913` 的 0 B view，确定性 catalog 毛释放 664,354,816 B。Raw/view 全表行数、身份、日期范围、32 月五字段和审计投影一致；owner/ACL/Raw reader/拒写 trigger 正确，三类 DML 均返回 `55000`。

两类市场情绪查询结果 hash 一致、view 下推 Raw、无临时块，切换后时延未超过 20% 停止线。回收 Web 与 Ops 连接池后，正式 TaskRun `10182` 对 `2026-08-28` 成功执行 `5800+1968` 两页，读取/保存 `7,768/7,768`，reject、去重、重试均为 0；Raw/view 当日各 7,768 行和身份、五字段差异 0，全表仍为 4,608,112 行。第一次远端 Python 创建命令只在解释阶段失败，服务未执行且无 TaskRun/Tushare 请求，回查后才正式创建唯一任务。

schedule #2/#24 经 config revision `133/134` 原样恢复 active，下一次仍为 `2026-08-31 21:02/18:30+08`；九个常驻服务与两个健康端点正常，最终开放任务、锁和长事务为 0。维护期间远端另有一次 fast-forward 到 `4e54dec8`，该提交是 `da84a32a` 的后继且只改财务数据集文档，不涉及本项运行合同。M3a 据此通过；M3b 已登记为两个自然父任务分别验收，不创建额外任务或重复请求源端。

### 2.41 2026-08-31 最后四项 M3b 统一结案事实

本轮只读复用三个既有自然父 TaskRun，没有创建额外任务、修改 schedule 或重复请求 Tushare：schedule #24 的 TaskRun `10343` 为 `success`，schedule #4 的 TaskRun `10359` 为 `success`，schedule #2 的 TaskRun `10371` 因范围外 `anns_d` 节点失败而为 `partial_success`。验收按一期合同逐个读取目标 `TaskRunNode`，没有用父任务顶层状态代替节点证据。

1. `stk_auction_c`：TaskRun `10343` node `16058` 为 1 个空短页、读取/保存 `0/0`；TaskRun `10371` node `16129` 为 1 页短页、读取/保存 `5,551/5,551`。两个目标节点均 0 reject/去重/重试且未截断；最终 Raw/view 各 5,551 行和身份，九字段差异为 0，符合 18:30 未就绪、21:02 就绪的既有源端时序。
2. `stk_limit`：TaskRun `10343/10371` 的 node `16064/16135` 均为两页 `5,800+1,968`、读取/保存 `7,768/7,768`，0 reject/去重/重试且未截断；最终 Raw/view 各 7,768 行、五字段差异为 0。第二轮完成后行数不增，证明同日自然重跑幂等。
3. `moneyflow_ths`：TaskRun `10359` node `16110` 为 1 页短页、读取/保存 `5,210/5,210`，0 reject/去重/重试；最终 Raw/view 各 5,210 行和身份，13 字段差异为 0。
4. `moneyflow_dc`：同一 TaskRun 的 node `16111` 为两页 `6,000+7`、读取/保存 `6,007/6,007`，0 reject/去重/重试；最终 Raw/view 各 6,007 行和身份，15 字段差异为 0。

四张 Raw 表的目标日行均由对应最终 node 执行窗口刷新，四个 Serving relation 均继续为 `relkind=v`、0 B，Raw relation 继续为物理表。`P1-B3-stk_auction_c-M3b`、`P1-B3-moneyflow_ths-M3b`、`P1-B4-moneyflow_dc-M3b` 与 `P1-B4-stk_limit-M3b` 据此全部通过，一期 12 项正式结案。

父工作流的范围外残余单独备案：TaskRun `10343` 的 `irm_qa_sh` node 读取 260、保存 259、reject 1，reason code 为 `write.duplicate_conflict_key_in_batch:row_key_hash`；TaskRun `10371` 的 `anns_d` node 因 Tushare `40203` 当日 200,000 次配额耗尽而失败。它们不属于本期 12 个目标 node，不重新打开 raw 直出结论；`anns_d` 既有批内身份冲突 TODO 也继续保留。
...
| 6 | `raw_tushare.stk_limit` | 661,782,528 B（631.13 MiB） | raw 直出 P1-B4 | 2026-08-29 M0 当前值；raw 必须留 SSD；从 Track A 移除 |
...
| `moneyflow_dc` | 1,131,995,136 B（约 1.054 GiB） | **36 月、4,151,016 行、15 业务字段双向差异为 0** | 是 | **M0/M1/M2/M3a/M3b 全部通过并结案；生产 revision 161、0 B raw-backed view，已释放 1,131,995,136 B；Raw 固定留 SSD** |
...
| `moneyflow_ths` | 468.18 MiB | **21 月、2,077,033 行、13 业务字段双向差异为 0** | 是 | **M0/M1/M2/M3a/M3b 全部通过并结案；revision 159 已切换 0 B view 并释放 490,921,984 B；Raw 固定留 SSD** |
| `stk_auction_c` | 372.19 MiB | **20 月、2,255,593 行、9 业务字段双向差异为 0** | 是 | **M0/M1/M2/M3a/M3b 全部通过并结案；生产 revision 158、0 B raw-backed view，已释放 390,266,880 B** |
...
| `stk_limit` | 664,354,816 B（633.58 MiB） | **32 月、4,608,112 行、5 业务字段双向差异为 0** | 是 | **M0/M1/M2/M3a/M3b 全部通过并结案；生产 revision 162、0 B Serving view，已释放 664,354,816 B；Raw 固定留 SSD** |
...
2026-08-27 又完成 `moneyflow_ind_dc` 当前 339,268 行、36 个自然月和全表 18 字段的双向差集；2026-08-28 完成 `dc_daily` 当前 634,116 行、32 个自然月和全表 13 字段，`suspend_d` 当前 640,504 行、320 个自然月和包含 `id/row_key_hash` 的 6 个业务字段，以及 `stk_auction_o` 当前 2,183,621 行、20 个自然月和全部 9 个业务字段的双向差集；2026-08-29 又依次完成 `stk_auction_c` 当前 2,255,593 行、20 个月 9 字段，`moneyflow_ths` 当前 2,077,033 行、21 个月 13 字段，以及 `moneyflow_dc` 当前 4,151,016 行、36 个月 15 字段的双向差集，差异均为 0。因此已有全量等价证据的严格候选共 18 组，但各次证据属于不同生产时点，不能伪装成同一快照求和。

一期各项已确认释放：P1-B0 237,568 B、P1-B1 行业 9,756,672 B、概念 43,958,272 B、margin 344,064 B、`moneyflow_ind_dc` 88,358,912 B、`dc_daily` 161,710,080 B、`suspend_d` 222,199,808 B、`stk_auction_o` 382,353,408 B、`stk_auction_c` 390,266,880 B、`moneyflow_ths` 490,921,984 B、`moneyflow_dc` 1,131,995,136 B、`stk_limit` 664,354,816 B，累计 **3,586,457,600 B（约 3,420.31 MiB / 3.340 GiB）**。该数字只表示各自切换时已删除 relation 的确定性 catalog 字节，不用瞬时 `df` 差值替代；普通 vacuum 不计入释放量，也不拿它替换 2026-08-23 同时点的 3.305 GiB 基线。
...
| P1-B3 | 9 | `stk_auction_c` | `raw_tushare.stk_auction_c` → `core_serving.equity_auction_close` | 372.19 MiB | 2,255,593 / 2,255,593 | **M0/M1/M2/M3a/M3b 全部通过并结案；已释放 390,266,880 B** |
| P1-B3 | 10 | `moneyflow_ths` | `raw_tushare.moneyflow_ths` → `core_serving.equity_moneyflow_ths` | 468.18 MiB | 2,077,033 / 2,077,033 | **M0/M1/M2/M3a/M3b 全部通过并结案；已释放 490,921,984 B** |
| P1-B4 | 11 | `moneyflow_dc` | `raw_tushare.moneyflow_dc` → `core_serving.equity_moneyflow_dc` | 1,131,995,136 B（约 1.054 GiB） | 4,151,016 / 4,151,016 | **M0/M1/M2/M3a/M3b 全部通过并结案；已释放 1,131,995,136 B** |
| P1-B4 | 12 | `stk_limit` | `raw_tushare.stk_limit` → `core_serving.equity_stk_limit` | 664,354,816 B（633.58 MiB） | 4,608,112 / 4,608,112 | **M0/M1/M2/M3a/M3b 全部通过并结案；已释放 664,354,816 B** |
|  |  | **合计** |  | **3,548,766,208 B，约 3.305 GiB** |  |  |

内部批次大小仍以 2026-08-23 全名单同一时点基线统计：P1-B0 237,568 B；P1-B1 54,050,816 B（约 51.55 MiB）；P1-B2 471,261,184 B（约 449.43 MiB）；P1-B3 1,244,495,872 B（约 1.159 GiB）；P1-B4 1,778,720,768 B（约 1.657 GiB）。`moneyflow_ind_dc`、`dc_daily`、`suspend_d`、`stk_auction_o`、`stk_auction_c`、`moneyflow_ths` 与 `moneyflow_dc` 的各自 M0 当前 Serving 值已分别刷新为 88,358,912 B、161,710,080 B、222,199,808 B、382,353,408 B、390,266,880 B、490,921,984 B 与 1,131,995,136 B；不同 M0 时点不混合重算批次合计。

`moneyflow_ths` 的 2026-08-29 M0 已按当前代码和生产事实独立闭环：Raw/Serving 各 2,077,033 行，日期 `2024-12-19..2026-08-28`；21 个自然月全部 13 个业务字段双向差集为 0，月峰值 116,993 行，业务行最大 134 B。16 MiB work_mem 下全范围逐月双向对账约 25.6 秒，因此本项固定 150,000/150,001 行月门禁。五类 Raw/Serving 查询结果一致、索引等价、Raw 最大正向退化 9.43%，无需复制 `stk_auction_o` 的 vacuum。Lake 已直接读取 Raw，唯一自动入口为 schedule #4；历史 Serving 有 35,990 行 `created_at != updated_at`，按一期既有边界不承诺该审计字段历史值透明。完整证据和 M1 禁止项见一期 LLD 的本数据集专节。

`P1-B3-moneyflow_ths-M1` 已严格按上述独立合同完成：Definition 只切换 storage delivery 为 Raw-only，Raw ORM 只补两个生产既存索引 metadata；独立 revision `20260829_000159` 接真实 head 158，显式固化 13 字段、`(trade_date, ts_code)` 身份、两个二级索引、150,000 行/层/月、Raw→Serving 锁序、依赖/ACL/comment/tablespace 和 DML 拒绝门禁。专项及共享回归覆盖 planner/filter/6,000 行分页不漂移、Raw-only writer、freshness Raw target、ServingPublish 旁路和 migration offline SQL/禁止项。M1 当时未连接数据库、请求 Tushare、部署、创建 TaskRun 或修改 schedule；后续 M2/M3a 的闭环证据见第 2.31/2.32 节。
...
| P1-B3 | `stk_auction_o` → `stk_auction_c` → `moneyflow_ths` | 验证百万行级数据等价、切换和查询性能 | **三项 M0/M1/M2/M3a/M3b 全部通过并结案** |
| P1-B4 | `moneyflow_dc` → `stk_limit` | 释放一期主要空间并验收市场情绪消费者 | **两项 M0/M1/M2/M3a/M3b 全部通过并结案** |
...
5. `moneyflow_ths-M0/M1/M2/M3a/M3b` 已独立完成 150,000/150,001 行、13 字段、schedule #4 暂停/恢复、Raw ORM 索引 metadata、历史审计时间边界、生产查询、最小 TaskRun 和自然数据链验收；不得复制竞价数据集的 160,000 行、#2/#24 或 vacuum 结论；
6. Track A 与一期外 `daily_basic/dc_member` 是否推进，继续后置，不由本 LLD 自动授权。

## 9. 一期结案后的残余不确定性与独立 TODO

1. 一期 12 组都已完成精确行数、唯一身份、全部业务字段的有界双向差集、生产 raw 直出切换及自然 M3b 数据链验收；`stk_limit` 生产最终复核为 32 月五字段全 0、月峰值 177,009，TaskRun `10182/10343/10371` 均通过。
...
7. Heat 上游时间合同已由 schedule #4/#36 在 `2026-08-27` 的自然成功链闭环；一期 12 项的 M0/M1/M2/M3a/M3b 均已完成并结案。`moneyflow_ind_dc` 与 `stk_auction_o` 的标准部署提前迁移偏差继续作为历史教训保留。
8. `anns_d` 的既有批内 `row_key_hash` 冲突仍是独立数据质量 TODO；2026-08-31 TaskRun `10371` 又因 Tushare `40203` 当日 200,000 次调用配额耗尽而失败。TaskRun `10343` 的 `irm_qa_sh` node 另有 1 行同 reason code reject。这些范围外问题不重新打开一期 12 项结论，但需要在各自专项继续审计。
