...
更新时间：2026-09-12。本文维护导航、待办边界和历史去向，不再重复定义日常维护规则。
...
本轮仅处理部署技能说明、Wealth README 与治理账本收尾，修改完成待 review，详见 §4.12。此前 §4.6–4.11 已提交；没有其他已批准待实施批次。用户随后批准将本机两份技能入口改为仓库文件的符号链接，已完成路径与读取核验，不宣称当前会话缓存已刷新。

本轮 review、提交结束后，通用治理转为按具体错误或实现变更维护，不再自动开启下一轮全仓扫描；不代表全仓每份文档均已验收。
...
状态：已按用户指令提交 `ef1f2b8a`。仅文档；其他任务改动保留。
...
状态：已按用户指令提交 `c26a641e`。不改业务代码、截图基线或生产数据。
...
状态：已按用户指令提交 `7ad169ff`。仅文档及 frontend AGENTS 中同一过期事实，未改 UI、API、测试代码、截图或配置。
...
状态：已按用户指令提交 `d9f87f90`。保留三份文档，不改部署/启动/预检脚本，不安装、不启停、不迁移、不访问生产。
...
状态：已按用户指令提交 `c005526b`。三份均保留，不改业务代码、API、数据库或执行器。
...
状态：已按用户指令提交 `b51d688c`。仅修改根 README、docs/AGENTS、维护基线及本记录；未改代码、脚本、配置、索引或数据。
...
<a id="skills-wealth-governance-closeout-20260912"></a>

### 4.12 2026-09-12 操作技能与项目入口收尾

状态：三批修改完成，待统一 review，未提交。仓库内四份说明，另按用户后续批准替换本机两个 SKILL.md 为符号链接；未修改 AGENTS.local、程序、业务配置或数据。

| 批次 | 处理 | 依据 |
| --- | --- | --- |
| 两份部署技能 | G1：本地启动前明确隐式安装检查；远程撤下“三服务即全部”的验收，按实际发布模式检查，区别只读/重启/发布。G2：公共矩阵回链正式流程，保留入口与故障边界 | local-build-and-run 安装分支、deploy wrapper/layered 的九类服务与模式分支、scripts/AGENTS 和正式流程 |
| Wealth README | G1：撤下“仅骨架、没有市场总览”；G2：保留独立前端、阅读导航与实际命令，补 /wealth/ base 和代理边界，不宣称模块全部验收 | WealthRouter、package.json、vite.config.ts 与 wealth/AGENTS |
| 治理账本 | 当前入口指向本节，§4.6–4.11 状态补已核实提交 SHA；历史证据、数量与锚点保留，不把旧阶段重新列为待办 | Git 提交记录及对应文件变更 |

使用文档治理、技能编写与前端 QA 指引，分别约束历史事实、技能授权范围及 Wealth 独立工程边界。仅说明更新，不改变依赖矩阵、API、CLI 或部署行为。未安装、启动、访问生产或运行部署/迁移。
