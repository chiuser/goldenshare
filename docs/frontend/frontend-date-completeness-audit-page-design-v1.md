...
1. 自动审计 Tab 可创建、暂停、恢复、删除计划。
2. 列表展示计划名称、审计窗口、cron、状态、最近结论和下次运行时间。
3. 配置 Drawer 支持固定范围和滚动窗口。
4. 日历口径第一版只展示“默认 A 股交易日历”，不暴露港股或自定义交易所选择。
5. 自动计划只创建独立审计 run，不跳任务中心，不复用 TaskRun。
