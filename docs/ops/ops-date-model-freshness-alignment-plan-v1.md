...
| `stk_period_bar_week` | `week_last_open_day` | 同上 | 同上 |
| `stk_period_bar_adj_week` | `week_last_open_day` | 同上，本次问题触发点 | 同上 |
| `index_monthly` | `month_last_open_day` | expected date 被算成最新开市日；actual 可能取到非月末日期 | 只按已到达的月末交易日锚点判断 |
| `stk_period_bar_month` | `month_last_open_day` | 同上 | 同上 |
| `stk_period_bar_adj_month` | `month_last_open_day` | 同上 | 同上 |
...
3. 对 shared table 继续保留必要的 `freq=week/month` 过滤。
4. 对 `month_window_has_data` 改为判断窗口内是否存在数据。
