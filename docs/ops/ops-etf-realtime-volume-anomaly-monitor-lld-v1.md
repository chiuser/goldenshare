...
6. 每行直接展示 ETF 代码/名称、交易所、监控分组、展示排序、启用监控开关和操作按钮。
7. 未加入监控池的行显示“添加”按钮；点击后立即提交该行的分组、排序和启用状态，不需要滚动到底部或再点击统一保存。
8. 添加成功后，该行按钮立即显示为浅绿色、置灰的“已添加”；抽屉保持打开，运营可以继续添加其他行。
9. 已在监控池中的 ETF 直接显示“已添加”，行内配置控件只读，不可重复添加。
10. 新增和编辑页面均不展示备注输入，也不提交备注字段；后端已有的 `note` 字段不作为本轮交互入口。
