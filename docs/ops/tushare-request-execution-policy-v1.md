...
## 4. 源资料定位：不要只按同名 API 算接入

原 2026-05-03 接入盘点的手工数量与分类已失效，正文下线；当前注册、接口资料和项目验收分别按 §1 查询，不再维护第二份长名单。保留其中仍有用的定位提示：

- `trade_cal` 在源索引有 0026、0137 两份资料；内部泛交易所日历映射不能按文档篇数重复算成两个数据集。
- `index_daily` 对应 0095 指数日线，不能因同名就把 0155 南华期货指数日线算入同一接入结论。
- `stk_mins` 对应 0370 股票分钟，不对应 0387 ETF 分钟；后者已有独立 `etf_mins` Definition。名称相近不等于共享合同。
- 0315/0316/0317 线上资料不存在，是旧盘点中用户确认的历史事实，不是本轮线上复核；本地源资料及索引仍保留，是否可用需按当前来源另行验证。

注册表中有定义只证明代码接入，不证明生产已启用或完成数据验收。旧盘点全文从 Git 历史追溯，本次不删除源文档、源索引或现有数据集。
