...
- 分页：`limit=8000`（文档单次上限），`offset` 递增分页。
- 默认请求：默认按 `start_date/end_date` 过滤交易日，逐 `trade_date` 请求；若只给 `trade_date` 则按单日请求。
- 自动维护：不加入每日收盘后维护工作流；通过 `remote_kpl_list_ready` 源站探测，在次日 08:30 发布后按已发布目标交易日创建维护任务。
...
- 默认请求：按 `ts_code x freq x datetime_window` 规划请求；未传 `ts_code` 时按股票池全量扇出；用户必须选择 `freq`。
- 用户传参规则：用户可显式传 `ts_code`、`freq`；`freq` 支持 `1min/5min/15min/30min/60min` 多选，按频度扇出请求；不暴露具体小时、分钟、秒输入，也不暴露股票池 offset/limit。
- 时间执行：用户侧仍选择交易日或交易日区间；程序内部转换为 `09:00:00~19:00:00` datetime 大窗口。`trade_date` 单日生成一个窗口，`start_date/end_date` 区间生成一个连续窗口，不再逐交易日拆分，不再拆上午/下午 session；执行计划使用分钟行情专用维护规划，不复用普通交易日维护规划。
...
- 分页：`limit=6000`（文档单次上限），`offset` 递增分页。
- 默认请求：按自然月最后一天锚点请求；`freq` 在实现内固定为“月线”，不对用户暴露。
- 用户传参规则：用户只可显式传 `ts_code`；`freq` 为系统内置固定参数，不接受用户输入。
- 时间执行：区间维护由 planner 生成范围内自然月最后一天，作为源接口 `trade_date`。
...
- 源文档：`docs/sources/tushare/股票数据/行情数据/0365_股票周_月线行情(复权--每日更新).md`
- 分页：`limit=6000`（文档单次上限），`offset` 递增分页。
- 默认请求：按自然周周五锚点请求；`freq` 在实现内固定为“周线”，不对用户暴露。
- 用户传参规则：用户只可显式传 `ts_code`；`freq` 为系统内置固定参数，不接受用户输入。
- 时间执行：区间维护由 planner 生成范围内自然周周五，作为源接口 `trade_date`。
