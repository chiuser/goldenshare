...
状态：下游监控设计基线，待 ETF 实时分钟流 M0 验证后开发。现有“批次差额当分钟成交额”的实现已废止，不得继续用于监控、归档或基准。

创建日期：2026-08-22
最近修订：2026-08-24
适用范围：ETF 实时采样、真实分钟归档、异动规则、Feishu 通知、Ops 配置与复盘。
...
2. 消费 ETF 实时分钟流已发布的 `1MIN` 最终分钟事实，形成监控基准与收盘归档。
3. `1/5/15` 分钟窗口的单 ETF 规则、告警和 Feishu 通知。
4. 收盘后将真实分钟事实归档到 PostgreSQL，供下一个交易日比较。
...
| ETF 实时分钟流 | 只消费该流标记为 `final` 且 `quality=valid` 的 `1MIN` 分钟事实，用于历史基准、收盘归档和窗口最终确认 | 绕过 realtime state contract 直接请求 Tushare；使用未最终确定的行作为基准 |
...
监控、归档和告警代码开始前，上游 ETF 实时分钟流必须至少完成：
...
flowchart LR
  ...
  E --> J["收盘归档"]
  J --> K["ops.etf_realtime_minute_stat"]
```
...
4. 真实分钟历史只来自 ETF 分钟流的最终分钟事实，从不使用 `rt_etf_k` 差额替代。
5. 所有监控、归档、通知失败都必须与实时采集和 Redis current pointer 隔离。
...
收盘归档只消费 ETF 分钟流的 `1MIN final/valid` 分钟事实和采集日志：

1. 同一 ETF、交易日、分钟桶只保留最后一条有效源端事实。
2. 已知请求过但无有效源端行的分钟，使用真实交易日与真实分钟桶写 `missing`；绝不使用 `date.min/time.min`。
3. `missing/invalid` 不填 0，也不能参与后续基准。
4. 重复归档同一交易日按主键 upsert，结果幂等。
5. `rt_etf_k` 的 batch、累计值和差额不进入 `ops.etf_realtime_minute_stat`。
...
`keep_recent_batches=260` 不再是 `rt_etf_k` 监控算法的依赖；全市场批次留存只服务快照/健康。分钟归档所需的日内事实由 ETF 分钟流独立维护的分钟事实与采集日志承担。
...
2. 获得运营对三张旧语义表清理重建的单独确认：`minute_stat`、`monitor_rule`、`alert`。监控池表保留。
3. 迁移重建后，运营重新录入新规则；不得复用旧 `observe_ratio/alert_ratio/strong_ratio`。
4. 第一个完整交易日只采集真实分钟事实并收盘归档，不告警。
5. 第二个交易日起，以前一开市交易日为基准启用告警。
...
| M2 | 真实分钟归档与上一开市日基准读取 | 只读取 `1MIN final/valid`，不写差额分钟值 |
| M3 | 盘中信号、事件/投递模型、Feishu 升级 | 告警先提交，发送后回写 |
| M4 | CLI 编排、规则/API/页面契约改造 | 不新增 systemd 服务 |
| M5 | 迁移、部署、首日采集、次日告警验收 | 清表须单独批准 |

必须覆盖：分钟源参数差异、两段采样请求预算、段级 `captured_at`、窗口首尾曲线计算、午休、采样中断、昨日基准缺失、同窗口升级、冷却、Feishu 失败隔离、分钟归档幂等和首日无基准。
